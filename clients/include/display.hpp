...
inline void display_timing_info_generate(std::ostream& out, int n, S name, T t, Ts... ts)
{
    ...
    rocgraph_record_timing(values[0], values[1], values[2]);
...
inline void display_timing_info_main(S name, Ts... ts)
{
    ...
    //
    {
        ...
        else
        {
            ...
            rocgraph_record_output_legend(out_legend.str());
        }
    }
    ...
    else
    {
        ...
        // store the string.
        rocgraph_record_output(out.str());
    }
}

inline void rocgraph_get_matrixname(const char* f, char* name)
{
    int n = 0;
    while(f[n] != '\0')
        ++n;
...
#define display_timing_info(...)                                                           \
    ...
    {                                                                                      \
        char matrixname[64];                                                               \
        if(rocgraph_arguments_has_datafile(arg))                                           \
            rocgraph_get_matrixname(&arg.filename[0], &matrixname[0]);                     \
        const char* importname                                                             \
            = (rocgraph_arguments_has_datafile(arg) ? &matrixname[0]                       \
                                                    : rocgraph_matrix2string(arg.matrix)); \
        const char* ctypename = rocgraph_datatype2string(arg.compute_type);                \
        const char* itypename = rocgraph_indextype2string(arg.index_type_I);               \
        const char* jtypename = rocgraph_indextype2string(arg.index_type_J);               \
                                                                                           \
