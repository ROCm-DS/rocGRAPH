...
#include "rocgraph_clients_are_gt.hpp"
...
bool rocgraph_clients_expect_gt(const char* a_name,
                                ...
                                int32_t     file_line)
{
    const bool are_gt = rocgraph_clients_are_gt(a, b);
    if(are_gt == false)
    {
        std::cerr << "*** ERROR ROCGRAPH_CLIENTS_EXPECT_GT(a,b)" << std::endl;
...
#define INSTANTIATE(T)                                              \
    template bool rocgraph_clients_expect_gt(const char* a_name,    \
                                             const T     a,         \
                                             const char* b_name,    \
                                             const T     b,         \
                                             const char* file_name, \
