 ...
 */
                 ...
                 Lambda       op = raft::identity_op())
    {
        ...
        {
            detail::scatterImpl<DataT, 16 / MaxPerElem, Lambda, IdxT, TPB>(
                out, in, idx, len, op, stream);
        }
        else if(8 / MaxPerElem && bytes % 8 == 0)
        {
            detail::scatterImpl<DataT, 8 / MaxPerElem, Lambda, IdxT, TPB>(
                out, in, idx, len, op, stream);
        }
        else if(4 / MaxPerElem && bytes % 4 == 0)
        {
            detail::scatterImpl<DataT, 4 / MaxPerElem, Lambda, IdxT, TPB>(
                out, in, idx, len, op, stream);
        }
        else if(2 / MaxPerElem && bytes % 2 == 0)
        {
            detail::scatterImpl<DataT, 2 / MaxPerElem, Lambda, IdxT, TPB>(
                out, in, idx, len, op, stream);
        }
        else if(1 / MaxPerElem)
        {
            detail::scatterImpl<DataT, 1 / MaxPerElem, Lambda, IdxT, TPB>(
                out, in, idx, len, op, stream);
        }
        else
        {
