 ...
 */
    template <typename lambda_t,
              ...
              enable_if_mdbuffer<mdbuffer_type>* = nullptr>
    decltype(auto)
        memory_type_dispatcher(raft::resources const& res, lambda_t&& f, mdbuffer_type&& buf)
    {
        if(is_host_device_accessible(buf.mem_type()))
        {
            // First see if functor has been specialized for this exact memory type
            if constexpr(detail::is_callable_for_memory_type<lambda_t,
                                                             mdbuffer_type,
                                                             memory_type::managed>)
            {
        ...
        if constexpr(detail::
                         is_callable_for_memory_type<lambda_t, mdbuffer_type, memory_type::device>)
        {
            if(is_device_accessible(buf.mem_type()))
            {
                return f(
                    mdbuffer{res, buf, memory_type::device}.template view<memory_type::device>());
            }
        ...
        {
            return f(
                mdbuffer{res, buf, memory_type::managed}.template view<memory_type::managed>());
        }
...
#pragma nv_diag_default 128
    ...
    decltype(auto)
        memory_type_dispatcher(raft::resources const& res, lambda_t&& f, mdspan_type view)
    {
    ...
    decltype(auto)
        memory_type_dispatcher(raft::resources const& res, lambda_t&& f, mdspan_type view)
    {
        return memory_type_dispatcher(
            res, std::forward<lambda_t>(f), mdbuffer_type{res, mdbuffer{view}});
    }
