 ...
 */
    ...
    namespace detail
    {
        ...
        template <typename I>
        constexpr inline I
            div_rounding_up_safe(std::integral_constant<bool, true>, I dividend, I divisor) noexcept
        {
 ...
 */
    ...
    constexpr inline auto
        is_a_power_of_two(I val) noexcept -> std::enable_if_t<std::is_integral<I>::value, bool>
    {
 ...
 */
    template <typename T>
    constexpr inline auto
        bound_by_power_of_two(T x) noexcept -> std::enable_if_t<std::is_integral<T>::value, T>
    {
        if(is_a_power_of_two(x))
        {
            return x;
        }
 ...
 */
    ...
    _RAFT_HOST_DEVICE inline void
        wmul_64bit(uint64_t& res_hi, uint64_t& res_lo, uint64_t a, uint64_t b)
    {
#ifdef __CUDA_ARCH__
