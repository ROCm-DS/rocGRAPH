 ...
 */
    ...
    inline std::enable_if_t<std::is_floating_point_v<MathT>>
        predict_core(const raft::resources&         handle,
                     ...
                     rmm::device_async_resource_ref mr)
    {
        ...
        case raft::distance::DistanceType::L2SqrtExpanded:
        {
            ...
            auto centroidsNorm = raft::make_device_mdarray<MathT, IdxT>(
                handle, mr, make_extents<IdxT>(n_clusters));
 ...
 */
                                ...
                                rmm::device_async_resource_ref mr)
    {
        ...
        else
        {
            ...
            cub::TransformInputIterator<MathT, MappingOpT, const T*> mapping_itr(dataset,
                                                                                 mapping_op);
 ...
 */
                              ...
                              MappingOpT      mapping_op)
    {
        ...
        // skip big clusters
        if(csize > static_cast<IdxT>(average * threshold))
            return;
 ...
 */
                        ...
                        rmm::device_async_resource_ref device_memory) -> bool
    {
        ...
        constexpr static std::array kPrimes{
 ...
 */
                            ...
                            rmm::device_async_resource_ref device_memory)
    {
        ...
        for(uint32_t iter = 0; iter < n_iters; iter++)
        {
            ...
            if(iter > 0
               && adjust_centers(cluster_centers,
                                 n_clusters,
                                 dim,
                                 dataset,
                                 n_rows,
                                      ...
                                      const CounterT* mesocluster_sizes)
    {
        ...
        RAFT_EXPECTS(
            fine_clusters_csum[n_mesoclusters] == n_clusters,
 ...
 */
                             ...
                             rmm::device_async_resource_ref device_memory) -> IdxT
    {
        ...
        rmm::device_uvector<MathT> mc_trainset_buf(
            mesocluster_size_max * dim, stream, device_memory);
        rmm::device_uvector<MathT> mc_trainset_norm_buf(
            mesocluster_size_max, stream, device_memory);
        ...
        for(IdxT i = 0; i < n_mesoclusters; i++)
        {
            ...
            cub::TransformInputIterator<MathT, MappingOpT, const T*> mapping_itr(dataset_mptr,
                                                                                 mapping_op);
 ...
 */
   ...
   * supported by atomicAdd: find a supported CounterT based on the IdxT. */
        ...
        rmm::device_uvector<CounterT> mesocluster_sizes_buf(
            n_mesoclusters, stream, &managed_memory);
        {
        ...
        {
            RAFT_LOG_WARN("build_hierarchical: built unbalanced mesoclusters (max_mesocluster_size "
                          "== %u > %u). "
