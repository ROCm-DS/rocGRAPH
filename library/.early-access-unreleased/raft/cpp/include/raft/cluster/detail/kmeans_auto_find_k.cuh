...
namespace raft::cluster::detail
{
    ...
    void compute_dispersion(raft::resources const&                         handle,
                            ...
                            idx_t                                          d)
    {
        auto centroids_const_view = raft::make_device_matrix_view<const value_t, idx_t>(
            centroids_view.data_handle(), val, d);
        ...
        detail::countLabels(
            handle, labels.data_handle(), clusterSizes.data_handle(), n, val, workspace);
                ...
                value_t                                        tol     = 1e-2)
    {
        ...
        objective[1]
            = (n - right) / (right - 1) * clusterDispertionView[right] / resultsView[right];
        while(left < right - 1)
        {
            ...
            {
                centroids_view = raft::make_device_matrix_view<value_t, idx_t>(
                    centroids.data_handle(), mid, d);
                compute_dispersion<value_t, idx_t>(handle,
                                                   X,
                                                   params,
                                                   centroids_view,
            ...
            objective[0]
                = (n - left) / (left - 1) * clusterDispertionView[left] / resultsView[left];
            objective[1]
                = (n - right) / (right - 1) * clusterDispertionView[right] / resultsView[right];
        ...
        objective[0] = (n - left) / (left - 1) * clusterDispertionView[left] / resultsView[left];
        objective[1]
            = (n - oldmid) / (oldmid - 1) * clusterDispertionView[oldmid] / resultsView[oldmid];
        ...
        if(best_k[0] != oldmid)
        {
            auto centroids_view = raft::make_device_matrix_view<value_t, idx_t>(
                centroids.data_handle(), best_k[0], d);
