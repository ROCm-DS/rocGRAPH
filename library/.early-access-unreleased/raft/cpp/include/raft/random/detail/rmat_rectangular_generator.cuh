    ...
    namespace random
    {
        namespace detail
        {
                                        ...
                                        raft::random::PCGenerator& gen)
            {
                ...
                {
                    src_id |= (IdxT(src_bit) << (r_scale - curr_depth - 1));
                }
                if(curr_depth < c_scale)
                {
                    dst_id |= (IdxT(dst_bit) << (c_scale - curr_depth - 1));
                }
            }

            template <typename IdxT>
            DI void store_ids(IdxT* out,
                              IdxT* out_src,
                              IdxT* out_dst,
                              IdxT  src_id,
                              IdxT  dst_id,
                                        ...
                                        raft::random::RngState r)
            {
                ...
                {
                    gen_and_update_bits(
                        src_id, dst_id, a, a + b, a + b + c, r_scale, c_scale, i, gen);
                }
                for(; i < r_scale; ++i)
                {
                    gen_and_update_bits(
                        src_id, dst_id, a + b, a + b, ProbT(1), r_scale, c_scale, i, gen);
                }
                for(; i < c_scale; ++i)
                {
                    gen_and_update_bits(
                        src_id, dst_id, a + c, ProbT(1), ProbT(1), r_scale, c_scale, i, gen);
                }
                store_ids(out, out_src, out_dst, src_id, dst_id, idx, n_edges);
            }
 ...
 */
            template <typename IdxT, typename ProbT>
            void rmat_rectangular_gen_impl(
                ...
                IdxT                                                    c_scale)
            {
                ...
                RAFT_EXPECTS(
                    theta.extent(0) == expected_theta_len,
                    "rmat_rectangular_gen: "
