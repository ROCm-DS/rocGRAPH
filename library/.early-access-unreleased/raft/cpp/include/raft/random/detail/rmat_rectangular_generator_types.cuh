 ...
 */
            ...
            public:
                using out_view_type
                    = raft::device_mdspan<IdxT,
                                          raft::extents<IdxT, raft::dynamic_extent, 2>,
                                          raft::row_major>;
            ...
            private:
                ...
                {
                public:
                    output_pair(const out_src_view_type& src, const out_dst_view_type& dst)
                        : src_(src)
                        , dst_(dst)
                    {
                    ...
                    bool empty() const
                    {
                        return src_.extent(0) == 0 && dst_.extent(0) == 0;
                    }
                ...
                public:
                    ...
                    bool empty() const
                    {
                        return out_.extent(0) == 0 && pair_.empty();
                    }
   ...
   */
                rmat_rectangular_gen_output(const out_src_view_type& src,
                                            const out_dst_view_type& dst)
   ...
   */
                bool empty() const
                {
                    ...
                    { // std::holds_alternative<output_triple>(data_)
   ...
   */
                std::optional<out_view_type> out_view() const
                {
                    ...
                    { // if (std::holds_alternative<>(output_pair))
                        return std::nullopt;
                    }
                }
   ...
   */
                IdxT number_of_edges() const
                {
                    ...
                    { // if (std::holds_alternative<output_triple>(data_))
