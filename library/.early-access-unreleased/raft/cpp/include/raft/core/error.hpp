 ...
 */
    ...
    private:
        ...
        void collect_call_stack() noexcept
        {
            ...
            oss << std::endl
                << "Obtained " << (depth - kSkipFrames) << " stack frames" << std::endl;
            ...
            for(int i = kSkipFrames; i < depth; i++)
            {
                   ...
                   && mangled_name + 1 < offset_begin)
                {
                    ...
                    *offset_end++   = 0;
                    ...
                    if(status == 0)
                    { // Success: substitute the real name
                        oss << backtrace_line << ": " << real_name << " +" << offset_begin
                            << offset_end;
                    }
                    else
                    { // Couldn't demangle
                        oss << backtrace_line << ": " << mangled_name << " +" << offset_begin
                            << offset_end;
                    }
                    free(real_name);
                }
                else
                { // Couldn't match the symbol name
                    oss << backtrace_line;
                }
                oss << std::endl;
            }
 ...
 */
    ...
    {
        explicit non_cuda_build_error(char const* const message)
            : raft::exception(message)
        {
        }
        explicit non_cuda_build_error(std::string const& message)
            : raft::exception(message)
        {
        }
    };
...
#define THROW(fmt, ...)                                                                    \
    ...
    {                                                                                      \
        int size1 = std::snprintf(                                                         \
            nullptr, 0, "exception occurred! file=%s line=%d: ", __FILE__, __LINE__);      \
