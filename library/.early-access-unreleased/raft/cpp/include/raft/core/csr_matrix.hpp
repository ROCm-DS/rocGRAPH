 ...
 */
   ...
   * @return span containing underlying indptr array
   */
        span<indptr_type, is_device> get_indptr() override
        {
            return indptr_;
        }
   ...
   * @return span containing underlying indices array
   */
        span<indices_type, is_device> get_indices() override
        {
            return indices_;
        }
 ...
 */
    ...
    public:
        ...
        compressed_structure(compressed_structure const&) noexcept(
            std::is_nothrow_copy_constructible_v<indptr_container_type>)
            = default;
        compressed_structure(compressed_structure&&) noexcept(
            std::is_nothrow_move_constructible<indptr_container_type>::value)
            = default;
   ...
   */
        view_type view()
        {
            if(this->get_nnz() == 0)
            {
                RAFT_LOG_WARN("Cannot create compressed_structure.view() because it has not been "
                              "initialized (sparsity "
                              "is 0)");
            }
            auto indptr_span
                = raft::span<IndptrType, is_device>(c_indptr_.data(), this->get_n_rows() + 1);
            auto indices_span
                = raft::span<IndicesType, is_device>(c_indices_.data(), this->get_nnz());
   ...
   */
    ...
    public:
        ...
        static constexpr auto get_sparsity_type()
        {
            return sparsity_type;
        }
        using sparse_matrix_type = sparse_matrix<
            ElementType,
            structure_type,
            csr_matrix_view<ElementType, IndptrType, IndicesType, NZType, is_device>,
            is_device,
