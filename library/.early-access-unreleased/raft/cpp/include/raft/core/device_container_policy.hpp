 ...
 */
    ...
    public:
        ...
        device_uvector(device_uvector const& that)
            : data_{that.data_, that.data_.stream()}
        {
        }
   ...
   */
        explicit device_uvector(std::size_t size, rmm::cuda_stream_view stream)
            : data_{size, stream}
        {
        }
        /**
   ...
   */
        ...
        {
            return device_reference<T const>{thrust::device_ptr<T const>{data_.data() + i},
                                             data_.stream()};
        }
 ...
 */
    ...
    public:
        ...
        explicit device_uvector_policy(rmm::device_async_resource_ref mr) noexcept
            : mr_(mr)
        {
        }
