 ...
 */
    ...
    inline constexpr bool is_input_host_mdspan_v
        = std::conjunction_v<is_input_host_mdspan_t<Tn>...>;

    template <typename... Tn>
    inline constexpr bool is_output_host_mdspan_v
        = std::conjunction_v<is_output_host_mdspan_t<Tn>...>;
 ...
 */
    template <typename ElementType,
              typename IndexType    = std::uint32_t,
              typename LayoutPolicy = layout_right_padded<ElementType>>
    auto constexpr make_host_aligned_matrix_view(ElementType* ptr,
                                                 IndexType    n_rows,
                                                 IndexType    n_cols)
    {
        ...
        return host_aligned_matrix_view<ElementType, IndexType, LayoutPolicy>{aligned_pointer,
                                                                              extents};
    }
