...
namespace raft
{
    ...
    namespace detail
    {
        ...
        struct padding
        {
            ...
            static_assert(ByteAlignment % sizeof(ValueType) == 0
                              || sizeof(ValueType) % ByteAlignment == 0,
 ...
 */
    ...
    using ensure_integral_extents
        = std::enable_if_t<std::conjunction_v<std::is_integral<Extents>...>>;
 ...
 */
    ...
    RAFT_INLINE_FUNCTION auto
        unravel_index_impl(I idx, std::experimental::extents<IndexType, Extents...> shape)
    {
        ...
        {
            auto s
                = static_cast<std::remove_const_t<std::remove_reference_t<I>>>(shape.extent(dim));
            if(s & (s - 1))
            {
                auto t     = idx / s;
                index[dim] = idx - t * s;
 ...
 */
    ...
    RAFT_INLINE_FUNCTION auto
        unravel_index(Idx idx, extents<IndexType, Exts...> shape, LayoutPolicy const& layout)
    {
        ...
        auto constexpr kIs64
            = sizeof(std::remove_cv_t<std::remove_reference_t<Idx>>) == sizeof(uint64_t);
 ...
 */
    ...
    host_device_accessor<std::experimental::default_accessor<std::add_const_t<ElementType>>,
                         MemType>
        accessor_of_const(
            host_device_accessor<std::experimental::default_accessor<ElementType>, MemType> a)
    {
        return {a};
    }
