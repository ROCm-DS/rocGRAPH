 ...
 */
   ...
   */
        auto view() noexcept
        {
            return static_cast<Base*>(this)->view();
        }
        /**
 ...
 */
        template <typename T>
        inline constexpr bool is_array_interface_v
            = is_array_interface<std::remove_const_t<T>>::value;
    } // namespace detail
...
#define RAFT_MDARRAY_CTOR_CONSTEXPR
   ...
   */
        RAFT_MDARRAY_CTOR_CONSTEXPR mdarray(raft::resources const&       handle,
            ...
            : cp_(cp)
            , map_(m)
            , c_(cp_.create(handle, map_.required_span_size()))
        {
        }

        RAFT_MDARRAY_CTOR_CONSTEXPR
        mdarray(raft::resources const& handle, mapping_type const& m, container_policy_type& cp)
            : cp_(cp)
            , map_(m)
            , c_(cp_.create(handle, map_.required_span_size()))
        {
...
#undef RAFT_MDARRAY_CTOR_CONSTEXPR
   ...
   */
        auto view() noexcept
        {
            return view_type(c_.data(), map_, cp_.make_accessor_policy());
        }
        /**
   * @brief Get an mdspan<const T>
   */
        ...
        [[nodiscard]] auto data_handle() noexcept -> pointer
        {
            return c_.data();
        }
        [[nodiscard]] constexpr auto data_handle() const noexcept -> const_pointer
        {
            return c_.data();
        }
   ...
   */
        ...
        [[nodiscard]] RAFT_INLINE_FUNCTION static constexpr auto
            rank_dynamic() noexcept -> rank_type
        {
            return extents_type::rank_dynamic();
        }
        [[nodiscard]] RAFT_INLINE_FUNCTION static constexpr auto
            static_extent(size_t r) noexcept -> index_type
        {
   ...
   */
        [[nodiscard]] RAFT_INLINE_FUNCTION constexpr auto
            extent(size_t r) const noexcept -> index_type
        {
