 ...
 */
   ...
   * @param n_cols total number of columns in matrix
   */
        sparse_structure(row_type n_rows, col_type n_cols)
            : n_rows_(n_rows)
            , n_cols_(n_cols)
            , nnz_(0)
        {
   ...
   * @param nnz
   */
        virtual void initialize_sparsity(nnz_type nnz)
        {
            nnz_ = nnz;
        }
 ...
 */
    ...
    public:
        ...
        sparse_matrix_view(raft::span<ElementType, is_device> element_span,
            ...
            : element_span_(element_span)
            , structure_view_(structure_view)
        {
   ...
   */
        span<element_type, is_device> get_elements()
        {
            return element_span_;
        }
 ...
 */
    ...
    public:
        ...
        sparse_matrix(raft::resources const& handle,
                      ...
                      col_type               n_cols,
                      nnz_type               nnz
                      = 0) noexcept(std::is_nothrow_default_constructible_v<container_type>)
            : structure_{handle, n_rows, n_cols, nnz}
            , cp_{}
        ...
        sparse_matrix(raft::resources const& handle, structure_type structure) noexcept(
            ...
            , cp_{}
            , c_elements_{cp_.create(handle, structure_.get_nnz())} {};
