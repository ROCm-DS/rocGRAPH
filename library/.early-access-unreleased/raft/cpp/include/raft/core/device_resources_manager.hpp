 ...
 */
    ...
    private:
        ...
        struct resource_params
        {
            ...
            // resource.
            std::vector<std::pair<std::shared_ptr<rmm::mr::device_memory_resource>, int>>
                workspace_mrs{};
        ...
        struct resource_components
        {
            ...
            resource_components(int device_id, resource_params const& params)
                ...
                , pools_{[&params, this]() {
                    ...
                    if(params.pool_size != 0)
                    {
                        ...
                        {
                            result.push_back(
                                std::make_shared<rmm::cuda_stream_pool>(params.pool_size));
                        }
                    }
                ...
                , pool_mr_{[&params, this]() {
                    ...
                    auto result        = std::shared_ptr<
                               rmm::mr::pool_memory_resource<rmm::mr::cuda_memory_resource>>{nullptr};
                    ...
                    if(params.max_mem_pool_size.value_or(1) != 0)
                    {
                        ...
                        if(upstream != nullptr)
                        {
                            result = std::make_shared<
                                ...
                                upstream,
                                params.init_mem_pool_size.value_or(
                        ...
                        {
                            RAFT_LOG_WARN("Pool allocation requested, but other memory resource "
                ...
                , workspace_mr_{[&params, this]() {
                    ...
                    auto iter
                        = std::find_if(std::begin(params.workspace_mrs),
                                       std::end(params.workspace_mrs),
                                       [this](auto&& pair) { return pair.second == device_id_; });
                    if(iter != std::end(params.workspace_mrs))
                    {
                        result = iter->first;
                    }
                    return result;
            ...
            [[nodiscard]] auto get_stream() const
            {
                ...
                if(stream_count() != 0)
                {
                    result = streams_->get_stream(get_thread_id() % stream_count());
                }
                return result;
            }
            ...
            // this manager
            [[nodiscard]] auto get_workspace_memory_resource()
            {
                return workspace_mr_;
            }
        ...
        {
            thread_local auto thread_resources
                = std::vector<std::optional<raft::device_resources>>([]() {
                      auto result = 0;
            ...
            if(!thread_resources[device_id])
            {
                ...
                auto component_iter
                    = std::find_if(std::begin(per_device_components_),
                ...
                // components
                thread_resources[device_id].emplace(
                    component_iter->get_stream(),
        ...
        void set_streams_per_device_(std::optional<std::size_t> num_streams)
        {
            ...
            {
                RAFT_LOG_WARN("Attempted to set device_resources_manager properties after "
                                            ...
                                            int device_id)
        {
            ...
            {
                auto iter
                    = std::find_if(std::begin(params_.workspace_mrs),
   ...
   * and a warning will be emitted.
   */
        static void set_stream_pools_per_device(std::size_t num_pools,
                                                std::size_t num_streams
                                                = rmm::cuda_stream_pool::default_size)
        {
   ...
   */
        static void
            set_workspace_memory_resource(std::shared_ptr<rmm::mr::device_memory_resource> mr,
