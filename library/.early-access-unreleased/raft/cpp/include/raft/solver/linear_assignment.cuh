 ...
 */
   ...
   */
        weight_t getDualObjectiveValue(int spId)
        {
            weight_t result;
            raft::update_host(
                &result, obj_val_dual_v.data() + spId, 1, resource::get_cuda_stream(handle_));
            RAFT_CHECK_CUDA(resource::get_cuda_stream(handle_));
            return result;
        }
    ...
    private:
        ...
        int hungarianStep1()
        {
            ...
            while(true)
            {
                if((next = hungarianStep2()) == 6)
                    break;

                if((next = hungarianStep3()) == 5)
                    break;
        ...
        int hungarianStep6()
        {
            ...
            detail::calcObjValDual(
                handle_, obj_val_dual_v.data(), d_vertices_dev, batchsize_, size_);
