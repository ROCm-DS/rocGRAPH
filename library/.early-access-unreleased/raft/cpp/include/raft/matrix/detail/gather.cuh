...
#pragma unroll
                ...
                {
                    IndexT tid = threadIdx.x
                                 + (Policy::work_per_thread * static_cast<IndexT>(blockIdx.x) + wid)
                                       * Policy::n_threads;
                    if(tid < len)
                    {
                        IndexT i_dst = tid / D;
 ...
 */
                            ...
                            cudaStream_t         stream)
            {
                ...
                if(len < static_cast<IndexT>(32 * TPB * n_sm))
                {
                    ...
                    IndexT n_blocks
                        = raft::ceildiv(map_length * D, static_cast<IndexT>(Policy::stride));
 ...
 */
                           ...
                           cudaStream_t         stream)
            {
                typedef typename std::iterator_traits<MapIteratorT>::value_type MapValueT;
                gatherImpl(
                    in, D, N, map, stencil, map_length, out, pred_op, raft::identity_op(), stream);
            }
...
#pragma omp for
                        ...
                        raft::device_matrix_view<T, MatIdxT>    output)
            {
                ...
                raft::copy(indices_host.data_handle(),
                           indices.data_handle(),
                           n_train,
...
#pragma omp parallel num_threads(n_threads)
                {
                    ...
                    for(MatIdxT device_offset = 0; device_offset < n_train;
                        device_offset += max_batch_size)
                    {
                        MatIdxT batch_size
                            = std::min<IdxT>(max_batch_size, n_train - device_offset);
...
#pragma omp master
                        ...
                        {
                            gather_buff(dataset,
                                        make_const_mdspan(indices_host.view()),
                                        host_offset,
                                        view2);
                        }
#pragma omp master
