    ...
    namespace matrix
    {
        namespace detail
        {
                ...
                IndexT         batch_size)
            {
                ...
                if(map_length <= 0 || m <= 0 || n <= 0 || batch_size < 0)
                    return;

                RAFT_EXPECTS(map_length <= m,
                             "Length of map should be <= number of rows for inplace gather");
                ...
                auto scratch_space
                    = raft::make_device_vector<MatrixT, IndexT>(handle, map_length * batch_size);
                ...
                for(IndexT bid = 0; bid < n_batches; bid++)
                {
                    ...
                    raft::linalg::map_offset(
                        handle,
                        raft::make_device_vector_view(scratch_space.data_handle(),
                                                      map_length * cols_per_batch),
                        gather_op);
                    ...
                    thrust::for_each(
                        exec_policy, counting, counting + map_length * cols_per_batch, copy_op);
                }
            }
