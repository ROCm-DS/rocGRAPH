...
MDSPAN_STATIC_TEST(
    // -1 - 2 - 3 + 7 + 8 + 9 = 18
    stdex::mdspan<double, stdex::extents<size_t, simple_static_submdspan_test_1(-1)>>{nullptr}
        .extent(0)
    == 18);
#endif
...
constexpr bool mixed_submdspan_left_test_2()
{
    ...
    auto s
        = stdex::mdspan<int, stdex::extents<size_t, 3, stdex::dynamic_extent>, stdex::layout_left>(
            data, 5);
    int result = 0;
...
constexpr bool submdspan_single_element_stress_test_impl_2(std::integer_sequence<size_t, Idxs...>)
{
    ...
    auto ss_all_dyn
        = stdex::submdspan(s_dyn, _repeated_with_idxs_t<stdex::full_extent_t, Idxs>{}...);
    auto val     = __MDSPAN_OP(ss_all, (_repeated_ptrdiff_t<0, Idxs>...));
    auto val_dyn = __MDSPAN_OP(ss_all_dyn, (_repeated_ptrdiff_t<0, Idxs>...));
    auto ss_pair = stdex::submdspan(
        s, _repeated_with_idxs_t<std::pair<ptrdiff_t, ptrdiff_t>, Idxs>{0, 1}...);
    auto ss_pair_dyn = stdex::submdspan(
        s_dyn, _repeated_with_idxs_t<std::pair<ptrdiff_t, ptrdiff_t>, Idxs>{0, 1}...);
