...
namespace std
{
    namespace experimental
    {
        ...
        namespace details
        {
            ...
            static_assert(
                std::is_same<iota_index_sequence_t<3, 6>, std::index_sequence<3, 4, 5>>::value,
        ...
        namespace details
        {
            ...
            MDSPAN_INLINE_FUNCTION constexpr ReturnExtents
                layout_left_extents_helper(const stdex::extents<typename InnerExtents::index_type,
                                                                UnpaddedExtent>& unpadded_extent,
            ...
            MDSPAN_INLINE_FUNCTION constexpr ReturnExtents
                layout_right_extents_helper(const InnerExtents&                   inner_extents,
                                            const stdex::extents<typename InnerExtents::index_type,
                                                                 UnpaddedExtent>& unpadded_extent,
                ...
                unpadded_extent_right(const stdex::extents<IndexType, InputExtents...>& input)
            {
                ...
                return stdex::extents<IndexType, input_type::static_extent(rank - 1)>{
                    input.extent(rank - 1)};
            }
        ...
        struct layout_left_padded
        {
            ...
            public:
                ...
                MDSPAN_INLINE_FUNCTION constexpr mapping(const extents_type& ext)
                    : inner_mapping_(
                          details::pad_extents_left(ext, padding_extents_type{padding_stride}))
                ...
                MDSPAN_INLINE_FUNCTION constexpr mapping(
                    ...
                    : inner_mapping_(
                          details::pad_extents_left(ext, padding_extents_type{padding_value}))
                    , unpadded_extent_(details::unpadded_extent_left(ext))
                {
                ...
                MDSPAN_INLINE_FUNCTION_DEFAULTED _MDSPAN_CONSTEXPR_14_DEFAULTED mapping&
                    operator=(const mapping&) noexcept
                    = default;

                MDSPAN_INLINE_FUNCTION
                constexpr extents_type extents() const noexcept
                {
                    return details::layout_left_extents<extents_type>(unpadded_extent_,
                ...
                MDSPAN_TEMPLATE_REQUIRES(
                    ...
                    /* requires */ (sizeof...(Indices) == Extents::rank()
                                    ...
                                    && _MDSPAN_FOLD_AND(_MDSPAN_TRAIT(std::is_nothrow_constructible,
                                                                      index_type,
                                                                      Indices) /*&& ...*/)))
                MDSPAN_INLINE_FUNCTION
        ...
        struct layout_right_padded
        {
            ...
            public:
                ...
                MDSPAN_INLINE_FUNCTION constexpr mapping(const extents_type& ext)
                    : inner_mapping_(
                          details::pad_extents_right(ext, padding_extents_type{padding_stride}))
                ...
                MDSPAN_INLINE_FUNCTION constexpr mapping(
                    ...
                    std::enable_if_t<
                        ...
                        && std::is_nothrow_constructible<index_type, Size>::value>* = nullptr)
                    : inner_mapping_(
                          details::pad_extents_right(ext, padding_extents_type{padding_value}))
                    , unpadded_extent_(details::unpadded_extent_right(ext))
                {
                    // We don't have to check padding_value here, because the
                    // padding_extents_type constructor already has a precondition.
                ...
                MDSPAN_INLINE_FUNCTION _MDSPAN_CONSTEXPR_14 bool is_exhaustive() const noexcept
                {
                    return extents_type::rank() == 0 ? true
                                                     : inner_mapping_.extent(Extents::rank() - 1)
                                                           == unpadded_extent_.extent(0);
                }
                MDSPAN_INLINE_FUNCTION static constexpr bool is_strided() noexcept
                {
                    return true;
