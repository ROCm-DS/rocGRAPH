 ...
 */
        ...
        bool evaluateSVDByL2Norm(raft::resources const& handle,
                                 ...
                                 cudaStream_t           stream)
        {
            return detail::evaluateSVDByL2Norm(
                handle, A_d, U, S_vec, V, n_rows, n_cols, k, tol, stream);
        }
 ...
 */
                    ...
                    = std::nullopt)
        {
            ...
            if(U)
            {
                RAFT_EXPECTS(in.extent(0) == U.value().extent(0)
                                 && in.extent(1) == U.value().extent(1),
                             "U should have dimensions m * n");
                left_sing_vecs_ptr = U.value().data_handle();
            }
            if(V)
            {
                RAFT_EXPECTS(in.extent(1) == V.value().extent(0)
                                 && in.extent(1) == V.value().extent(1),
 ...
 */
        template <typename ValueType, typename IndexType>
        void
            svd_eig(raft::resources const&                                                handle,
                    raft::device_matrix_view<const ValueType, IndexType, raft::col_major> in,
        ...
        void svd_eig(raft::resources const&                                                handle,
                     ...
                     UType&& U = std::nullopt)
        {
            std::optional<raft::device_matrix_view<ValueType, IndexType, raft::col_major>>
                U_optional = std::forward<UType>(U);
 ...
 */
            ...
            raft::device_matrix_view<ValueType, IndexType, raft::col_major>       out)
        {
            ...
            RAFT_EXPECTS(
                out.extent(0) == U.extent(0) && out.extent(1) == V.extent(1),
