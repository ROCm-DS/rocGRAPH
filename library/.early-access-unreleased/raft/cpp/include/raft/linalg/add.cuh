 ...
 */
            ...
            raft::device_scalar_view<const typename InType::value_type, ScalarIdxType> scalar)
        {
            ...
            if(out.size() <= std::numeric_limits<std::uint32_t>::max())
            {
                addDevScalar<in_value_t, out_value_t, std::uint32_t>(
                    out.data_handle(),
                    in.data_handle(),
                    scalar.data_handle(),
                    static_cast<std::uint32_t>(out.size()),
            ...
            {
                addDevScalar<in_value_t, out_value_t, std::uint64_t>(
                    out.data_handle(),
                    in.data_handle(),
                    scalar.data_handle(),
                    static_cast<std::uint64_t>(out.size()),
 ...
 */
            ...
            raft::host_scalar_view<const typename InType::value_type, ScalarIdxType> scalar)
        {
            ...
            if(out.size() <= std::numeric_limits<std::uint32_t>::max())
            {
                addScalar<in_value_t, out_value_t, std::uint32_t>(
                    out.data_handle(),
                    in.data_handle(),
                    *scalar.data_handle(),
                    static_cast<std::uint32_t>(out.size()),
            ...
            {
                addScalar<in_value_t, out_value_t, std::uint64_t>(
                    out.data_handle(),
                    in.data_handle(),
                    *scalar.data_handle(),
                    static_cast<std::uint64_t>(out.size()),
