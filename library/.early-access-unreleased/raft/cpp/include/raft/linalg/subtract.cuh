 ...
 */
        ...
        void subtract(raft::resources const& handle, InType in1, InType in2, OutType out)
        {
            ...
            {
                subtract<in_value_t, out_value_t, std::uint32_t>(
                    out.data_handle(),
                    in1.data_handle(),
                    in2.data_handle(),
            ...
            {
                subtract<in_value_t, out_value_t, std::uint64_t>(
                    out.data_handle(),
                    in1.data_handle(),
                    in2.data_handle(),
 ...
 */
            ...
            raft::host_scalar_view<const typename InType::element_type, ScalarIdxType> scalar)
        {
            ...
            if(out.size() <= std::numeric_limits<std::uint32_t>::max())
            {
                subtractScalar<in_value_t, out_value_t, std::uint32_t>(
                    out.data_handle(),
                    in.data_handle(),
                    *scalar.data_handle(),
                    static_cast<std::uint32_t>(out.size()),
            ...
            {
                subtractScalar<in_value_t, out_value_t, std::uint64_t>(
                    out.data_handle(),
                    in.data_handle(),
                    *scalar.data_handle(),
                    static_cast<std::uint64_t>(out.size()),
