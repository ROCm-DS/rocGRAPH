 ...
 */
                  ...
                  std::optional<ScalarViewType> beta  = std::nullopt)
        {
            ...
            if(is_A_col_major)
            {
                ...
                RAFT_EXPECTS(y.extent(0) == A.extent(0),
                             "Number of rows of A and length of y should be equal");
            }
            else
            {
                RAFT_EXPECTS(x.extent(0) == A.extent(0),
                             "Number of rows of A and length of x should be equal");
                RAFT_EXPECTS(y.extent(0) == A.extent(1),
                             "Number of columns of A and length of y should be equal");
            }

            constexpr auto device_mode
                = std::is_same_v<ScalarViewType,
                                 raft::device_scalar_view<ValueType, ScalarIdxType>>;
            ...
            if constexpr(device_mode)
            {
                if(!alpha)
                {
                    alpha = alpha_device.view();
                }
                if(!beta)
                {
                    beta = beta_device.view();
                }
            }
            else
            {
