 ...
 */
        ...
        {
            detail::eigSelDC(
                handle, in, n_rows, n_cols, n_eig_vals, eig_vectors, eig_vals, memUsage, stream);
        }
 ...
 */
        ...
        {
            detail::eigJacobi(
                handle, in, n_rows, n_cols, eig_vectors, eig_vals, stream, tol, sweeps);
        }
 ...
 */
        ...
        {
            RAFT_EXPECTS(in.size() == eig_vectors.size(),
                         "Size mismatch between Input and Eigen Vectors");
            RAFT_EXPECTS(eig_vals.size() == in.extent(1),
                         "Size mismatch between Input and Eigen Values");
 ...
 */
            ...
            EigVecMemUsage                                                        memUsage)
        {
            RAFT_EXPECTS(eig_vectors.size() == n_eig_vals * in.extent(0),
                         "Size mismatch between Input and Eigen Vectors");
            RAFT_EXPECTS(eig_vals.size() == n_eig_vals,
                         "Size mismatch between Input and Eigen Values");
