 ...
 */
        ...
        void powerScalar(
            out_t* out, const in_t* in, const in_t scalar, IdxType len, cudaStream_t stream)
        {
 ...
 */
        ...
        void power(raft::resources const& handle, InType in1, InType in2, OutType out)
        {
            ...
            {
                power<in_value_t, out_value_t, std::uint32_t>(
                    out.data_handle(),
                    in1.data_handle(),
                    in2.data_handle(),
            ...
            {
                power<in_value_t, out_value_t, std::uint64_t>(
                    out.data_handle(),
                    in1.data_handle(),
                    in2.data_handle(),
 ...
 */
            ...
            const raft::host_scalar_view<const typename InType::value_type, ScalarIdxType> scalar)
        {
            ...
            if(out.size() <= std::numeric_limits<std::uint32_t>::max())
            {
                powerScalar<in_value_t, out_value_t, std::uint32_t>(
                    out.data_handle(),
                    in.data_handle(),
                    *scalar.data_handle(),
                    static_cast<std::uint32_t>(out.size()),
            ...
            {
                powerScalar<in_value_t, out_value_t, std::uint64_t>(
                    out.data_handle(),
                    in.data_handle(),
                    *scalar.data_handle(),
                    static_cast<std::uint64_t>(out.size()),
