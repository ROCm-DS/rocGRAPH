        ...
        namespace detail
        {
                                     ...
                                     math_t                 eps = -1)
            {
                ...
                else
                {
                    ...
                    RAFT_CUBLAS_TRY(cublasCopy(
                        resource::get_cublas_handle(handle), n - 1, A_row, ld, A_new, 1, stream));
                }
                ...
                if(n > 1)
                {
                    ...
                    RAFT_CUBLAS_TRY(cublasdot(
                        resource::get_cublas_handle(handle), n - 1, A_new, 1, A_new, 1, s, stream));

                    if(uplo == CUBLAS_FILL_MODE_LOWER)
                    {
                        // Copy back the L_12 elements as the n-th row of L
                        RAFT_CUBLAS_TRY(cublasCopy(resource::get_cublas_handle(handle),
                                                   n - 1,
                                                   A_new,
                                                   1,
                                                   A_row,
