    ...
    namespace linalg
    {
        namespace detail
        {
                                ...
                                bool                   gen_V)
            {
                ...
                RAFT_EXPECTS((k + p) < std::min(n_rows, n_cols),
                             "k + p must be < min(n_rows, n_cols)");
                RAFT_EXPECTS(!gen_U || (U != nullptr),
                             "computation of U vector requested but found nullptr");
                RAFT_EXPECTS(!gen_V || (V != nullptr),
                             "computation of V vector requested but found nullptr");
#if CUDART_VERSION < 11050
                RAFT_EXPECTS(gen_U && gen_V,
                             "not computing U or V is not supported in CUDA version < 11.5");
#endif
 ...
 */
                               ...
                               cudaStream_t           stream)
            {
                ...
                if(!use_bbt)
                {
                    ...
                    raft::linalg::qrGetQR(
                        handle, Bt.data(), Qhat.data(), Rhat.data(), n, l, stream);
                    ...
                    raft::matrix::slice(
                        handle,
                        make_device_matrix_view<const math_t, int, col_major>(
                            S_vec_tmp.data(), 1, l),
                ...
                else
                {
                    ...
                    if(gen_left_vec)
                    {
                        ...
                        raft::matrix::col_reverse(
                            handle, make_device_matrix_view<math_t, int, col_major>(U, m, k));
                    }
                    ...
                    if(gen_right_vec)
                    {
                        ...
                        raft::linalg::gemm(handle,
                                           ...
                                           stream);
                        raft::matrix::col_reverse(
                            handle, make_device_matrix_view<math_t, int, col_major>(V, n, k));
                    }
                }
            }
