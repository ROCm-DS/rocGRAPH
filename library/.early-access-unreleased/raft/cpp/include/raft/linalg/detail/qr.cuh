 ...
 */
            ...
            void qrGetQ_inplace(raft::resources const& handle,
                                math_t*                Q,
                                int                    n_rows,
                                int                    n_cols,
                                cudaStream_t           stream)
            {
                ...
                RAFT_CUSOLVER_TRY(
                    cusolverDngeqrf_bufferSize(cusolver, n_rows, n_cols, Q, n_rows, &ws_size));
                ...
                RAFT_CUSOLVER_TRY(cusolverDnorgqr_bufferSize(
                    cusolver, n_rows, n_cols, n_cols, Q, n_rows, tau.data(), &ws_size));
                         ...
                         cudaStream_t           stream)
            {
                ...
                RAFT_CUDA_TRY(
                    cudaMemsetAsync(tau.data(), 0, sizeof(math_t) * std::min(m, n), stream));
                int R_full_nrows = m, R_full_ncols = n;
                RAFT_CUDA_TRY(cudaMemcpyAsync(
                    R_full.data(), M, sizeof(math_t) * m * n, cudaMemcpyDeviceToDevice, stream));
                ...
                raft::matrix::upper_triangular<math_t, int>(
                    ...
                    make_device_matrix_view<math_t, int, col_major>(
                        R, std::min(m, n), std::min(m, n)));

                RAFT_CUDA_TRY(cudaMemcpyAsync(
                    Q, R_full.data(), sizeof(math_t) * m * n, cudaMemcpyDeviceToDevice, stream));
