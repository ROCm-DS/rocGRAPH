 ...
 */
        ...
        void
            rsvd_fixed_rank(raft::resources const& handle,
                            ...
                            VType&&                                        V_in)
        {
            ...
            if(U)
            {
                RAFT_EXPECTS(M.extent(0) == U.value().extent(0),
                             "Number of rows in M should be equal to U");
            ...
            if(V)
            {
                RAFT_EXPECTS(M.extent(1) == V.value().extent(1),
                             "Number of columns in M should be equal to V");
 ...
 */
        ...
        void rsvd_fixed_rank_symmetric_jacobi(Args... args)
        {
            rsvd_fixed_rank_symmetric_jacobi(
                std::forward<Args>(args)..., std::nullopt, std::nullopt);
        }
