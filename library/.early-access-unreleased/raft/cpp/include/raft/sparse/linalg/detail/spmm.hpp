 ...
 */
                    ...
                    raft::device_matrix_view<ValueType, IndexType, LayoutPolicyZ>&       z)
                {
                    ...
                    ASSERT(is_row_major || is_col_major,
                           "Both matrices need to be either row or col major");
                    return is_row_major;
                }
 ...
 */
                ...
                {
                    auto opX
                        = trans_x ? CUSPARSE_OPERATION_TRANSPOSE : CUSPARSE_OPERATION_NON_TRANSPOSE;
                    auto opY
                        = trans_y ? CUSPARSE_OPERATION_TRANSPOSE : CUSPARSE_OPERATION_NON_TRANSPOSE;
                    ...
                    rmm::device_uvector<ValueType> tmp(bufferSize,
                                                       resource::get_cuda_stream(handle));

                    RAFT_CUSPARSE_TRY(
                        raft::sparse::detail::cusparsespmm(resource::get_cusparse_handle(handle),
                                                           opX,
                                                           opY,
                                                           alpha,
                                                           descr_x,
