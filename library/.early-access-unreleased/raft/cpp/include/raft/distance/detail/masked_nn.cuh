...
namespace raft
{
    namespace distance
    {
        namespace detail
        {
            ...
            __launch_bounds__(P::Nthreads, 2) RAFT_KERNEL
...
#pragma unroll
                        for(int j = 0; j < P::AccColsPerTh; ++j)
                        {
                            ...
                            {
                                val[i] = pairRed_op(
                                    accrowid + i * P::AccThRows + tile_idx_m, tmp, val[i]);
                            }
                        }
                    }
                };
...
#pragma unroll
                          ...
                          updateReducedVal<P, OutT, IdxT, KVPair, ReduceOpT>(
                              mutex, min, val, red_op, m, tile_idx_m);
 ...
 */
                                   ...
                                   bool                   initOutBuffer)
            {
                ...
                static_assert(P::Mblk == 64,
                              "masked_l2_nn_impl only supports a policy with 64 rows per block.");
                ...
                RAFT_CUDA_TRY(cudaMemsetAsync(
                    ws_adj64.data(), 0, ws_adj64.size() * sizeof(uint64_t), stream));
                RAFT_CUDA_TRY(cudaMemsetAsync(
                    ws_fused_nn.data(), 0, ws_fused_nn.size() * sizeof(int), stream));

                // Compress boolean adjacency matrix to bitfield.
                auto adj_view = raft::make_device_matrix_view<const bool, int>(adj, m, num_groups);
                auto adj64_view = raft::make_device_matrix_view<uint64_t, int>(
                    ws_adj64.data(), m_div_64, num_groups);
                ...
                auto core_lambda
                    = [] __device__(DataT & acc, DataT & x, DataT & y) { acc += x * y; };
