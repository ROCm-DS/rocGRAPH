                            ...
                            tile_idx_n += grid_stride_n)
                        {
                            ...
                            if(distance_op.use_norms)
                            {
                                ...
                                distance_op.template epilog<Policy>(
                                    acc, regxn, regyn, tile_idx_n, tile_idx_m);
                            ...
                            else
                            {
                                ...
                                // https://en.cppreference.com/w/cpp/language/dependent_name)
                                distance_op.template epilog<Policy>(
                                    acc, nullptr, nullptr, tile_idx_n, tile_idx_m);
                                // And any possible additional epilogs
                                epilog_op(acc, nullptr, nullptr, tile_idx_n, tile_idx_m);
                            }
                            if(writeOut)
                            {
                                store_output(tile_idx_m, tile_idx_n);
                            }
                        }
...
#pragma unroll
                ...
                DI void accumulate()
                {
                    ...
                    constexpr int unroll_count
                        = decltype(distance_op)::expensive_inner_loop ? 1 : num_iterations;
...
#pragma unroll
                        for(int j = 0; j < P::AccColsPerTh; ++j)
                        {
                            ...
                            if(rowId < this->m && colId < this->n)
                            {
                                ...
                                dOutput[std::size_t(rowId) * this->n + colId]
                                    = fin_op(acc[i][j], 0);
                            }
                        }
                    }
