 ...
 */
    ...
    struct l2_exp_cutlass_op
    {
        ...
        __device__ l2_exp_cutlass_op() noexcept
            : sqrt(false)
        {
        }
        __device__ l2_exp_cutlass_op(bool isSqrt) noexcept
            : sqrt(isSqrt)
        {
        }
 ...
 */
    ...
    struct l2_exp_distance_op
    {
        ...
        l2_exp_distance_op(bool sqrt_) noexcept
            : sqrt(sqrt_)
        {
        }
...
#pragma unroll
         ...
         */
                    acc[i][j]
                        = val * (val > 0)
                          * !((val * val < get_clamp_precision<DataT>()) * (regxn[i] == regyn[j]));
                }
            }
            if(sqrt)
            {
