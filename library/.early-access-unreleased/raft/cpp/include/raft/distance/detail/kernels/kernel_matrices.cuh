            ...
            math_t* inout, int ld, int rows, int cols, bool is_row_major, cudaStream_t stream)
        {
            ...
            if(ld == n_minor)
            {
                polynomial_kernel_nopad<<<raft::ceildiv<size_t>((size_t)rows * cols, 128),
                                          128,
                                          0,
                                          stream>>>(inout, rows * cols, exponent, gain, offset);
            }
            else
            {
 ...
 */
    ...
    class TanhKernel : public GramMatrixBase<math_t>
    {
            ...
            math_t* inout, int ld, int rows, int cols, bool is_row_major, cudaStream_t stream)
        {
            ...
            if(ld == n_minor)
            {
                tanh_kernel_nopad<<<raft::ceildiv<size_t>((size_t)rows * cols, 128),
                                    128,
                                    0,
                                    stream>>>(inout, rows * cols, gain, offset);
            }
            else
            {
                int n1                         = is_row_major ? cols : rows;
                int n2                         = is_row_major ? rows : cols;
                auto [grid_shape, block_shape] = generateLaunchConfig2dElementwiseOp(n1, n2);
                tanh_kernel<<<grid_shape, block_shape, 0, stream>>>(
                    inout, ld, n1, n2, gain, offset);
            }
            RAFT_CUDA_TRY(cudaPeekAtLastError());
        }
   ...
   * @param offset
   */
        TanhKernel(math_t gain, math_t offset)
            : GramMatrixBase<math_t>()
            , gain(gain)
            , offset(offset)
        {
