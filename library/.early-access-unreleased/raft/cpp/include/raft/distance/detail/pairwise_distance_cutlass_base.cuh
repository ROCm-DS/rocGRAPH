    ...
    namespace distance
    {
        namespace detail
        {
            ...
            template <typename DataT,
                      ...
                      typename OpT,
                      bool isRowMajor>
            std::enable_if_t<ops::has_cutlass_op<OpT>::value>
                cutlassDistanceKernel(const DataT* x,
                                      const DataT* y,
                                      const DataT* xn,
                                      const DataT* yn,
                                      IdxT         m,
                                      ...
                                      cudaStream_t stream)
            {
                ...
                using EpilogueOutputOp
                    = cutlass::epilogue::thread::PairwiseDistanceEpilogueElementwise<
                        DataT, // ElementC_
                ...
                constexpr uint32_t max_batch_size
                    = gridYZMax * cutlassDistKernel::ThreadblockShape::kN;
                ...
                for(IdxT i = 0; i < numNbatches; i++)
                {
                    ...
                    RAFT_CUTLASS_TRY(
                        cutlassDist_op.initialize(arguments, workspace.data(), stream));
