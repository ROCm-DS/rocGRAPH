...
namespace raft
{
    namespace distance
    {
        namespace detail
        {
            ...
            RAFT_KERNEL initBinMutexKernel(cuda::binary_semaphore<cuda::thread_scope_device>* mut,
                                           IdxT                                               m)
            {
                                        ...
                                        cudaStream_t stream)
            {
                using EpilogueOutputOp
                    = cutlass::epilogue::thread::FusedDistanceNNEpilogueElementwise<
                        DataT, // ElementC_
                ...
                rmm::device_uvector<cuda::binary_semaphore<cuda::thread_scope_device>> bin_mutex(
                    m, stream);
                ...
                using fusedDistanceNNKernel = typename cutlass::gemm::kernel::FusedDistanceNNGemm<
                    DataT,
                    Alignment,
                    DataT,
                    Alignment,
                ...
                int           thread_blocks     = rowTiles < full_wave
                                                      ? (totalTiles < full_wave ? totalTiles : full_wave)
                                                      : rowTiles;
                ...
                RAFT_CUTLASS_TRY(
                    fusedDistanceNN_op.initialize(arguments, workspace.data(), stream));
