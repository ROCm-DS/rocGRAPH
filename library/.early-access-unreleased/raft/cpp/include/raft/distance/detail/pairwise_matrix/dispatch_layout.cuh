 ...
 */
    ...
    size_t alignment_of_2d_array(const DataT* x, size_t stride)
    {
        ...
        for(int align = 16; align >= 0; align /= 2)
        {
            ...
            if(base_aligned && stride_aligned)
            {
                return align;
            }
        }
 ...
 */
    ...
    int determine_vec_len(pairwise_matrix_params<IdxT, DataT, OutT, FinOpT> params)
    {
        ...
        int vec_len_aligned
            = (byte_alignment % sizeof(DataT) == 0) ? byte_alignment / sizeof(DataT) : 1;
 ...
 */
    ...
    auto dispatch_layout(bool row_major, int vec_len, F&& f)
    {
        if(row_major)
        {
            ...
            {
            case 4:
                return f(std::true_type(), vec_len_constant<4>());
            case 2:
                return f(std::true_type(), vec_len_constant<2>());
            default:
                return f(std::true_type(), vec_len_constant<1>());
            }
        }
        else
        {
            ...
            {
            case 4:
                return f(std::false_type(), vec_len_constant<4>());
            case 2:
                return f(std::false_type(), vec_len_constant<2>());
            default:
                return f(std::false_type(), vec_len_constant<1>());
            }
        }
    }
