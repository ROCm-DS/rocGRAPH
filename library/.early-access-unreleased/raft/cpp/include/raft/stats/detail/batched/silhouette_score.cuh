 ...
 */
                                                       ...
                                                       value_idx        dist_cols)
                {
                    ...
                    {
                        atomicAdd(&a[pw_row_id],
                                  distances[row_id * dist_cols + col_id]
                                                                  ...
                                                                  label_idx&             n_labels)
                {
                    ...
                    raft::stats::detail::countLabels(
                        y, cluster_counts.data(), n_rows, n_labels, workspace, stream);
                ...
                rmm::device_uvector<value_t>
                    get_pairwise_distance(raft::resources const&       handle,
                ...
                template <typename value_t, typename value_idx, typename label_idx>
                value_t silhouette_score(raft::resources const&       handle,
                                         const value_t*               X,
                                         value_idx                    n_rows,
                                         value_idx                    n_cols,
                                         const label_idx*             y,
                                         ...
                                         = raft::distance::DistanceType::L2Unexpanded)
                {
                    ...
                    for(value_idx i = 0; i < n_rows; i += chunk)
                    {
                        for(value_idx j = 0; j < n_rows; j += chunk)
                        {
                            ...
                            auto chunk_stream
                                = resource::get_next_usable_stream(handle, i + chunk * j);
                    ...
                    raft::linalg::
                        reduce<value_t, value_t, value_idx, raft::identity_op, raft::min_op>(
                            b_ptr,
                            b_ptr,
                            n_labels,
                            n_rows,
                    ...
                    raft::linalg::
                        binaryOp<value_t, raft::stats::detail::SilOp<value_t>, value_t, value_idx>(
                            a_ptr,
                            a_ptr,
                            b_ptr,
                            n_rows,
                            raft::stats::detail::SilOp<value_t>(),
                            stream);
