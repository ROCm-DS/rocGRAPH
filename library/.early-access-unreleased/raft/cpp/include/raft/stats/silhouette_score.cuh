 ...
 */
                               ...
                               = raft::distance::DistanceType::L2Unexpanded)
        {
            return detail::silhouette_score(handle,
                                            ...
                                            labels,
                                            nLabels,
                                            silhouette_scorePerSample,
                                            stream,
                                            metric);
        }

        template <typename value_t, typename value_idx, typename label_idx>
        value_t silhouette_score_batched(raft::resources const&       handle,
                                         value_t*                     X,
                                         value_idx                    n_rows,
                                         value_idx                    n_cols,
                                         label_idx*                   y,
 ...
 */
        ...
        {
            RAFT_EXPECTS(labels.extent(0) == X_in.extent(0),
                         "Size mismatch between labels and data");
 ...
 */
        template <typename value_t, typename label_t, typename idx_t>
        value_t
            silhouette_score(raft::resources const&                                          handle,
                             raft::device_matrix_view<const value_t, idx_t, raft::row_major> X_in,
                             raft::device_vector_view<const label_t, idx_t>                  labels,
                             ...
                             = raft::distance::DistanceType::L2Unexpanded)
        {
            std::optional<raft::device_vector_view<value_t, idx_t>> opt_scores
