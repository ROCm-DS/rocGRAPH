 ...
 */
            ...
            public:
                ...
                device_shared_wrapper_t(device_shared_wrapper_t&& other)
   ...
   */
                void set(rocgraph::mtmg::handle_t const& handle, wrapped_t&& obj)
                {
                    std::lock_guard<std::mutex> lock(lock_);

                    auto pos = objects_.find(handle.get_rank());
                    ROCGRAPH_EXPECTS(pos == objects_.end(), "Cannot overwrite wrapped object");
   ...
   */
                wrapped_t& get(rocgraph::mtmg::handle_t const& handle)
                {
                    std::lock_guard<std::mutex> lock(lock_);

                    auto pos = objects_.find(handle.get_rank());
                    ROCGRAPH_EXPECTS(pos != objects_.end(), "Uninitialized wrapped object");
   ...
   */
                wrapped_t const& get(rocgraph::mtmg::handle_t const& handle) const
                {
                    std::lock_guard<std::mutex> lock(lock_);
