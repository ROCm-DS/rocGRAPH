 ...
 *
 * A rocgraph edgelist needs to contain all of the edges necessary to create the graph
 ...
 */
   ...
   */
            ...
            {
                if(edgelist.use_weight())
                    wgt_ = std::make_optional(std::vector<weight_t>(thread_buffer_size));
   ...
   */
                        ...
                        std::optional<edge_type_t> edge_type)
            {
                ...
                if(wgt)
                    (*wgt_)[current_pos_] = *wgt;
                if(edge_id)
                    (*edge_id_)[current_pos_] = *edge_id;
                if(edge_type)
                    (*edge_type_)[current_pos_] = *edge_type;
   ...
   */
                        ...
                        std::optional<raft::host_span<edge_type_t const>> edge_type)
            {
                ...
                while(count > 0)
                {
                    ...
                    std::copy(src.begin() + pos,
                              src.begin() + pos + copy_count,
                              src_.begin() + current_pos_);
                    std::copy(dst.begin() + pos,
                              dst.begin() + pos + copy_count,
                              dst_.begin() + current_pos_);
                    if(wgt)
                        std::copy(wgt.begin() + pos,
                                  wgt.begin() + pos + copy_count,
                                  wgt_->begin() + current_pos_);
                    if(edge_id)
   ...
   */
            ...
            {
                edgelist_.append(handle,
                                 raft::host_span<vertex_t const>{src_.data(), current_pos_},
                                 raft::host_span<vertex_t const>{dst_.data(), current_pos_},
                                 wgt_ ? std::make_optional(raft::host_span<weight_t const>{
                                            wgt_->data(), current_pos_})
                                      : std::nullopt,
                                 edge_id_ ? std::make_optional(raft::host_span<edge_t const>{
                                                edge_id_->data(), current_pos_})
                                          : std::nullopt,
                                 edge_type_ ? std::make_optional(raft::host_span<edge_type_t const>{
                                                  edge_type_->data(), current_pos_})
                                            : std::nullopt);
