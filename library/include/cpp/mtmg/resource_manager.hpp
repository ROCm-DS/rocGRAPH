 ...
 */
   ...
   */
            void register_local_gpu(int global_rank, rmm::cuda_device_id local_device_id)
            {
                ...
                ROCGRAPH_EXPECTS(remote_rank_set_.find(global_rank) == remote_rank_set_.end(),
                                 "cannot register same global_rank as local and remote");
                ROCGRAPH_EXPECTS(local_rank_map_.find(global_rank) == local_rank_map_.end(),
                                 "cannot register same global_rank multiple times");

                int num_gpus_this_node;
                RAFT_CUDA_TRY(hipGetDeviceCount(&num_gpus_this_node));

                ROCGRAPH_EXPECTS((local_device_id.value() >= 0)
                                     && (local_device_id.value() < num_gpus_this_node),
    ...
    auto per_device_it = per_device_rmm_resources_.insert(
      std::pair{global_rank, std::make_shared<rmm::mr::cuda_memory_resource>()});
                ...
                rmm::mr::set_per_device_resource(local_device_id,
                                                 per_device_it.first->second.get());
            }
   ...
   */
            std::unique_ptr<instance_manager_t>
                create_instance_manager(std::vector<int> ranks_to_include,
                                        ...
                                        size_t           n_streams = 16) const
            {
                ...
                //        to be managed by RAFT instead of rocgraph::mtmg
                ...
                auto gpu_row_comm_size
                    = static_cast<int>(std::sqrt(static_cast<double>(ranks_to_include.size())));
                ...
                for(size_t i = 0; i < local_ranks_to_include.size(); ++i)
                {
                    ...
                    RAFT_NCCL_TRY(ncclCommInitRank(
                        nccl_comms[i].get(), ranks_to_include.size(), instance_manager_id, rank));
                ...
                for(size_t i = 0; i < local_ranks_to_include.size(); ++i)
                {
                    running_threads.emplace_back([instance_manager_id,
                        ...
                        rocgraph::partition_manager::init_subcomm(*handles[idx], gpu_row_comm_size);
                    });
                }

                std::for_each(
                    running_threads.begin(), running_threads.end(), [](auto& t) { t.join(); });
   ...
   */
            std::vector<int> registered_ranks() const
            {
                ...
                std::transform(local_rank_map_.begin(),
                               local_rank_map_.end(),
                               registered_ranks.begin(),
        ...
        private:
            ...
            std::map<int, std::shared_ptr<rmm::mr::device_memory_resource>>
