...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        struct typecast_t
        {
            ...
            {
                return static_cast<output_t>(val);
            }
        };
        ...
        struct pack_bool_t
        {
            ...
            {
                auto first = i * rocgraph::packed_bools_per_word();
                auto last  = std::min((i + 1) * rocgraph::packed_bools_per_word(), num_bools);
                packed_bool_container_t ret{0};
                for(auto j = first; j < last; ++j)
                {
                    if(*(bool_first + j))
                    {
                        auto mask = rocgraph::packed_bool_mask(j);
                        ret |= mask;
                    }
                }
                return ret;
        ...
        struct check_bit_set_t
        {
            ...
            static_assert(
                std::is_same_v<typename thrust::iterator_traits<PackedBoolIterator>::value_type,
                               packed_bool_container_t>);
            __device__ bool operator()(T idx) const
            {

                auto offset = idx - idx_first;
                return static_cast<bool>(*(bitmap_first + rocgraph::packed_bool_offset(offset))
                                         & rocgraph::packed_bool_mask(offset));
            }
        };
        ...
        struct indirection_t
        {
            ...
            __device__ typename thrust::iterator_traits<Iterator>::value_type
        ...
        struct is_equal_t
        {
            ...
            __device__ bool operator()(T val) const
            {
                return val == compare;
            }
        };
