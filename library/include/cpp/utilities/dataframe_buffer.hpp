...
namespace rocgraph
{
    ...
    namespace detail
    {
                                                  ...
                                                  rmm::cuda_stream_view stream_view)
        {
            return std::make_tuple(
                rmm::device_uvector<typename thrust::tuple_element<Is, TupleType>::type>(
    ...
    {
        static_assert(
            is_std_tuple_of_arithmetic_vectors<std::remove_cv_t<BufferType>>::value
        ...
        {
            std::apply(
                [new_buffer_capacity, stream_view](auto&&... args) {
                    (args.reserve(new_buffer_capacity, stream_view), ...);
                },
                buffer);
        }
        else
                                 ...
                                 rmm::cuda_stream_view stream_view)
    {
        ...
        if constexpr(is_std_tuple_of_arithmetic_vectors<std::remove_cv_t<BufferType>>::value)
        {
            std::apply([new_buffer_size, stream_view](
                           auto&&... args) { (args.resize(new_buffer_size, stream_view), ...); },
                       buffer);
        }
        else
        {
    ...
    void shrink_to_fit_dataframe_buffer(BufferType& buffer, rmm::cuda_stream_view stream_view)
    {
        ...
        if constexpr(is_std_tuple_of_arithmetic_vectors<std::remove_cv_t<BufferType>>::value)
        {
            std::apply([stream_view](auto&&... args) { (args.shrink_to_fit(stream_view), ...); },
                       buffer);
        }
        else
        {
