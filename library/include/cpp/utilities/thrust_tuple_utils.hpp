...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        template <typename TupleType, std::size_t... Is>
        constexpr TupleType
            thrust_tuple_of_arithmetic_numeric_limits_lowest(std::index_sequence<Is...>)
        {
            return thrust::make_tuple(
                std::numeric_limits<
                    typename thrust::tuple_element<Is, TupleType>::type>::lowest()...);
        }

        template <typename TupleType, std::size_t... Is>
        constexpr TupleType
            thrust_tuple_of_arithmetic_numeric_limits_max(std::index_sequence<Is...>)
        {
            return thrust::make_tuple(
    ...
    private:
        ...
        static constexpr bool is_valid
            = std::is_arithmetic_v<T> || std::is_same_v<T, thrust::null_type>;
    ...
    struct compute_thrust_tuple_element_sizes
    {
        auto operator()() const
        {
            ...
            detail::compute_thrust_tuple_element_sizes_impl<TupleType, size_t{0}, tuple_size>()
                .compute(ret);
            return ret;
        }
    };
    ...
    {
        __device__ typename thrust::tuple_element<I, TupleType>::type
            operator()(TupleType tup) const
        {
