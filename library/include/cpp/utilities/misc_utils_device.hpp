...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        std::tuple<std::vector<vertex_t>, std::vector<offset_t>>
            compute_offset_aligned_element_chunks(raft::handle_t const&             handle,
                                                  ...
                                                  vertex_t approx_element_chunk_size)
        {
            ...
            auto num_chunks
                = (num_elements + approx_element_chunk_size - 1) / approx_element_chunk_size;
            ...
            if(num_chunks > 1)
            {
                ...
                rmm::device_uvector<offset_t> d_element_offsets(d_chunk_offsets.size(),
                                                                handle.get_stream());
                                                         ...
                                                         rmm::cuda_stream_view stream_view)
        {
            ...
            {
                thrust::fill(
                    rmm::exec_policy(stream_view), results.begin(), results.end(), idx_t{0});
                ...
                thrust::for_each(
                    ...
                    offsets.begin() + 1,
                    offsets.end(),
                    [results = raft::device_span<idx_t>(results.data(), results.size())] __device__(
                        offset_t offset) {
                        if(offset < static_cast<offset_t>(results.size()))
                        {
                            hip::atomic_ref<idx_t, hip::thread_scope_device> atomic_counter(
                                results[offset]);
