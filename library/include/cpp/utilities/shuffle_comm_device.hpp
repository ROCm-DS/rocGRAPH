...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        struct compute_group_id_count_pair_t
        {
            ...
            {
                static_assert(
                    std::is_same_v<typename thrust::iterator_traits<GroupIdIterator>::value_type,
                                   int>);
                auto lower_it = thrust::lower_bound(
                    thrust::seq, group_id_first, group_id_last, static_cast<int>(i));
                auto upper_it = thrust::upper_bound(
                    thrust::seq, lower_it, group_id_last, static_cast<int>(i));
                return thrust::make_tuple(
                    static_cast<int>(i), static_cast<size_t>(thrust::distance(lower_it, upper_it)));
            }
        };
                                               ...
                                               rmm::cuda_stream_view              stream_view)
        {
            ...
            raft::update_host(
                tx_counts.data(), d_tx_value_counts.data(), comm_size, stream_view.value());
            raft::update_host(
                rx_counts.data(), d_rx_value_counts.data(), comm_size, stream_view.value());
            ...
            return std::make_tuple(
                tx_counts, tx_offsets, tx_dst_ranks, rx_counts, rx_offsets, rx_src_ranks);
        }
        ...
        struct key_group_id_less_t
        {
            ...
            int             pivot{};
            __device__ bool operator()(key_type k) const
            {
                return key_to_group_id_op(k) < pivot;
            }
        };
        ...
        struct value_group_id_less_t
        {
            ...
            int              pivot{};
            __device__ bool  operator()(value_type v) const
            {
                return value_to_group_id_op(v) < pivot;
            }
        };
                             ...
                             rmm::cuda_stream_view stream_view)
        {
            ...
            thrust::transform(
                ...
                [value_to_group_id_op, group_first, counts = counts.data()] __device__(
                    ...
                    return thrust::make_tuple(
                        group_id, counter.fetch_add(size_t{1}, hip::std::memory_order_relaxed));
                });
            ...
            auto tmp_value_buffer = allocate_dataframe_buffer<
                typename thrust::iterator_traits<ValueIterator>::value_type>(num_values,
                                                                             stream_view);
            auto input_triplet_first = thrust::make_zip_iterator(thrust::make_tuple(
                value_first, group_ids.begin(), intra_partition_offsets.begin()));
            ...
            thrust::for_each(
                ...
                [group_first,
                 ...
                 output_first
                 = get_dataframe_buffer_begin(tmp_value_buffer)] __device__(auto triplet) {
                             ...
                             rmm::cuda_stream_view stream_view)
        {
            ...
            auto tmp_key_buffer = allocate_dataframe_buffer<
                typename thrust::iterator_traits<KeyIterator>::value_type>(num_keys, stream_view);
            auto tmp_value_buffer = allocate_dataframe_buffer<
                typename thrust::iterator_traits<ValueIterator>::value_type>(num_keys, stream_view);
            auto input_quadraplet_first = thrust::make_zip_iterator(thrust::make_tuple(
                key_first, value_first, group_ids.begin(), intra_partition_offsets.begin()));
            auto tmp_kv_pair_first      = thrust::make_zip_iterator(
                thrust::make_tuple(get_dataframe_buffer_begin(tmp_key_buffer),
            ...
            thrust::for_each(rmm::exec_policy(stream_view),
                             ...
                             [group_first,
                              ...
                              output_first  = tmp_kv_pair_first] __device__(auto quadraplet) {
                                 ...
                                 auto offset   = displacements[group_id - group_first]
                                               + thrust::get<3>(quadraplet);
                                 *(output_first + offset) = thrust::make_tuple(
                                     thrust::get<0>(quadraplet), thrust::get<1>(quadraplet));
                             });
            thrust::copy(rmm::exec_policy(stream_view),
                         tmp_kv_pair_first,
                             ...
                             rmm::cuda_stream_view stream_view)
        {
            ...
            {
                auto tmp_key_buffer = allocate_dataframe_buffer<
                    typename thrust::iterator_traits<KeyIterator>::value_type>(first_partition_size,
                                                                               stream_view);
                auto tmp_value_buffer = allocate_dataframe_buffer<
                    typename thrust::iterator_traits<ValueIterator>::value_type>(
                    first_partition_size, stream_view);
        ...
        template <typename ValueIterator, typename ValueToGroupIdOp>
        ValueIterator
            mem_frugal_partition(ValueIterator    value_first,
                                 ValueIterator    value_last,
                                 ValueToGroupIdOp value_to_group_id_op,
                                 ...
                                 rmm::cuda_stream_view stream_view)
        {
            ...
            auto tmp_buffer = allocate_dataframe_buffer<
                typename thrust::iterator_traits<ValueIterator>::value_type>(second_size,
                                                                             stream_view);
            ...
            auto num_chunks
                = (num_elements + max_elements_per_iteration - 1) / max_elements_per_iteration;
            ...
            for(size_t i = 0; i < num_chunks; ++i)
            {
                output_chunk_first = thrust::copy_if(
                    ...
                    output_chunk_first,
                    value_group_id_greater_equal_t<
                        typename thrust::iterator_traits<ValueIterator>::value_type,
                        ValueToGroupIdOp>{value_to_group_id_op, pivot});
            }
        ...
        // favorable to the pool allocator).
        template <typename KeyIterator, typename ValueIterator, typename KeyToGroupIdOp>
        std::tuple<KeyIterator, ValueIterator>
            mem_frugal_partition(KeyIterator    key_first,
                                 KeyIterator    key_last,
                                 ValueIterator  value_first,
                                 KeyToGroupIdOp key_to_group_id_op,
                                 int pivot, // group Id less than pivot goes to the first partition
                                 rmm::cuda_stream_view stream_view)
        {
            ...
            auto first_size   = static_cast<size_t>(thrust::count_if(
                ...
                key_first,
                key_last,
                key_group_id_less_t<typename thrust::iterator_traits<KeyIterator>::value_type,
                                      KeyToGroupIdOp>{key_to_group_id_op, pivot}));
            auto second_size  = num_elements - first_size;

            auto tmp_key_buffer = allocate_dataframe_buffer<
                typename thrust::iterator_traits<KeyIterator>::value_type>(second_size,
                                                                           stream_view);
            auto tmp_value_buffer = allocate_dataframe_buffer<
                typename thrust::iterator_traits<ValueIterator>::value_type>(second_size,
                                                                             stream_view);
            ...
            auto kv_pair_first
                = thrust::make_zip_iterator(thrust::make_tuple(key_first, value_first));
            auto output_chunk_first = thrust::make_zip_iterator(
                thrust::make_tuple(get_dataframe_buffer_begin(tmp_key_buffer),
                                   get_dataframe_buffer_begin(tmp_value_buffer)));
            for(size_t i = 0; i < num_chunks; ++i)
            {
                output_chunk_first = thrust::copy_if(
                    ...
                    output_chunk_first,
                    kv_pair_group_id_greater_equal_t<
                        typename thrust::iterator_traits<KeyIterator>::value_type,
                        typename thrust::iterator_traits<ValueIterator>::value_type,
                        KeyToGroupIdOp>{key_to_group_id_op, pivot});
            }
        ...
        void mem_frugal_groupby(
            ...
            int              num_groups,
            size_t
                mem_frugal_threshold, // take the memory frugal approach (instead of thrust::sort) if #
    ...
    // validated. The else path should be eventually deleted.
                            ...
                            thrust::sort(rmm::exec_policy(stream_view),
                                         ...
                                         [value_to_group_id_op] __device__(auto lhs, auto rhs) {
                                             return value_to_group_id_op(lhs)
                                                    < value_to_group_id_op(rhs);
                                         });
#endif
                        }
                    }
                    else
                    {
                        ...
                        auto          num_elements = static_cast<size_t>(
                            thrust::distance(value_firsts[i], value_lasts[i]));
                        auto first_chunk_partition_first
                            = mem_frugal_partition(value_firsts[i],
                                                   value_firsts[i] + num_elements / 2,
                                                   value_to_group_id_op,
                                                   pivot,
                                                   stream_view);
                        auto second_chunk_partition_first
                            = mem_frugal_partition(value_firsts[i] + num_elements / 2,
                                                   value_lasts[i],
                                                   value_to_group_id_op,
                                                   pivot,
                                                   stream_view);
                        auto no_less_size = static_cast<size_t>(thrust::distance(
                            first_chunk_partition_first, value_firsts[i] + num_elements / 2));
                        auto less_size    = static_cast<size_t>(thrust::distance(
                            value_firsts[i] + num_elements / 2, second_chunk_partition_first));
                       ...
                       < mem_frugal_threshold)
                    {
                        if(group_lasts[i] - group_firsts[i] == 2)
                        {
                            auto kv_pair_first = thrust::make_zip_iterator(
                                thrust::make_tuple(key_firsts[i], value_firsts[i]));
                            thrust::partition(
                                rmm::exec_policy(stream_view),
                                kv_pair_first,
                                kv_pair_first + thrust::distance(key_firsts[i], key_lasts[i]),
                                kv_pair_group_id_less_t<
                                    typename thrust::iterator_traits<KeyIterator>::value_type,
                                    typename thrust::iterator_traits<ValueIterator>::value_type,
                                    KeyToGroupIdOp>{key_to_group_id_op, pivot});
                        }
    ...
    // validated. The else path should be eventually deleted.
                    ...
                    else
                    {
                        ...
                        auto                                   num_elements
                            = static_cast<size_t>(thrust::distance(key_firsts[i], key_lasts[i]));
                        auto first_chunk_partition_first
                            = mem_frugal_partition(key_firsts[i],
                                                   ...
                                                   stream_view);
                        auto second_chunk_partition_first
                            = mem_frugal_partition(key_firsts[i] + num_elements / 2,
                                                   key_lasts[i],
                                                   value_firsts[i] + num_elements / 2,
                                                   key_to_group_id_op,
                                                   pivot,
                        ...
                        auto no_less_size = static_cast<size_t>(
                            thrust::distance(std::get<0>(first_chunk_partition_first),
                                             key_firsts[i] + num_elements / 2));
                        auto less_size = static_cast<size_t>(
                            thrust::distance(key_firsts[i] + num_elements / 2,
                                             std::get<0>(second_chunk_partition_first)));
                        ...
                        second_first = std::make_tuple(
                            key_firsts[i] + ((num_elements / 2 - no_less_size) + less_size),
                            value_firsts[i] + ((num_elements / 2 - no_less_size) + less_size));
                        if(pivot - group_firsts[i] > 1)
                        {
                            group_firsts.push_back(group_firsts[i]);
                                                  ...
                                                  rmm::cuda_stream_view stream_view)
    {
        ...
        auto group_id_first = thrust::make_transform_iterator(
            tx_value_first, [value_to_group_id_op] __device__(auto value) -> int {
                        ...
                        rmm::cuda_stream_view       stream_view)
    {
        ...
        auto rx_value_buffer = allocate_dataframe_buffer<
            typename thrust::iterator_traits<TxValueIterator>::value_type>(
                                             ...
                                             rmm::cuda_stream_view stream_view)
    {
        ...
        rmm::device_uvector<typename thrust::iterator_traits<VertexIterator>::value_type> rx_keys(
            rx_offsets.size() > 0 ? rx_offsets.back() + rx_counts.back() : size_t{0}, stream_view);
        auto rx_value_buffer = allocate_dataframe_buffer<
            typename thrust::iterator_traits<ValueIterator>::value_type>(rx_keys.size(),
                                                                         stream_view);
