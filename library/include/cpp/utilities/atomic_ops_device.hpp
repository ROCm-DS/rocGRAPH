...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        __device__ constexpr TupleType
            thrust_tuple_atomic_and(Iterator iter, TupleType tup, std::index_sequence<Is...>)
        {
            return thrust::make_tuple(atomicAnd(
                &(thrust::raw_reference_cast(thrust::get<Is>(*iter))), thrust::get<Is>(tup))...);
        }

        template <typename Iterator, typename TupleType, std::size_t... Is>
        __device__ constexpr TupleType
            thrust_tuple_atomic_or(Iterator iter, TupleType tup, std::index_sequence<Is...>)
        {
            return thrust::make_tuple(atomicOr(
                &(thrust::raw_reference_cast(thrust::get<Is>(*iter))), thrust::get<Is>(tup))...);
        }

        template <typename Iterator, typename TupleType, std::size_t... Is>
        __device__ constexpr TupleType
            thrust_tuple_atomic_add(Iterator iter, TupleType tup, std::index_sequence<Is...>)
        {
            return thrust::make_tuple(atomicAdd(
                &(thrust::raw_reference_cast(thrust::get<Is>(*iter))), thrust::get<Is>(tup))...);
        }

        template <typename Iterator, typename TupleType, std::size_t... Is>
        __device__ constexpr TupleType thrust_tuple_elementwise_atomic_cas(
            Iterator iter, TupleType comp_tup, TupleType val_tup, std::index_sequence<Is...>)
        {
            return thrust::make_tuple(
                atomicCAS(&(thrust::raw_reference_cast(thrust::get<Is>(*iter))),
                          thrust::get<Is>(comp_tup),
                          thrust::get<Is>(val_tup))...);
        }

        template <typename Iterator, typename TupleType, std::size_t... Is>
        __device__ constexpr TupleType thrust_tuple_elementwise_atomic_min(
            Iterator iter, TupleType tup, std::index_sequence<Is...>)
        {
            return thrust::make_tuple(atomicMin(
                &(thrust::raw_reference_cast(thrust::get<Is>(*iter))), thrust::get<Is>(tup))...);
        }

        template <typename Iterator, typename TupleType, std::size_t... Is>
        __device__ constexpr TupleType thrust_tuple_elementwise_atomic_max(
            Iterator iter, TupleType tup, std::index_sequence<Is...>)
        {
            return thrust::make_tuple(atomicMax(
                &(thrust::raw_reference_cast(thrust::get<Is>(*iter))), thrust::get<Is>(tup))...);
        }
    ...
    template <typename Iterator, typename T>
    __device__ std::enable_if_t<
        is_thrust_tuple<typename thrust::iterator_traits<Iterator>::value_type>::value
            && is_thrust_tuple<T>::value,
        void>
        atomic_add(Iterator iter, T value)
    {
