...
*******************************************/
        ...
        public:
            ...
            // moved the packed_bool test to classes below that use them.
            //static_assert(std::is_same_v<typename thrust::iterator_traits<ValueIterator>::value_type, value_t>);
            ...
            edge_major_property_view_t(
                ...
                raft::host_span<raft::device_span<vertex_t const> const>
                                                  edge_partition_key_chunk_start_offsets,
            ...
            {
                return key_chunk_size_;
            }

            std::vector<ValueIterator> const& value_firsts() const
            {
                return edge_partition_value_firsts_;
            }
...
*******************************************/
        ...
        public:
            ...
            ValueIterator value_first() const
            {
                return value_first_;
            }
        ...
        public:
            ...
            using buffer_type
                = decltype(allocate_dataframe_buffer<
                           std::conditional_t<std::is_same_v<T, bool>, packed_bool_container_t, T>>(
                ...
                for(size_t i = 0; i < edge_partition_major_range_firsts_.size(); ++i)
                {
                    ...
                    buffer_size
                        = std::is_same_v<T, bool>
                              ? rocgraph::packed_bool_size(edge_partition_major_range_sizes[i])
                              : edge_partition_major_range_sizes[i];

                    buffers_.push_back(
                        allocate_dataframe_buffer<std::conditional_t<std::is_same_v<T, bool>,
                ...
                , edge_partition_major_range_firsts_(edge_partition_major_range_firsts)
            {
                ...
                for(size_t i = 0; i < edge_partition_major_range_firsts_.size(); ++i)
                {
                    ...
                    buffer_size = std::is_same_v<T, bool>
                                      ? rocgraph::packed_bool_size(edge_partition_keys[i].size())
        ...
        *******************************************/
        ...
        public:
            ...
            edge_minor_property_t(raft::handle_t const& handle,
                ...
                buffer_(allocate_dataframe_buffer<
                        std::conditional_t<std::is_same_v<T, bool>, packed_bool_container_t, T>>(
                    std::is_same_v<T, bool> ? rocgraph::packed_bool_size(buffer_size) : buffer_size,
                    handle.get_stream()))
                , minor_range_first_(minor_range_first)
            {
            }
            ...
            edge_minor_property_t(raft::handle_t const&             handle,
                ...
                , buffer_(allocate_dataframe_buffer<
                          std::conditional_t<std::is_same_v<T, bool>, packed_bool_container_t, T>>(
                      std::is_same_v<T, bool> ? rocgraph::packed_bool_size(keys.size())
                                              : keys.size(),
                      handle.get_stream()))
                , minor_range_first_(minor_range_first)
            {
    ...
    public:
            ...
            : property_(handle)
        {
                    ...
                    else
                    {
                        ...
                        edge_partition_key_chunk_start_offsets_
                            = std::vector<raft::device_span<vertex_t const>>(
                                graph_view.number_of_local_edge_partitions());
                        std::vector<vertex_t> major_range_firsts(
                            graph_view.number_of_local_edge_partitions());
                        for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
                        {
                            (*edge_partition_keys_)[i]
                                = *(graph_view.local_sorted_unique_edge_srcs(i));
                            (*edge_partition_key_chunk_start_offsets_)[i]
                                = *(graph_view.local_sorted_unique_edge_src_chunk_start_offsets(i));
                            major_range_firsts[i]
                        ...
                        property_ = detail::edge_major_property_t<vertex_t, T>(
                            ...
                            raft::host_span<raft::device_span<vertex_t const>>(
                                (*edge_partition_keys_).data(), (*edge_partition_keys_).size()),
                            raft::host_span<raft::device_span<vertex_t const>>(
            ...
            else
            {
                ...
                {
                    std::vector<vertex_t> major_range_sizes(
                        graph_view.number_of_local_edge_partitions(), vertex_t{0});
                    std::vector<vertex_t> major_range_firsts(
                        graph_view.number_of_local_edge_partitions());
                    for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
                    {
            ...
            if(key_chunk_size)
            {
                if constexpr(GraphViewType::is_multi_gpu)
                {
                    if constexpr(GraphViewType::is_storage_transposed)
                    {
                        ...
                        {
                            (*edge_partition_keys_)[i]
                                = *(graph_view.local_sorted_unique_edge_dsts(i));
                            (*edge_partition_key_chunk_start_offsets_)[i]
...
*******************************************/
    ...
    public:
        ...
        auto view() const
        {
            return detail::edge_endpoint_dummy_property_view_t{};
        }
    };
...
*******************************************/
    ...
    auto view_concat(detail::edge_major_property_view_t<vertex_t, Iters, Types> const&... views)
    {
        ...
        {
            return detail::
                edge_major_property_view_t<vertex_t, concat_value_iterator, concat_value_type>(
                    *(first_view.keys()),
...
*******************************************/
    ...
    auto view_concat(detail::edge_minor_property_view_t<vertex_t, Iters, Types> const&... views)
    {
        ...
        {
            return detail::
                edge_minor_property_view_t<vertex_t, concat_value_iterator, concat_value_type>(
                    *(first_view.keys()),
                    *(first_view.key_chunk_start_offsets()),
                    *(first_view.key_chunk_size()),
