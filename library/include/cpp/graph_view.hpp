 ...
 * minor_comm_rank). rocgraph::partition_manager is responsible for mapping (major_comm_rank,
 ...
 */
    ...
    public:
                      ...
                      major_comm_size, minor_comm_size, major_comm_rank, minor_comm_rank))
        {
            ...
            ROCGRAPH_EXPECTS(
                std::is_sorted(vertex_partition_range_offsets_.begin(),
            ...
            for(size_t i = 0; i < edge_partition_major_value_start_offsets_.size(); ++i)
            {
                ...
                start_offset += local_edge_partition_major_range_last(i)
                                - local_edge_partition_major_range_first(i);
            }
        }
        ...
        {
            return vertex_partition_range_offsets_[partition_idx * major_comm_size_
                                                   + major_comm_rank_];
        }
        ...
        {
            return local_edge_partition_minor_range_last()
                   - local_edge_partition_minor_range_first();
        }
...
#ifndef NDEBUG
#warning low_degree_threshold does not seem to be used here and can't be initialized as constexpr using host_warp_size.
#endif
        ...
        public:
            ...
            graph_base_t(vertex_t           number_of_vertices,
            ...
            std::enable_if_t<std::is_signed<vertex_type>::value, bool>
                is_valid_vertex(vertex_type v) const
            {
                return ((v >= 0) && (v < number_of_vertices_));
            }

            template <typename vertex_type = vertex_t>
            std::enable_if_t<std::is_unsigned<vertex_type>::value, bool>
                is_valid_vertex(vertex_type v) const
            {
            ...
            {
                return properties_.is_symmetric;
            }
            bool is_multigraph() const
            {
                return properties_.is_multigraph;
            }
    ...
    public:
        ...
        vertex_t local_edge_partition_src_range_size() const
        {
            ...
            { // source range can be non-contiguous
                vertex_t ret{0};
        ...
        std::enable_if_t<!transposed, vertex_t>
            local_edge_partition_src_range_size(size_t partition_idx) const
        {
        ...
        template <bool transposed = is_storage_transposed>
        std::enable_if_t<!transposed, vertex_t>
            local_edge_partition_src_range_first(size_t partition_idx) const
        {
        ...
        template <bool transposed = is_storage_transposed>
        std::enable_if_t<!transposed, vertex_t>
            local_edge_partition_src_range_last(size_t partition_idx) const
        {
        ...
        template <bool transposed = is_storage_transposed>
        std::enable_if_t<!transposed, vertex_t>
            local_edge_partition_src_value_start_offset(size_t partition_idx) const
        {
        ...
        {
            if constexpr(store_transposed)
            { // destination range can be non-contiguous
                vertex_t ret{0};
                for(size_t i = 0; i < partition_.number_of_local_edge_partitions(); ++i)
                {
                    ret += partition_.local_edge_partition_major_range_size(i);
        ...
        template <bool transposed = is_storage_transposed>
        std::enable_if_t<transposed, vertex_t>
            local_edge_partition_dst_range_size(size_t partition_idx) const
        {
        ...
        template <bool transposed = is_storage_transposed>
        std::enable_if_t<transposed, vertex_t>
            local_edge_partition_dst_range_first(size_t partition_idx) const
        {
            return partition_.local_edge_partition_major_range_first(partition_idx);
        }
        ...
        template <bool transposed = is_storage_transposed>
        std::enable_if_t<transposed, vertex_t>
            local_edge_partition_dst_range_last(size_t partition_idx) const
        {
            return partition_.local_edge_partition_major_range_last(partition_idx);
        }
        ...
        template <bool transposed = is_storage_transposed>
        std::enable_if_t<transposed, vertex_t>
            local_edge_partition_dst_value_start_offset(size_t partition_idx) const
        {
        ...
        std::optional<std::vector<vertex_t>>
            local_edge_partition_segment_offsets(size_t partition_idx) const
        {
            auto num_segments_per_vertex_partition
                = edge_partition_segment_offsets_.size() / edge_partition_offsets_.size();
            return std::vector<vertex_t>(edge_partition_segment_offsets_.begin()
                                             + partition_idx * num_segments_per_vertex_partition,
        ...
        {
            return vertex_partition_view_t<vertex_t, true>(
                this->number_of_vertices(),
                this->local_vertex_partition_range_first(),
                this->local_vertex_partition_range_last());
        }

        edge_partition_view_t<vertex_t, edge_t, true>
            local_edge_partition_view(size_t partition_idx) const
        {
        ...
        {
            ROCGRAPH_EXPECTS(!(this->has_edge_mask()), "unimplemented.");
        ...
        void attach_edge_mask(
            edge_property_view_t<edge_t, packed_bool_container_t const*, bool> edge_mask_view)
        {
        ...
        std::optional<edge_property_view_t<edge_t, packed_bool_container_t const*, bool>>
            edge_mask_view() const
        {
    ...
    public:
        ...
        constexpr vertex_t local_vertex_partition_range_first() const
        {
            return vertex_t{0};
        }
        ...
        constexpr bool in_local_vertex_partition_range_nocheck(vertex_t v) const
        {
            return true;
        }

        constexpr size_t number_of_local_edge_partitions() const
        {
            return size_t(1);
        }
        ...
        std::optional<std::vector<vertex_t>>
            local_edge_partition_segment_offsets(size_t partition_idx = 0) const
        {
            assert(partition_idx == 0);
        ...
        edge_partition_view_t<vertex_t, edge_t, false>
            local_edge_partition_view(size_t partition_idx = 0) const
        {
