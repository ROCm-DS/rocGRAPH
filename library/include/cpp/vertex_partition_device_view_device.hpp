...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            __host__ __device__ std::enable_if_t<std::is_signed<vertex_type>::value, bool>
                                is_valid_vertex(vertex_type v) const noexcept
            {
                return ((v >= 0) && (v < number_of_vertices_));
            }

            template <typename vertex_type = vertex_t>
            __host__ __device__ std::enable_if_t<std::is_unsigned<vertex_type>::value, bool>
                                is_valid_vertex(vertex_type v) const noexcept
            {
    ...
    public:
        ...
        __host__ __device__ bool in_local_vertex_partition_range_nocheck(vertex_t v) const noexcept
        {
            return (v >= local_vertex_partition_range_first_)
                   && (v < local_vertex_partition_range_last_);
        }
    ...
    public:
        ...
        __host__ __device__ constexpr bool
            in_local_vertex_partition_range_nocheck(vertex_t v) const noexcept
        {
            return true;
        }

        __host__ __device__ constexpr vertex_t
            local_vertex_partition_offset_from_vertex_nocheck(vertex_t v) const noexcept
        {
            return v;
        }
