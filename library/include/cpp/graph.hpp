...
/** @defgroup cpp_api rocGRAPH C++ API
 *  @{
 */
...
struct graph_meta_t<vertex_t, edge_t, multi_gpu, std::enable_if_t<multi_gpu>> {
  ...
  vertex_t num_local_unique_edge_dsts{};
    ...
    public:
        ...
        graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu> view() const
        {
            ...
            for(size_t i = 0; i < offsets.size(); ++i)
            {
                ...
                if(dcs_nzd_vertices)
                {
                    (*dcs_nzd_vertices)[i] = raft::device_span<vertex_t const>(
                        (*edge_partition_dcs_nzd_vertices_)[i].data(),
            ...
            if(local_sorted_unique_edge_srcs_)
            {
                ...
                { // minor
                    local_sorted_unique_edge_srcs = raft::device_span<vertex_t const>(
                        (*local_sorted_unique_edge_srcs_).begin(),
                        (*local_sorted_unique_edge_srcs_).end());
                ...
                { // major
                    local_sorted_unique_edge_srcs = std::vector<raft::device_span<vertex_t const>>(
                    ...
                    {
                        (*local_sorted_unique_edge_srcs)[i] = raft::device_span<vertex_t const>(
                            (*local_sorted_unique_edge_srcs_)[i].begin(),
            ...
            if(local_sorted_unique_edge_dsts_)
            {
                if constexpr(store_transposed)
                { // major
                    local_sorted_unique_edge_dsts = std::vector<raft::device_span<vertex_t const>>(
                    ...
                    {
                        (*local_sorted_unique_edge_dsts)[i] = raft::device_span<vertex_t const>(
                            (*local_sorted_unique_edge_dsts_)[i].begin(),
                ...
                { // minor
                    local_sorted_unique_edge_dsts = raft::device_span<vertex_t const>(
                        (*local_sorted_unique_edge_dsts_).begin(),
                        (*local_sorted_unique_edge_dsts_).end());
                    local_sorted_unique_edge_dst_chunk_start_offsets
                        = raft::device_span<vertex_t const>(
    ...
    template <typename vertex_t>
    __host__ __device__ std::enable_if_t<std::is_signed<vertex_t>::value, bool>
                        is_valid_vertex(vertex_t num_vertices, vertex_t v)
    {
        return (v >= 0) && (v < num_vertices);
    }

    template <typename vertex_t>
    __host__ __device__ std::enable_if_t<std::is_unsigned<vertex_t>::value, bool>
                        is_valid_vertex(vertex_t num_vertices, vertex_t v)
    {
