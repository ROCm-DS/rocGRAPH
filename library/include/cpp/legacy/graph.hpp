 ...
 */
   ...
   */
            ...
            void set_handle(raft::handle_t* handle_in)
            {
                handle = handle_in;
            }
 ...
 */
   ...
   */
            GraphCOOView()
                : GraphViewBase<vertex_t, edge_t, weight_t>(nullptr, 0, 0)
            {
            }
   ...
   */
            GraphCOOView(vertex_t* src_indices,
                         ...
                         vertex_t  number_of_vertices,
                         edge_t    number_of_edges)
                : GraphViewBase<vertex_t, edge_t, weight_t>(
                      edge_data, number_of_vertices, number_of_edges)
 ...
 */
   ...
   */
            ...
            GraphCOO(GraphCOOView<vertex_t, edge_t, weight_t> const& graph,
                ...
                , src_indices_p(
                      graph.src_indices, graph.number_of_edges * sizeof(vertex_t), stream, mr)
                , dst_indices_p(
                      graph.dst_indices, graph.number_of_edges * sizeof(vertex_t), stream, mr)
            {
                if(graph.has_data())
                {
                    edge_data_p = rmm::device_buffer{
                        graph.edge_data, graph.number_of_edges * sizeof(weight_t), stream, mr};
                }
            }
            ...
            vertex_t number_of_vertices(void)
            {
                return number_of_vertices_p;
            }
            edge_t number_of_edges(void)
            {
                return number_of_edges_p;
            }
            vertex_t* src_indices(void)
            {
                return static_cast<vertex_t*>(src_indices_p.data());
            }
            vertex_t* dst_indices(void)
            {
                return static_cast<vertex_t*>(dst_indices_p.data());
            }
            weight_t* edge_data(void)
            {
                return static_cast<weight_t*>(edge_data_p.data());
            }
            ...
            {
                return GraphCOOView<vertex_t, edge_t, weight_t>(src_indices(),
                                                                dst_indices(),
                                                                edge_data(),
                                                                number_of_vertices_p,
                                                                number_of_edges_p);
            }

            bool has_data(void)
            {
                return nullptr != edge_data_p.data();
            }
        };
 ...
 */
   ...
   */
            ...
            edge_t* offsets(void)
            {
                return static_cast<edge_t*>(offsets_p.data());
            }
            vertex_t* indices(void)
            {
                return static_cast<vertex_t*>(indices_p.data());
            }
            weight_t* edge_data(void)
            {
                return static_cast<weight_t*>(edge_data_p.data());
