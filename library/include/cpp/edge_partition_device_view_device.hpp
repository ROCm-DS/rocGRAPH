...
namespace rocgraph
{
    ...
    namespace detail
    {
            ...
            raft::device_span<vertex_t const> dcs_nzd_vertices, vertex_t major)
        {
            ...
            auto it = thrust::lower_bound(
                thrust::seq, dcs_nzd_vertices.begin(), dcs_nzd_vertices.end(), major);
                    ...
                    if constexpr(use_dcs)
                    {
                        ...
                        else
                        {
                            auto major_hypersparse_idx
                                = major_hypersparse_idx_from_major_nocheck_impl(dcs_nzd_vertices,
                                                                                major);
                            if(major_hypersparse_idx)
                            {
                                idx = (major_hypersparse_first - major_range_first)
            ...
            __device__ return_type_t operator()(vertex_t major) const
            {
                if constexpr(multi_gpu)
                {
                    ...
                    if constexpr(use_dcs)
                    {
                        ...
                        {
                            idx = major - major_range_first;
                            return static_cast<return_type_t>(count_set_bits(
                                mask_first, offsets[idx], offsets[idx + 1] - offsets[idx]));
                        }
                        else
                        {
                            auto major_hypersparse_idx
                ...
                {
                    return static_cast<return_type_t>(count_set_bits(
                        mask_first, offsets[major], offsets[major + 1] - offsets[major]));
                }
            }
        };
        ...
        public:
            ...
            __host__ __device__ edge_t const* offsets() const
            {
                return offsets_.data();
            }
            __host__ __device__ vertex_t const* indices() const
            {
                return indices_.data();
            }

            __device__ vertex_t
                major_idx_from_local_edge_idx_nocheck(edge_t local_edge_idx) const noexcept
            {
                return static_cast<vertex_t>(thrust::distance(
                    offsets_.begin() + 1,
                    thrust::upper_bound(
                        thrust::seq, offsets_.begin() + 1, offsets_.end(), local_edge_idx)));
            }

            // major_idx == major offset if CSR/CSC, major_offset != major_idx if DCSR/DCSC
            __device__ thrust::tuple<vertex_t const*, edge_t, edge_t>
    ...
    public:
        edge_partition_device_view_t(edge_partition_view_t<vertex_t, edge_t, multi_gpu> view)
            : detail::edge_partition_device_view_base_t<vertex_t, edge_t>(view.offsets(),
                                                                          view.indices())
        ...
        rmm::device_uvector<edge_t> compute_local_degrees(MajorIterator         major_first,
                                                          ...
                                                          rmm::cuda_stream_view stream) const
        {
            rmm::device_uvector<edge_t> local_degrees(thrust::distance(major_first, major_last),
                                                      stream);
            if(dcs_nzd_vertices_)
            {
                assert(major_hypersparse_first_);
                                            ...
                                            rmm::cuda_stream_view stream) const
        {
            ...
            if(dcs_nzd_vertices_)
            {
                ...
                thrust::transform(rmm::exec_policy(stream),
                                  ...
                                  thrust::make_counting_iterator(this->major_range_last()),
                                  local_degrees.begin(),
                                  detail::local_degree_with_mask_op_t<vertex_t,
                                                                      edge_t,
                                                                      edge_t,
                                                                      multi_gpu,
                                                                      true,
        ...
        __host__ __device__ vertex_t major_range_first() const noexcept
        {
            return major_range_first_;
        }

        __host__ __device__ vertex_t major_range_last() const noexcept
        {
            return major_range_last_;
        }
        ...
        __host__ __device__ vertex_t minor_range_first() const noexcept
        {
            return minor_range_first_;
        }

        __host__ __device__ vertex_t minor_range_last() const noexcept
        {
            return minor_range_last_;
        }
        ...
        __device__ thrust::optional<vertex_t>
                   major_idx_from_major_nocheck(vertex_t major) const noexcept
        {
            if(major_hypersparse_first_ && (major >= *major_hypersparse_first_))
            {
                auto major_hypersparse_idx = detail::major_hypersparse_idx_from_major_nocheck_impl(
                    *dcs_nzd_vertices_, major);
        ...
        __device__ thrust::optional<vertex_t>
                   major_hypersparse_idx_from_major_nocheck(vertex_t major) const noexcept
        {
            if(dcs_nzd_vertices_)
            {
                return detail::major_hypersparse_idx_from_major_nocheck_impl(*dcs_nzd_vertices_,
                                                                             major);
            }
            else
            {
        ...
        // major_hypersparse_idx: index within the hypersparse segment
        __device__ thrust::optional<vertex_t>
            major_from_major_hypersparse_idx_nocheck(vertex_t major_hypersparse_idx) const noexcept
        {
            return dcs_nzd_vertices_
        ...
        {
            return dcs_nzd_vertices_
                       ? thrust::optional<vertex_t const*>{(*dcs_nzd_vertices_).data()}
                       : thrust::nullopt;
        }

        __host__ __device__ thrust::optional<vertex_t> dcs_nzd_vertex_count() const
        {
            return dcs_nzd_vertices_ ? thrust::optional<vertex_t>{static_cast<vertex_t>(
                                           (*dcs_nzd_vertices_).size())}
                                     : thrust::nullopt;
        }
    ...
    public:
        ...
        __host__ __device__ constexpr vertex_t major_range_first() const noexcept
        {
            return vertex_t{0};
        }
        ...
        __host__ __device__ constexpr vertex_t minor_range_first() const noexcept
        {
            return vertex_t{0};
        }
