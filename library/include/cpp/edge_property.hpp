...
namespace rocgraph
{
    ...
    public:
        ...
        std::vector<edge_t> const& edge_counts() const
        {
            return edge_partition_edge_counts_;
        }
    ...
    public:
        ...
        edge_property_t(raft::handle_t const& handle, GraphViewType const& graph_view)
        {
            ...
            for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
            {
                auto num_edges = static_cast<size_t>(
                    graph_view.local_edge_partition_view(i).number_of_edges());
                size_t buffer_size;

                buffer_size
                    = std::is_same_v<T, bool> ? rocgraph::packed_bool_size(num_edges) : num_edges;
        ...
        template <typename value_type = T,
                  typename            = std::enable_if_t<!std::is_same_v<value_type, bool>>>
        edge_property_t(std::vector<buffer_type>&& buffers)
        ...
        auto view() const
        {
            ...
            return edge_property_view_t<edge_type, const_value_iterator, T>(
