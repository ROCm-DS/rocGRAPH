...
namespace rocgraph
{
    ...
    namespace detail
    {
            ...
            ValueIterator                        value_iter)
        {
            ...
            auto const tid = threadIdx.x + blockIdx.x * blockDim.x;
            static_assert(transform_reduce_e_by_src_dst_key_kernel_block_size % raft::warp_size()
                          == 0);
            ...
            ValueIterator                        value_iter)
        {
            ...
            using BlockScan
                = hipcub::BlockScan<edge_t, transform_reduce_e_by_src_dst_key_kernel_block_size>;
        ...
        std::tuple<rmm::device_uvector<vertex_t>, BufferType>
            reduce_to_unique_kv_pairs(rmm::device_uvector<vertex_t>&& keys,
                                      ...
                                      hipStream_t                     stream)
        {
            ...
            auto                          value_for_unique_key_buffer
                = allocate_dataframe_buffer<value_t>(unique_keys.size(), stream);
                                              ...
                                              ReduceOp                  reduce_op)
        {
            ...
            for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
            {
                ...
                if(edge_partition_e_mask)
                {
                    ...
                    edge_offsets_with_mask = rmm::device_uvector<edge_t>(
                        edge_partition.major_range_size() + 1, handle.get_stream());
                ...
                auto tmp_value_buffer
                    = allocate_dataframe_buffer<T>(tmp_keys.size(), handle.get_stream());
                    ...
                    if(segment_offsets)
                    {
                        ...
                        if((*segment_offsets)[1] > 0)
                        {
                            ...
                            THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                                (detail::transform_reduce_by_src_dst_key_high_degree<
                                    edge_src_key,
                                    GraphViewType>),
                                ...
                                edge_offsets_with_mask
                                    ? thrust::make_optional<raft::device_span<edge_t const>>(
                                          (*edge_offsets_with_mask).data(),
                                          (*edge_offsets_with_mask).size())
                                    : thrust::nullopt,
                                e_op,
                                tmp_keys.data(),
                        ...
                        if((*segment_offsets)[2] - (*segment_offsets)[1] > 0)
                        {
                            raft::grid_1d_warp_t update_grid(
                                ...
                                handle.get_device());
                            THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                                (detail::transform_reduce_by_src_dst_key_mid_degree<edge_src_key,
                                                                                    GraphViewType>),
                                dim3(update_grid.num_blocks),
                                dim3(update_grid.block_size),
                                0,
                        ...
                        if((*segment_offsets)[3] - (*segment_offsets)[2] > 0)
                        {
                            raft::grid_1d_thread_t update_grid(
                                ...
                                handle.get_device_properties().maxGridSize[0]);
                            THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                                (detail::transform_reduce_by_src_dst_key_low_degree<edge_src_key,
                                                                                    GraphViewType>),
                                dim3(update_grid.num_blocks),
                                dim3(update_grid.block_size),
                                0,
                           ...
                           && (*(edge_partition.dcs_nzd_vertex_count()) > 0))
                        {
                            raft::grid_1d_thread_t update_grid(
                                *(edge_partition.dcs_nzd_vertex_count()),
                                ...
                                handle.get_device_properties().maxGridSize[0]);
                            THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                                (detail::transform_reduce_by_src_dst_key_hypersparse<
                                    edge_src_key,
                                    GraphViewType>),
                                dim3(update_grid.num_blocks),
                                dim3(update_grid.block_size),
                ...
                std::tie(tmp_keys, tmp_value_buffer)
                    = reduce_to_unique_kv_pairs<vertex_t, T>(std::move(tmp_keys),
                ...
                if(GraphViewType::is_multi_gpu)
                {
                    ...
                    auto                          rx_value_for_unique_key_buffer
 ...
 * assigned to this process in multi-GPU). Use  rocgraph::edge_src_property_t::view(). Use
 ...
 * rocgraph::edge_dst_property_t::view(). Use update_edge_dst_property to fill the wrapper.
 * @param edge_value_input Wrapper used to access edge input property values (for the edges assigned
 * to this process in multi-GPU). Use either rocgraph::edge_property_t::view() (if @p e_op needs to
 * access edge property values) or rocgraph::edge_dummy_property_t::view() (if @p e_op does not
 * access edge property values).
