 ...
 * There are pre-defined reduction operators in include/rocgraph/prims/reduce_op.cuh. It is
 ...
 */
               ...
               bool                     do_expensive_check = false)
    {
        ...
        static_assert(std::is_same_v<std::remove_cv_t<typename thrust::iterator_traits<
                                         VertexValueInputIterator>::value_type>,
                                     std::remove_cv_t<T>>);
        ...
        if(graph_view.number_of_vertices() == vertex_t{0})
        {
            return init;
        }
        ...
        else
        {
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                else
                {
                    ...
                    else if(local_reduction_size > vertex_t{0})
                    { // use the last element as local_init
                        rmm::device_scalar<T> tmp(handle.get_stream());
                        thrust::copy(
                ...
                if constexpr(reduce_op::has_identity_element_v<ReduceOp>)
                {
                    ...
                    else
                    {
                        auto rets = host_scalar_gather(
                            handle.get_comms(), *local_result, int{0}, handle.get_stream());
                        if(handle.get_comms().get_rank() == int{0})
                        {
                            ret = std::reduce(
                                rets.begin(), rets.end(), ReduceOp::identity_element, reduce_op);
                        }
                        ret = host_scalar_bcast(
                            handle.get_comms(), ret, int{0}, handle.get_stream());
                    }
                }
                else
                { // no guarantee that every GPU has valid local_result
                    auto rets = host_scalar_gather(handle.get_comms(),
                    ...
                    if(handle.get_comms().get_rank() == int{0})
                    {
                        ...
                        auto vertex_partition_range_offsets
                            = graph_view.vertex_partition_range_offsets();
                        for(size_t i = 0; i < rets.size(); ++i)
                        {
                            if(vertex_partition_range_offsets[i + size_t{1}]
                        ...
                        ret = std::reduce(valid_rets.begin(),
                                          valid_rets.end() - size_t{1},
                                          valid_rets.back(),
                                          reduce_op);
                    }
