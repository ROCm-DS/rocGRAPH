...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        {
            auto packed_output_offset = rocgraph::packed_bool_offset(offset);
            auto packed_output_mask   = rocgraph::packed_bool_mask(offset);
            if(val)
            {
            ...
            {
                atomicAnd(value_first + packed_output_offset,
                          rocgraph::packed_bool_negate_mask(packed_output_mask));
            }
        }
                        ...
                        PackedBoolOutputIterator output_first)
        {
            ...
            auto packed_size = rocgraph::packed_bool_size(num_bools);

            // static_assert(std::is_same_v<decltype(*output_first), packed_bool_container_t>);
        ...
        {
            auto num_bools = static_cast<size_t>(thrust::distance(input_first, input_last));
            auto num_first_word_bools
                = std::min(num_bools, rocgraph::packed_bools_per_word() - intraword_start_offset);
            auto num_aligned_bools
                = (num_bools - num_first_word_bools)
                  - (num_bools - num_first_word_bools) % rocgraph::packed_bools_per_word();
            ...
            thrust::for_each(
                ...
                [intraword_start_offset,
                 ...
                 last_word_input_bool_first
                 = input_first + (num_first_word_bools + num_aligned_bools),
                 first_word_output = output_first,
                 last_word_output
                 = output_first
                   + ((num_first_word_bools > 0 ? 1 : 0)
                      + rocgraph::packed_bool_size(num_aligned_bools))] __device__(size_t i) {
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                if(edge_partition_keys)
                {
                    ...
                    for(int i = 0; i < minor_comm_size; ++i)
                    {
                        auto major_range_vertex_partition_id
                            = compute_local_edge_partition_major_range_vertex_partition_id_t{
                                ...
                                major_comm_rank,
                                minor_comm_rank}(i);
                        max_rx_size = std::max(max_rx_size,
                                               graph_view.vertex_partition_range_size(
                                                   major_range_vertex_partition_id));
                    }
                    auto rx_value_buffer = allocate_dataframe_buffer<
                        ...
                        packed_bool ? rocgraph::packed_bool_size(max_rx_size) : max_rx_size,
                        handle.get_stream());
                    ...
                    for(int i = 0; i < minor_comm_size; ++i)
                    {
                        ...
                        if constexpr(packed_bool)
                        {
                            ...
                            device_bcast(
                                ...
                                rx_value_first,
                                rx_value_first,
                                rocgraph::packed_bool_size(graph_view.vertex_partition_range_size(
                                    major_range_vertex_partition_id)),
                                i,
                                handle.get_stream());
                                ...
                                [rx_value_first,
                                 v_first = graph_view.vertex_partition_range_first(
                                    ...
                                    return static_cast<bool>(
                                        *(rx_value_first + rocgraph::packed_bool_offset(v_offset))
                                        & rocgraph::packed_bool_mask(v_offset));
                                });
                            pack_bools(handle,
                                       bool_first,
                                        ...
                                        EdgeMajorPropertyOutputWrapper edge_major_property_output)
        {
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                auto max_rx_size = std::reduce(
                    rx_counts.begin(), rx_counts.end(), size_t{0}, [](auto lhs, auto rhs) {
                ...
                for(int i = 0; i < minor_comm_size; ++i)
                {
                    auto edge_partition = edge_partition_device_view_t<vertex_t,
                                                                       edge_t,
                    ...
                    if(i == minor_comm_rank)
                    {
                        ...
                        if constexpr(packed_bool)
                        {
                            auto bool_first = thrust::make_transform_iterator(
                                vertex_first,
                                [vertex_property_input_first,
                                 vertex_partition] __device__(auto v) -> bool {
                                    auto v_offset
                                        = vertex_partition
                                              .local_vertex_partition_offset_from_vertex_nocheck(v);
                                    return static_cast<bool>(
                                        *(vertex_property_input_first
                                          + rocgraph::packed_bool_offset(v_offset))
                                        & rocgraph::packed_bool_mask(v_offset));
                                });
                            pack_bools(handle,
                                       bool_first,
                        ...
                        else
                        {
                            auto map_first = thrust::make_transform_iterator(
                                vertex_first, [vertex_partition] __device__(auto v) -> vertex_t {
                    ...
                    device_bcast(minor_comm,
                                 rx_value_first,
                                 rx_value_first,
                                 packed_bool ? rocgraph::packed_bool_size(rx_counts[i])
                                             : rx_counts[i],
                                 i,
                                 handle.get_stream());
                    ...
                    if(edge_partition_keys)
                    {
                        thrust::for_each(
                            ...
                            [rx_vertex_first = rx_vertices.begin(),
                             ...
                             = edge_partition_value_firsts[i]] __device__(size_t i) {
                                ...
                                {
                                    auto edge_partition_offset
                                        = thrust::distance(edge_partition_key_first, it);
                                    if constexpr(packed_bool)
                                    {
                                        auto rx_value = static_cast<bool>(
                                            *(rx_value_first + rocgraph::packed_bool_offset(i))
                                            & rocgraph::packed_bool_mask<rx_value_first>(i));
                                        packed_bool_atomic_set(edge_partition_value_first,
                                    ...
                                    else
                                    {
                                        ...
                                        *(edge_partition_value_first + edge_partition_offset)
                                            = rx_value;
                                    }
                                }
                            });
                        ...
                        if constexpr(packed_bool)
                        {
                            thrust::for_each(
                                ...
                                [edge_partition,
                                 ...
                                 rx_value_first,
                                 output_value_first
                                 = edge_partition_value_firsts[i]] __device__(auto i) {
                                    auto rx_vertex = *(rx_vertex_first + i);
                                    auto rx_value  = static_cast<bool>(
                                        *(rx_value_first + rocgraph::packed_bool_offset(i))
                                        & rocgraph::packed_bool_mask(i));
                                    auto major_offset
                                        = edge_partition.major_offset_from_major_nocheck(rx_vertex);
                                    packed_bool_atomic_set(
                                        output_value_first, major_offset, rx_value);
                                });
                        }
                        else
                        {
                            auto map_first = thrust::make_transform_iterator(
                                rx_vertices.begin(),
                                [edge_partition] __device__(auto v) -> vertex_t {
            ...
            else
            {
                ...
                {
                    auto val_first = thrust::make_permutation_iterator(vertex_property_input_first,
                                                                       vertex_first);
                    thrust::scatter(handle.get_thrust_policy(),
                                    val_first,
                                        ...
                                        EdgeMinorPropertyOutputWrapper edge_minor_property_output)
        {
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                using bcast_buffer_type
                    = decltype(allocate_dataframe_buffer<std::conditional_t<
                                   packed_bool,
                                   packed_bool_container_t,
                ...
                num_concurrent_bcasts
                    = std::min(num_concurrent_bcasts, static_cast<size_t>(major_comm_size));
                auto num_rounds
                    = (static_cast<size_t>(major_comm_size) + num_concurrent_bcasts - size_t{1})
                      / num_concurrent_bcasts;
                ...
                if(packed_bool || edge_partition_keys)
                {
                    ...
                    for(size_t i = 0; i < num_concurrent_bcasts; ++i)
                    {
                        ...
                        (*rx_value_buffers)
                            .push_back(allocate_dataframe_buffer<std::conditional_t<
                                           packed_bool,
                                           packed_bool_container_t,
                                           typename EdgeMinorPropertyOutputWrapper::value_type>>(
                                packed_bool ? rocgraph::packed_bool_size(max_size) : max_size,
                                handle.get_stream()));
                    }
                }
                    ...
                    for(int i = 0; i < major_comm_size; ++i)
                    {
                        auto minor_range_vertex_partition_id
                            = compute_local_edge_partition_minor_range_vertex_partition_id_t{
                                ...
                                major_comm_rank,
                                minor_comm_rank}(i);
                        rx_counts[i] = graph_view.vertex_partition_range_size(
                            minor_range_vertex_partition_id);
                    }
                            ...
                            if(j == major_comm_rank)
                            {
                                auto minor_range_vertex_partition_id
                                    = compute_local_edge_partition_minor_range_vertex_partition_id_t{
                                        ...
                                        minor_comm_rank}(j);
                                auto rx_value_first
                                    = get_dataframe_buffer_begin((*rx_value_buffers)[i]);
                                pack_bools(handle,
                                           vertex_property_input_first,
                                           vertex_property_input_first
                                               + graph_view.vertex_partition_range_size(
                        ...
                        if(j < static_cast<size_t>(major_comm_size))
                        {
                            ...
                            auto rx_value_first
                                = rx_value_buffers
                                      ? get_dataframe_buffer_begin((*rx_value_buffers)[i])
                                      : edge_partition_value_first
                                            + std::get<std::vector<size_t>>(
                                                key_offsets_or_rx_displacements)[j];
                            if constexpr(packed_bool)
                            {
                        ...
                        for(size_t i = 0; i < num_concurrent_bcasts; ++i)
                        {
                            ...
                            if(j < static_cast<size_t>(major_comm_size))
                            {
                                ...
                                if constexpr(packed_bool)
                                {
                                    ...
                                    {
                                        auto key_offsets
                                            = std::get<raft::host_span<vertex_t const>>(
                                                key_offsets_or_rx_displacements);
                                        ...
                                        auto bool_first = thrust::make_transform_iterator(
                                            ...
                                            [rx_value_first,
                                             v_first = graph_view.vertex_partition_range_first(
                                                 minor_range_vertex_partition_id)] __device__(auto
                                                                                                  v)
                                                -> bool {
                                                auto v_offset = v - v_first;
                                                return static_cast<bool>(
                                        ...
                                        pack_unaligned_bools(
                                            ...
                                            edge_partition_value_first
                                                + rocgraph::packed_bool_offset(key_offsets[j]),
                                            key_offsets[j] % rocgraph::packed_bools_per_word());
                                    }
                                    else
                                    {
                                        auto rx_displacements = std::get<std::vector<size_t>>(
                                            key_offsets_or_rx_displacements);
                                        auto bool_first = thrust::make_transform_iterator(
                                            thrust::make_counting_iterator(vertex_t{0}),
                                        ...
                                        pack_unaligned_bools(
                                            ...
                                            bool_first
                                                + graph_view.vertex_partition_range_size(
                                                    minor_range_vertex_partition_id),
                                            edge_partition_value_first
                                                + rocgraph::packed_bool_offset(rx_displacements[j]),
                                            rx_displacements[j]
                                ...
                                else
                                {
                                    ...
                                    auto key_offsets = std::get<raft::host_span<vertex_t const>>(
                                        key_offsets_or_rx_displacements);
                    ...
                    if(edge_partition_keys)
                    {
                        thrust::for_each(
                            ...
                            [rx_vertex_first = rx_vertices.begin(),
                             rx_value_first,
                             subrange_key_first
                             = (*edge_partition_keys).begin() + (*key_offsets)[i],
                             subrange_key_last
                             = (*edge_partition_keys).begin() + (*key_offsets)[i + 1],
                             ...
                             subrange_start_offset      = (*key_offsets)[i]] __device__(auto i) {
                                ...
                                auto it    = thrust::lower_bound(
                                    thrust::seq, subrange_key_first, subrange_key_last, minor);
                                if((it != subrange_key_last) && (*it == minor))
                                {
                                    ...
                                    {
                                        auto rx_value = *(rx_value_first + i);
                                        *(edge_partition_value_first + subrange_start_offset
                                          + subrange_offset)
                                            = rx_value;
                                    }
                                }
                        ...
                        if constexpr(packed_bool)
                        {
                            thrust::for_each(
                                ...
                                [edge_partition,
                                 ...
                                 rx_value_first,
                                 output_value_first
                                 = edge_partition_value_first] __device__(auto i) {
                                    auto rx_vertex = *(rx_vertex_first + i);
                                    auto rx_value  = static_cast<bool>(
                                        *(rx_value_first + rocgraph::packed_bool_offset(i))
                                        & rocgraph::packed_bool_mask<rx_vertex_first>(i));
                                    auto minor_offset
                                        = edge_partition.minor_offset_from_minor_nocheck(rx_vertex);
                                    packed_bool_atomic_set(
                                        output_value_first, minor_offset, rx_value);
                                });
                        }
                        else
                        {
 ...
 */
        ...
        bool do_expensive_check = false)
    {
        if(do_expensive_check)
        {
            ...
            ROCGRAPH_EXPECTS(num_invalids == 0,
                             "Invalid input argument: invalid or non-local vertices in "
                             "[vertex_first, vertex_last).");
        }
