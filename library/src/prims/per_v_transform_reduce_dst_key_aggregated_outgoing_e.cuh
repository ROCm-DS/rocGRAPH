...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        {
            edge_t            base_offset{};
            __device__ edge_t operator()(edge_t offset) const
            {
        ...
        struct tuple_to_minor_comm_rank_t
        {
            ...
            __device__ std::enable_if_t<!std::is_same_v<edge_value_type, thrust::nullopt_t>, int>
                       operator()(thrust::tuple<vertex_t, vertex_t, edge_value_t>
                               val /* major, minor key, edge value */) const
            {
        ...
        struct call_key_aggregated_e_op_t
        {
            ...
            template <typename edge_value_type = edge_value_t>
            __device__
                std::enable_if_t<!std::is_same_v<edge_value_type, thrust::nullopt_t>, e_op_result_t>
                operator()(thrust::tuple<vertex_t, vertex_t, edge_value_t>
                               val /* major, minor key, aggregated edge value */) const
            {
                ...
                return key_aggregated_e_op(major,
                                           minor_key,
                                           major_val,
                                           edge_minor_key_value_map.find(minor_key),
        ...
        struct invalidate_if_not_first_in_run_t
        {
            ...
            {
                return ((i == 0) || (majors[i] != majors[i - 1]))
                           ? majors[i]
 ...
 * destinations assigned to this process in multi-GPU). Use  rocgraph::edge_dst_property_t::view().
 ...
 * in multi-GPU). `rocgraph::detail::compute_gpu_id_from_ext_vertex_t` is used to map keys to
 ...
 */
        ...
        bool                      do_expensive_check = false)
    {
        ...
        static_assert(std::is_same_v<typename KVStoreViewType::key_type,
        ...
        using optional_edge_value_buffer_value_type = std::
            conditional_t<!std::is_same_v<edge_value_t, thrust::nullopt_t>, edge_value_t, void>;

        static_assert(
            std::is_same_v<edge_value_t, thrust::nullopt_t> || std::is_arithmetic_v<edge_value_t>,
            "Currently only scalar values are supported, should be extended to support "
            "thrust::tuple of "
        ...
        [[maybe_unused]] auto mem_frugal_threshold = static_cast<size_t>(
        ...
        for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
        {
            ...
            if(edge_partition_e_mask)
            {
                ...
                if(edge_partition.dcs_nzd_vertices())
                {
                    ...
                    degrees_with_mask = rmm::device_uvector<edge_t>(
                        major_sparse_range_size + *(edge_partition.dcs_nzd_vertex_count()),
                        handle.get_stream());
                    auto major_first = thrust::make_transform_iterator(
                        ...
                        [major_sparse_range_size,
                         ...
                         dcs_nzd_vertices
                         = *(edge_partition.dcs_nzd_vertices())] __device__(vertex_t i)
                            -> vertex_t {
                            if(i < major_sparse_range_size)
                            { // sparse
                ...
                offsets_with_mask = rmm::device_uvector<edge_t>(degrees_with_mask.size() + 1,
                                                                handle.get_stream());
            ...
            if(tmp_majors.size() > 0)
            {
                ...
                auto [h_vertex_offsets, h_edge_offsets]
                    = detail::compute_offset_aligned_element_chunks(
                        ...
                        raft::device_span<edge_t const>(
                            offsets_with_mask ? (*offsets_with_mask).data()
                ...
                for(size_t j = 0; j < num_chunks; ++j)
                {
                    max_chunk_size
                        = std::max(max_chunk_size,
                                   static_cast<size_t>(h_edge_offsets[j + 1] - h_edge_offsets[j]));
                }
                ...
                [[maybe_unused]] auto         unreduced_key_aggregated_edge_values
                    = detail::allocate_optional_dataframe_buffer<
                        optional_edge_value_buffer_value_type>(unreduced_majors.size(),
                                                               handle.get_stream());
                ...
                for(size_t j = 0; j < num_chunks; ++j)
                {
                        ...
                        if constexpr(!std::is_same_v<edge_value_t, thrust::nullopt_t>)
                        {
                            detail::copy_if_mask_set(
                                ...
                                thrust::make_zip_iterator(
                                    minor_key_first, edge_partition_e_value_input.value_first())
                                    + unmasked_ranges[0],
                                thrust::make_zip_iterator(
                                    minor_key_first, edge_partition_e_value_input.value_first())
                                    + unmasked_ranges[1],
                    ...
                    auto input_key_first = thrust::make_zip_iterator(unreduced_majors.begin(),
                                                                     unreduced_minor_keys.begin());
                    ...
                    else
                    {
                        reduced_size += thrust::distance(
                            ...
                            thrust::copy_if(
                                ...
                                output_key_first + reduced_size,
                                rocgraph::detail::is_first_in_run_t<decltype(input_key_first)>{
                                    input_key_first}));
                    }
                }
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                if constexpr(!std::is_same_v<edge_value_t, thrust::nullopt_t>)
                {
                    auto triplet_first = thrust::make_zip_iterator(
                        ...
                        detail::get_optional_dataframe_buffer_begin<edge_value_t>(
                            tmp_key_aggregated_edge_values));
                    d_tx_value_counts = rocgraph::groupby_and_count(
                        triplet_first,
                ...
                if(edge_src_value_input.keys())
                {
                    ...
                    std::vector<size_t>           tx_edge_major_value_counts(minor_comm_size);
                    {
                        ...
                        rmm::device_uvector<int> minor_comm_ranks(tmp_majors.size(),
                                                                  handle.get_stream());
                        thrust::tabulate(
                            handle.get_thrust_policy(),
                            ...
                            [minor_comm_rank_lasts = raft::device_span<size_t const>(
                                 minor_comm_rank_lasts.data(),
                                 minor_comm_rank_lasts.size())] __device__(size_t i) {
                                auto it = thrust::upper_bound(thrust::seq,
                                                              minor_comm_rank_lasts.begin(),
                                                              minor_comm_rank_lasts.end(),
                                                              i);
                                return static_cast<int>(
                                    thrust::distance(minor_comm_rank_lasts.begin(), it));
                            });
                        ...
                        auto pair_first
                            = thrust::make_zip_iterator(minor_comm_ranks.begin(), majors.begin());
                        thrust::sort(handle.get_thrust_policy(),
                                     pair_first,
                                     pair_first + minor_comm_ranks.size());
                        ...
                        rmm::device_uvector<size_t> d_tx_edge_major_value_lasts(
                            minor_comm_size, handle.get_stream());
                    ...
                    auto edge_major_values = allocate_dataframe_buffer<edge_src_value_t>(
                        majors.size(), handle.get_stream());
                    thrust::transform(
                        handle.get_thrust_policy(),
                        majors.begin(),
                        majors.end(),
                        ...
                        [edge_partition,
                         edge_partition_src_value_input] __device__(vertex_t major) {
                            return edge_partition_src_value_input.get(
                                edge_partition.major_offset_from_major_nocheck(major));
                        });

                    std::tie(majors, std::ignore) = shuffle_values(minor_comm,
                                                                   majors.begin(),
...

                    {
                        ...
                        auto unique_pair_last
                            = thrust::unique_by_key(handle.get_thrust_policy(),
                                                    ...
                                                    get_dataframe_buffer_begin(edge_major_values));
                        majors.resize(
                            thrust::distance(majors.begin(), thrust::get<0>(unique_pair_last)),
                            handle.get_stream());
                        resize_dataframe_buffer(
                            edge_major_values, majors.size(), handle.get_stream());
                ...
                [[maybe_unused]] auto         rx_key_aggregated_edge_values
                    = detail::allocate_optional_dataframe_buffer<
                        optional_edge_value_buffer_value_type>(0, handle.get_stream());
                auto mem_frugal_flag = host_scalar_allreduce(
                    minor_comm,
                    tmp_majors.size() > mem_frugal_threshold ? int{1} : int{0},
                    raft::comms::op_t::MAX,
                    handle.get_stream());
                if(mem_frugal_flag)
                { // trade-off potential parallelism to lower peak memory
                    std::tie(rx_majors, std::ignore) = shuffle_values(
                        minor_comm, tmp_majors.begin(), h_tx_value_counts, handle.get_stream());
                    ...
                    detail::shrink_to_fit_optional_dataframe_buffer<
                        optional_edge_value_buffer_value_type>(tmp_key_aggregated_edge_values,
                                                               handle.get_stream());
                }
                else
                {
                    if constexpr(!std::is_same_v<edge_value_t, thrust::nullopt_t>)
                    {
                        ...
                        std::forward_as_tuple(
                            std::tie(rx_majors, rx_minor_keys, rx_key_aggregated_edge_values),
                            std::ignore)
                            = shuffle_values(
                                minor_comm, triplet_first, h_tx_value_counts, handle.get_stream());
                    }
                    else
                    {
                        auto pair_first
                        ...
                        std::forward_as_tuple(std::tie(rx_majors, rx_minor_keys), std::ignore)
                            = shuffle_values(
                                minor_comm, pair_first, h_tx_value_counts, handle.get_stream());
                    }
                    tmp_majors.resize(0, handle.get_stream());
                    tmp_majors.shrink_to_fit(handle.get_stream());
                    tmp_minor_keys.resize(0, handle.get_stream());
                ...
                auto key_pair_first
                    = thrust::make_zip_iterator(rx_majors.begin(), rx_minor_keys.begin());
                if constexpr(!std::is_same_v<edge_value_t, thrust::nullopt_t>)
                {
                    if(rx_majors.size() > mem_frugal_threshold)
                    { // trade-off parallelism to lower peak memory
                ...
                else
                {
                    ...
                    auto num_uniques
                        = thrust::distance(key_pair_first,
                                           thrust::unique(handle.get_thrust_policy(),
                                                          key_pair_first,
                                                          key_pair_first + rx_majors.size()));
                    tmp_majors.resize(num_uniques, handle.get_stream());
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                rmm::device_uvector<vertex_t> unique_minor_keys(tmp_minor_keys.size(),
                                                                handle.get_stream());
                thrust::copy(handle.get_thrust_policy(),
                             tmp_minor_keys.begin(),
                             tmp_minor_keys.end(),
            ...
            auto major_value_map_device_view
                = (GraphViewType::is_multi_gpu && edge_src_value_input.keys())
                      ? thrust::make_optional<detail::kv_binary_search_store_device_view_t<
                            decltype(multi_gpu_major_value_map_ptr->view())>>(
                            multi_gpu_major_value_map_ptr->view())
                      : thrust::nullopt;
            std::conditional_t<KVStoreViewType::binary_search,
                ...
                dst_key_value_map_device_view(GraphViewType::is_multi_gpu
                                                  ? multi_gpu_minor_key_value_map_ptr->view()
                                                  : kv_store_view);
            if constexpr(!std::is_same_v<edge_value_t, thrust::nullopt_t>)
            {
                auto triplet_first = thrust::make_zip_iterator(
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                multi_gpu_minor_key_value_map_ptr.reset();
            }
            tmp_minor_keys.resize(0, handle.get_stream());
            tmp_minor_keys.shrink_to_fit(handle.get_stream());
            detail::resize_optional_dataframe_buffer<optional_edge_value_buffer_value_type>(
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                auto rx_sizes
                    = host_scalar_gather(minor_comm, tmp_majors.size(), i, handle.get_stream());
