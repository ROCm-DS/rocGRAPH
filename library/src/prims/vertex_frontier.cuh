...
namespace rocgraph
{
    ...
    public:
        using key_type = std::
            conditional_t<std::is_same_v<tag_t, void>, vertex_t, thrust::tuple<vertex_t, tag_t>>;
        static bool constexpr is_sorted_unique = sorted_unique;

        static_assert(std::is_same_v<tag_t, void> || std::is_arithmetic_v<tag_t>);

        using optional_buffer_type = std::conditional_t<std::is_same_v<tag_t, void>,
                                                        std::byte /* dummy */,
   ...
   */
        ...
        void insert(vertex_t vertex)
        {
            ...
            else
            {
                ...
                raft::update_device(
                    vertices_.data(), &vertex, size_t{1}, handle_ptr_->get_stream());
            }
        }
   ...
   */
        ...
        void insert(thrust::tuple<vertex_t, tag_type> key)
        {
            if(vertices_.size() > 0)
            {
                rmm::device_scalar<vertex_t> tmp_vertex(thrust::get<0>(key),
                                                        handle_ptr_->get_stream());
                rmm::device_scalar<tag_t> tmp_tag(thrust::get<1>(key), handle_ptr_->get_stream());
                auto                      pair_first = thrust::make_zip_iterator(
                    thrust::make_tuple(tmp_vertex.data(), tmp_tag.data()));
            ...
            else
            {
                ...
                auto pair_first = thrust::make_tuple(
                    thrust::make_zip_iterator(vertices_.begin(), tags_.begin()));
   ...
   */
        ...
        {
            static_assert(std::is_same_v<typename std::iterator_traits<VertexIterator>::value_type,
                                         vertex_t>);
            ...
            {
                vertices_.resize(thrust::distance(vertex_first, vertex_last),
                                 handle_ptr_->get_stream());
                thrust::copy(
                    handle_ptr_->get_thrust_policy(), vertex_first, vertex_last, vertices_.begin());
            }
        }
   ...
   */
        ...
        void insert(KeyIterator key_first, KeyIterator key_last)
        {
            ...
            if(vertices_.size() > 0)
            {
                ...
                {
                    rmm::device_uvector<vertex_t> merged_vertices(
                        vertices_.size() + thrust::distance(key_first, key_last),
                        handle_ptr_->get_stream());
                    rmm::device_uvector<tag_t> merged_tags(merged_vertices.size(),
                                                           handle_ptr_->get_stream());
                    auto                       old_pair_first = thrust::make_zip_iterator(
                        thrust::make_tuple(vertices_.begin(), tags_.begin()));
        ...
        void resize(size_t size)
        {
            ...
            {
                tags_.resize(size, handle_ptr_->get_stream());
            }
        }
        ...
        void shrink_to_fit()
        {
            ...
            if constexpr(!std::is_same_v<tag_t, void>)
            {
                tags_.shrink_to_fit(handle_ptr_->get_stream());
            }
        }
    ...
    public:
        ...
        vertex_frontier_t(raft::handle_t const& handle, size_t num_buckets)
            : handle_ptr_(&handle)
        {
        ...
        key_bucket_t<vertex_t, tag_t, multi_gpu, sorted_unique_key_bucket>&
            bucket(size_t bucket_idx)
        {
            return buckets_[bucket_idx];
        }

        key_bucket_t<vertex_t, tag_t, multi_gpu, sorted_unique_key_bucket> const&
            bucket(size_t bucket_idx) const
        {
            return buckets_[bucket_idx];
        }
                          ...
                          SplitOp                    split_op)
        {
            ...
            thrust::transform(handle_ptr_->get_thrust_policy(),
                              ...
                              [split_op] __device__(auto key) {
                                  ...
                                  return static_cast<uint8_t>(split_op_result ? *split_op_result
                                                                              : kInvalidBucketIdx);
                              });
            ...
            auto pair_first = thrust::make_zip_iterator(
                thrust::make_tuple(bucket_indices.begin(), this_bucket.begin()));
            auto new_this_bucket_size = static_cast<size_t>(thrust::distance(
                pair_first,
                thrust::
                    stable_partition( // stable_partition to maintain sorted order within each bucket
                        handle_ptr_->get_thrust_policy(),
                        pair_first,
                        pair_first + bucket_indices.size(),
                        [this_bucket_idx = static_cast<uint8_t>(this_bucket_idx)] __device__(
                            auto pair) { return thrust::get<0>(pair) == this_bucket_idx; })));
                               ...
                               std::vector<size_t> const& to_bucket_indices)
        {
            ...
            auto pair_first
                = thrust::make_zip_iterator(thrust::make_tuple(bucket_idx_first, key_first));
                ...
                auto next_bucket_size = static_cast<size_t>(thrust::distance(
                    ...
                    thrust::
                        stable_partition( // stable_partition to maintain sorted order within each bucket
                            ...
                            pair_last,
                            [next_bucket_idx
                             = static_cast<uint8_t>(to_bucket_indices[0])] __device__(auto pair) {
                                return thrust::get<0>(pair) == next_bucket_idx;
                            })));
                insert_bucket_indices = to_bucket_indices;
                insert_offsets        = {0, next_bucket_size};
                insert_sizes          = {next_bucket_size,
                                         static_cast<size_t>(
                                    thrust::distance(pair_first + next_bucket_size, pair_last))};
            }
            else
            {
                ...
                rmm::device_uvector<uint8_t> d_indices(to_bucket_indices.size(),
                                                       handle_ptr_->get_stream());
            ...
            for(size_t i = 0; i < insert_offsets.size(); ++i)
            {
                bucket(insert_bucket_indices[i])
                    .insert(key_first + insert_offsets[i],
                            key_first + (insert_offsets[i] + insert_sizes[i]));
            }
        }
