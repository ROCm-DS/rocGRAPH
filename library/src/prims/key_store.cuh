...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        using hipco_storage_type
            = hipco::experimental::aow_storage<1>; ///< hipco window storage type
        ...
        struct key_hipco_store_contains_device_view_t
        {
            ...
            using hipco_store_device_ref_type =
                typename ViewType::hipco_set_type::template ref_type<
                    hipco::experimental::contains_tag>;
            ...
            __device__ bool contains(key_type key) const
            {
                return hipco_store_device_ref.contains(key);
            }
        ...
        struct key_hipco_store_insert_device_view_t
        {
            ...
            {
                hipco_store_device_ref.insert(key);
            }
        ...
        public:
                          ...
                          rmm::cuda_stream_view stream) const
            {
                thrust::transform(rmm::exec_policy(stream),
                                  ...
                                  key_last,
                                  value_first,
                                  key_binary_search_contains_op_t<KeyIterator>{store_key_first_,
                                                                               store_key_last_});
            }
        ...
        public:
            ...
            key_hipco_store_view_t(hipco_set_type const* store)
                : hipco_store_(store)
            {
            }
            ...
            {
                return hipco_store_->get_empty_key_sentinel();
            }
        ...
        public:
            ...
            key_binary_search_store_t(rmm::cuda_stream_view stream)
        ...
        public:
                           ...
                           rmm::cuda_stream_view stream)
            {
                ...
                size_ += hipco_store_->insert_if(
                    key_first, key_last, stencil_first, pred_op, stream.value());
            }
            ...
            hipco_set_type const* hipco_store_ptr() const
            {
        ...
        private:
                ...
                auto hipco_size
                    = std::max(static_cast<size_t>(static_cast<double>(num_keys) / load_factor),
                               static_cast<size_t>(num_keys)
                                   + 1); // hipco::static_map requires at least one empty slot
            ...
            size_t capacity_{0};
            size_t size_{
                0}; // caching as hipco_store_->size() is expensive (this scans the entire slots to
 ...
 * use_binary_search == true) or hash-table (cuCollection, when use_binary_search = false) */
    ...
    public:
        ...
        key_store_t(rmm::cuda_stream_view stream)
            : store_(stream)
        {
        }
        ...
        template <typename KeyIterator, bool binary_search = use_binary_search>
        std::enable_if_t<!binary_search, void>
            insert(KeyIterator key_first, KeyIterator key_last, rmm::cuda_stream_view stream)
        {
