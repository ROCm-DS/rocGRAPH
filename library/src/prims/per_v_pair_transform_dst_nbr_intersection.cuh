...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        {
            using vertex_t = typename thrust::tuple_element<
                0,
                typename thrust::iterator_traits<VertexPairIterator>::value_type>::type;
        ...
        struct call_intersection_op_t
        {
            edge_partition_device_view_t<typename GraphViewType::vertex_type,
                ...
                edge_partition{};
            thrust::optional<raft::device_span<typename GraphViewType::vertex_type const>>
                                                       unique_vertices;
            ...
            __device__ void operator()(size_t i) const
            {
                ...
                if constexpr(!std::is_same_v<edge_property_value_t, thrust::nullopt_t>)
                {
                    ...
                    property_values1 = raft::device_span<edge_property_value_t const>(
                        nbr_intersection_property_values1 + nbr_offsets[i],
                        nbr_intersection_property_values1 + +nbr_offsets[i + 1]);
                }
                ...
                if(unique_vertices)
                {
                    src_prop = *(vertex_property_first
                                 + thrust::distance((*unique_vertices).begin(),
                ...
                *(major_minor_pair_value_output_first + index) = intersection_op(
                    src, dst, src_prop, dst_prop, intersection, property_values0, property_values1);
            }
        };
 ...
 * to this process in multi-GPU). Use either rocgraph::edge_property_t::view() (if @p intersection_op
 * needs to access edge property values) or rocgraph::edge_dummy_property_t::view() (if @p
 ...
 */
        ...
        bool                          do_expensive_check = false)
    {
        ...
        using result_t =
            typename thrust::iterator_traits<VertexPairValueOutputIterator>::value_type;
        ...
        std::optional<decltype(allocate_dataframe_buffer<property_t>(size_t{0},
        ...
        if constexpr(GraphViewType::is_multi_gpu)
        {
            ...
            auto elem0_first = thrust::make_transform_iterator(
                vertex_pair_first,
                rocgraph::thrust_tuple_get<
                    typename thrust::iterator_traits<VertexPairIterator>::value_type,
                    0>{});
        ...
        raft::update_host(h_edge_partition_group_sizes.data(),
                          ...
                          handle.get_stream());
        handle.sync_stream();
        std::vector<size_t> h_edge_partition_group_displacements(
            h_edge_partition_group_sizes.size());
        ...
        for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
        {
            ...
            auto max_chunk_size
                = static_cast<size_t>(handle.get_device_properties().multiProcessorCount)
                  * (1 << 15);
            auto max_num_chunks
                = (h_edge_partition_group_sizes[i] + max_chunk_size - 1) / max_chunk_size;
            if constexpr(GraphViewType::is_multi_gpu)
            {
            ...
            if(h_chunk_sizes.size() > size_t{1})
            {
                auto d_chunk_sizes = groupby_and_count(
                    ...
                    detail::compute_chunk_id_t<VertexPairIterator>{vertex_pair_first,
                                                                   max_num_chunks},
            ...
            for(size_t j = 0; j < h_chunk_sizes.size(); ++j)
            {
                ...
                if constexpr(!std::is_same_v<edge_property_value_t, thrust::nullopt_t>)
                {
                    std::tie(intersection_offsets,
                             ...
                             r_nbr_intersection_property_values1)
