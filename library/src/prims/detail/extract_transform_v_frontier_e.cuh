...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        template <typename BufferKeyOutputIterator,
                  ...
                  typename e_op_result_t>
        __device__ void
            warp_push_buffer_elements(BufferKeyOutputIterator   buffer_key_output_first,
                                      BufferValueOutputIterator buffer_value_output_first,
                                      hip::atomic_ref<size_t, hip::thread_scope_device>& buffer_idx,
                                      int                                                lane_id,
                                      e_op_result_t e_op_result)
        {
            packed_bool_container_t ballot
                = __ballot_sync(raft::warp_full_mask(), e_op_result ? uint64_t{1} : uint64_t{0});
            if(ballot > 0)
            {
                size_t warp_buffer_start_idx{};
                if(lane_id == 0)
                {
                    auto increment = rocgraph::bit_count(ballot);
                    warp_buffer_start_idx
                        = buffer_idx.fetch_add(increment, hip::std::memory_order_relaxed);
                }
                warp_buffer_start_idx
                    = __shfl_sync(raft::warp_full_mask(), warp_buffer_start_idx, int{0});
                if(e_op_result)
                {
                    auto buffer_warp_offset = rocgraph::bit_count(
                        ballot
                        & rocgraph::packed_bool_negate_mask((raft::warp_full_mask() << lane_id)));
            ...
            EdgeOp                                         e_op)
        {
            ...
            using e_op_result_t = typename edge_op_result_type<
                key_t,
            ...
            [[maybe_unused]] vertex_t
                major_start_offset{}; // relevant only when hypersparse is true
            ...
            __shared__ edge_t
                warp_key_local_edge_offsets[extract_transform_v_frontier_e_kernel_block_size];
            ...
            {
                auto min_key_idx
                    = static_cast<vertex_t>(idx - (idx % raft::warp_size())); // inclusive
                auto max_key_idx = static_cast<vertex_t>(
                    std::min(static_cast<size_t>(min_key_idx) + raft::warp_size(),
                ...
                if(lane_id < static_cast<int32_t>(max_key_idx - min_key_idx))
                {
                    ...
                    if constexpr(hypersparse)
                    {
                        ...
                        if(major_hypersparse_idx)
                        {
                            ...
                            warp_key_local_edge_offsets[threadIdx.x]
                                = edge_partition.local_offset(major_idx);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        warp_key_local_edge_offsets[threadIdx.x]
                            = edge_partition.local_offset(major_offset);
                    }
                }
                ...
                {
                    for(size_t i = lane_id; i < rounded_up_num_edges_this_warp;
                        i += raft::warp_size())
                    {
                        ...
                        if(i < static_cast<size_t>(num_edges_this_warp))
                        {
                            auto key_idx_this_warp = static_cast<vertex_t>(
                                thrust::distance(this_warp_inclusive_sum_first,
                                                 thrust::upper_bound(thrust::seq,
                                                                     this_warp_inclusive_sum_first,
                                                                     this_warp_inclusive_sum_last,
                                                                     i)));
                            auto local_edge_offset
                                = warp_key_local_edge_offsets[warp_id * raft::warp_size()
                                                              + key_idx_this_warp]
            ...
            EdgeOp                                         e_op)
        {
            ...
            auto const tid = threadIdx.x + blockIdx.x * blockDim.x;
            static_assert(extract_transform_v_frontier_e_kernel_block_size % raft::warp_size()
                          == 0);
            auto const warp_id = threadIdx.x / raft::warp_size();
            auto const lane_id = tid % raft::warp_size();
            ...
            while(idx < static_cast<size_t>(thrust::distance(key_first, key_last)))
            {
                ...
                auto rounded_up_local_out_degree
                    = ((static_cast<size_t>(local_out_degree) + (raft::warp_size() - 1))
                       / raft::warp_size())
                      * raft::warp_size();
        ...
        template <bool incoming, // iterate over incoming edges (incoming == true) or outgoing edges
                  ...
                  typename EdgeOp>
        std::tuple<decltype(allocate_optional_dataframe_buffer<OutputKeyT>(
                       size_t{0}, rmm::cuda_stream_view{})),
                   decltype(allocate_optional_dataframe_buffer<OutputValueT>(
                       size_t{0}, rmm::cuda_stream_view{}))>
                                           ...
                                           bool do_expensive_check = false)
        {
            ...
            using e_op_result_t = typename rocgraph::detail::edge_op_result_type<
                key_t,
                typename GraphViewType::vertex_type,
            ...
            static_assert(
                !std::is_same_v<output_key_t, void>
            ...
            auto value_buffer = allocate_optional_dataframe_buffer<output_value_t>(
            ...
            for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
            {
                ...
                vertex_t edge_partition_frontier_size
                    = static_cast<vertex_t>(local_frontier_sizes[i]);
                ...
                if constexpr(GraphViewType::is_multi_gpu)
                {
                    ...
                    resize_dataframe_buffer(edge_partition_frontier_key_buffer,
                ...
                auto max_pushes
                    = edge_partition.compute_number_of_edges(edge_partition_frontier_major_first,
                                                             edge_partition_frontier_major_last,
                                                             handle.get_stream());
                ...
                if(segment_offsets)
                {
                    ...
                    rmm::device_uvector<vertex_t> d_thresholds(h_thresholds.size(),
                                                               handle.get_stream());
                    raft::update_device(d_thresholds.data(),
                                        h_thresholds.data(),
                                        h_thresholds.size(),
                                        handle.get_stream());
                    rmm::device_uvector<vertex_t> d_offsets(d_thresholds.size(),
                                                            handle.get_stream());
                    ...
                    raft::update_host(
                        h_offsets.data(), d_offsets.data(), d_offsets.size(), handle.get_stream());
                    ...
                    if(h_offsets[2] - h_offsets[1] > 0)
                    {
                        raft::grid_1d_thread_t update_grid(
                            ...
                            handle.get_device_properties().maxGridSize[0]);
                        THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                            (extract_transform_v_frontier_e_hypersparse_or_low_degree<
                                false,
                                GraphViewType>),
                            dim3(update_grid.num_blocks),
                            dim3(update_grid.block_size),
            ...
            resize_optional_dataframe_buffer<output_value_t>(
                value_buffer, new_buffer_size, handle.get_stream());
            shrink_to_fit_optional_dataframe_buffer<output_value_t>(value_buffer,
                                                                    handle.get_stream());
