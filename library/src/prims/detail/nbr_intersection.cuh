...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        struct major_to_group_idx_t
        {
            ...
            {
                auto vertex_partition_id = compute_vertex_partition_id_from_int_vertex_t<vertex_t>{
                    vertex_partition_range_lasts}(major);
        ...
        struct update_rx_major_local_degree_t
        {
            ...
            __device__ void operator()(size_t idx) const
            {
                auto it = thrust::upper_bound(thrust::seq,
                                              rx_reordered_group_lasts.begin(),
                                              rx_reordered_group_lasts.end(),
                                              idx);
                auto major_comm_rank
                    = static_cast<int>(thrust::distance(rx_reordered_group_lasts.begin(), it));
                auto offset_in_local_edge_partition
                    = idx
                      - (major_comm_rank == int{0}
                             ? reordered_idx_first
                             : rx_reordered_group_lasts[major_comm_rank - int{1}]);
                auto major        = rx_majors[rx_group_firsts[major_comm_rank * minor_comm_size
                ...
                if(edge_partition_e_mask && (local_degree > edge_t{0}))
                {
                    ...
                    local_degree      = static_cast<edge_t>(count_set_bits(
                        (*edge_partition_e_mask).value_first(), local_offset, local_degree));
                }
        ...
        struct update_rx_major_local_nbrs_t
        {
            ...
            __device__ void operator()(size_t idx)
            {
                using edge_property_value_t =
                    typename edge_partition_e_input_device_view_t::value_type;
                ...
                if(multi_gpu
                   && (edge_partition.major_hypersparse_first()
                       && (major >= *(edge_partition.major_hypersparse_first()))))
                {
                    ...
                    {
                        auto major_idx = (*(edge_partition.major_hypersparse_first())
                ...
                size_t output_start_offset = local_nbr_offsets_for_rx_majors
                    [rx_group_firsts[major_comm_rank * minor_comm_size + local_edge_partition_idx]
                     + offset_in_local_edge_partition];
                ...
                static_assert(!edge_partition_e_input_device_view_t::has_packed_bool_element,
                              "unimplemented.");
                if(local_degree > 0)
                {
                    if(edge_partition_e_mask)
                    {
                        ...
                        if constexpr(!std::is_same_v<edge_property_value_t, thrust::nullopt_t>)
                        {
                            auto input_first = thrust::make_zip_iterator(
                                indices, edge_partition_e_value_input.value_first());
                            copy_if_mask_set(
                                input_first,
                                mask_first,
        ...
        struct pick_min_degree_t
        {
            ...
            __device__ edge_t operator()(thrust::tuple<vertex_t, vertex_t> pair) const
            {
                ...
                if constexpr(std::is_same_v<FirstElementToIdxMap, void*>)
                {
                    ...
                    {
                        auto local_offset = edge_partition.local_offset(*major_idx);
                        local_degree0     = count_set_bits(
                            (*edge_partition_e_mask).value_first(), local_offset, local_degree0);
                    }
                }
                else
                {
                    auto idx      = first_element_to_idx_map.find(major0);
                    local_degree0 = static_cast<edge_t>(first_element_offsets[idx + 1]
                                                        - first_element_offsets[idx]);
                }
                ...
                if constexpr(std::is_same_v<SecondElementToIdxMap, void*>)
                {
                    ...
                    {
                        auto local_offset = edge_partition.local_offset(*major_idx);
                        local_degree1     = count_set_bits(
                            (*edge_partition_e_mask).value_first(), local_offset, local_degree1);
                    }
                }
                else
                {
                    auto idx      = second_element_to_idx_map.find(major1);
                    local_degree1 = static_cast<edge_t>(second_element_offsets[idx + 1]
                                                        - second_element_offsets[idx]);
                }
        ...
        template <bool check_edge_mask,
                  ...
                  typename edge_t>
        __device__ edge_t
            set_intersection_by_key_with_mask(InputKeyIterator0    input_key_first0,
        ...
        struct copy_intersecting_nbrs_and_update_intersection_size_t
        {
            ...
            __device__ edge_t operator()(size_t i)
            {
                ...
                else
                {
                    ...
                    local_degree0
                        = static_cast<edge_t>(first_element_offsets[idx + 1] - local_edge_offset0);
                }
                ...
                else
                {
                    ...
                    local_degree1
                        = static_cast<edge_t>(second_element_offsets[idx + 1] - local_edge_offset1);
                }
                ...
                {
                    intersection_size = set_intersection_by_key_with_mask<false>(
                        indices0,
                        indices1,
                        edge_property_values0,
                        edge_property_values1,
                        static_cast<packed_bool_container_t const*>(nullptr),
                        nbr_intersection_indices.begin(),
                        nbr_intersection_e_property_values0,
                        nbr_intersection_e_property_values1,
                        local_edge_offset0,
                ...
                thrust::fill(thrust::seq,
                             nbr_intersection_indices.begin()
                                 + (nbr_intersection_offsets[i] + intersection_size),
        ...
        struct strided_accumulate_t
        {
            ...
            __device__ edge_t operator()(size_t i) const
            {
                ...
                {
                    accumulated_size
                        += rx_nbr_intersection_sizes[edge_partition_input_size * j + i];
                }
                             ...
                             bool                   do_expensive_check = false)
        {
            ...
            using edge_partition_e_input_device_view_t = std::conditional_t<
                std::is_same_v<edge_property_value_t, thrust::nullopt_t>,
            ...
            static_assert(
                std::is_same_v<typename thrust::iterator_traits<VertexPairIterator>::value_type,
                               thrust::tuple<vertex_t, vertex_t>>);

            size_t input_size
                = static_cast<size_t>(thrust::distance(vertex_pair_first, vertex_pair_last));
            ...
            {
                auto is_sorted = thrust::is_sorted(
                    handle.get_thrust_policy(), vertex_pair_first, vertex_pair_last);
                if constexpr(GraphViewType::is_multi_gpu)
                {
                    auto& comm = handle.get_comms();
                    is_sorted  = static_cast<bool>(host_scalar_allreduce(comm,
                                                                        static_cast<int>(is_sorted),
                                                                        raft::comms::op_t::MIN,
                                                                        handle.get_stream()));
                }
                ROCGRAPH_EXPECTS(is_sorted,
                                 "Invalid input arguments: input vertex pairs should be sorted.");

                auto num_invalid_pairs = count_invalid_vertex_pairs(
                    handle, graph_view, vertex_pair_first, vertex_pair_last);
                ROCGRAPH_EXPECTS(num_invalid_pairs == 0,
            ...
            std::optional<std::unique_ptr<kv_store_t<vertex_t, vertex_t, false>>>
                major_to_idx_map_ptr{std::nullopt}; // idx to major_nbr_offsets
            ...
            [[maybe_unused]] auto major_e_property_values
                = rocgraph::detail::allocate_optional_dataframe_buffer<
                    optional_property_buffer_value_type>(0, handle.get_stream());
                ...
                if(intersect_minor_nbr[1])
                {
                    ...
                    rmm::device_uvector<vertex_t> unique_majors(input_size, handle.get_stream());
                    {
                        auto second_element_first = thrust::make_transform_iterator(
                            vertex_pair_first,
                            thrust_tuple_get<thrust::tuple<vertex_t, vertex_t>, size_t{1}>{});
                        ...
                        if(minor_comm_size > 1)
                        {
                            ...
                            auto rx_counts = host_scalar_allgather(
                                minor_comm, unique_majors.size(), handle.get_stream());
                            std::vector<size_t> rx_displacements(rx_counts.size());
                            std::exclusive_scan(rx_counts.begin(),
                                                rx_counts.end(),
                                                rx_displacements.begin(),
                                                size_t{0});
                            rmm::device_uvector<vertex_t> rx_unique_majors(
                                rx_displacements.back() + rx_counts.back(), handle.get_stream());
                    ...
                    {
                        auto h_vertex_partition_range_lasts
                        ...
                        auto d_tx_group_counts
                            = groupby_and_count(unique_majors.begin(),
                                                unique_majors.end(),
                        ...
                        {
                            tx_counts[i]
                                = std::reduce(h_tx_group_counts.begin() + minor_comm_size * i,
                                              h_tx_group_counts.begin() + minor_comm_size * (i + 1),
                                              size_t{0});
                        }

                        std::tie(rx_majors, rx_major_counts) = shuffle_values(
                            major_comm, unique_majors.begin(), tx_counts, handle.get_stream());
                    ...
                    rmm::device_uvector<edge_t>   local_degrees_for_rx_majors(size_t{0},
                                                                            handle.get_stream());
                    rmm::device_uvector<vertex_t> local_nbrs_for_rx_majors(size_t{0},
                                                                           handle.get_stream());
                    ...
                    std::vector<size_t> local_nbr_counts{};
                    {
                        rmm::device_uvector<size_t> rx_reordered_group_counts(
                            ...
                            handle
                                .get_stream()); // reorder using local edge partition index as the primary key and
                        ...
                        rmm::device_uvector<size_t> d_rx_reordered_group_lasts(
                            rx_reordered_group_counts.size(), handle.get_stream());
                        thrust::inclusive_scan(handle.get_thrust_policy(),
                                               rx_reordered_group_counts.begin(),
                                               rx_reordered_group_counts.end(),
                                               d_rx_reordered_group_lasts.begin());
                        std::vector<size_t> h_rx_reordered_group_lasts(
                        ...
                        rmm::device_uvector<size_t> rx_group_firsts(rx_group_counts.size(),
                                                                    handle.get_stream());
                        ...
                        {
                            auto edge_partition
                                = edge_partition_device_view_t<vertex_t,
                                                               edge_t,
                            ...
                            auto reordered_idx_first
                                = (i == size_t{0})
                                      ? size_t{0}
                                      : h_rx_reordered_group_lasts[i * major_comm_size - 1];
                            auto reordered_idx_last
                                = h_rx_reordered_group_lasts[(i + 1) * major_comm_size - 1];
                            thrust::for_each(
                                handle.get_thrust_policy(),
                                thrust::make_counting_iterator(reordered_idx_first),
                                thrust::make_counting_iterator(reordered_idx_last),
                                update_rx_major_local_degree_t<vertex_t,
                                                               edge_t,
                                                               GraphViewType::is_multi_gpu>{
                                    major_comm_size,
                                    minor_comm_size,
                                    ...
                                    i,
                                    raft::device_span<size_t const>(
                                        d_rx_reordered_group_lasts.data() + i * major_comm_size,
                                        major_comm_size),
                                    raft::device_span<size_t const>(rx_group_firsts.data(),
                                                                    rx_group_firsts.size()),
                                    raft::device_span<vertex_t const>(rx_majors.data(),
                                                                      rx_majors.size()),
                        ...
                        auto degree_first
                            = thrust::make_transform_iterator(local_degrees_for_rx_majors.begin(),
                        ...
                        local_nbrs_for_rx_majors.resize(
                            local_nbr_offsets_for_rx_majors.back_element(handle.get_stream()),
                            handle.get_stream());
                        ...
                        if constexpr(!std::is_same_v<edge_property_value_t, thrust::nullopt_t>)
                        {
                            local_e_property_values_for_rx_majors.resize(
                                local_nbrs_for_rx_majors.size(), handle.get_stream());
                            optional_local_e_property_values
                                = local_e_property_values_for_rx_majors.data();
                        }
                        ...
                        std::vector<size_t> h_rx_offsets(rx_major_counts.size() + size_t{1},
                                                         size_t{0});
                        std::inclusive_scan(rx_major_counts.begin(),
                                            rx_major_counts.end(),
                                            h_rx_offsets.begin() + 1);
                        rmm::device_uvector<size_t> d_rx_offsets(h_rx_offsets.size(),
                                                                 handle.get_stream());
                        raft::update_device(d_rx_offsets.data(),
                                            h_rx_offsets.data(),
                                            h_rx_offsets.size(),
                                            handle.get_stream());
                        rmm::device_uvector<size_t> d_local_nbr_counts(rx_major_counts.size(),
                                                                       handle.get_stream());
                        thrust::tabulate(handle.get_thrust_policy(),
                                         d_local_nbr_counts.begin(),
                                         d_local_nbr_counts.end(),
                                         compute_local_nbr_count_per_rank_t{
                                             raft::device_span<size_t const>(d_rx_offsets.data(),
                                                                             d_rx_offsets.size()),
                                             raft::device_span<size_t const>(
                                                 local_nbr_offsets_for_rx_majors.data(),
                                                 local_nbr_offsets_for_rx_majors.size())});
                    ...
                    {
                        rmm::device_uvector<edge_t> local_degrees_for_unique_majors(
                            size_t{0}, handle.get_stream());
                        std::tie(local_degrees_for_unique_majors, std::ignore)
                            = shuffle_values(major_comm,
                                             ...
                                             rx_major_counts,
                                             handle.get_stream());
                        major_nbr_offsets = rmm::device_uvector<edge_t>(
                            local_degrees_for_unique_majors.size() + 1, handle.get_stream());
                        (*major_nbr_offsets)
                        ...
                        thrust::inclusive_scan(handle.get_thrust_policy(),
                                               degree_first,
                                               degree_first
                                                   + local_degrees_for_unique_majors.size(),
            ...
            std::optional<std::unique_ptr<kv_store_t<vertex_t, vertex_t, false>>>
                minor_to_idx_map_ptr{std::nullopt}; // idx to minor_nbr_offsets
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                std::vector<size_t> input_lasts(input_counts.size());
                {
                    ...
                    for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); i++)
                    {
                        if constexpr(GraphViewType::is_storage_transposed)
                        {
                            h_edge_partition_major_range_lasts[i]
                                = graph_view.local_edge_partition_dst_range_last(i);
                        }
                        else
                        {
                            h_edge_partition_major_range_lasts[i]
                    ...
                    auto first_element_first = thrust::make_transform_iterator(
                        vertex_pair_first,
                        thrust_tuple_get<thrust::tuple<vertex_t, vertex_t>, size_t{0}>{});
                    thrust::lower_bound(handle.get_thrust_policy(),
                                        ...
                                        d_lasts.begin());
                    raft::update_host(
                        input_lasts.data(), d_lasts.data(), d_lasts.size(), handle.get_stream());
                    handle.sync_stream();
                    std::adjacent_difference(
                        input_lasts.begin(), input_lasts.end(), input_counts.begin());
                }

                std::vector<rmm::device_uvector<edge_t>> edge_partition_nbr_intersection_sizes{};
                std::vector<rmm::device_uvector<vertex_t>>
                    edge_partition_nbr_intersection_indices{};
                edge_partition_nbr_intersection_sizes.reserve(
                    graph_view.number_of_local_edge_partitions());
                edge_partition_nbr_intersection_indices.reserve(
                    graph_view.number_of_local_edge_partitions());
                ...
                for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
                {
                    ...
                    std::exclusive_scan(rx_v_pair_counts.begin(),
                                        ...
                                        size_t{0});
                    auto aggregate_rx_v_pair_size
                        = rx_v_pair_displacements.back() + rx_v_pair_counts.back();

                    // 4.1. All-gather vertex pairs & locally intersect

                    rmm::device_uvector<edge_t> rx_v_pair_nbr_intersection_sizes(
                        size_t{0}, handle.get_stream());
                    rmm::device_uvector<vertex_t> rx_v_pair_nbr_intersection_indices(
                        size_t{0}, handle.get_stream());
                    ...
                    std::vector<size_t> rx_v_pair_nbr_intersection_index_tx_counts(size_t{0});
                    {
                        ...
                        if(intersect_minor_nbr[0] && intersect_minor_nbr[1])
                        {
                            auto second_element_to_idx_map
                                = detail::kv_hipco_store_find_device_view_t(
                                    (*major_to_idx_map_ptr)->view());
                            thrust::transform(
                                handle.get_thrust_policy(),
                                get_dataframe_buffer_begin(vertex_pair_buffer),
                        ...
                        rx_v_pair_nbr_intersection_offsets.set_element_to_zero_async(
                            size_t{0}, handle.get_stream());
                        auto size_first = thrust::make_transform_iterator(
                            rx_v_pair_nbr_intersection_sizes.begin(),
                            rocgraph::detail::typecast_t<edge_t, size_t>{});
                        thrust::inclusive_scan(handle.get_thrust_policy(),
                                               size_first,
                        ...
                        {
                            auto common_nbr_and_e_property_values_begin
                            ...
                            rx_v_pair_nbr_intersection_indices.resize(
                                thrust::distance(common_nbr_and_e_property_values_begin, last),
                                handle.get_stream());
                            ...
                            rx_v_pair_nbr_intersection_e_property_values0.resize(
                                rx_v_pair_nbr_intersection_indices.size(), handle.get_stream());
                            rx_v_pair_nbr_intersection_e_property_values0.shrink_to_fit(
                                handle.get_stream());

                            rx_v_pair_nbr_intersection_e_property_values1.resize(
                                rx_v_pair_nbr_intersection_indices.size(), handle.get_stream());
                            rx_v_pair_nbr_intersection_e_property_values1.shrink_to_fit(
                                handle.get_stream());
                        }
                        ...
                        std::inclusive_scan(rx_v_pair_counts.begin(),
                                            rx_v_pair_counts.end(),
                                            h_rx_v_pair_lasts.begin());
                        ...
                        std::adjacent_difference(
                            h_rx_v_pair_nbr_intersection_index_tx_lasts.begin(),
                    ...
                    rmm::device_uvector<edge_t> combined_nbr_intersection_sizes(
                        size_t{0}, handle.get_stream());
                    rmm::device_uvector<size_t> combined_nbr_intersection_offsets(
                        size_t{0}, handle.get_stream());
                    rmm::device_uvector<size_t> gathered_nbr_intersection_offsets(
                        size_t{0}, handle.get_stream());
                    std::vector<size_t> gathered_nbr_intersection_index_rx_counts(size_t{0});
                    {
                        ...
                        rmm::device_uvector<edge_t> gathered_nbr_intersection_sizes(
                            rx_v_pair_counts[minor_comm_rank] * minor_comm_size,
                            handle.get_stream());
                        ...
                        thrust::tabulate(handle.get_thrust_policy(),
                                         ...
                                         strided_accumulate_t<edge_t>{
                                             raft::device_span<edge_t const>(
                                                 gathered_nbr_intersection_sizes.data(),
                                                 gathered_nbr_intersection_sizes.size()),
                                             rx_v_pair_counts[minor_comm_rank],
                                             minor_comm_size});

                        combined_nbr_intersection_offsets.resize(
                            rx_v_pair_counts[minor_comm_rank] + 1, handle.get_stream());
                        combined_nbr_intersection_offsets.set_element_to_zero_async(
                            size_t{0}, handle.get_stream());
                        ...
                        thrust::inclusive_scan(handle.get_thrust_policy(),
                                               combined_size_first,
                                               combined_size_first
                                                   + combined_nbr_intersection_sizes.size(),
                                               combined_nbr_intersection_offsets.begin() + 1);

                        gathered_nbr_intersection_offsets.resize(
                            gathered_nbr_intersection_sizes.size() + 1, handle.get_stream());
                        gathered_nbr_intersection_offsets.set_element_to_zero_async(
                            size_t{0}, handle.get_stream());
                        ...
                        raft::update_host(
                            h_lasts.data(), d_lasts.data(), d_lasts.size(), handle.get_stream());
                        handle.sync_stream();
                        gathered_nbr_intersection_index_rx_counts.resize(h_lasts.size());
                        std::adjacent_difference(h_lasts.begin(),
                                                 h_lasts.end(),
                                                 gathered_nbr_intersection_index_rx_counts.begin());
                    }

                    rmm::device_uvector<vertex_t> combined_nbr_intersection_indices(
                        size_t{0}, handle.get_stream());

                    [[maybe_unused]] auto combined_nbr_intersection_e_property_values0
                        = rocgraph::detail::allocate_optional_dataframe_buffer<
                            optional_property_buffer_value_type>(size_t{0}, handle.get_stream());
...

                    {
                        ...
                        std::vector<size_t> tx_displacements(
                        ...
                        std::vector<size_t> rx_displacements(
                        ...
                        rmm::device_uvector<vertex_t> gathered_nbr_intersection_indices(
                            rx_displacements.back()
                                + gathered_nbr_intersection_index_rx_counts.back(),
                            handle.get_stream());
                        ...
                        combined_nbr_intersection_indices.resize(
                            gathered_nbr_intersection_indices.size(), handle.get_stream());

                        [[maybe_unused]] auto gathered_nbr_intersection_e_property_values0
                            = rocgraph::detail::allocate_optional_dataframe_buffer<
                                optional_property_buffer_value_type>(
                                rx_displacements.back()
                                    + gathered_nbr_intersection_index_rx_counts.back(),
                                handle.get_stream());
                        ...
                        if constexpr(!std::is_same_v<edge_property_value_t, thrust::nullopt_t>)
                        {
                            ...
                            combined_nbr_intersection_e_property_values0.resize(
                                gathered_nbr_intersection_e_property_values0.size(),
                                handle.get_stream());
                            ...
                            combined_nbr_intersection_e_property_values1.resize(
                                gathered_nbr_intersection_e_property_values1.size(),
                                handle.get_stream());
                        }
                    ...
                    edge_partition_nbr_intersection_sizes.push_back(
                        std::move(combined_nbr_intersection_sizes));
                ...
                {
                    num_nbr_intersection_indices
                        += edge_partition_nbr_intersection_indices[i].size();
                }
                ...
                nbr_intersection_offsets.resize(nbr_intersection_sizes.size() + size_t{1},
                                                handle.get_stream());
                nbr_intersection_offsets.set_element_to_zero_async(size_t{0}, handle.get_stream());
                auto size_first = thrust::make_transform_iterator(
                    nbr_intersection_sizes.begin(), detail::typecast_t<edge_t, size_t>{});
                thrust::inclusive_scan(handle.get_thrust_policy(),
                                       size_first,
            ...
            else
            {
                ...
                auto edge_partition_e_value_input
                    = edge_partition_e_input_device_view_t(edge_value_input, 0);
                auto edge_partition_e_mask
                    = edge_mask_view ? thrust::make_optional<
                                           detail::edge_partition_edge_property_device_view_t<
                                               edge_t,
                                               packed_bool_container_t const*,
                                               bool>>(*edge_mask_view, 0)
                                     : thrust::nullopt;
                ...
                nbr_intersection_offsets.resize(nbr_intersection_sizes.size() + 1,
                                                handle.get_stream());
                nbr_intersection_offsets.set_element_to_zero_async(size_t{0}, handle.get_stream());
                auto size_first = thrust::make_transform_iterator(
                    nbr_intersection_sizes.begin(), detail::typecast_t<edge_t, size_t>{});
                ...
                nbr_intersection_indices.resize(
                    nbr_intersection_offsets.back_element(handle.get_stream()),
                    handle.get_stream());
                ...
                if constexpr(!std::is_same_v<edge_property_value_t, thrust::nullopt_t>)
                {
                    ...
                    optional_nbr_intersection_e_property_values0
                        = nbr_intersection_e_property_values0.data();
                    optional_nbr_intersection_e_property_values1
                        = nbr_intersection_e_property_values1.data();
                }
    ...
    // thrust::remove_if, and thrust::copy_if (https://github.com/NVIDIA/thrust/issues/1302)
                ...
                [[maybe_unused]] auto tmp_property_values0
                    = rocgraph::detail::allocate_optional_dataframe_buffer<
                        optional_property_buffer_value_type>(tmp_indices.size(),
                                                             handle.get_stream());
                        ...
                        num_copied += static_cast<size_t>(thrust::distance(
                            ...
                            thrust::copy_if(handle.get_thrust_policy(),
                                            ...
                                            nbr_intersection_indices.begin() + num_scanned
                                                + this_scan_size,
                                            tmp_indices.begin() + num_copied,
                                            detail::is_not_equal_t<vertex_t>{
                                                invalid_vertex_id<vertex_t>::value})));
                    }
                    else
                    {
                        auto zipped_itr_to_indices_and_e_property_values_begin
                            = thrust::make_zip_iterator(
                        ...
                        auto zipped_itr_to_tmps_begin = thrust::make_zip_iterator(
                            thrust::make_tuple(tmp_indices.begin(),
                                               tmp_property_values0.begin(),
                                               tmp_property_values1.begin()));
