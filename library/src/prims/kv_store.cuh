...
// under cpp/include/rocgraph; this requires exposing hipco in the public interface and is currently
// problematic.
...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        using hipco_storage_type = cuco::storage<1>; ///< hipco window storage type
        ...
        struct kv_hipco_insert_if_and_increment_t
        {
            ...
            __device__ size_t operator()(size_t i)
            {
                if(pred_op(*(stencil_first + i)) == false)
                {
                    return invalid_idx;
                }
        ...
        struct kv_hipco_store_find_device_view_t
        {
            ...
            using hipco_store_device_ref_type =
                typename ViewType::hipco_map_type::template ref_type<cuco::find_tag>;
        ...
        public:
            using key_type
                = std::remove_cv_t<typename thrust::iterator_traits<KeyIterator>::value_type>;
            using value_type
                = std::remove_cv_t<typename thrust::iterator_traits<ValueIterator>::value_type>;
            ...
            KeyIterator store_key_first() const
            {
                return store_key_first_;
            }

            KeyIterator store_key_last() const
            {
                return store_key_last_;
            }

            ValueIterator store_value_first() const
            {
        ...
        public:
            ...
            using hipco_map_type = cuco::static_map<
                key_t,
                      ...
                      rmm::cuda_stream_view stream) const
            {
                ...
                {
                    rmm::device_uvector<size_t> indices(thrust::distance(key_first, key_last),
                                                        stream);
            ...
            auto hipco_store_find_device_ref() const
            {
                return hipco_store_->ref(cuco::find);
            }
            ...
            key_t invalid_key() const
            {
                return hipco_store_->empty_key_sentinel();
            }
        ...
        public:
            ...
            kv_binary_search_store_t(
                ...
                ,
                bool
                    key_sorted /* if set to true, assume that the input data is sorted and skip sorting (which
            ...
            auto retrieve_all(rmm::cuda_stream_view stream)
            {
                ...
                auto                       tmp_store_values = allocate_dataframe_buffer<value_t>(
                    size_dataframe_buffer(store_values_), stream);
                thrust::copy(rmm::exec_policy(stream),
                             store_keys_.begin(),
                             store_keys_.end(),
                             tmp_store_keys.begin());
                thrust::copy(rmm::exec_policy(stream),
                             get_dataframe_buffer_begin(store_values_),
            ...
            key_t const* store_key_first() const
            {
                return store_keys_.cbegin();
            }

            key_t const* store_key_last() const
            {
                return store_keys_.cend();
            }

            auto store_value_first() const
            {
        ...
        public:
            ...
            {
                static_assert(
                    std::is_same_v<typename thrust::iterator_traits<KeyIterator>::value_type,
                                   key_t>);
                static_assert(
                    std::is_same_v<typename thrust::iterator_traits<ValueIterator>::value_type,
                                   value_t>);
                        ...
                        rmm::cuda_stream_view stream)
            {
                ...
                if constexpr(std::is_arithmetic_v<value_t>)
                {
                    ...
                    size_
                        += hipco_store_->insert(pair_first, pair_first + num_keys, stream.value());
                }
                else
                {
                    auto old_store_value_size
                        = size_optional_dataframe_buffer<value_t>(store_values_);
                    ...
                    auto mutable_device_ref = hipco_store_->ref(cuco::insert_and_find);
                                   ...
                                   rmm::cuda_stream_view stream)
            {
                ...
                if constexpr(std::is_arithmetic_v<value_t>)
                {
                    ...
                    thrust::for_each(
                        ...
                        pair_first,
                        pair_first + num_keys,
                        detail::kv_hipco_insert_and_assign_t<decltype(mutable_device_ref),
                                                             key_t,
                                                             value_t>{mutable_device_ref});
                ...
                else
                {
                    ...
                    thrust::sequence(
                        rmm::exec_policy(), kv_indices.begin(), kv_indices.end(), size_t{0});
                    ...
                    kv_indices.resize(
                        thrust::distance(kv_indices.begin(),
                    ...
                    thrust::for_each(
                        ...
                        [key_first,
                         ...
                         store_value_first
                         = get_optional_dataframe_buffer_begin<value_t>(store_values_),
                         device_ref = hipco_store_->ref(cuco::find)] __device__(auto kv_idx) {
            ...
            auto retrieve_all(rmm::cuda_stream_view stream)
            {
                ...
                if constexpr(std::is_arithmetic_v<value_t>)
                {
                    ...
                    auto pair_last
                        = hipco_store_->retrieve_all(keys.begin(), values.begin(), stream.value());
                ...
                else
                {
                    ...
                    auto                        pair_last
                        = hipco_store_->retrieve_all(keys.begin(), indices.begin(), stream.value());
                    // FIXME: this resize (& shrink_to_fit) shouldn't be necessary if size_ is exact
                    keys.resize(thrust::distance(keys.begin(), std::get<0>(pair_last)), stream);
            ...
            hipco_map_type const* hipco_store_ptr() const
            {
                return hipco_store_.get();
            }

            template <typename type = value_t>
            std::enable_if_t<!std::is_arithmetic_v<type>, const_value_iterator>
                store_value_first() const
            {
        ...
        private:
                          ...
                          rmm::cuda_stream_view stream)
            {
                ...
                auto hipco_size
                    = std::max(static_cast<size_t>(static_cast<double>(num_keys) / load_factor),
                               static_cast<size_t>(num_keys)
                                   + 1); // cuco::static_map requires at least one empty slot
 ...
 * false) */
    ...
    public:
        ...
        kv_store_t(rmm::cuda_stream_view stream)
            : store_(stream)
        {
        }
        ...
        kv_store_t(
            size_t
                capacity /* one can expect good performance till the capacity, the actual underlying
                       capacity can be larger (for performance & correctness reasons) */
            ,
            key_t
                invalid_key /* invalid key shouldn't appear in any *iter in [key_first, key_last) */
            ,
        ...
        auto retrieve_all(rmm::cuda_stream_view stream) const
        {
            return store_.retrieve_all(stream);
        }

        // kv_store_t becomes empty after release
        auto release(rmm::cuda_stream_view stream)
        {
            return store_.release(stream);
        }
        ...
        auto view() const
        {
            ...
            else
            {
                if constexpr(std::is_arithmetic_v<value_t>)
                {
                    return detail::kv_hipco_store_view_t<key_t, value_t const*>(
                        store_.hipco_store_ptr());
                }
                else
                {
                    return detail::kv_hipco_store_view_t<key_t,
                                                         decltype(store_.store_value_first())>(
                        store_.hipco_store_ptr(),
                        store_.store_value_first(),
                        store_.invalid_value());
                }
