...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        struct transform_and_atomic_reduce_t
        {
            ...
            __device__ void operator()(edge_t i) const
            {
                ...
                if(e_op_result != identity_element)
                {
                    ...
                    {
                        reduce_op::atomic_reduce<ReduceOp>(
                            result_value_output, minor_offset, e_op_result);
                    }
                    else
                    {
                        reduce_op::atomic_reduce<ReduceOp>(result_value_output + minor_offset,
                                                           e_op_result);
                    }
                }
            }
        ...
        __global__ static void per_v_transform_reduce_e_hypersparse(
            ...
            ReduceOp reduce_op)
        {
            static_assert(update_major
                          || reduce_op::has_compatible_raft_comms_op_v<
            ...
            auto       major_start_offset = static_cast<size_t>(
            ...
            {
                auto major        = *(edge_partition.major_from_major_hypersparse_idx_nocheck(
                    static_cast<vertex_t>(idx)));
                auto major_offset = edge_partition.major_offset_from_major_nocheck(major);
                auto major_idx    = major_start_offset
                ...
                {
                    auto transform_op = [&edge_partition_e_mask,
                                         &call_e_op,
                                         identity_element,
            ...
            ReduceOp reduce_op)
        {
            ...
            while(idx < static_cast<size_t>(major_range_last - major_range_first))
            {
                ...
                [[maybe_unused]] auto reduced_e_op_result
                    = lane_id == 0 ? init
                                   : identity_element; // relevant only if update_major == true
                if(edge_partition_e_mask)
                {
                    for(edge_t i = lane_id; i < local_degree; i += raft::warp_size())
                    {
                        if((*edge_partition_e_mask).get(edge_offset + i))
                        {
                            ...
                            {
                                auto minor_offset
                                    = edge_partition.minor_offset_from_minor_nocheck(indices[i]);
            ...
            ReduceOp reduce_op)
        {
            ...
            [[maybe_unused]] __shared__ typename BlockReduce::TempStorage
                temp_storage; // relevant only if update_major == true
            ...
            while(idx < static_cast<size_t>(major_range_last - major_range_first))
            {
                ...
                {
                    reduced_e_op_result
                        = BlockReduce(temp_storage).Reduce(reduced_e_op_result, reduce_op);
                    if(threadIdx.x == 0)
                    {
                        *(result_value_output + idx) = reduced_e_op_result;
                    }
        ...
        {
            static_assert(
                ReduceOp::pure_function && reduce_op::has_compatible_raft_comms_op_v<ReduceOp>
            ...
            if constexpr(update_major)
            {
                ...
                if(segment_offsets)
                { // no vertices in the zero degree segment are visited
            ...
            else
            {
                if constexpr(GraphViewType::is_multi_gpu)
                {
                    ...
                    if(view.keys())
                    { // defer applying the initial value to the end as minor_tmp_buffer may not
                    ...
                    fill_edge_minor_property(
                        handle, graph_view, minor_init, minor_tmp_buffer->mutable_view());
                }
                else
                {
                    thrust::fill(handle.get_thrust_policy(),
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                   ...
                   && (handle.get_stream_pool_size() >= max_segments))
                {
                    ...
                    size_t num_streams = std::min(
                        static_cast<size_t>(minor_comm_size) * max_segments,
            ...
            if constexpr(GraphViewType::is_multi_gpu && update_major)
            {
                ...
                if(stream_pool_indices)
                {
                    ...
                    for(size_t i = 0; i < num_concurrent_loops; ++i)
                    {
                        ...
                        major_tmp_buffers.push_back(
                            allocate_dataframe_buffer<T>(max_size, handle.get_stream()));
                    }
                }
                else
                {
                    ...
                    major_tmp_buffers.push_back(allocate_dataframe_buffer<T>(
                        *std::max_element(major_tmp_buffer_sizes.begin(),
                                          major_tmp_buffer_sizes.end()),
                        handle.get_stream()));
                }
            }
            else
            ...
            for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
            {
                ...
                if constexpr(update_major)
                {
                    if constexpr(GraphViewType::is_multi_gpu)
                    {
                        auto& minor_comm
                            = handle.get_subcomm(rocgraph::partition_manager::minor_comm_name());
                        auto const minor_comm_rank = minor_comm.get_rank();
                        major_init                 = (static_cast<int>(i) == minor_comm_rank)
                                                         ? init
                ...
                if constexpr(GraphViewType::is_multi_gpu)
                {
                    ...
                    {
                        output_buffer = edge_partition_minor_output_device_view_t(
                            minor_tmp_buffer->mutable_view());
                    }
                }
                else
                {
                ...
                if(segment_offsets)
                {
                    ...
                    if(edge_partition.dcs_nzd_vertex_count())
                    {
                        auto exec_stream
                            = stream_pool_indices
                                  ? handle.get_stream_from_stream_pool(
                                        (i * max_segments) % (*stream_pool_indices).size())
                                  : handle.get_stream();

                        if constexpr(update_major)
                        { // this is necessary as we don't visit every vertex in the
                            // hypersparse segment
                        ...
                        if(*(edge_partition.dcs_nzd_vertex_count()) > 0)
                        {
                            ...
                            if constexpr(update_major)
                            {
                                segment_output_buffer += (*segment_offsets)[3];
                            }
                            THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                                (detail::per_v_transform_reduce_e_hypersparse<update_major,
                                                                              GraphViewType>),
                    ...
                    if((*segment_offsets)[3] - (*segment_offsets)[2] > 0)
                    {
                        auto exec_stream
                            = stream_pool_indices
                                  ? handle.get_stream_from_stream_pool(
                                        (i * max_segments + 1) % (*stream_pool_indices).size())
                                  : handle.get_stream();
                        raft::grid_1d_thread_t update_grid(
                        ...
                        if constexpr(update_major)
                        {
                            segment_output_buffer += (*segment_offsets)[2];
                        }
                        THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                            (detail::per_v_transform_reduce_e_low_degree<update_major,
                                                                         GraphViewType>),
                            dim3(update_grid.num_blocks),
                            dim3(update_grid.block_size),
                            0,
                    ...
                    if((*segment_offsets)[2] - (*segment_offsets)[1] > 0)
                    {
                        auto exec_stream
                            = stream_pool_indices
                                  ? handle.get_stream_from_stream_pool(
                                        (i * max_segments + 2) % (*stream_pool_indices).size())
                                  : handle.get_stream();
                        ...
                        if constexpr(update_major)
                        {
                            segment_output_buffer += (*segment_offsets)[1];
                        }
                        THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                            (detail::per_v_transform_reduce_e_mid_degree<update_major,
                                                                         GraphViewType>),
                            dim3(update_grid.num_blocks),
                            dim3(update_grid.block_size),
                            0,
                    ...
                    if((*segment_offsets)[1] > 0)
                    {
                        auto exec_stream
                            = stream_pool_indices
                                  ? handle.get_stream_from_stream_pool(
                                        (i * max_segments + 3) % (*stream_pool_indices).size())
                                  : handle.get_stream();
                        raft::grid_1d_block_t update_grid(
                            ...
                            handle.get_device());
                        THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                            (detail::per_v_transform_reduce_e_high_degree<update_major,
                                                                          GraphViewType>),
                            dim3(update_grid.num_blocks),
                            dim3(update_grid.block_size),
                            0,
                ...
                if constexpr(GraphViewType::is_multi_gpu && update_major)
                {
                    ...
                    else
                    {
                        size_t reduction_size = static_cast<size_t>(
                            segment_offsets ? *((*segment_offsets).rbegin()
                                                + 1) /* exclude the zero degree segment */
                                            : edge_partition.major_range_size());
                        device_reduce(minor_comm,
                                      major_buffer_first,
                                      vertex_value_output_first,
            ...
            if constexpr(GraphViewType::is_multi_gpu && !update_major)
            {
                ...
                auto view = minor_tmp_buffer->view();
                if(view.keys())
                { // applying the initial value is deferred to here
                    ...
                    for(int i = 0; i < major_comm_size; ++i)
                    {
                        auto this_segment_vertex_partition_id
                            = compute_local_edge_partition_minor_range_vertex_partition_id_t{
                                ...
                                major_comm_rank,
                                minor_comm_rank}(i);
                        max_vertex_partition_size = std::max(max_vertex_partition_size,
                                                             graph_view.vertex_partition_range_size(
                                                                 this_segment_vertex_partition_id));
                    }
                    auto tx_buffer       = allocate_dataframe_buffer<T>(max_vertex_partition_size,
                                                                  handle.get_stream());
                    ...
                    {
                        auto minor_init
                            = (major_comm_rank == i) ? init : ReduceOp::identity_element;
                        auto this_segment_vertex_partition_id
                            = compute_local_edge_partition_minor_range_vertex_partition_id_t{
                                major_comm_size,
                                minor_comm_size,
                        ...
                        auto value_first = thrust::make_transform_iterator(
                            view.value_first(), [reduce_op, minor_init] __device__(auto val) -> T {
                                return reduce_op(val, minor_init);
                            });
                        thrust::scatter(
                ...
                else
                {
                    ...
                    vertex_t minor_range_first = graph_view.vertex_partition_range_first(
                    ...
                    for(int i = 0; i < major_comm_size; ++i)
                    {
                        auto this_segment_vertex_partition_id
                            = compute_local_edge_partition_minor_range_vertex_partition_id_t{
                                ...
                                major_comm_rank,
                                minor_comm_rank}(i);
                        auto offset = graph_view.vertex_partition_range_first(
                                          this_segment_vertex_partition_id)
                                      - minor_range_first;
                        device_reduce(major_comm,
