    ...
    namespace detail
    {
                                      ...
                                      EdgeMajorPropertyOutputWrapper edge_major_property_output)
        {
            ...
            for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
            {
                ...
                else
                {
                    ...
                    {
                        num_buffer_elements = static_cast<size_t>(
                                 ...
                                 T>())
                {
                    ...
                    thrust::fill_n(handle.get_thrust_policy(),
                                   value_firsts[i],
                                   rocgraph::packed_bool_size(num_buffer_elements),
                                   packed_input);
                }
                else
                {
                    thrust::fill_n(
                        handle.get_thrust_policy(), value_firsts[i], num_buffer_elements, input);
                }
            }
        }
                                      ...
                                      EdgeMinorPropertyOutputWrapper edge_minor_property_output)
        {
            ...
            else
            {
                ...
                {
                    num_buffer_elements
                        = static_cast<size_t>(graph_view.local_edge_partition_src_range_size());
                }
                else
                {
                    num_buffer_elements
            ...
            if constexpr(rocgraph::has_packed_bool_element<decltype(value_first), T>())
            {
