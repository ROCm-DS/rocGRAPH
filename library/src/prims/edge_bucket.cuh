...
namespace rocgraph
{
   ...
   */
        ...
        void insert(vertex_t src, vertex_t dst, tag_type tag)
        {
            ...
            else
            {
                ...
                auto triplet_first = thrust::make_zip_iterator(
                    thrust::make_tuple(majors_.data(), minors_.data(), tags_.data()));
                thrust::fill(handle_ptr_->get_thrust_policy(),
                             triplet_first,
                             triplet_first + 1,
   ...
   */
        ...
        {
            static_assert(
                std::is_same_v<typename thrust::iterator_traits<VertexIterator>::value_type,
                               vertex_t>);
            ...
            {
                majors_.resize(thrust::distance(major_first, major_last),
                               handle_ptr_->get_stream());
                minors_.resize(majors_.size(), handle_ptr_->get_stream());
                thrust::copy(handle_ptr_->get_thrust_policy(),
                             new_pair_first,
   ...
   */
                    ...
                    TagIterator    tag_first)
        {
            static_assert(
                std::is_same_v<typename thrust::iterator_traits<VertexIterator>::value_type,
                               vertex_t>);
            static_assert(
                std::is_same_v<typename thrust::iterator_traits<TagIterator>::value_type, tag_t>);

            auto major_first       = src_major ? src_first : dst_first;
            auto major_last        = major_first + thrust::distance(src_first, src_last);
            auto minor_first       = src_major ? dst_first : src_first;
            auto new_triplet_first = thrust::make_zip_iterator(
                thrust::make_tuple(major_first, minor_first, tag_first));
            ...
            if(majors_.size() > 0)
            {
                if constexpr(sorted_unique)
                {
                    ...
                    rmm::device_uvector<vertex_t> merged_minors(merged_majors.size(),
                                                                handle_ptr_->get_stream());
                    rmm::device_uvector<tag_t>    merged_tags(merged_majors.size(),
                                                           handle_ptr_->get_stream());
                    auto                          old_triplet_first = thrust::make_zip_iterator(
                        thrust::make_tuple(majors_.begin(), minors_.begin(), tags_.begin()));
                    auto merged_triplet_first = thrust::make_zip_iterator(thrust::make_tuple(
                ...
                else
                {
                    ...
                    thrust::copy(handle_ptr_->get_thrust_policy(),
                                 ...
                                 thrust::make_zip_iterator(thrust::make_tuple(
                                     majors_.begin(), minors_.begin(), tags_.begin()))
                                     + cur_size);
                }
            }
            else
        ...
        auto const src_begin() const
        {
            return src_major ? majors_.begin() : minors_.begin();
        }
        ...
        auto const src_end() const
        {
            return (src_major ? majors_.begin() : minors_.begin()) + size();
        }
        ...
        auto const dst_begin() const
        {
            return src_major ? minors_.begin() : majors_.begin();
        }
        ...
        auto const dst_end() const
        {
            return (src_major ? minors_.begin() : majors_.begin()) + size();
        }
