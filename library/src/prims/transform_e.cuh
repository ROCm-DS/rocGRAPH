...
namespace rocgraph
{
    ...
    namespace detail
    {
            ...
            EdgeOp                              e_op)
        {
            ...
            // static_assert(raft::warp_size() == rocgraph::packed_bools_per_word());
            ...
            auto       idx     = static_cast<edge_t>(rocgraph::packed_bool_offset(tid));

            auto num_edges = edge_partition.number_of_edges();
            while(idx < static_cast<edge_t>(rocgraph::packed_bool_size(num_edges)))
            {
                [[maybe_unused]] packed_bool_container_t edge_mask = rocgraph::
                    packed_bool_full_mask(); // relevant only when check_edge_mask is true
                if constexpr(check_edge_mask)
                {
                    edge_mask = *(edge_partition_e_mask.value_first() + idx);
                }
                auto local_edge_idx = idx * static_cast<edge_t>(rocgraph::packed_bools_per_word())
                                      + static_cast<edge_t>(lane_id);
                ...
                if(local_edge_idx < num_edges)
                {
                    ...
                    if constexpr(check_edge_mask)
                    {
                        compute_predicate = ((edge_mask & rocgraph::packed_bool_mask(lane_id))
                    ...
                    {
                        auto major_idx
                            = edge_partition.major_idx_from_local_edge_idx_nocheck(local_edge_idx);
                ...
                packed_bool_container_t new_val = __ballot_sync(raft::warp_full_mask(), predicate);
                if(lane_id == 0)
                {
                    ...
                    if constexpr(check_edge_mask)
                    {
                        ...
                        *(edge_partition_e_value_output.value_first() + idx)
                            = (old_val & rocgraph::packed_bool_negate_mask(edge_mask)) | new_val;
                    }
                    else
                    {
        ...
        struct update_e_value_t
        {
            ...
            __device__ void
                operator()(thrust::tuple<typename GraphViewType::vertex_type,
                        ...
                        if(edge_partition_e_mask.get(edge_offset + thrust::distance(indices, it)))
                        {
                            auto e_op_result
                                = e_op(src,
                                       ...
                                       edge_partition_src_value_input.get(src_offset),
                                       edge_partition_dst_value_input.get(dst_offset),
                                       edge_partition_e_value_input.get(
                                           edge_offset + thrust::distance(indices, it)));
                            edge_partition_e_value_output.set(
                                edge_offset + thrust::distance(indices, it), e_op_result);
                        }
                    }
                    else
                    {
 ...
 * assigned to this process in multi-GPU). Use rocgraph::edge_property_t::mutable_view().
 ...
 */
                     ...
                     bool                     do_expensive_check = false)
    {
        ...
        using edge_partition_e_output_device_view_t
            = detail::edge_partition_edge_property_device_view_t<
                edge_t,
                ...
                if(edge_partition_e_mask)
                {
                    thrust::for_each(
                        ...
                        detail::update_e_value_t<
                            ...
                            EdgeOp,
                            edge_partition_e_output_device_view_t>{edge_partition,
                                                                   edge_partition_src_value_input,
                                                                   edge_partition_dst_value_input,
                                                                   edge_partition_e_value_input,
                                                                   *edge_partition_e_mask,
                     ...
                     bool                     do_expensive_check = false)
    {
        ...
        if(do_expensive_check)
        {
            ROCGRAPH_EXPECTS(thrust::is_sorted(handle.get_thrust_policy(),
                                               edge_first,
                                               edge_first + edge_list.size()),
                             "Invalid input arguments: edge_list is not sorted.");
        }

        std::vector<size_t> edge_partition_offsets(graph_view.number_of_local_edge_partitions() + 1,
                                                   0);
            ...
            if(do_expensive_check)
            {
                ROCGRAPH_EXPECTS(
                    thrust::count_if(
                         ...
                         edge_partition_e_mask] __device__(thrust::tuple<vertex_t, vertex_t> edge) {
                            ...
                            if(edge_partition_e_mask)
                            {
                                auto upper_it = thrust::upper_bound(
                                    thrust::seq, lower_it, indices + local_degree, minor);
                        ...
                        == 0,
                    "Invalid input arguments: edge_list contains edges that do not exist in the "
                    "input graph.");
            }
