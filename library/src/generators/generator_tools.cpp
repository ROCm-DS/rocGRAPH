...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        {
            auto size = std::transform_reduce(input.begin(),
                                              input.end(),
                                              size_t{0},
                                              std::plus<size_t>{},
                                              [](auto const& element) { return element.size(); });
    ...
    std::tuple<rmm::device_uvector<vertex_t>, rmm::device_uvector<vertex_t>>
        scramble_vertex_ids(raft::handle_t const&           handle,
        ...
        thrust::transform(handle.get_thrust_policy(),
                          ...
                          [lgN] __device__(auto pair) {
                              return thrust::make_tuple(
                                  detail::scramble(thrust::get<0>(pair), lgN),
    ...
    {
        ROCGRAPH_EXPECTS(sources.size() == dests.size(),
                         "sources and dests vertex lists must be the same size");

        if(optional_d_weights)
        {
            ROCGRAPH_EXPECTS(sources.size() == optional_d_weights.value().size(),
            ...
            thrust::for_each_n(
                ...
                sources.size(),
                [](auto tuple) {
                    ROCGRAPH_EXPECTS(thrust::get<0>(tuple).size() != thrust::get<1>(tuple).size(),
                                     "source vertex and dest vertex uvectors must be same size");
                    ROCGRAPH_EXPECTS(thrust::get<0>(tuple).size() != thrust::get<2>(tuple).size(),
        ...
        if(remove_multi_edges)
        {
            ...
            if(optional_d_weights)
            {
                thrust::sort(handle.get_thrust_policy(),
                             thrust::make_zip_iterator(thrust::make_tuple(
                                 srcs_v.begin(), dsts_v.begin(), weights_v.begin())),
                             thrust::make_zip_iterator(
                                 thrust::make_tuple(srcs_v.end(), dsts_v.end(), weights_v.end())));

                auto pair_first
                    = thrust::make_zip_iterator(thrust::make_tuple(srcs_v.begin(), dsts_v.begin()));
                auto end_iter = thrust::unique_by_key(handle.get_thrust_policy(),
                                                      pair_first,
            ...
            bool                                           check_diagonal)
    {
        ...
        if(check_diagonal)
        {
            if(optional_d_weights_v)
            {
                auto edge_first               = thrust::make_zip_iterator(thrust::make_tuple(
                    d_src_v.begin(), d_dst_v.begin(), (*optional_d_weights_v).begin()));
            ...
            else
            {
                auto edge_first = thrust::make_zip_iterator(
                    thrust::make_tuple(d_src_v.begin(), d_dst_v.begin()));
                auto strictly_triangular_last = thrust::partition(
                    handle.get_thrust_policy(),
                    edge_first,
                    edge_first + d_src_v.size(),
        ...
        return std::make_tuple(std::move(d_src_v),
                               std::move(d_dst_v),
                               optional_d_weights_v ? std::move(optional_d_weights_v)
                                                    : std::nullopt);
    }

    template rmm::device_uvector<int32_t> scramble_vertex_ids(
        raft::handle_t const& handle, rmm::device_uvector<int32_t>&& vertices, size_t lgN);

    template rmm::device_uvector<int64_t> scramble_vertex_ids(
        raft::handle_t const& handle, rmm::device_uvector<int64_t>&& vertices, size_t lgN);
