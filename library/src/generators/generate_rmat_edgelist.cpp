...
namespace rocgraph
{
    ...
    std::tuple<rmm::device_uvector<vertex_t>, rmm::device_uvector<vertex_t>>
        generate_rmat_edgelist(raft::handle_t const&   handle,
    ...
    {
        ROCGRAPH_EXPECTS((size_t{1} << scale)
        ...
        rmm::device_uvector<float> rands(std::min(num_edges, max_edges_to_generate_per_iteration)
                                             * 2 * scale,
                                         handle.get_stream());
        ...
        {
            return rocgraph::scramble_vertex_ids<vertex_t>(
    ...
    std::vector<std::tuple<rmm::device_uvector<vertex_t>, rmm::device_uvector<vertex_t>>>
        generate_rmat_edgelists(raft::handle_t const&    handle,
                                ...
                                bool                     scramble_vertex_ids)
    {
        ROCGRAPH_EXPECTS(min_scale > 0, "minimum graph scale is 1.");
        ROCGRAPH_EXPECTS(size_t{1} << max_scale
                             <= static_cast<size_t>(std::numeric_limits<vertex_t>::max()),
                         "Invalid input argument: scale too large for vertex_t.");
        ...
        else
        {
            ...
            thrust::transform(handle.get_thrust_policy(),
                              ...
                              [min_scale, range] __device__(auto rnd) {
                                  return min_scale
                                         + static_cast<vertex_t>(static_cast<float>(range) * rnd)
                                               % range;
                              });
        }
