            ...
            result_t                                                      default_value)
        {
            ...
            if constexpr(multi_gpu)
            {
                ...
                rocgraph::detail::sequence_fill(
                    stream, vertex_pos.data(), vertex_pos.size(), size_t{0});
                ...
                std::forward_as_tuple(
                    local_vertices, std::tie(vertex_gpu_ids, vertex_pos), std::ignore)
                ...
                size_t new_size = thrust::distance(
                    ...
                    thrust::remove_if(
                        ...
                        thrust::make_zip_iterator(
                            local_vertices.begin(), vertex_gpu_ids.begin(), vertex_pos.begin()),
                        thrust::make_zip_iterator(
                            local_vertices.end(), vertex_gpu_ids.end(), vertex_pos.end()),
                        [check = rocgraph::detail::check_out_of_range_t<vertex_t>{
                             vertex_partition_view.local_vertex_partition_range_first(),
                             vertex_partition_view
                                 .local_vertex_partition_range_last()}] __device__(auto tuple) {
            ...
            rocgraph::detail::scalar_fill(stream, result.data(), result.size(), default_value);
            ...
            if constexpr(multi_gpu)
            {
                ...
                //
                std::forward_as_tuple(
                    std::ignore, std::tie(std::ignore, vertex_pos, tmp_result), std::ignore)
                    = groupby_gpu_id_and_shuffle_kv_pairs(
                        handle.raft_handle().get_comms(),
                        vertex_gpu_ids.begin(),
                        vertex_gpu_ids.end(),
                        thrust::make_zip_iterator(
                            local_vertices.begin(), vertex_pos.begin(), result.begin()),
                        thrust::identity{},
                        stream);
