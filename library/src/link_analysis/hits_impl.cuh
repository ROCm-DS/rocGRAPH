...
namespace rocgraph
{
    namespace detail
    {
                       ...
                       ReduceOp              reduce_op)
        {
            ...
            ROCGRAPH_EXPECTS(hubs_norm > 0, "Norm is required to be a positive value.");
                                          ...
                                          bool                  do_expensive_check)
        {
            ...
            if(has_initial_hubs_guess && do_expensive_check)
            {
                auto num_negative_values = count_if_v(
                    handle, graph_view, hubs, [] __device__(auto, auto val) { return val < 0.0; });
                ROCGRAPH_EXPECTS(
            ...
            {
                detail::normalize(
                    handle, graph_view, hubs, result_t{0.0}, reduce_op::plus<result_t>{});
            }
            ...
            else
            {
                fill_edge_src_property(
                    handle, graph_view, result_t{1.0} / num_vertices, prev_src_hubs);
            ...
            while(true)
            {
                ...
                per_v_transform_reduce_incoming_e(
                    ...
                    [] __device__(auto, auto, auto prev_src_hub_value, auto, auto) {
                        return prev_src_hub_value;
                    },
                    result_t{0},
                ...
                {
                    ROCGRAPH_FAIL("HITS failed to converge.");
                }
            }
            ...
            {
                detail::normalize(
                    handle, graph_view, prev_hubs, result_t{0.0}, reduce_op::plus<result_t>{});
                detail::normalize(
                    handle, graph_view, authorities, result_t{0.0}, reduce_op::plus<result_t>{});
            }
