...
namespace rocgraph
{
    namespace detail
    {
            ...
            bool                                                         do_expensive_check)
        {
            ...
            if(num_vertices == 0)
            {
                return centrality_algorithm_metadata_t{0, true};
            }
            ...
            if(do_expensive_check)
            {
                if(precomputed_vertex_out_weight_sums)
                {
                    ...
                    ROCGRAPH_EXPECTS(num_negative_precomputed_vertex_out_weight_sums == 0,
                                     "Invalid input argument: outgoing edge weight sum values "
                ...
                if(edge_weight_view)
                {
                    auto num_negative_edge_weights = count_if_e(
                        ...
                        *edge_weight_view,
                        [] __host__ __device__(vertex_t, vertex_t, auto, auto, weight_t w) {
                            return w < 0.0;
                        });
                    ROCGRAPH_EXPECTS(num_negative_edge_weights == 0,
                                     "Invalid input argument: input edge weights should have "
                                     "non-negative values.");
                }
                ...
                if constexpr(GraphViewType::is_multi_gpu)
                {
                    auto num_gpus_with_valid_personalization_vector
                        = host_scalar_allreduce(handle.get_comms(),
                                                ...
                                                raft::comms::op_t::SUM,
                                                handle.get_stream());
                    ROCGRAPH_EXPECTS((num_gpus_with_valid_personalization_vector == 0)
                                         || (num_gpus_with_valid_personalization_vector
                                             == handle.get_comms().get_size()),
                                     "Invalid input argument: personalization vectors should be "
                                     "provided in either no ranks or "
                                     "all ranks.");
                }
                ...
                if(aggregate_personalization_vector_size > 0)
                {
                    ...
                    auto num_invalid_vertices = thrust::count_if(
                        ...
                        [vertex_partition] __device__(auto val) {
                            return !(
                                vertex_partition.is_valid_vertex(val)
                    ...
                    ROCGRAPH_EXPECTS(num_invalid_vertices == 0,
                                     "Invalid input argument: peresonalization vertices have "
                                     "invalid vertex IDs.");
                    ...
                    rmm::device_uvector<vertex_t> check_for_duplicates(
                        std::get<0>(*personalization).size(), handle.get_stream());
                    ...
                    ROCGRAPH_EXPECTS(static_cast<size_t>(num_uniques)
                                         == check_for_duplicates.size(),
                                     "Invalid input argument: personalization vertices not contain "
                                     "duplicate entries.");
                }
            }
            ...
            if(!precomputed_vertex_out_weight_sums)
            {
                ...
                else
                {
                    ...
                    tmp_vertex_out_weight_sums  = rmm::device_uvector<weight_t>(
                        tmp_vertex_out_degrees.size(), handle.get_stream());
            ...
            if(aggregate_personalization_vector_size > 0)
            {
                ...
                if constexpr(GraphViewType::is_multi_gpu)
                {
                    personalization_sum = host_scalar_allreduce(handle.get_comms(),
                                                                personalization_sum,
                                                                raft::comms::op_t::SUM,
                                                                handle.get_stream());
                }
                ROCGRAPH_EXPECTS(personalization_sum > 0.0,
            ...
            // old PageRank values
            rmm::device_uvector<result_t> old_pageranks(
                pull_graph_view.local_vertex_partition_range_size(), handle.get_stream());
            edge_src_property_t<GraphViewType, result_t> edge_src_pageranks(handle,
                                                                            pull_graph_view);
            size_t                                       iter{0};
            while(true)
            {
                ...
                thrust::transform(
                    ...
                    thrust::make_zip_iterator(
                        pageranks.end(),
                        vertex_out_weight_sums
                            + pull_graph_view.local_vertex_partition_range_size()),
                    pageranks.begin(),
                    [] __device__(auto val) {
                        auto const pagerank       = thrust::get<0>(val);
                        auto const out_weight_sum = thrust::get<1>(val);
                ...
                update_edge_src_property(
                    handle, pull_graph_view, pageranks.data(), edge_src_pageranks);
                ...
                if(aggregate_personalization_vector_size > 0)
                {
                    ...
                    thrust::for_each(
                        handle.get_thrust_policy(),
                        thrust::make_zip_iterator(
                            thrust::make_tuple(std::get<0>(*personalization).begin(),
                                               std::get<1>(*personalization).begin())),
                        thrust::make_zip_iterator(
                            thrust::make_tuple(std::get<0>(*personalization).end(),
                                               std::get<1>(*personalization).end())),
                        [vertex_partition,
                         ...
                         alpha] __device__(auto val) {
                            ...
                            *(pageranks
                              + vertex_partition.local_vertex_partition_offset_from_vertex_nocheck(
                                  v))
                                += (dangling_sum * alpha + static_cast<result_t>(1.0 - alpha))
                ...
                auto diff_sum = transform_reduce_v(
                    ...
                    pull_graph_view,
                    thrust::make_zip_iterator(
                        thrust::make_tuple(pageranks.begin(), old_pageranks.begin())),
                    [] __device__(auto, auto val) {
                        return std::abs(thrust::get<0>(val) - thrust::get<1>(val));
                    },
                    result_t{0.0});
                  ...
                  bool                           do_expensive_check)
    {
        ...
        ROCGRAPH_EXPECTS(
            (personalization_vertices.has_value() == false)
        ...
        if(has_initial_guess)
        {
            ...
            {
                auto num_negative_values
                    = count_if_v(handle, graph_view, pageranks, [] __device__(auto, auto val) {
                          return val < 0.0;
                      });
                ROCGRAPH_EXPECTS(
                    num_negative_values == 0,
        ...
        auto metadata = detail::pagerank(
            ...
            precomputed_vertex_out_weight_sums
                ...
                : std::nullopt,
            personalization_vertices ? std::make_optional(std::make_tuple(
                                           raft::device_span<vertex_t const>{
                                               *personalization_vertices,
                                               static_cast<size_t>(*personalization_vector_size)},
                                           raft::device_span<result_t const>{
        ...
        ROCGRAPH_EXPECTS(metadata.converged_, "PageRank failed to converge.");
    }
    ...
    std::tuple<rmm::device_uvector<result_t>, centrality_algorithm_metadata_t> pagerank(
        ...
        std::optional<edge_property_view_t<edge_t, weight_t const*>> edge_weight_view,
        std::optional<raft::device_span<weight_t const>> precomputed_vertex_out_weight_sums,
        std::optional<std::tuple<raft::device_span<vertex_t const>,
                                 raft::device_span<result_t const>>> personalization,
        ...
        bool                                                         do_expensive_check)
    {
        rmm::device_uvector<result_t> local_pageranks(
