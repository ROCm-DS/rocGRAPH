...
struct CStableChecker
{
    ...
    {
        thrust::for_each(
            d_flag_.begin(), d_flag_.end(), [flag] __device__(int& val) { val = flag; });
    }
...
struct SCC_Data
{
    ...
    ByteT const* get_Cptr(void) const
    {
        return p_d_C_;
    }
    ...
    void get_labels(IndexT* d_labels) const
    {
        ...
        thrust::transform(thrust::device,
                          ...
                          [n, p_d_C = p_d_C_] __device__(IndexT k) {
                              ...
                              auto pos
                                  = thrust::find_if(thrust::seq, begin, end, [one](IndexT entry) {
                                        return (entry == one);
                                    });
    ...
    size_t run_scc(IndexT* d_labels)
    {
        ...
        do
        {
            ...
            thrust::for_each(
                ...
                [nrows, p_d_C = p_d_C_, p_d_Cprev, p_d_flag, p_d_ro, p_d_ci] __device__(
                    size_t indx) {
                    ByteT one{1};
