    ...
    namespace
    {
                ...
                typename GraphViewType::edge_type                           degree_sum_threshold)
        {
            ...
            vertex_t max_scan_size
                = static_cast<vertex_t>(handle.get_device_properties().multiProcessorCount)
                  * vertex_t{16384};
            ...
            while((candidate_first + num_scanned < candidate_last)
                  && (degree_sum < degree_sum_threshold) && (num_new_roots < max_new_roots))
            {
                auto scan_size = std::min(max_scan_size,
                                          static_cast<vertex_t>(thrust::distance(
                                              candidate_first + num_scanned, candidate_last)));

                rmm::device_uvector<vertex_t> tmp_new_roots(scan_size, handle.get_stream());
                rmm::device_uvector<vertex_t> tmp_indices(tmp_new_roots.size(),
                                                          handle.get_stream());
                ...
                auto output_pair_first = thrust::make_zip_iterator(
                    thrust::make_tuple(tmp_new_roots.begin(), tmp_indices.begin()));
                ...
                if(tmp_new_roots.size() > 0)
                {
                    ...
                    thrust::transform(
                        ...
                        [vertex_partition, degrees] __device__(auto v) {
                            return degrees[vertex_partition
                    ...
                    if(last != tmp_cumulative_degrees.end())
                    {
                        ++last;
                    }
                    auto tmp_num_new_roots = std::min(static_cast<vertex_t>(thrust::distance(
                                                          tmp_cumulative_degrees.begin(), last)),
        ...
        struct e_op_t
        {
            ...
            __device__ thrust::optional<vertex_t>
                           ...
                           thrust::nullopt_t) const
            {
                ...
                auto old        = dst_components.elementwise_atomic_cas(
                    dst_offset, invalid_component_id<vertex_t>::value, tag);
                if(old != invalid_component_id<vertex_t>::value && old != tag)
                { // conflict
                    static_assert(sizeof(unsigned long long int) == sizeof(size_t));
                    auto edge_idx
                        = atomicAdd(reinterpret_cast<unsigned long long int*>(num_edge_inserts),
        ...
        struct v_op_t
        {
            ...
            decltype(thrust::make_zip_iterator(
                thrust::make_tuple(static_cast<vertex_type*>(nullptr),
                                   static_cast<vertex_type*>(nullptr)))) edge_buffer_first{};
                ...
                operator()(thrust::tuple<vertex_type, vertex_type> tagged_v, int /* v_val */) const
            {
                ...
                bool                         success
                    = v_component.compare_exchange_strong(old, tag, hip::std::memory_order_relaxed);
                if(!success && (old != tag))
                { // conflict
                    return thrust::make_tuple(
                        thrust::optional<size_t>{bucket_idx_conflict},
                ...
                else
                {
                    return thrust::make_tuple(
                        ...
                        success ? thrust::optional<std::byte>{std::byte{0}} /* dummy */
                                : thrust::nullopt);
                }
            }
            ...
            __device__ std::enable_if_t<
                !multi_gpu,
                thrust::tuple<thrust::optional<size_t>, thrust::optional<std::byte>>>
                operator()(thrust::tuple<vertex_type, vertex_type> /* tagged_v */,
                           int /* v_val */) const
            {
                                              ...
                                              bool do_expensive_check)
        {
            ...
            ROCGRAPH_EXPECTS(push_graph_view.is_symmetric(),
                             "Invalid input argument: input graph should be symmetric for weakly "
            ...
            auto degree_sum_threshold
                = static_cast<edge_t>(handle.get_device_properties().multiProcessorCount)
                  * edge_t{1024};
            ...
            while(true)
            {
                ...
                level_component_vectors.push_back(rmm::device_uvector<vertex_t>(
                    num_levels == 0 ? vertex_t{0}
                                    : level_graph_view.local_vertex_partition_range_size(),
                    handle.get_stream()));
                ...
                auto pair_first = thrust::make_zip_iterator(
                    thrust::make_tuple(thrust::make_counting_iterator(
                                           level_graph_view.local_vertex_partition_range_first()),
                                       degrees.begin()));
                    ...
                    thrust::distance(
                        ...
                        thrust::copy_if(
                            ...
                            thrust::make_counting_iterator(
                                level_graph_view.local_vertex_partition_range_first()),
                            thrust::make_counting_iterator(
                                level_graph_view.local_vertex_partition_range_last()),
                            new_root_candidates.begin(),
                ...
                auto high_degree_partition_last = thrust::stable_partition(
                    ...
                    [vertex_partition,
                     ...
                     threshold = static_cast<edge_t>(std::ceil(std::sqrt(
                         static_cast<double>(degree_sum_threshold) * 2.0)))] __device__(auto v) {
                        return degrees[vertex_partition
                                           .local_vertex_partition_offset_from_vertex_nocheck(v)]
                               >= threshold;
                    });
                ...
                auto max_new_roots = std::max(
                    static_cast<vertex_t>(new_root_candidates.size() * max_new_roots_ratio),
                    vertex_t{1});
                ...
                if(GraphViewType::is_multi_gpu)
                {
                    ...
                    auto first_candidate_degrees = host_scalar_gather(
                        comm, first_candidate_degree, int{0}, handle.get_stream());
                    auto new_root_candidate_counts = host_scalar_gather(
                        comm, new_root_candidates.size(), int{0}, handle.get_stream());
                    ...
                    if(comm_rank == 0)
                    {
                        ...
                        if(std::reduce(first_candidate_degrees.begin(),
                                       first_candidate_degrees.end())
                           > degree_sum_threshold * comm_size)
                        {
                            std::vector<std::tuple<edge_t, int>> degree_gpu_id_pairs(comm_size);
                            for(int i = 0; i < comm_size; ++i)
                            {
                                degree_gpu_id_pairs[i]
                                    = std::make_tuple(first_candidate_degrees[i], i);
                            }
                            ...
                            for(size_t i = 0; i < degree_gpu_id_pairs.size(); ++i)
                            {
                                ...
                                if(sum > degree_sum_threshold * comm_size)
                                {
                                    break;
                                }
                            }
                        ...
                        // compression in the worst case.
                                ...
                                <= static_cast<vertex_t>(handle.get_comms().get_size()
                                                         * std::ceil(1.0 / max_new_roots_ratio)))
                        {
                            std::vector<int> gpu_ids{};
                            gpu_ids.reserve(std::reduce(new_root_candidate_counts.begin(),
                                                        new_root_candidate_counts.end()));
                            for(size_t i = 0; i < new_root_candidate_counts.size(); ++i)
                            {
                                gpu_ids.insert(gpu_ids.end(),
                                               new_root_candidate_counts[i],
                                               static_cast<int>(i));
                            }
                            std::random_device rd{};
                            std::shuffle(gpu_ids.begin(), gpu_ids.end(), std::mt19937(rd()));
                            gpu_ids.resize(std::max(
                                static_cast<vertex_t>(gpu_ids.size() * max_new_roots_ratio),
                                vertex_t{1}));
                        ...
                        init_max_new_roots = host_scalar_scatter(
                            comm, init_max_new_root_counts, int{0}, handle.get_stream());
                    }
                    else
                    {
                        init_max_new_roots = host_scalar_scatter(
                            comm, std::vector<vertex_t>{}, int{0}, handle.get_stream());
                    }
                ...
                vertex_frontier_t<vertex_t, vertex_t, GraphViewType::is_multi_gpu, true>
                ...
                while(true)
                {
                    if((edge_count < degree_sum_threshold)
                       && (next_candidate_offset
                           < static_cast<vertex_t>(new_root_candidates.size())))
                    {
                        auto [new_roots, num_scanned, degree_sum]
                            = accumulate_new_roots<GraphViewType>(
                                handle,
                                vertex_partition,
                                level_components,
                                degrees.data(),
                        ...
                        thrust::sort(
                            handle.get_thrust_policy(), new_roots.begin(), new_roots.end());
                        ...
                        auto pair_first = thrust::make_zip_iterator(
                            thrust::make_tuple(new_roots.begin(), new_roots.begin()));
                    ...
                    if constexpr(GraphViewType::is_multi_gpu)
                    {
                        update_edge_dst_property(
                            handle,
                            level_graph_view,
                            thrust::get<0>(vertex_frontier.bucket(bucket_idx_cur)
                                               .begin()
                                               .get_iterator_tuple()),
                            thrust::get<0>(
                                vertex_frontier.bucket(bucket_idx_cur).end().get_iterator_tuple()),
                            level_components,
                            edge_dst_components);
                    }
                            ...
                            e_op_t<vertex_t, decltype(get_dataframe_buffer_begin(edge_buffer))>{
                                GraphViewType::is_multi_gpu
                                    ? detail::edge_partition_endpoint_property_device_view_t<
                                          vertex_t,
                                          vertex_t*>(edge_dst_components.mutable_view())
                                    : detail::edge_partition_endpoint_property_device_view_t<
                                          vertex_t,
                                          vertex_t*>(
                                          detail::edge_minor_property_view_t<vertex_t, vertex_t*>(
                    ...
                    if(GraphViewType::is_multi_gpu)
                    {
                        ...
                        resize_dataframe_buffer(edge_buffer,
                                                cur_num_edge_inserts + conflict_bucket.size(),
                                                handle.get_stream());
                        thrust::for_each(
                            handle.get_thrust_policy(),
                            ...
                            [vertex_partition,
                             ...
                             num_edge_inserts = num_edge_inserts.data()] __device__(auto tagged_v) {
                                ...
                                auto tag = thrust::get<1>(tagged_v);
                                static_assert(sizeof(unsigned long long int) == sizeof(size_t));
                                auto edge_idx = atomicAdd(
                                    reinterpret_cast<unsigned long long int*>(num_edge_inserts),
                                    static_cast<unsigned long long int>(1));
                                // keep only the edges in the lower triangular part
                                *(edge_buffer_first + edge_idx)
                                    = tag >= old ? thrust::make_tuple(tag, old)
                                                 : thrust::make_tuple(old, tag);
                            });
                    ...
                    if(new_num_edge_inserts > old_num_edge_inserts)
                    {
                        ...
                        auto unique_edge_last = thrust::unique(handle.get_thrust_policy(),
                                                               edge_first,
                                                               edge_first + new_num_edge_inserts);
                    ...
                    edge_count = thrust::transform_reduce(
                        handle.get_thrust_policy(),
                        thrust::get<0>(
                            vertex_frontier.bucket(bucket_idx_cur).begin().get_iterator_tuple()),
                        thrust::get<0>(
                            vertex_frontier.bucket(bucket_idx_cur).end().get_iterator_tuple()),
                        [vertex_partition, degrees = degrees.data()] __device__(auto v) {
                            return degrees[vertex_partition
                ...
                if(GraphViewType::is_multi_gpu)
                {
                    ...
                    aggregate_num_inserts = host_scalar_allreduce(
                        comm, num_inserts, raft::comms::op_t::SUM, handle.get_stream());
                }
                ...
                if(aggregate_num_inserts > 0)
                {
                    ...
                    auto output_first = thrust::make_zip_iterator(thrust::make_tuple(
                                            thrust::get<1>(input_first.get_iterator_tuple()),
                    ...
                    if(GraphViewType::is_multi_gpu)
                    {
                        ...
                        auto unique_edge_last
                            = thrust::unique(handle.get_thrust_policy(), edge_first, edge_last);
                        resize_dataframe_buffer(
                            edge_buffer,
                            static_cast<size_t>(thrust::distance(edge_first, unique_edge_last)),
                            handle.get_stream());
            ...
            for(size_t i = 0; i < num_levels - 1; ++i)
            {
                ...
                rmm::device_uvector<vertex_t> next_local_vertices(
                    level_renumber_map_vectors[next_level].size(), handle.get_stream());
