...
*/
            ...
            bool do_expensive_check)
    {
        ...
        {
            source_start = rocgraph::host_scalar_allgather(
                handle.get_comms(), num_sources, handle.get_stream());
            num_sources = std::reduce(source_start.begin(), source_start.end());
            std::exclusive_scan(
                source_start.begin(), source_start.end(), source_start.begin(), size_t{0});
        }
        ...
        for(size_t i = 0; i < num_sources; i++)
        {
            ...
            rmm::device_uvector<vertex_t> local_reach(
                graph_view.local_vertex_partition_range_size(), worker_stream_view);
            ...
            if constexpr(multi_gpu)
            {
                ...
                if(gpu == handle.get_comms().get_rank())
                    source = raft::device_span<vertex_t const>{
                        source_vertex.data() + i - source_start[gpu], 1};
                else
                    source = raft::device_span<vertex_t const>{source_vertex.data(), size_t{0}};
            }

            rocgraph::bfs<vertex_t, edge_t, multi_gpu>(multi_gpu ? handle : light_handle,
                                                       graph_view,
                                                       reached[i].data(),
                                                       nullptr,
                                                       source.data(),
        ...
        // extract
        return rocgraph::extract_induced_subgraphs(
            handle,
            graph_view,
            edge_weight_view,
...
namespace rocgraph
{
                    ...
                    vertex_t                                                     radius)
    {
        ...
        ROCGRAPH_EXPECTS(n_subgraphs > 0, "Need at least one source to extract the egonet from");
        ROCGRAPH_EXPECTS(n_subgraphs < graph_view.number_of_vertices(),
                         "Can't have more sources to extract from than vertices in the graph");
        ROCGRAPH_EXPECTS(radius > 0, "Radius should be at least 1");
        ROCGRAPH_EXPECTS(radius < graph_view.number_of_vertices(), "radius is too large");
                    ...
                    bool                                                         do_expensive_check)
    {
        ...
        return extract(
            handle, graph_view, edge_weight_view, source_vertex, radius, do_expensive_check);
    }
