    ...
    namespace detail
    {
        ...
        struct key_aggregated_edge_op_t
        {
                           ...
                           weight_t new_cluster_sum) const
            {
                ...
                if(src_cluster == neighbor_cluster)
                    new_cluster_sum -= cluster_subtract;

                weight_t delta_modularity
                    = 2
                      * (((new_cluster_sum - old_cluster_sum) / total_edge_weight)
        ...
        struct reduce_op_t
        {
            ...
            inline static type const identity_element = thrust::make_tuple(
        ...
        struct count_updown_moves_op_t
        {
            bool up_down{};
            __device__ auto
                operator()(thrust::tuple<vertex_t, thrust::tuple<vertex_t, weight_t>> p) const
            {
        ...
        {
            bool            up_down{};
            __device__ auto operator()(vertex_t                          old_cluster,
                                       thrust::tuple<vertex_t, weight_t> p) const
            {
            ...
            weight_t                             resolution)
        {
            ...
            {
                sum_degree_squared = host_scalar_allreduce(handle.get_comms(),
                                                           sum_degree_squared,
                                                           raft::comms::op_t::SUM,
                                                           handle.get_stream());
            }
            ...
            weight_t Q
                = sum_internal / total_edge_weight
        ...
        template <typename vertex_t, typename edge_t, typename weight_t, bool multi_gpu>
        std::tuple<rocgraph::graph_t<vertex_t, edge_t, false, multi_gpu>,
                   std::optional<
                ...
                raft::device_span<vertex_t>                                       labels)
        {
            ...
            rmm::device_uvector<vertex_t> numbering_indices((*numbering_map).size(),
                                                            handle.get_stream());
            ...
            bool up_down)
        {
            ...
            auto zipped_src_device_view
                = multi_gpu
                      ...
                      : view_concat(detail::edge_major_property_view_t<vertex_t, weight_t const*>(
                                        vertex_weights_v.data()),
                                    detail::edge_major_property_view_t<vertex_t, vertex_t const*>(
                                        next_clusters_v.data()),
                                    detail::edge_major_property_view_t<vertex_t, weight_t const*>(
                                        vertex_cluster_weights_v.data()),
                                    detail::edge_major_property_view_t<
                                        vertex_t,
            ...
            per_v_transform_reduce_dst_key_aggregated_outgoing_e(
                ...
                *edge_weight_view,
                ...
                rocgraph::get_dataframe_buffer_begin(output_buffer));

            int nr_moves = thrust::count_if(
                handle.get_thrust_policy(),
                thrust::make_zip_iterator(thrust::make_tuple(
                    next_clusters_v.begin(), rocgraph::get_dataframe_buffer_begin(output_buffer))),
                thrust::make_zip_iterator(thrust::make_tuple(
                    next_clusters_v.end(), rocgraph::get_dataframe_buffer_end(output_buffer))),
                                    ...
                                    vertex_t> const&                         src_clusters_cache)
        {
            ...
            auto [cluster_keys, cluster_values] = rocgraph::transform_reduce_e_by_src_key(
                handle,
                graph_view,
