...
// FIXME: Disable random moves in refinement phase for now.
        ...
        std::tuple<rmm::device_uvector<typename GraphViewType::vertex_type>,
            ...
            refine_clustering(
                ...
                std::optional<edge_property_view_t<typename GraphViewType::edge_type,
                ...
                rmm::device_uvector<typename GraphViewType::vertex_type>&&
                                                                    louvain_assignment_of_vertices,
                ...
                bool up_down)
        {
            ...
            if(GraphViewType::is_multi_gpu)
            {
                ...
                rocgraph::detail::compute_gpu_id_from_ext_vertex_t<vertex_t> vertex_to_gpu_id_op{
                    comm_size, major_comm_size, minor_comm_size};

                vertex_louvain_cluster_weights
                    = rocgraph::collect_values_for_keys(handle,
            ...
            per_v_transform_reduce_outgoing_e(
                ...
                *edge_weight_view,
                ...
                weight_t{0},
                rocgraph::reduce_op::plus<weight_t>{},
                weighted_cut_of_vertices_to_louvain.begin());

            rmm::device_uvector<uint8_t> singleton_and_connected_flags(
                graph_view.local_vertex_partition_range_size(), handle.get_stream());

            auto wcut_deg_and_cluster_vol_triple_begin = thrust::make_zip_iterator(
                thrust::make_tuple(weighted_cut_of_vertices_to_louvain.begin(),
                                   weighted_degree_of_vertices.begin(),
                                   vertex_louvain_cluster_weights.begin()));
            auto wcut_deg_and_cluster_vol_triple_end = thrust::make_zip_iterator(
                thrust::make_tuple(weighted_cut_of_vertices_to_louvain.end(),
            ...
            thrust::transform(
                ...
                [resolution,
                 total_edge_weight] __device__(auto wcut_wdeg_and_louvain_volume) -> uint8_t {
            ...
            edge_src_property_t<GraphViewType, uint8_t>  src_singleton_and_connected_flag_cache(
                handle);
            ...
            while(true)
            {
                ...
                if(nr_remaining_active_vertices == 0)
                {
                    break;
                }
                ...
                auto src_input_property_values
                    = GraphViewType::is_multi_gpu
                          ? view_concat(src_louvain_assignment_cache.view(),
                ...
                auto dst_input_property_values
                    = GraphViewType::is_multi_gpu
                          ? view_concat(dst_louvain_assignment_cache.view(),
                                        dst_leiden_assignment_cache.view())
                          : view_concat(
                                detail::edge_minor_property_view_t<vertex_t, vertex_t const*>(
                                    louvain_assignment_of_vertices.data(), vertex_t{0}),
                                detail::edge_minor_property_view_t<vertex_t, vertex_t const*>(
                ...
                rmm::device_uvector<vertex_t> leiden_keys_used_in_edge_reduction(
                    0, handle.get_stream());
                ...
                std::forward_as_tuple(leiden_keys_used_in_edge_reduction,
                    ...
                    = rocgraph::transform_reduce_e_by_dst_key(
                        handle,
                        graph_view,
                ...
                if(GraphViewType::is_multi_gpu)
                {
                    ...
                    rmm::device_uvector<vertex_t> d_partitions_range_lasts(
                    ...
                    louvain_of_leiden_keys_used_in_edge_reduction
                        = rocgraph::collect_values_for_keys(
                            handle,
                ...
                auto                                                 gain_and_dst_output_pairs
                    = allocate_dataframe_buffer<thrust::tuple<weight_t, vertex_t>>(
                        graph_view.local_vertex_partition_range_size(), handle.get_stream());
                ...
                per_v_transform_reduce_dst_key_aggregated_outgoing_e(
                    ...
                    *edge_weight_view,
                    ...
                    rocgraph::get_dataframe_buffer_begin(gain_and_dst_output_pairs));
                ...
                auto     gain_and_dst_first
                    = rocgraph::get_dataframe_buffer_cbegin(gain_and_dst_output_pairs);
                auto gain_and_dst_last
                    = rocgraph::get_dataframe_buffer_cend(gain_and_dst_output_pairs);

                auto vertex_begin = thrust::make_counting_iterator(
                    graph_view.local_vertex_partition_range_first());
                auto vertex_end = thrust::make_counting_iterator(
                ...
                {
                    rocgraph::resize_dataframe_buffer(
                        gain_and_dst_output_pairs, 0, handle.get_stream());
                    rocgraph::shrink_to_fit_dataframe_buffer(gain_and_dst_output_pairs,
                                                             handle.get_stream());
                    break;
                }
                ...
                std::optional<rmm::device_uvector<weight_t>> d_weights = std::make_optional(
                    rmm::device_uvector<weight_t>(nr_valid_tuples, handle.get_stream()));
                ...
                rocgraph::graph_t<vertex_t, edge_t, store_transposed, multi_gpu> decision_graph(
                    handle);
                ...
                std::tie(
                    decision_graph, coarse_edge_weights, std::ignore, std::ignore, renumber_map)
                    = create_graph_from_edgelist<vertex_t,
                        ...
                        std::nullopt,
                        std::nullopt,
                        rocgraph::graph_properties_t{false, false},
                        true,
                        false);
                ...
                rmm::device_uvector<vertex_t> numbering_indices((*renumber_map).size(),
                                                                handle.get_stream());
                detail::sequence_fill(handle.get_stream(),
                ...
                if(GraphViewType::is_multi_gpu)
                {
                    vertices_in_mis = rocgraph::detail::
                ...
                thrust::for_each(
                    ...
                    [dst_first         = thrust::get<1>(gain_and_dst_first.get_iterator_tuple()),
                     ...
                     v_first
                     = graph_view.local_vertex_partition_range_first()] __device__(vertex_t v) {
                ...
                //
                rmm::device_uvector<vertex_t> dst_vertices(vertices_in_mis.size(),
                                                           handle.get_stream());
                thrust::transform(
                    handle.get_thrust_policy(),
                    vertices_in_mis.begin(),
                ...
                dst_vertices.resize(
                    static_cast<size_t>(thrust::distance(dst_vertices.begin(),
                ...
                if constexpr(GraphViewType::is_multi_gpu)
                {
                    ...
                    thrust::sort(
                        handle.get_thrust_policy(), dst_vertices.begin(), dst_vertices.end());

                    dst_vertices.resize(static_cast<size_t>(thrust::distance(
                                            dst_vertices.begin(),
                                            thrust::unique(handle.get_thrust_policy(),
                                                           dst_vertices.begin(),
                                                           dst_vertices.end()))),
            ...
            auto nr_unique_leiden_clusters = static_cast<size_t>(
                thrust::distance(leiden_keys_to_read_louvain.begin(),
                                 thrust::unique(handle.get_thrust_policy(),
                                                leiden_keys_to_read_louvain.begin(),
                                                leiden_keys_to_read_louvain.end())));
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                //   rocgraph::detail::shuffle_ext_vertices_to_local_gpu_by_vertex_partitioning(
                //     handle, std::move(leiden_keys_to_read_louvain));

                leiden_keys_to_read_louvain
                    = rocgraph::detail::shuffle_int_vertices_to_local_gpu_by_vertex_partitioning(
                        handle,
                ...
                nr_unique_leiden_clusters = static_cast<size_t>(
                    thrust::distance(leiden_keys_to_read_louvain.begin(),
                                     thrust::unique(handle.get_thrust_policy(),
                                                    leiden_keys_to_read_louvain.begin(),
                                                    leiden_keys_to_read_louvain.end())));
            ...
            if(GraphViewType::is_multi_gpu)
            {
                ...
                rocgraph::detail::compute_gpu_id_from_int_vertex_t<vertex_t> vertex_to_gpu_id_op{
                    raft::device_span<vertex_t const>(d_partitions_range_lasts.data(),
                                                      d_partitions_range_lasts.size()),
                    major_comm_size,
                    minor_comm_size};
            ...
            {
                lovain_of_leiden_cluster_keys.resize(leiden_keys_to_read_louvain.size(),
                                                     handle.get_stream());
