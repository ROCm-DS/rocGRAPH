...
namespace rocgraph
{
    namespace legacy
    {
        ...
        public:
                                ...
                                vertex_t const* d_cluster)
            {
                ...
                weight_t Q = thrust::transform_reduce(
                    ...
                    [d_deg = deg.data(),
                     d_inc = inc.data(),
                     total_edge_weight,
                     resolution] __device__(vertex_t community) {
            ...
            Dendrogram<vertex_t> const& get_dendrogram() const
            {
                return *dendrogram_;
            }

            Dendrogram<vertex_t>& get_dendrogram()
            {
                return *dendrogram_;
            }

            std::unique_ptr<Dendrogram<vertex_t>> move_dendrogram()
            {
                return std::move(dendrogram_);
            }

            virtual weight_t operator()(size_t max_level, weight_t resolution)
            {
                weight_t total_edge_weight = thrust::reduce(
                    handle_.get_thrust_policy(), weights_v_.begin(), weights_v_.end());
                ...
                while(dendrogram_->num_levels() < max_level)
                {
                    ...
                    weight_t new_Q
                        = update_clustering(total_edge_weight, resolution, current_graph);
        ...
        public:
            void compute_vertex_and_cluster_weights(graph_type const& graph)
            {
                ...
                thrust::for_each(handle_.get_thrust_policy(),
                                 ...
                                 [d_offsets,
                                  ...
                                  d_weights,
                                  d_vertex_weights,
                                  d_cluster_weights] __device__(vertex_t src) {
                                     weight_t sum = thrust::reduce(thrust::seq,
                                                                   d_weights + d_offsets[src],
                                               ...
                                               graph_type const& graph)
            {
                ...
                rmm::device_uvector<vertex_t> next_cluster_v(dendrogram_->current_level_size(),
                                                             handle_.get_stream());
                rmm::device_uvector<weight_t> delta_Q_v(graph.number_of_edges,
                                                        handle_.get_stream());
                ...
                weight_t new_Q = modularity(
                    total_edge_weight, resolution, graph, dendrogram_->current_level_begin());
                                          ...
                                          rmm::device_uvector<weight_t>& delta_Q_v)
            {
                ...
                thrust::fill(handle_.get_thrust_policy(),
                             ...
                             vertex_t{-1});
                thrust::fill(
                    handle_.get_thrust_policy(), delta_Q_v.begin(), delta_Q_v.end(), weight_t{0.0});
                thrust::fill(handle_.get_thrust_policy(),
                                  ...
                                  d_old_cluster_sum] __device__(edge_t loc) {
                                     ...
                                     if(src != dst)
                                     {
                                         ...
                                         while(d_cluster_hash[offset] != new_cluster)
                                         {
                                             ...
                                             {
                                                 atomicCAS(
                                                     d_cluster_hash + offset, -1, new_cluster);
                                             }
                                             else
                                             {
                                                 h      = (h + 1) % n_edges;
                                 ...
                                 [total_edge_weight,
                                  ...
                                  d_cluster_weights] __device__(edge_t loc) {
                                     ...
                                     if(new_cluster >= 0)
                                     {
                                         ...
                                         d_delta_Q[loc]
                                             = 2
                                               * (((d_new_cluster_sum[loc] - d_old_cluster_sum[src])
                                                   / total_edge_weight)
                                                  - resolution
                                                        * (a_new * k_k - a_old * k_k + k_k * k_k)
                                                        / (total_edge_weight * total_edge_weight));
                                     }
                                     else
                                     {
                              ...
                              bool                           up_down)
            {
                ...
                vertex_t final_size
                    = thrust::distance(temp_vertices_v.data(), cluster_reduce_end.first);
            ...
            {
                rmm::device_uvector<vertex_t> new_src_v(graph.number_of_edges,
                                                        handle_.get_stream());
                rmm::device_uvector<vertex_t> new_dst_v(graph.number_of_edges,
                                                        handle_.get_stream());
                rmm::device_uvector<weight_t> new_weight_v(graph.number_of_edges,
                                                           handle_.get_stream());
                ...
                thrust::stable_sort_by_key(handle_.get_thrust_policy(),
                                           new_dst_v.begin(),
                                           new_dst_v.end(),
                ...
                auto new_start = thrust::make_zip_iterator(
                    thrust::make_tuple(src_indices_v_.data(), graph.indices));
                auto new_end
