...
namespace
{
    ...
    struct update_functor
    {
        ValT min_value;
        ValT ensemble_size;
        update_functor(ValT minv, ValT es)
            : min_value(minv)
            , ensemble_size(es)
        {
        }
 ...
 */
    ...
    void get_permutation_vector(T size, T seed, T* permutation, rmm::cuda_stream_view stream_view)
    {
        ...
        thrust::transform(
            rmm::exec_policy(stream_view), index, index + size, randoms_v.begin(), prg());
    ...
    class EcgLouvain : public rocgraph::legacy::Louvain<graph_type>
    {
    public:
        ...
        EcgLouvain(raft::handle_t const& handle, graph_type const& graph, vertex_t seed)
            : rocgraph::legacy::Louvain<graph_type>(handle, graph)
            , seed_(seed)
        {
        }
...
namespace rocgraph
{
             ...
             vertex_t*                                               clustering)
    {
        ...
        for(vertex_t i = 0; i < ensemble_size; i++)
        {
            ...
            THROW_IF_HIPLAUNCHKERNELGGL_ERROR((match_check_kernel),
                                              grid,
                                              block,
                                              0,
                                              handle.get_stream(),
        ...
        rocgraph::louvain(handle, louvain_graph, clustering, size_t{100});
    }

    // Explicit template instantiations.
    template void
        ecg<int32_t, int32_t, float>(raft::handle_t const&,
                                     legacy::GraphCSRView<int32_t, int32_t, float> const& graph,
                                     float    min_weight,
                                     int32_t  ensemble_size,
                                     int32_t* clustering);
    template void
        ecg<int32_t, int32_t, double>(raft::handle_t const&,
