...
namespace rocgraph
{
    ...
    struct unroll_edge
    {
        ...
        __host__ __device__ thrust::tuple<vertex_t, vertex_t> operator()(edge_t i) const
        {
            ...
            {
                idx = static_cast<size_t>(thrust::distance(transposed_valid_edge_last, itr)
                                          + num_valid_edges);
            }
            else
            {
            ...
            return atomic_counter.fetch_sub(edge_t{1}, hip::std::memory_order_relaxed);
        }
    };
    ...
    rmm::device_uvector<vertex_t>
        compute_prefix_sum(raft::handle_t const&             handle,
                           ...
                           raft::device_span<vertex_t const> query_vertices)
    {
        ...
        auto count_first = thrust::make_transform_iterator(
            ...
            [query_vertices,
             ...
             sorted_vertices
             = sorted_vertices.begin()] __host__ __device__(size_t idx) -> vertex_t {
                                         ...
                                         raft::device_span<vertex_t const> invalid_edgelist_dsts)
    {
        ...
        auto edges_not_overcomp = thrust::remove_if(
            ...
            [num_invalid_edges = invalid_edgelist_dsts.size(),
             ...
             invalid_last
             = thrust::make_zip_iterator(invalid_edgelist_dsts.end(),
                                         invalid_edgelist_srcs.end())] __host__ __device__(auto e) {
                                 ...
                                 raft::device_span<edge_t>                     num_triangles)
    {
        ...
        auto prefix_sum_invalid
            = compute_prefix_sum(handle,
                                 raft::device_span<vertex_t const>(
                                     edgelist_dsts.data() + num_valid_edges, num_invalid_edges),
                                 raft::device_span<vertex_t const>(
                                     edgelist_dsts.data() + num_valid_edges, num_invalid_edges));
        ...
        thrust::for_each(
            ...
            [num_valid_edges,
             ...
             incoming_edges_to_r
             = get_dataframe_buffer_begin(incoming_edges_to_r)] __host__ __device__(auto idx) {
                ...
                thrust::copy(thrust::seq,
                             ...
                             incoming_edges_to_r_first_valid
                                 + (prefix_sum_valid[idx + 1] - prefix_sum_valid[idx]),
                ...
                thrust::copy(thrust::seq,
                             ...
                             incoming_edges_to_r + prefix_sum_invalid[idx]
                                 + prefix_sum_valid[idx + 1]);
                ...
                if constexpr(is_q_r_edge)
                {
                    ...
                    thrust::copy(thrust::seq,
                                 ...
                                 potential_closing_edges_first_valid
                                     + (prefix_sum_valid[idx + 1] - prefix_sum_valid[idx]),
                                 potential_closing_edges + prefix_sum_valid[idx]
                                     + prefix_sum_invalid[idx]);
                    ...
                    thrust::copy(thrust::seq,
                                 ...
                                 potential_closing_edges_first_invalid
                                     + (prefix_sum_invalid[idx + 1] - prefix_sum_invalid[idx]),
                                 potential_closing_edges + prefix_sum_invalid[idx]
                                     + prefix_sum_valid[idx + 1]);
                }
                else
                {
        ...
        resize_dataframe_buffer(
            potential_closing_edges, num_edges_not_overcomp, handle.get_stream());
        ...
        if constexpr(!is_q_r_edge)
        {
            ...
            resize_dataframe_buffer(
                potential_closing_edges, num_edges_not_overcomp, handle.get_stream());
            resize_dataframe_buffer(
                incoming_edges_to_r, num_edges_not_overcomp, handle.get_stream());
        }
        ...
        thrust::for_each(
            ...
            [num_triangles = num_triangles.begin(),
             ...
             invalid_last  = thrust::make_zip_iterator(
                 edgelist_dsts.end(),
                 edgelist_srcs.end())] __host__ __device__(auto potential_or_incoming_e) {
                ...
                auto transposed_invalid_edge = thrust::make_tuple(thrust::get<1>(incoming_e_to_r),
                                                                  thrust::get<1>(potential_e));

                if constexpr(!is_q_r_edge)
                {
                    transposed_invalid_edge = thrust::make_tuple(thrust::get<1>(incoming_e_to_r),
                                                                 thrust::get<0>(potential_e));
                }
                auto itr = thrust::lower_bound(
                    thrust::seq, invalid_first, invalid_last, transposed_invalid_edge);
                if(itr != invalid_last)
                {
        ...
        thrust::for_each(
            ...
            unroll_edge<vertex_t,
                        edge_t,
                        decltype(thrust::make_zip_iterator(edgelist_srcs.begin(),
                                                           edgelist_dsts.begin()))>{
                num_valid_edges,
    ...
    namespace
    {
        ...
        {
            __host__ __device__ thrust::optional<thrust::tuple<vertex_t, vertex_t>>
            ...
            {
                return src != dst
                           ? thrust::optional<thrust::tuple<vertex_t, vertex_t>>{thrust::make_tuple(
                                 src, dst)}
                           : thrust::nullopt;
            }
        };
        ...
        {
            __host__ __device__ thrust::optional<thrust::tuple<vertex_t, vertex_t, weight_t>>
                                operator()(vertex_t src,
                           vertex_t dst,
                ...
                return (src_out_degree < dst_out_degree)
                           ...
                           : (((src_out_degree == dst_out_degree)
                               && (src < dst) /* tie-breaking using vertex ID */)
                                  ? thrust::optional<
                                        thrust::tuple<vertex_t, vertex_t, weight_t>>{thrust::
                                                                                         make_tuple(
                                                                                             src,
                                                                                             dst,
                                                                                             wgt)}
                           ...
                           thrust::nullopt_t) const
            {
                return (src_out_degree < dst_out_degree)
                           ...
                           : (((src_out_degree == dst_out_degree)
                               && (src < dst) /* tie-breaking using vertex ID */)
                                  ? thrust::optional<
                                        thrust::tuple<vertex_t, vertex_t>>{thrust::make_tuple(src,
                                                                                              dst)}
                                  : thrust::nullopt);
            }
        };
                ...
                bool                                                         do_expensive_check)
    {
        ...
        std::optional<graph_t<vertex_t, edge_t, false, multi_gpu>>      modified_graph{handle};
        std::optional<graph_view_t<vertex_t, edge_t, false, multi_gpu>> modified_graph_view{
            std::nullopt};
        ...
        if(graph_view.count_self_loops(handle) > edge_t{0})
        {
            ...
            {
                std::tie(srcs, dsts, std::ignore, std::ignore, std::ignore) = detail::
            ...
            std::tie(*modified_graph, std::ignore, std::ignore, std::ignore, renumber_map)
                = create_graph_from_edgelist<vertex_t,
                    ...
                    std::nullopt,
                    std::nullopt,
                    rocgraph::graph_properties_t{true, graph_view.is_multigraph()},
                    true);
  ...
  */
...

        {
            ...
            auto vertex_partition_range_lasts
                = renumber_map ? std::make_optional<std::vector<vertex_t>>(
                                     cur_graph_view.vertex_partition_range_lasts())
                               : std::nullopt;

            auto out_degrees = cur_graph_view.compute_out_degrees(handle);
            edge_src_property_t<decltype(cur_graph_view), edge_t> edge_src_out_degrees(
                handle, cur_graph_view);
            edge_dst_property_t<decltype(cur_graph_view), edge_t> edge_dst_out_degrees(
                handle, cur_graph_view);
            ...
            if constexpr(multi_gpu)
            {
                std::tie(srcs, dsts, wgts, std::ignore, std::ignore) = detail::
                    shuffle_ext_vertex_pairs_with_values_to_local_gpu_by_edge_partitioning<vertex_t,
                                                                                           edge_t,
                                                                                           weight_t,
                                                                                           int32_t>(
            ...
            std::tie(
                modified_graph.value(), edge_weight, std::ignore, std::ignore, tmp_renumber_map)
                = create_graph_from_edgelist<vertex_t,
                                             edge_t,
                                             weight_t,
                                             edge_t,
                    ...
                    std::nullopt,
                    std::nullopt,
                    rocgraph::graph_properties_t{false /* now asymmetric */,
                                                 cur_graph_view.is_multigraph()},
                    false);

            modified_graph_view = (*modified_graph).view();
            if(renumber_map)
            { // collapse renumber_map
                unrenumber_int_vertices<vertex_t, multi_gpu>(handle,
                                                             (*tmp_renumber_map).data(),
                                                             (*tmp_renumber_map).size(),
                                                             (*renumber_map).data(),
...

        {
            ...
            /* edge_property_bool */
            rocgraph::edge_property_t<decltype(cur_graph_view), bool> edge_mask(handle,
                                                                                cur_graph_view);
            rocgraph::fill_edge_property(handle, cur_graph_view, true, edge_mask);
            ...
            while(true)
            {
                ...
                // edges.
                auto invalid_transposed_edge_triangle_count_first = thrust::stable_partition(
                    handle.get_thrust_policy(),
                    transposed_edge_triangle_count_pair_first,
                    transposed_edge_triangle_count_pair_first + edgelist_srcs.size(),
                    [k] __host__ __device__(auto e) {
                        auto num_triangles = thrust::get<1>(e);
                        return num_triangles >= k - 2;
                    });
                auto num_invalid_edges = static_cast<size_t>(thrust::distance(
                    invalid_transposed_edge_triangle_count_first,
                    transposed_edge_triangle_count_pair_first + edgelist_srcs.size()));
                ...
                thrust::for_each(handle.get_thrust_policy(),
                                 thrust::make_counting_iterator<edge_t>(0),
                                 thrust::make_counting_iterator<edge_t>(num_invalid_edges),
                                 [num_triangles = raft::device_span<edge_t>(
                                      num_triangles.data() + num_valid_edges, num_invalid_edges),
                                  intersection_offsets = raft::device_span<size_t const>(
                                      intersection_offsets.data(),
                                      intersection_offsets.size())] __host__ __device__(auto i) {
                                     num_triangles[i]
                ...
                thrust::tabulate(
                    ...
                    generate_p_r_or_q_r_from_p_q<vertex_t, edge_t, true>{
                        ...
                        raft::device_span<vertex_t>(edgelist_srcs.data() + num_valid_edges,
                                                    num_invalid_edges),
                ...
                // create the pair (p, q)
                rocgraph::unroll_p_r_or_q_r_edges<vertex_t, edge_t, false, true>(
                    handle,
                    cur_graph_view,
                    num_invalid_edges,
                    num_valid_edges,
                ...
                // case 3: unroll (p, r)
                rocgraph::unroll_p_r_or_q_r_edges<vertex_t, edge_t, false, false>(
                    handle,
                    cur_graph_view,
                    num_invalid_edges,
                    num_valid_edges,
                ...
                auto num_edges_with_triangles = static_cast<size_t>(thrust::distance(
                    transposed_edge_triangle_count_pair_first, edges_with_triangle_last));
                ...
                rocgraph::edge_bucket_t<vertex_t, void, true, multi_gpu, true>
                    edges_with_no_triangle(handle);
                ...
                if(edge_weight_view)
                {
                    rocgraph::transform_e(
                        ...
                        rocgraph::edge_src_dummy_property_t{}.view(),
                        rocgraph::edge_dst_dummy_property_t{}.view(),
                        *edge_weight_view,
                        [] __host__ __device__(
                ...
                else
                {
                    rocgraph::transform_e(
                        ...
                        rocgraph::edge_dst_dummy_property_t{}.view(),
                        rocgraph::edge_dummy_property_t{}.view(),
                        [] __host__ __device__(auto src,
                                               auto dst,
                                               thrust::nullopt_t,
                                               thrust::nullopt_t,
                                               thrust::nullopt_t) { return false; },
                        edge_mask.mutable_view(),
                        false);
                }
                cur_graph_view.attach_edge_mask(edge_mask.view());
