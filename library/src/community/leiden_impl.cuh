...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        vertex_t remove_duplicates(raft::handle_t const&          handle,
                                   rmm::device_uvector<vertex_t>& input_array)
        {
            ...
            auto nr_unique_elements = static_cast<vertex_t>(thrust::distance(
                input_array.begin(),
                thrust::unique(
                    handle.get_thrust_policy(), input_array.begin(), input_array.end())));
            ...
            if constexpr(multi_gpu)
            {
                ...
                nr_unique_elements = host_scalar_allreduce(handle.get_comms(),
                                                           nr_unique_elements,
                                                           raft::comms::op_t::SUM,
                                                           handle.get_stream());
            }
        ...
        std::pair<std::unique_ptr<Dendrogram<vertex_t>>, weight_t>
            leiden(raft::handle_t const&                                        handle,
                   ...
                   weight_t                                                     theta = 1.0)
        {
            using graph_t = rocgraph::graph_t<vertex_t, edge_t, store_transposed, multi_gpu>;
            using graph_view_t
            ...
            std::optional<edge_property_t<graph_view_t, weight_t>> coarsen_graph_edge_weight(
                handle);
...
//  graph in the hierarchical decomposition
                ...
                rmm::device_uvector<weight_t> vertex_weights = compute_out_weight_sums(
                ...
                else
                {
                    ...
                    auto num_unique_louvain_clusters_in_refined_partition = thrust::count_if(
                    ...
                    cluster_keys.resize(num_unique_louvain_clusters_in_refined_partition,
                                        handle.get_stream());
                    ...
                    if constexpr(graph_view_t::is_multi_gpu)
                    {
                        ...
                        num_unique_louvain_clusters_in_refined_partition = thrust::count_if(
                            handle.get_thrust_policy(),
                            thrust::make_counting_iterator(size_t{0}),
...
//  Update the clustering assignment, this is the main loop of Louvain
                ...
                while(new_Q > (cur_Q + 1e-4))
                {
                    ...
                    louvain_assignment_for_vertices = detail::update_clustering_by_delta_modularity(
                        handle,
                        current_graph_view,
                        current_edge_weight_view,
                        total_edge_weight,
                ...
                rmm::device_uvector<vertex_t> copied_louvain_partition(
                    dendrogram->current_level_size(), handle.get_stream());
                ...
                terminate
                    = terminate
                      || (nr_unique_louvain_clusters == current_graph_view.number_of_vertices());
                ...
                std::pair<rmm::device_uvector<vertex_t>, rmm::device_uvector<vertex_t>>
                    leiden_to_louvain_map{rmm::device_uvector<vertex_t>(0, handle.get_stream()),
                ...
                if(!terminate)
                {
                    auto nr_unique_leiden
                        = static_cast<vertex_t>(leiden_to_louvain_map.first.size());
                    if(graph_view_t::is_multi_gpu)
                    {
                        nr_unique_leiden = host_scalar_allreduce(handle.get_comms(),
                                                                 nr_unique_leiden,
                                                                 raft::comms::op_t::SUM,
                                                                 handle.get_stream());
                    }
                    ...
                    terminate = terminate
                                || (nr_unique_leiden == current_graph_view.number_of_vertices());
                    ...
                    if(nr_unique_leiden < current_graph_view.number_of_vertices())
                    {
                        ...
                        louvain_of_refined_graph.resize(
                            current_graph_view.local_vertex_partition_range_size(),
                            handle.get_stream());
                        ...
                        relabel<vertex_t, multi_gpu>(
                            handle,
                            std::make_tuple(
                                static_cast<vertex_t const*>(leiden_to_louvain_map.first.begin()),
                        ...
                        thrust::sort(handle.get_thrust_policy(),
                                     thrust::make_zip_iterator(numbering_map->begin(),
                                                               numeric_sequence.begin()),
                                     thrust::make_zip_iterator(numbering_map->end(),
                                                               numeric_sequence.end()));

                        size_t new_size
                            = thrust::distance(numbering_map->begin(),
                                 ...
                                 size_t                         num_nodes)
        {
            ...
            if constexpr(multi_gpu)
            {
                auto unique_cluster_range_lasts
                    = rocgraph::partition_manager::compute_partition_range_lasts(
            ...
            rmm::device_uvector<vertex_t> numbering_indices(unique_cluster_ids.size(),
                                                            handle.get_stream());
            detail::sequence_fill(handle.get_stream(),
        ...
        template <typename vertex_t, typename edge_t, bool multi_gpu>
        void flatten_leiden_dendrogram(
               ...
               weight_t                                                     theta = 1.0)
    {
        ...
        std::tie(dendrogram, modularity) = detail::leiden(
            handle, rng_state, graph_view, edge_weight_view, max_level, resolution, theta);
