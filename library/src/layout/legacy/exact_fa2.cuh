...
namespace rocgraph
{
    namespace detail
    {
                       ...
                       internals::GraphBasedDimRedCallback* callback              = nullptr)
        {
            ...
            graph.degree(d_mass, rocgraph::legacy::DegreeDirection::OUT);
            RAFT_CHECK_CUDA(stream_view.value());
            ...
            for(int iter = 0; iter < max_iter; ++iter)
            {
                ...
                const float s
                    = thrust::reduce(handle.get_thrust_policy(), swinging.begin(), swinging.end());
                const float t
                    = thrust::reduce(handle.get_thrust_policy(), traction.begin(), traction.end());
