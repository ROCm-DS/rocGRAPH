 ...
 */
        __global__ static __launch_bounds__(THREADS1, FACTOR1) void BoundingBoxKernel(
            int* restrict startd,
            int* restrict childd,
            int* restrict massd,
 ...
 */
        __global__ static __launch_bounds__(THREADS2, FACTOR2) void TreeBuildingKernel(
            int* restrict childd,
...
#pragma unroll
                        for(int i = 0; i < 4; i++)
                        {
                            ...
                            if(ch >= 0)
                            {
                                const float m = (ch >= N) ? (cnt += countd[ch],
                                                             mass[i * THREADS3 + threadIdx.x])
                                                          : (cnt++, massd[ch]);
                ...
                else
                {
                    ...
                    for(int i = 0; i < 4; i++)
                    {
                        ...
                        if((ch < N) or (mass[i * THREADS3 + threadIdx.x] >= 0)
                           or ((mass[i * THREADS3 + threadIdx.x] = massd[ch]) >= 0))
                            j--;
                    }
                }
 ...
 */
            ...
            for(int k = threadIdx.x + blockIdx.x * blockDim.x; k < N; k += blockDim.x * gridDim.x)
            {
                ...
                do
                {
                    ...
                    while(pd < 4 && depth < THREADS5)
                    {
                        ...
                        {
                            const float tdist_2
                                = __fdividef(scaling_ratio * massd[i] * massd[n], dxy1);
                            vx += dx * tdist_2;
                            vy += dy * tdist_2;
                        }
                        else
            ...
            const int   n)
        {
            ...
            for(int i = threadIdx.x + blockIdx.x * blockDim.x; i < n; i += gridDim.x * blockDim.x)
            {
                ...
                // Update positions
                float factor = speed / (1.0 + std::sqrt(speed * swinging[i]));
                Y_x[i] += dx * factor;
                Y_y[i] += dy * factor;
            }
        }
