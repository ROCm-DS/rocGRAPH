...
namespace rocgraph
{
    namespace detail
    {
            ...
            bool                                                         do_expensive_check)
        {
            ...
            if(num_vertices == 0)
            {
                return rmm::device_uvector<weight_t>(0, handle.get_stream());
            }
            ...
            // Power iteration
            rmm::device_uvector<weight_t> old_centralities(centralities.size(),
                                                           handle.get_stream());

            edge_src_property_t<GraphViewType, weight_t> edge_src_centralities(handle,
                                                                               pull_graph_view);
            ...
            while(true)
            {
                ...
                if(edge_weight_view)
                {
                    per_v_transform_reduce_incoming_e(
                        ...
                        edge_dst_dummy_property_t{}.view(),
                        *edge_weight_view,
                        [] __device__(vertex_t, vertex_t, auto src_val, auto, weight_t w) {
                            return src_val * w;
                        },
                        weight_t{0},
                ...
                else
                {
                    per_v_transform_reduce_incoming_e(
                        ...
                        [] __device__(vertex_t, vertex_t, auto src_val, auto, auto) {
                            return src_val;
                        },
                        weight_t{0},
                        reduce_op::plus<weight_t>{},
                ...
                auto diff_sum = transform_reduce_v(
                    ...
                    thrust::make_zip_iterator(
                        thrust::make_tuple(centralities.begin(), old_centralities.data())),
                ...
                {
                    ROCGRAPH_FAIL("Eigenvector Centrality failed to converge.");
                }
            }
        ...
        bool                                                         do_expensive_check)
    {
        ...
        ROCGRAPH_EXPECTS(epsilon >= 0.0, "Invalid input argument: epsilon should be non-negative.");
        if(initial_centralities)
            ROCGRAPH_EXPECTS(
                initial_centralities->size()
