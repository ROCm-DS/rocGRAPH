...
namespace
{
    ...
    struct brandes_e_op_t
    {
        ...
        __host__ __device__ thrust::optional<value_t>
            operator()(vertex_t, vertex_t, value_t src_sigma, vertex_t dst_distance, ignore_t) const
        {
...
namespace rocgraph
{
    namespace detail
    {
            ...
            bool                                                         do_expensive_check)
        {
            ...
            edge_src_property_t<graph_view_t<vertex_t, edge_t, false, multi_gpu>, edge_t>
                src_sigmas(handle, graph_view);
            edge_dst_property_t<graph_view_t<vertex_t, edge_t, false, multi_gpu>, vertex_t>
                dst_distances(handle, graph_view);
            ...
            if(vertex_frontier.bucket(bucket_idx_cur).size() > 0)
            {
                thrust::for_each(
                    ...
                    [d_sigma    = sigmas.begin(),
                     d_distance = distances.begin(),
                     vertex_partition] __host__ __device__(auto v) {
                        auto offset
            ...
            while(true)
            {
                ...
                auto [new_frontier, new_sigma] = transform_reduce_v_frontier_outgoing_e_by_dst(
                    handle,
                    graph_view,
                ...
                update_v_frontier(
                    ...
                    thrust::make_zip_iterator(distances.begin(), sigmas.begin()),
                    [bucket_idx_next,
                     hop] __host__ __device__(auto v, auto old_values, auto v_sigma) {
            ...
            bool                                                         do_expensive_check)
        {
            ...
            vertex_t diameter = transform_reduce_v(
                ...
                graph_view,
                distances.begin(),
                [] __host__ __device__(auto, auto d) {
            ...
            if(with_endpoints)
            {
                vertex_t count = count_if_v(
                    ...
                    graph_view,
                    distances.begin(),
                    [] __host__ __device__(auto, auto d) { return (d != invalid_distance); },
                    do_expensive_check);
                ...
                thrust::transform(handle.get_thrust_policy(),
                                  ...
                                  centralities.begin(),
                                  centralities.begin(),
                                  [count] __host__ __device__(auto d, auto centrality) {
                                      if(d == vertex_t{0})
                                      {
            ...
            for(vertex_t d = diameter; d > 1; --d)
            {
                per_v_transform_reduce_outgoing_e(
                    ...
                    dst_properties.view(),
                    rocgraph::edge_dummy_property_t{}.view(),
                    [d] __host__ __device__(auto, auto, auto src_props, auto dst_props, auto) {
                        if((thrust::get<0>(dst_props) == d)
                           && (thrust::get<0>(src_props) == (d - 1)))
                        {
            ...
            bool                                                         do_expensive_check)
        {
            ...
            for(vertex_t d = diameter; d > 0; --d)
            {
                ...
                //
                rocgraph::edge_bucket_t<vertex_t, void, true, multi_gpu, true> edge_list(handle);

                {
                    ...
                    auto new_edgelist_end
                        = thrust::unique(handle.get_thrust_policy(),
                                         thrust::make_zip_iterator(src.begin(), dst.begin()),
                                         thrust::make_zip_iterator(src.end(), dst.end()));

                    src.resize(thrust::distance(thrust::make_zip_iterator(src.begin(), dst.begin()),
                                                new_edgelist_end),
                               handle.get_stream());
                ...
                transform_e(
                    ...
                    [d] __host__ __device__(
                        auto src, auto dst, auto src_props, auto dst_props, auto edge_centrality) {
                        if((thrust::get<0>(dst_props) == d)
                           && (thrust::get<0>(src_props) == (d - 1)))
                        {
                            auto sigma_v = static_cast<weight_t>(thrust::get<1>(src_props));
            ...
            bool const                                                   do_expensive_check)
        {
            ...
            if constexpr(multi_gpu)
            {
                ...
                source_offsets[0] = 0;
                std::inclusive_scan(
                    source_counts.begin(), source_counts.end(), source_offsets.begin() + 1);
            ...
            for(size_t source_idx = 0; source_idx < num_sources; ++source_idx)
            {
                ...
                vertex_frontier_t<vertex_t, void, multi_gpu, true> vertex_frontier(handle,
                                                                                   num_buckets);

                if((source_idx >= source_offsets[my_rank])
                   && (source_idx < source_offsets[my_rank + 1]))
                {
                    vertex_frontier.bucket(bucket_idx_cur)
            ...
            if(scale_factor)
            {
                if(graph_view.number_of_vertices() > 2)
                {
                    ...
                    thrust::transform(handle.get_thrust_policy(),
                                      ...
                                      centralities.end(),
                                      centralities.begin(),
                                      [sf = *scale_factor] __host__ __device__(auto centrality) {
                                          return centrality / sf;
                                      });
                }
            }
                ...
                bool const                                                   do_expensive_check)
        {
            ...
            edge_property_t<graph_view_t<vertex_t, edge_t, false, multi_gpu>, weight_t>
                centralities(handle, graph_view);
            ...
            if(graph_view.has_edge_mask())
            {
                ...
                fill_edge_property(
                    handle, unmasked_graph_view, weight_t{0}, centralities, do_expensive_check);
            }
            else
            {
                fill_edge_property(
                    handle, graph_view, weight_t{0}, centralities, do_expensive_check);
            }
                ...
                if(graph_view.number_of_vertices() > 1)
                {
                    ...
                    for(size_t k = 0; k < counts.size(); k++)
                    {
                        thrust::transform(handle.get_thrust_policy(),
                                          ...
                                          firsts[k] + counts[k],
                                          mutable_firsts[k],
                                          [sf = *scale_factor] __host__ __device__(
                                              auto centrality) { return centrality / sf; });
                    }
                }
            }
