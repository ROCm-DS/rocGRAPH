...
namespace rocgraph
{
    namespace detail
    {
            ...
            bool            do_expensive_check)
        {
            ...
            if(do_expensive_check)
            {
                if(has_initial_guess)
                {
                    auto num_negative_values = count_if_v(
                        handle, pull_graph_view, katz_centralities, [] __device__(auto, auto val) {
                            return val < 0.0;
                        });
                    ROCGRAPH_EXPECTS(
            ...
            edge_src_property_t<GraphViewType, result_t> edge_src_katz_centralities(
            ...
            while(true)
            {
                ...
                if(betas != nullptr)
                {
                    auto val_first = thrust::make_zip_iterator(
                        thrust::make_tuple(new_katz_centralities, betas));
                ...
                auto diff_sum = transform_reduce_v(
                    ...
                    thrust::make_zip_iterator(
                        thrust::make_tuple(new_katz_centralities, old_katz_centralities)),
                ...
                {
                    ROCGRAPH_FAIL("Katz Centrality failed to converge.");
                }
            }
