...
namespace rocgraph
{
    namespace detail
    {
            ...
            bool      do_expensive_check = false)
        {
            ...
            ROCGRAPH_EXPECTS(std::get<0>(vertex_pairs).size() == std::get<1>(vertex_pairs).size(),
                             "vertex pairs have mismatched sizes");
            ROCGRAPH_EXPECTS(graph_view.is_symmetric(),
                             "similarity algorithms require an undirected(symmetric) graph");

            size_t num_vertex_pairs   = std::get<0>(vertex_pairs).size();
            auto   vertex_pairs_begin = thrust::make_zip_iterator(std::get<0>(vertex_pairs).data(),
            ...
            if(edge_weight_view)
            {
                rmm::device_uvector<weight_t> similarity_score(num_vertex_pairs,
                                                               handle.get_stream());
                    ...
                    *edge_weight_view,
                    ...
                    [functor] __device__(auto a,
                        ...
                        auto pair_first = thrust::make_zip_iterator(
                            intersected_properties_a.data(), intersected_properties_b.data());
                        thrust::tie(sum_of_min_weight_a_intersect_b,
                            ...
                            = thrust::transform_reduce(
                                ...
                                thrust::make_tuple(
                                    weight_t{0}, weight_t{0}, weight_t{0}, weight_t{0}),
                ...
                bool do_expensive_check = false)
        {
            ...
            // FIXME: See https://github.com/rapidsai/rocgraph/issues/4132
            //   Once that issue is resolved we can drop this check
            ROCGRAPH_EXPECTS(!graph_view.is_multigraph() || !edge_weight_view,
                ...
                if(vertices)
                {
                    ...
                    if constexpr(multi_gpu)
                    {
                        num_invalid_vertices
                            = rocgraph::host_scalar_allreduce(handle.get_comms(),
                                                              num_invalid_vertices,
                                                              raft::comms::op_t::SUM,
                                                              handle.get_stream());
                    }
            ...
            if(topk)
            {
                ...
                {
                    tmp_vertices.resize(graph_view.local_vertex_partition_range_size(),
                                        handle.get_stream());
                ...
                rmm::device_uvector<size_t> two_hop_degrees(degrees.size() + 1,
                                                            handle.get_stream());
                ...
                if(vertices)
                {
                    ...
                    thrust::gather(handle.get_thrust_policy(),
                                   thrust::make_transform_iterator(
                                       tmp_vertices.begin(),
                                       rocgraph::detail::shift_left_t<vertex_t>{
                                           graph_view.local_vertex_partition_range_first()}),
                                   thrust::make_transform_iterator(
                                       tmp_vertices.end(),
                                       rocgraph::detail::shift_left_t<vertex_t>{
                ...
                std::tie(batch_offsets, std::ignore) = compute_offset_aligned_element_chunks(
                    handle,
                    raft::device_span<size_t const>{two_hop_degree_offsets.data(),
                                                    two_hop_degree_offsets.size()},
                    sum_two_hop_degrees,
                    MAX_PAIRS_PER_BATCH);

                for(size_t batch_number = 0; batch_number < (batch_offsets.size() - 1);
                    ++batch_number)
                {
                    if(batch_offsets[batch_number + 1] > batch_offsets[batch_number])
                    {
                        ...
                        auto v1 = rocgraph::detail::expand_sparse_offsets(
                        ...
                        rocgraph::unrenumber_local_int_vertices(
                            ...
                            tmp_vertices.data() + batch_offsets[batch_number],
                            vertex_t{0},
                            static_cast<vertex_t>(batch_offsets[batch_number + 1]
                                                  - batch_offsets[batch_number]),
                            do_expensive_check);
                        ...
                        if constexpr(multi_gpu)
                        {
                            ...
                            std::tie(v1, v2, std::ignore, std::ignore, std::ignore) = detail::
                        ...
                        auto score = similarity(
                            handle,
                            graph_view,
                            edge_weight_view,
                            std::make_tuple(
                                raft::device_span<vertex_t const>{v1.data(), v1.size()},
                        ...
                        thrust::copy(handle.get_thrust_policy(),
                                     v1.begin(),
                                     v1.begin() + top_v1.size(),
                                     top_v1.begin());
                        thrust::copy(handle.get_thrust_policy(),
                                     v2.begin(),
                                     v2.begin() + top_v1.size(),
                                     top_v2.begin());
                        thrust::copy(handle.get_thrust_policy(),
                                     score.begin(),
                        ...
                        if constexpr(multi_gpu)
                        {
                            ...
                            auto rx_sizes = rocgraph::host_scalar_gather(
                            ...
                            if(is_root)
                            {
                                ...
                                rx_displs[0] = 0;
                                std::partial_sum(
                                    rx_sizes.begin(), rx_sizes.end() - 1, rx_displs.begin() + 1);
                                gathered_size = std::reduce(rx_sizes.begin(), rx_sizes.end());
                            }

                            rmm::device_uvector<vertex_t> gathered_v1(gathered_size,
                                                                      handle.get_stream());
                            rmm::device_uvector<vertex_t> gathered_v2(gathered_size,
                                                                      handle.get_stream());
                            rmm::device_uvector<weight_t> gathered_score(gathered_size,
                                                                         handle.get_stream());

                            rocgraph::device_gatherv(
                                handle.get_comms(),
                                thrust::make_zip_iterator(
                                    top_v1.begin(), top_v2.begin(), top_score.begin()),
                                thrust::make_zip_iterator(gathered_v1.begin(),
                                                          gathered_v2.begin(),
                        ...
                        {
                            raft::update_host(&similarity_threshold,
                                              top_score.data() + *topk - 1,
                                              1,
                                              handle.get_stream());

                            if constexpr(multi_gpu)
                            {
                                similarity_threshold = host_scalar_bcast(handle.get_comms(),
            ...
            else
            {
                ...
                else
                {
                    ...
                    thrust::sequence(handle.get_thrust_policy(),
                                     ...
                                     tmp_vertices.end(),
                                     graph_view.local_vertex_partition_range_first());
                    vertices_span = raft::device_span<vertex_t const>{tmp_vertices.data(),
                                                                      tmp_vertices.size()};
                }

                auto [offsets, v2]
                    = k_hop_nbrs(handle, graph_view, vertices_span, 2, do_expensive_check);
                ...
                rocgraph::unrenumber_local_int_vertices(handle,
                                                        v1.data(),
                                                        v1.size(),
                                                        vertices_span.data(),
                                                        vertex_t{0},
