...
private:
    ...
    ~memstat()
    {
        if(s_enabled)
        {
            ...
            {
                std::cerr << "rocgraph memstat memory leaks detected, use Python script "
                             "'rocgraph-memstat.py' to postprocess file '"
...
//
bool memstat::s_enabled = ROCGRAPH_ENVARIABLES.get(rocgraph::envariables::MEMSTAT);
bool memstat::s_force_managed
    = ROCGRAPH_ENVARIABLES.get(rocgraph::envariables::MEMSTAT_FORCE_MANAGED);
bool memstat::s_guards_enabled = ROCGRAPH_ENVARIABLES.get(rocgraph::envariables::MEMSTAT_GUARDS);
...
hipError_t rocgraph_free(void* mem, memstat_mode::value_t mode, const char* tag)
{
    hipError_t err = hipErrorInvalidValue;
    switch(mode)
    {
...
hipError_t rocgraph_malloc(void** mem, size_t nbytes, memstat_mode::value_t mode, const char* tag)
{
...
hipError_t
    rocgraph_free_async(void* mem, hipStream_t stream, memstat_mode::value_t mode, const char* tag)
{
    hipError_t err = hipErrorInvalidValue;
    switch(mode)
    {
...
hipError_t rocgraph_malloc_async(
...
memstat::memstat()
    : m_report_filename("rocgraph_memstat.json")
{
...
void memstat::add(void* address, size_t nbytes, memstat_mode::value_t mode, const char* tag)
{
    ...
    {
        THROW_WITH_MESSAGE_IF_ROCGRAPH_ERROR(rocgraph_status_internal_error,
...
void memstat::flush_report(bool finalize)
{
    if(this->m_data.size() >= 128 || finalize)
    {
        ...
        out.close();
        this->m_next_flush_report += rocgraph::min(m_data.size(), size_t(128));
        this->m_data.clear();
    }
}
...
hipError_t rocgraph_hip_free(void* mem, const char* tag)
{
    ...
    return rocgraph_free(mem, mode, tag);
}
...
hipError_t rocgraph_hip_malloc(void** mem, size_t nbytes, const char* tag)
{
    ...
    return rocgraph_malloc(mem, nbytes, mode, tag);
}

hipError_t rocgraph_hip_free_async(void* mem, hipStream_t stream, const char* tag)
{
    return rocgraph_free_async(mem, stream, memstat_mode::device, tag);
}

hipError_t rocgraph_hip_malloc_async(void** mem, size_t nbytes, hipStream_t stream, const char* tag)
{
    return rocgraph_malloc_async(mem, nbytes, stream, memstat_mode::device, tag);
}

hipError_t rocgraph_hip_host_free(void* mem, const char* tag)
{
    return rocgraph_free(mem, memstat_mode::host, tag);
}

hipError_t rocgraph_hip_host_malloc(void** mem, size_t nbytes, const char* tag)
{
    return rocgraph_malloc(mem, nbytes, memstat_mode::host, tag);
}

hipError_t rocgraph_hip_free_managed(void* mem, const char* tag)
{
    return rocgraph_free(mem, memstat_mode::managed, tag);
}

hipError_t rocgraph_hip_malloc_managed(void** mem, size_t nbytes, const char* tag)
{
    return rocgraph_malloc(mem, nbytes, memstat_mode::managed, tag);
}
