...
namespace rocgraph
{
    ...
    namespace
    {
        ...
        struct update_v_to_destination_index_t
        {
            ...
            __device__ void operator()(od_idx_t i) const
            {
                v_to_destination_indices[destinations[i]] = i;
            }
        };
        ...
        struct e_op_t
        {
            detail::kv_hipco_store_find_device_view_t<
                detail::kv_hipco_store_view_t<key_t, weight_t const*>>
                     key_to_dist_map{};
            tag_t    num_origins{};
            weight_t cutoff{};
            weight_t invalid_distance{};

            __device__ thrust::optional<thrust::tuple<tag_t, weight_t>>
                       operator()(thrust::tuple<vertex_t, tag_t> tagged_src,
                           vertex_t                       dst,
                           thrust::nullopt_t,
                           thrust::nullopt_t,
                           weight_t w) const
            {
                ...
                auto dst_val
                    = key_to_dist_map.find(aggregator(thrust::make_tuple(dst, origin_idx)));
                if(dst_val != invalid_distance)
                {
                    threshold = dst_val < threshold ? dst_val : threshold;
                }
                return (new_distance < threshold)
                           ? thrust::optional<thrust::tuple<tag_t, weight_t>>{thrust::make_tuple(
        ...
        struct insert_nbr_key_t
        {
            ...
            detail::key_hipco_store_insert_device_view_t<detail::key_hipco_store_view_t<key_t>>
                                                   key_set{};
        ...
        struct keep_t
        {
            ...
            detail::key_hipco_store_contains_device_view_t<detail::key_hipco_store_view_t<key_t>>
                key_set{};
        ...
        struct is_no_smaller_than_threshold_t
        {
            ...
            __device__ bool operator()(key_t key) const
            {
                return key_to_dist_map.find(key) >= threshold;
            }
        };
                                         ...
                                         size_t max_capacity_increment)
        {
            ...
            {
                return old_capacity
                       + raft::round_up_safe(new_min_size - old_capacity, max_capacity_increment);
            }
        }
        ...
        __global__ static void
            multi_partition_copy(InputIterator             input_first,
        ...
        {
            static_assert(max_num_partitions
            ...
            // & atomic operations)
            static_assert(
                static_cast<size_t>(multi_partition_copy_block_size)
                    * static_cast<size_t>(tmp_buffer_size)
                <= static_cast<size_t>(
                    std::numeric_limits<
                        int32_t>::max())); // int32_t is sufficient to store the maximum possible
            ...
            auto rounded_up_num_elems = ((num_elems + static_cast<size_t>(blockDim.x - 1))
            ...
            while(idx < rounded_up_num_elems)
            {
                ...
                for(int32_t i = 0; i < num_partitions; ++i)
                {
                    BlockScan(temp_storage)
                        .ExclusiveSum(static_cast<int32_t>(tmp_counts[i]),
                                      tmp_intra_block_offsets[i]);
                }
                if(threadIdx.x == (blockDim.x - 1))
                {
                    ...
                    {
                        auto increment
                            = static_cast<size_t>(tmp_intra_block_offsets[i] + tmp_counts[i]);
                        hip::atomic_ref<size_t, hip::thread_scope_device> atomic_counter(
                            partition_counters[i]);
                    ...
                    if(tmp_idx < num_elems)
                    {
                        ...
                        if(partition != static_cast<uint8_t>(max_num_partitions))
                        {
                            auto offset = block_start_offsets[partition]
                                          + static_cast<size_t>(tmp_intra_block_offsets[partition]
                                                                + tmp_offsets[i]);
                            *(output_buffer_ptrs[partition] + offset)
                                = key_op(*(input_first + tmp_idx));
                        }
                    }
                    tmp_idx += gridDim.x * blockDim.x;
                }
            ...
            size_t num_origins)
        {
            ...
            size_t                        keep_count{0};
            {
                std::tie(old_key_buffer, old_value_buffer)
                    = key_to_dist_map.release(handle.get_stream());
                ...
                for(size_t i = 0; i < far_buffers.size(); ++i)
                {
                    ...
                    num_edges += edge_partition.compute_number_of_edges(far_vertex_first,
                                                                        far_vertex_first
                                                                            + far_buffers[i].size(),
                                                                        handle.get_stream());
                }
                ...
                for(size_t i = 0; i < far_buffers.size(); ++i)
                {
                    ...
                    thrust::for_each(
                        ...
                        insert_nbr_key_t<vertex_t, edge_t, tag_t, key_t>{
                            ...
                            indices,
                            detail::key_hipco_store_insert_device_view_t(key_set.view()),
                            aggregate_vi_t<vertex_t, tag_t, key_t>{
                                static_cast<tag_t>(num_origins)}});
                }
                ...
                auto old_kv_pair_last = thrust::transform(
                    ...
                    keep_flags.begin(),
                    keep_t<key_t, weight_t>{
                        old_near_far_threshold,
                        detail::key_hipco_store_contains_device_view_t(key_set.view())});

                keep_count = thrust::count_if(handle.get_thrust_policy(),
                                              keep_flags.begin(),
                                              keep_flags.end(),
                                              thrust::identity<bool>{});
            }

            size_t new_kv_store_capacity
                = compute_kv_store_capacity(keep_count + min_extra_capacity,
                                            old_kv_store_capacity,
        ...
        rmm::device_uvector<weight_t> od_shortest_distances(
            raft::handle_t const& handle,
            GraphViewType const&  graph_view,
            edge_property_view_t<typename GraphViewType::edge_type, weight_t const*>
                                                                         edge_weight_view,
            ...
            bool                                                         do_expensive_check)
        {
            ...
            ROCGRAPH_EXPECTS(
                num_vertices != 0 || (origins.size() == 0 && destinations.size() == 0),
                "Invalid input argument: the input graph is empty but origins.size() > 0 or "
                "destinations.size() > 0.");

            ROCGRAPH_EXPECTS(static_cast<size_t>(num_vertices) * origins.size()
                                 <= std::numeric_limits<key_t>::max(),
                             "Invalid input arguments: graph_view.number_of_vertices() * "
                             "origins.size() too large, the "
                             "current implementation assumes that a vertex ID and a origin index "
                             "can be packed in a 64 "
                             "bit value.");

            ROCGRAPH_EXPECTS(
                origins.size() <= std::numeric_limits<od_idx_t>::max()
            ...
            if(do_expensive_check)
            {
                ...
                auto num_invalid_origins
                    = thrust::count_if(handle.get_thrust_policy(),
                                       origins.begin(),
                                       origins.end(),
                                       [num_vertices] __device__(auto v) {
                                           return !is_valid_vertex(num_vertices, v);
                                       });
                auto num_invalid_destinations
                    = thrust::count_if(handle.get_thrust_policy(),
                                       destinations.begin(),
                                       destinations.end(),
                                       [num_vertices] __device__(auto v) {
                ...
                ROCGRAPH_EXPECTS(num_invalid_origins == 0,
                                 "Invalid input arguments: origins contains invalid vertex IDs.");
                ROCGRAPH_EXPECTS(
                    num_invalid_destinations == 0,
                ...
                ROCGRAPH_EXPECTS(thrust::unique(handle.get_thrust_policy(),
                                                tmp_origins.begin(),
                                                tmp_origins.end())
                                     == tmp_origins.end(),
                                 "Invalid input arguments: origins should not have duplicates.");

                rmm::device_uvector<vertex_t> tmp_destinations(destinations.size(),
                                                               handle.get_stream());
                thrust::copy(handle.get_thrust_policy(),
                             destinations.begin(),
                             destinations.end(),
            ...
            size_t kv_store_capacity_increment = raft::round_up_safe(
                std::max(
                    static_cast<size_t>((static_cast<double>(total_global_mem) * 0.01)
            ...
            auto target_near_q_size
                = static_cast<size_t>(handle.get_device_properties().multiProcessorCount)
                  * size_t{
                      32
                      * 1024}; // increase the step size if the near queue size is smaller than the target
            ...
            if(num_vertices == 0 || num_edges == 0 || od_matrix.size() == 0)
            {
                return od_matrix;
            }

            rmm::device_uvector<od_idx_t> v_to_destination_indices(num_vertices,
                                                                   handle.get_stream());
            thrust::fill(handle.get_thrust_policy(),
            ...
            auto init_kv_store_capacity = compute_kv_store_capacity(
                init_kv_store_size, size_t{0}, kv_store_capacity_increment);
            ...
            {
                auto tagged_origin_first = thrust::make_zip_iterator(
                    origins.begin(), thrust::make_counting_iterator(od_idx_t{0}));
                vertex_frontier.bucket(bucket_idx_near)
                    .insert(tagged_origin_first, tagged_origin_first + origins.size());

                auto key_first
                    = thrust::make_transform_iterator(tagged_origin_first,
                                                      aggregate_vi_t<vertex_t, od_idx_t, key_t>{
            ...
            while(true)
            {
                ...
                rmm::device_uvector<weight_t> distance_buffer(0, handle.get_stream());
                {
                    ...
                    auto new_frontier_tagged_vertex_buffer
                        = allocate_dataframe_buffer<thrust::tuple<vertex_t, od_idx_t>>(
                            0, handle.get_stream());
                    ...
                    resize_dataframe_buffer(
                        new_frontier_tagged_vertex_buffer, 0, handle.get_stream());
                    shrink_to_fit_dataframe_buffer(new_frontier_tagged_vertex_buffer,
                                                   handle.get_stream());

                    std::tie(new_frontier_keys, distance_buffer)
                        = detail::sort_and_reduce_buffer_elements<key_t,
                                                                  weight_t,
...

                {
                    ...
                    if(key_to_dist_map.capacity() < new_min_capacity)
                    { // note that this is conservative as some keys may already exist in
                        // key_to_dist_map
                        auto new_kv_store_capacity
                            = compute_kv_store_capacity(new_min_capacity,
                                                        key_to_dist_map.capacity(),
                                                        kv_store_capacity_increment);
                        auto [old_key_buffer, old_value_buffer]
                            = key_to_dist_map.release(handle.get_stream());
                        key_to_dist_map
                            = kv_store_t<key_t, weight_t, false /* use_binary_search */>(
                                new_kv_store_capacity,
                                invalid_key,
                                invalid_distance,
                                handle.get_stream());
                    ...
                    thrust::transform_if(
                        ...
                        thrust::make_permutation_iterator(
                            ...
                            thrust::make_transform_iterator(
                                new_frontier_keys.begin(),
                                compute_od_matrix_index_t<vertex_t, od_idx_t, key_t>{
                                    raft::device_span<od_idx_t const>(
                                        v_to_destination_indices.data(),
                                        v_to_destination_indices.size()),
                                    static_cast<od_idx_t>(origins.size()),
                                    static_cast<od_idx_t>(destinations.size())})),
                        thrust::identity<weight_t>{},
...

                {
                    ...
                    rmm::device_uvector<size_t> d_counters(d_split_thresholds.size() + 1,
                                                           handle.get_stream());
                    thrust::fill(handle.get_thrust_policy(),
                                 d_counters.begin(),
                                 d_counters.end(),
                    ...
                    auto input_first = thrust::make_zip_iterator(new_frontier_keys.begin(),
                                                                 distance_buffer.begin());

                    size_t num_copied{0};
                    while(num_copied < num_tagged_vertices)
                    {
                        auto this_loop_size = std::min(key_buffer_capacity_increment,
                                                       num_tagged_vertices - num_copied);

                        std::vector<key_t*> h_buffer_ptrs(d_counters.size());
                        tmp_near_q_keys.resize(tmp_near_q_keys.size() + this_loop_size,
                                               handle.get_stream());
                            ...
                            if(far_buffers[i].size() + this_loop_size > far_buffers[i].capacity())
                            {
                                far_buffers[i].reserve(
                                    far_buffers[i].capacity()
                                        + raft::round_up_safe(this_loop_size,
                                                              key_buffer_capacity_increment),
                                    handle.get_stream());
                            }
                            far_buffers[i].resize(far_buffers[i].size() + this_loop_size,
                                                  handle.get_stream());
                            h_buffer_ptrs[i + 1] = far_buffers[i].data() + old_far_buffer_sizes[i];
                        }
                        rmm::device_uvector<key_t*> d_buffer_ptrs(h_buffer_ptrs.size(),
                                                                  handle.get_stream());
                        ...
                        THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                            (multi_partition_copy<static_cast<int32_t>(
                                 1 /* near queue */ + num_far_buffers)>),
                            ...
                            [split_thresholds = raft::device_span<weight_t const>(
                                 ...
                                 d_split_thresholds.size())] __device__(auto pair) {
                        ...
                        raft::update_host(h_counters.data(),
                                          d_counters.data(),
                                          d_counters.size(),
                                          handle.get_stream());
                        handle.sync_stream();
                        ...
                        for(size_t i = 0; i < num_far_buffers; ++i)
                        {
                            far_buffers[i].resize(old_far_buffer_sizes[i] + h_counters[i + 1],
                                                  handle.get_stream());
                        }
                    ...
                    tmp_near_q_keys.resize(
                        thrust::distance(tmp_near_q_keys.begin(),
                                         thrust::unique(handle.get_thrust_policy(),
                    ...
                    auto near_vi_first = thrust::make_transform_iterator(
                        tmp_near_q_keys.begin(),
                        split_vi_t<vertex_t, od_idx_t, key_t>{
                            static_cast<od_idx_t>(origins.size())});
                    vertex_frontier.bucket(bucket_idx_near)
                ...
                else
                {
                    ...
                    if(num_aggregate_far_keys > 0)
                    { // near queue is empty, split the far queue
                        std::vector<weight_t> invalid_thresholds(num_far_buffers);
                        for(size_t i = 0; i < invalid_thresholds.size(); ++i)
                        {
                            invalid_thresholds[i]
                                = (i == 0) ? near_far_threshold : next_far_thresholds[i - 1];
                        }
                        ...
                        do
                        {
                            ...
                            do
                            {
                                near_far_threshold = (num_far_buffers > 1)
                                                         ? next_far_thresholds[0]
                            ...
                            for(size_t i = 0; i < num_far_buffers; ++i)
                            {
                                ...
                                if(i == (num_far_buffers - 1))
                                {
                                    ...
                                    h_split_thresholds[0]
                                        = (num_far_buffers == num_near_q_insert_buffers)
                                              ? near_far_threshold
                                              : next_far_thresholds[(num_far_buffers
                                                                     - num_near_q_insert_buffers)
                                                                    - 1];
                                    ...
                                    rmm::device_uvector<weight_t> d_split_thresholds(
                                        h_split_thresholds.size(), handle.get_stream());
                                    raft::update_device(d_split_thresholds.data(),
                                                        h_split_thresholds.data(),
                                                        h_split_thresholds.size(),
                                                        handle.get_stream());

                                    rmm::device_uvector<key_t> tmp_buffer
                                        = std::move(far_buffers.back());
                                    std::vector<key_t*> h_buffer_ptrs(h_split_thresholds.size()
                                                                      + 1);
                                    ...
                                    for(size_t j = 0; j < h_buffer_ptrs.size(); ++j)
                                    {
                                        ...
                                        {
                                            new_near_q_keys.resize(old_size + tmp_buffer.size(),
                                                                   handle.get_stream());
                                        ...
                                        else
                                        {
                                            ...
                                            far_buffers[buffer_idx].reserve(
                                                raft::round_up_safe(
                                                    std::max(tmp_buffer.size(), size_t{1}),
                                                    key_buffer_capacity_increment),
                                                handle.get_stream());
                                            far_buffers[buffer_idx].resize(tmp_buffer.size(),
                                                                           handle.get_stream());
                                            h_buffer_ptrs[j] = far_buffers[buffer_idx].data();
                                        }
                                    }
                                    rmm::device_uvector<key_t*> d_buffer_ptrs(
                                        num_near_q_insert_buffers + 1, handle.get_stream());
                                    raft::update_device(d_buffer_ptrs.data(),
                                                        h_buffer_ptrs.data(),
                                                        h_buffer_ptrs.size(),
                                                        handle.get_stream());
                                    rmm::device_uvector<size_t> d_counters(
                                        num_near_q_insert_buffers + 1, handle.get_stream());
                                    thrust::fill(handle.get_thrust_policy(),
                                                 d_counters.begin(),
                                                 d_counters.end(),
                                    ...
                                    if(tmp_buffer.size() > 0)
                                    {
                                        raft::grid_1d_thread_t update_grid(
                                            ...
                                            multi_partition_copy_block_size,
                                            handle.get_device_properties().maxGridSize[0]);
                                        auto constexpr max_num_partitions = static_cast<int32_t>(
                                            1 /* near queue */ + num_far_buffers);
                                        THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                                            ...
                                            tmp_buffer.begin(),
                                            tmp_buffer.end(),
                                            raft::device_span<key_t*>(d_buffer_ptrs.data(),
                                                                      d_buffer_ptrs.size()),
                                            [key_to_dist_map
                                             = detail::kv_hipco_store_find_device_view_t(
                                                 key_to_dist_map.view()),
                                             split_thresholds = raft::device_span<weight_t const>(
                                                 d_split_thresholds.data(),
                                            ...
                                            raft::device_span<size_t>(d_counters.data(),
                                                                      d_counters.size()));
                                    }
                                    std::vector<size_t> h_counters(d_counters.size());
                                    raft::update_host(h_counters.data(),
                                    ...
                                    for(size_t j = 0; j < (num_near_q_insert_buffers + 1); ++j)
                                    {
                                        if(j == 0 && (num_far_buffers == num_near_q_insert_buffers))
                                        {
                                            new_near_q_keys.resize(old_size + h_counters[j],
                                                                   handle.get_stream());
                                        }
                                        else
                                        {
                                            auto buffer_idx
                                                = (num_far_buffers - num_near_q_insert_buffers)
                                                  + (j - 1);
                                            far_buffers[buffer_idx].resize(h_counters[j],
                                                                           handle.get_stream());
                                        }
                                    }
                                }
                                ...
                                {
                                    auto old_size = new_near_q_keys.size();
                                    new_near_q_keys.resize(old_size + far_buffers[i].size(),
                                                           handle.get_stream());
                                    auto last = thrust::copy_if(
                                        ...
                                        is_no_smaller_than_threshold_t<key_t, weight_t>{
                                            ...
                                            detail::kv_hipco_store_find_device_view_t(
                                                key_to_dist_map.view())});
                                    new_near_q_keys.resize(
                                        thrust::distance(new_near_q_keys.begin(), last),
                                        handle.get_stream());
                                }
                                else
                                {
                                    far_buffers[i - num_near_q_insert_buffers]
                                        = std::move(far_buffers[i]);
                                }
                            }
                            ...
                            thrust::sort(handle.get_thrust_policy(),
                                         ...
                                         new_near_q_keys.end());
                            new_near_q_keys.resize(
                                thrust::distance(new_near_q_keys.begin(),
                                                 thrust::unique(handle.get_thrust_policy(),
                                                                new_near_q_keys.begin(),
                                                                new_near_q_keys.end())),
                                handle.get_stream());
                        ...
                        auto next_near_far_threshold_num_inserts_estimate = static_cast<size_t>(
                            static_cast<double>(prev_near_far_threshold_num_inserts)
                        ...
                        if(key_to_dist_map.size() + next_near_far_threshold_num_inserts_estimate
                           >= key_to_dist_map.capacity())
                        { // if resize is likely to be necessary before reaching
                            // this check again
                            std::vector<raft::device_span<key_t const>> far_buffer_spans(
                                num_far_buffers);
                            for(size_t i = 0; i < num_far_buffers; ++i)
                            {
                                far_buffer_spans[i] = raft::device_span<key_t const>(
                                    far_buffers[i].data(), far_buffers[i].size());
                            }
                            key_to_dist_map = filter_key_to_dist_map(
                                handle,
                                graph_view,
                              ...
                              bool                                          do_expensive_check)
    {
        ...
        // reduction.
        auto delta
            = std::max(average_edge_weight * 0.5, std::numeric_limits<weight_t>::min() * 1e3);
