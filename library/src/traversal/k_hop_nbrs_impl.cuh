...
namespace rocgraph
{
    ...
    namespace
    {
        ...
        struct e_op_t
        {
            __device__ thrust::optional<size_t>
    ...
    namespace detail
    {
                       ...
                       bool do_expensive_check)
        {
            ...
            {
                start_vertex_counts = host_scalar_allgather(
            ...
            ROCGRAPH_EXPECTS(
                start_vertex_displacements.back() + start_vertex_counts.back() > 0,
                "Invalid input argument: input should have at least one starting vertex.");

            ROCGRAPH_EXPECTS(k > 0, "Invalid input argument: k should be a positive integer.");
            ...
            vertex_frontier_t<vertex_t, size_t, GraphViewType::is_multi_gpu, false> frontier(
                handle, num_buckets);

            auto key_first = thrust::make_zip_iterator(
                start_vertices.begin(),
                thrust::make_counting_iterator(
                    start_vertex_displacements[GraphViewType::is_multi_gpu
            ...
            {
                auto new_frontier_key_buffer = transform_reduce_v_frontier_outgoing_e_by_dst(
                    handle,
                    push_graph_view,
            ...
            {
                rmm::device_uvector<size_t> lasts(handle.get_comms().get_size(),
                                                  handle.get_stream());
                ...
                std::tie(start_vertex_indices, nbrs, std::ignore)
            ...
            rmm::device_uvector<size_t> offsets(start_vertices.size() + size_t{1},
                                                handle.get_stream());
