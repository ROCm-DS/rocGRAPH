...
namespace rocgraph
{
    ...
    namespace
    {
        ...
        struct e_op_t
        {
                ...
                vertex_t src, vertex_t dst, weight_t src_val, thrust::nullopt_t, weight_t w) const
            {
                ...
                return push
                           ? thrust::optional<thrust::tuple<weight_t, vertex_t>>{thrust::make_tuple(
                                 new_distance, src)}
                           : thrust::nullopt;
            }
        };
    ...
    namespace detail
    {
                  ...
                  bool                                do_expensive_check)
        {
            ...
            ROCGRAPH_EXPECTS(push_graph_view.is_valid_vertex(source_vertex),
            ...
            thrust::transform(
                ...
                thrust::make_counting_iterator(
                    push_graph_view.local_vertex_partition_range_first()),
                thrust::make_counting_iterator(push_graph_view.local_vertex_partition_range_last()),
                val_first,
                [source_vertex, invalid_vertex] __host__ __device__(auto val) {
                    auto distance = invalid_distance;
                    if(val == source_vertex)
                    {
                        distance = weight_t{0.0};
                    }
            ...
            auto delta = (static_cast<weight_t>(raft::host_warp_size(handle.get_device()))
                          * average_edge_weight)
            ...
            while(true)
            {
                ...
                update_v_frontier(
                    ...
                    distances,
                    thrust::make_zip_iterator(thrust::make_tuple(distances, predecessor_first)),
                    [bucket_idx_far, bucket_idx_next_near, near_far_threshold] __host__ __device__(
                        auto v, auto v_val, auto pushed_val) {
                        auto new_dist = thrust::get<0>(pushed_val);
                        auto update   = (new_dist < v_val);
                        return thrust::make_tuple(
                            update ? thrust::optional<size_t>{new_dist < near_far_threshold
                                                                  ? bucket_idx_next_near
                                                                  : bucket_idx_far}
                                   : thrust::nullopt,
                ...
                else if(vertex_frontier.bucket(bucket_idx_far).aggregate_size() > 0)
                    ...
                    while(true)
                    {
                        vertex_frontier.split_bucket(
                            ...
                            [vertex_partition,
                             ...
                             near_far_threshold,
                             bucket_idx_far,
                             bucket_idx_cur_near] __host__ __device__(auto v) {
                                auto dist = *(
                                    distances
                                    + vertex_partition
                                          .local_vertex_partition_offset_from_vertex_nocheck(v));
                                return dist >= old_near_far_threshold
                                           ? thrust::optional<size_t>{dist < near_far_threshold
                                                                          ? bucket_idx_cur_near
                                                                          : bucket_idx_far}
                                           : thrust::nullopt;
                            });
