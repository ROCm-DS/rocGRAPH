...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        struct compute_max_distance
        {
            ...
            {
                auto offset
                    = vertex_partition_.local_vertex_partition_offset_from_vertex_nocheck(v);

                return (predecessors_[offset] == invalid_vertex_) ? vertex_t{0}
                                                                  : distances_[offset];
            }
        };
        ...
        struct update_paths
        {
            ...
            void __device__ operator()(thrust::tuple<vertex_t, size_t> tuple)
            {
                ...
                if(next_v != invalid_vertex_)
                    paths_[offset] = next_v;
            }
        };
        ...
        {
            size_t __device__ operator()(size_t offset)
            {
                return offset - 1;
            }
        };
        ...
        std::tuple<rmm::device_uvector<vertex_t>, rmm::device_uvector<size_t>>
            shrink_extraction_list(raft::handle_t const&           handle,
                                   rmm::device_uvector<vertex_t>&& vertex_list,
                                   rmm::device_uvector<size_t>&&   path_offset)
        {
            auto constexpr invalid_vertex = invalid_vertex_id<vertex_t>::value;

            auto begin_iter = thrust::make_zip_iterator(
                thrust::make_tuple(vertex_list.begin(), path_offset.begin()));

            auto end_iter = thrust::remove_if(handle.get_thrust_policy(),
                                              begin_iter,
                                              begin_iter + vertex_list.size(),
                                              [invalid_vertex] __device__(auto p) {
                                                  return thrust::get<0>(p) == invalid_vertex;
                                              });
    ...
    std::tuple<rmm::device_uvector<vertex_t>, vertex_t>
    ...
    {
        ROCGRAPH_EXPECTS((graph_view.local_vertex_partition_range_size() == 0)
                             || (distances != nullptr),
        ...
        ROCGRAPH_EXPECTS((n_destinations == 0) || (destinations != nullptr),
        ...
        if constexpr(multi_gpu)
        {
            max_path_length = rocgraph::host_scalar_allreduce(
        ...
        thrust::for_each_n(handle.get_thrust_policy(),
                           thrust::make_zip_iterator(thrust::make_tuple(current_frontier.begin(),
                                                                        current_position.begin())),
        ...
        for(vertex_t count = 0; count < max_path_length; ++count)
        {
            ...
            thrust::for_each_n(handle.get_thrust_policy(),
                               thrust::make_zip_iterator(thrust::make_tuple(
                                   current_frontier.begin(), current_position.begin())),
                               current_frontier.size(),
                               detail::update_paths<vertex_t>{paths.data(), invalid_vertex});
        }
