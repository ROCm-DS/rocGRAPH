    ...
    namespace
    {
                ...
                func_t                                            func)
        {
            ...
            if(weights)
            {
                    ...
                    if(edge_types)
                    {
                        d_tx_value_counts = rocgraph::groupby_and_count(
                            thrust::make_zip_iterator(majors.begin(),
                                                      ...
                                                      edge_ids->begin(),
                                                      edge_types->begin()),
                            thrust::make_zip_iterator(majors.end(),
                                                      minors.end(),
                                                      weights->end(),
                                                      edge_ids->end(),
                                                      edge_types->end()),
                            [func] __device__(auto val) {
                                return func(thrust::get<0>(val), thrust::get<1>(val));
                            },
                            comm_size,
                    ...
                    else
                    {
                        d_tx_value_counts = rocgraph::groupby_and_count(
                            ...
                            thrust::make_zip_iterator(
                                majors.end(), minors.end(), weights->end(), edge_ids->end()),
                            [func] __device__(auto val) {
                                return func(thrust::get<0>(val), thrust::get<1>(val));
                            },
                            comm_size,
                ...
                else
                {
                    if(edge_types)
                    {
                        d_tx_value_counts = rocgraph::groupby_and_count(
                            thrust::make_zip_iterator(majors.begin(),
                                                      ...
                                                      edge_types->begin()),
                            thrust::make_zip_iterator(
                                majors.end(), minors.end(), weights->end(), edge_types->end()),
                            [func] __device__(auto val) {
                                return func(thrust::get<0>(val), thrust::get<1>(val));
                            },
                            comm_size,
                    ...
                    else
                    {
                        d_tx_value_counts = rocgraph::groupby_and_count(
                            thrust::make_zip_iterator(
                                majors.begin(), minors.begin(), weights->begin()),
            ...
            else
            {
                if(edge_ids)
                {
                    if(edge_types)
                    {
                        d_tx_value_counts = rocgraph::groupby_and_count(
                            thrust::make_zip_iterator(majors.begin(),
                                                      ...
                                                      edge_types->begin()),
                            thrust::make_zip_iterator(
                                majors.end(), minors.end(), edge_ids->end(), edge_types->end()),
                            [func] __device__(auto val) {
                                return func(thrust::get<0>(val), thrust::get<1>(val));
                            },
                            comm_size,
                    ...
                    else
                    {
                        d_tx_value_counts = rocgraph::groupby_and_count(
                            thrust::make_zip_iterator(
                                majors.begin(), minors.begin(), edge_ids->begin()),
                ...
                else
                {
                    if(edge_types)
                    {
                        d_tx_value_counts = rocgraph::groupby_and_count(
                            thrust::make_zip_iterator(
                                majors.begin(), minors.begin(), edge_types->begin()),
                            thrust::make_zip_iterator(
                                majors.end(), minors.end(), edge_types->end()),
                            [func] __device__(auto val) {
                                return func(thrust::get<0>(val), thrust::get<1>(val));
                            },
                            comm_size,
            ...
            { // trade-off potential parallelism to lower peak memory
                ...
                if(weights)
                {
                    std::tie(weights, std::ignore) = shuffle_values(
                        comm, (*weights).begin(), h_tx_value_counts, handle.get_stream());
                }

                if(edge_ids)
                {
                    std::tie(edge_ids, std::ignore) = shuffle_values(
                        comm, (*edge_ids).begin(), h_tx_value_counts, handle.get_stream());
                }

                if(edge_types)
                {
                    std::tie(edge_types, std::ignore) = shuffle_values(
                        comm, (*edge_types).begin(), h_tx_value_counts, handle.get_stream());
                }
            }
            else
            {
                if(weights)
                {
                    if(edge_ids)
                    {
                        ...
                        {
                            std::forward_as_tuple(std::tie(majors, minors, weights, edge_ids),
                                                  std::ignore)
                                = shuffle_values(comm,
                                                 thrust::make_zip_iterator(majors.begin(),
                                                                           minors.begin(),
                    ...
                    else
                    {
                        if(edge_types)
                        {
                            std::forward_as_tuple(std::tie(majors, minors, weights, edge_types),
                                                  std::ignore)
                                = shuffle_values(comm,
                                                 thrust::make_zip_iterator(majors.begin(),
                                                                           minors.begin(),
                        ...
                        {
                            std::forward_as_tuple(std::tie(majors, minors, weights), std::ignore)
                                = shuffle_values(comm,
                                                 thrust::make_zip_iterator(majors.begin(),
                                                                           minors.begin(),
                                                                           weights->begin()),
                ...
                else
                {
                    if(edge_ids)
                    {
                        if(edge_types)
                        {
                            std::forward_as_tuple(std::tie(majors, minors, edge_ids, edge_types),
                                                  std::ignore)
                                = shuffle_values(comm,
                                                 thrust::make_zip_iterator(majors.begin(),
                                                                           minors.begin(),
                        ...
                        {
                            std::forward_as_tuple(std::tie(majors, minors, edge_ids), std::ignore)
                                = shuffle_values(comm,
                                                 thrust::make_zip_iterator(majors.begin(),
                                                                           minors.begin(),
                                                                           edge_ids->begin()),
                    ...
                    else
                    {
                        if(edge_types)
                        {
                            std::forward_as_tuple(std::tie(majors, minors, edge_types), std::ignore)
                                = shuffle_values(comm,
                                                 thrust::make_zip_iterator(majors.begin(),
                                                                           minors.begin(),
                                                                           edge_types->begin()),
    ...
    namespace detail
    {
                ...
                std::optional<rmm::device_uvector<edge_type_t>>&& edge_types)
        {
            ...
            return shuffle_vertex_pairs_with_values_by_gpu_id_impl(
                ...
                std::move(edge_ids),
                std::move(edge_types),
                rocgraph::detail::compute_gpu_id_from_ext_edge_endpoints_t<vertex_t>{
                ...
                std::vector<vertex_t> const&                      vertex_partition_range_lasts)
        {
            ...
            return shuffle_vertex_pairs_with_values_by_gpu_id_impl(
                ...
                std::move(edge_ids),
                std::move(edge_types),
                rocgraph::detail::compute_gpu_id_from_int_edge_endpoints_t<vertex_t>{
