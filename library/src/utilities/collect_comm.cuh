...
namespace rocgraph
{
                                ...
                                KeyToGPUIdOp          key_to_gpu_id_op)
    {
        ...
        static_assert(
            std::is_same_v<typename thrust::iterator_traits<KeyIterator>::value_type, key_t>);
        ...
        rmm::device_uvector<key_t> unique_keys(
        ...
        auto values_for_unique_keys = allocate_dataframe_buffer<value_t>(0, handle.get_stream());
        {
            ...
            auto rx_values_for_unique_keys
        ...
        if constexpr(KVStoreViewType::binary_search)
        {
            unique_key_value_store
                = kv_store_t<key_t, value_t, true>(std::move(unique_keys),
        ...
        else
        {
            ...
            auto valid_kv_pair_last = thrust::remove_if(
            ...
            auto num_valid_pairs
                = static_cast<size_t>(thrust::distance(kv_pair_first, valid_kv_pair_last));
            unique_key_value_store = kv_store_t<key_t, value_t, false>(
                unique_keys.begin(),
    ...
    std::tuple<rmm::device_uvector<vertex_t>,
               decltype(allocate_dataframe_buffer<typename thrust::iterator_traits<
                            ValueIterator>::value_type>(0, hipStream_t{nullptr}))>
        collect_values_for_unique_int_vertices(
            raft::handle_t const&           handle,
                ...
                vertex_partition_range_lasts)
    {
        ...
        raft::copy(sorted_unique_int_vertices.data(),
        ...
        sorted_unique_int_vertices.resize(
            thrust::distance(sorted_unique_int_vertices.begin(), last), handle.get_stream());

        auto [unique_int_vertices, tmp_value_buffer]
    ...
    {
        auto rx_sizes = rocgraph::host_scalar_allgather(comms, d_input.size(), handle.get_stream());
