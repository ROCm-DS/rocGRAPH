        ...
        struct is_invalid_input_vertex_pair_t
        {
            ...
            __device__ bool operator()(thrust::tuple<vertex_t, vertex_t> pair) const
            {
                ...
                auto it = thrust::upper_bound(thrust::seq,
                                              ...
                                              major);
                if(it == edge_partition_major_range_lasts.end())
                {
                    return true;
                }
                auto edge_partition_idx = static_cast<size_t>(
                    thrust::distance(edge_partition_major_range_lasts.begin(), it));
                if(major < edge_partition_major_range_firsts[edge_partition_idx])
                {
                    return true;
                }
                return (minor < edge_partition_minor_range_first)
                       || (minor >= edge_partition_minor_range_last);
            }
        };
                                          ...
                                          VertexPairIterator    vertex_pair_last)
        {
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                if constexpr(GraphViewType::is_storage_transposed)
                {
                    ...
                    edge_partition_minor_range_last
                        = graph_view.local_edge_partition_src_range_last();
                }
                else
                {
                    edge_partition_minor_range_first
            ...
            if constexpr(GraphViewType::is_multi_gpu)
            {
                ...
                num_invalid_pairs = host_scalar_allreduce(
                    comm, num_invalid_pairs, raft::comms::op_t::SUM, handle.get_stream());
            }
