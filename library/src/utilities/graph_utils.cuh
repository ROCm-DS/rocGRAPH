...
#define US
        ...
        __inline__ __device__ value_t parallel_prefix_sum(count_t        n,
                                                          index_t const* ind,
                                                          value_t const* w)
        {
            count_t i, j, mn;
        ...
        T nrm2(size_t n, T* x)
        {
            rmm::cuda_stream_view stream_view;
            T                     init = 0;
            T result = sqrt(thrust::transform_reduce(rmm::exec_policy(stream_view),
                                                     thrust::device_pointer_cast(x),
                                                     thrust::device_pointer_cast(x + n),
                                                     square<T>(),
                                                     init,
        ...
        void printv(size_t n, T* vec, int offset)
        {
            ...
            thrust::copy(
                ...
                std::ostream_iterator<T>(
                    std::cout,
                    " ")); // Assume no RMM dependency; TODO: check / test (potential BUG !!!!!)
                               ...
                               ValueType*       bookmark)
        {
            ...
            nblocks.y  = 1;
            nblocks.z  = 1;
            THROW_IF_HIPLAUNCHKERNELGGL_ERROR((degree_coo<IndexType, IndexType>),
                                              nblocks,
                                              nthreads,
                                              0,
                                              stream_view.value(),
            ...
            THROW_IF_HIPLAUNCHKERNELGGL_ERROR((equi_prob3<IndexType, ValueType>),
                                              nblocks,
                                              nthreads,
                                              0,
                                              stream_view.value(),
            ...
            nblocks.y  = 1;
            nblocks.z  = 1;
            THROW_IF_HIPLAUNCHKERNELGGL_ERROR((flag_leafs_kernel<IndexType, ValueType>),
                                              nblocks,
                                              nthreads,
                                              0,
                                              stream_view.value(),
        ...
        __attribute__((visibility("hidden"))) __global__ void
            offsets_to_indices_kernel(const offsets_t* offsets, index_t v, index_t* indices)
        {
