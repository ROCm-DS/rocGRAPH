...
namespace rocgraph
{
    ...
    namespace
    {
        ...
        {
            __device__ edge_t operator()(edge_t d) const
            {
                return d * edge_t{2};
            }
        };
                     ...
                     bool                                                    do_expensive_check)
    {
        ...
        ROCGRAPH_EXPECTS((degree_type == k_core_degree_type_t::IN)
        ...
        ROCGRAPH_EXPECTS(k_first <= k_last, "Invalid input argument: k_first <= k_last.");

        if(do_expensive_check)
        {
            ROCGRAPH_EXPECTS(graph_view.count_self_loops(handle) == 0,
        ...
        { // in-degree == out-degree
            ...
            else
            {
                auto inout_degree_first = thrust::make_transform_iterator(
                    out_degrees.begin(), mult_degree_by_two_t<edge_t>{});
                thrust::copy(handle.get_thrust_policy(),
                             inout_degree_first,
        ...
        else
        {
            if(degree_type == k_core_degree_type_t::IN)
            {
                ...
                thrust::copy(
                    handle.get_thrust_policy(), in_degrees.begin(), in_degrees.end(), core_numbers);
            }
            ...
            else
            {
                auto in_degrees        = graph_view.compute_in_degrees(handle);
                auto out_degrees       = graph_view.compute_out_degrees(handle);
                auto degree_pair_first = thrust::make_zip_iterator(
                    thrust::make_tuple(in_degrees.begin(), out_degrees.begin()));
                thrust::transform(
        ...
        if(k_first > 1)
        {
            thrust::for_each(
                ...
                [k_first,
                 core_numbers,
                 v_first = graph_view.local_vertex_partition_range_first()] __device__(auto v) {
                    if(core_numbers[v - v_first] < k_first)
                    {
                        core_numbers[v - v_first] = edge_t{0};
                    }
                });
        }
        ...
        edge_dst_property_t<graph_view_t<vertex_t, edge_t, false, multi_gpu>, edge_t>
        ...
        if(graph_view.is_symmetric() && (degree_type == k_core_degree_type_t::INOUT)
           && ((k % 2) == 1))
        { // core numbers are always even numbers if symmetric and INOUT
            ++k;
        }
        while(k <= k_last)
        {
            ...
            if(aggregate_num_remaining_vertices == 0)
            {
                break;
            }
            ...
            remaining_vertices.resize(
                thrust::distance(remaining_vertices.begin(), less_than_k_first),
                handle.get_stream());
                           ...
                           || (degree_type == k_core_degree_type_t::INOUT)))
                    {
                        ...
                        update_v_frontier(
                            ...
                            [bucket_idx_next,
                             ...
                             v_first
                             = graph_view
                                   .local_vertex_partition_range_first()] __device__(auto v,
                                                                                     auto v_val,
                                                                                     auto
                                                                                         pushed_val) {
                                auto new_core_number
                                    = v_val >= pushed_val ? v_val - pushed_val : edge_t{0};
                                new_core_number
                                    = new_core_number < (k - delta) ? (k - delta) : new_core_number;
                                new_core_number
                                    = new_core_number < k_first ? edge_t{0} : new_core_number;
                                return thrust::make_tuple(
                                    thrust::optional<size_t>{bucket_idx_next},
            ...
            else
            {
                auto remaining_vertex_core_number_first = thrust::make_transform_iterator(
                    remaining_vertices.begin(),
                    v_to_core_number_t<vertex_t, edge_t>{
                        core_numbers, graph_view.local_vertex_partition_range_first()});
                ...
                if constexpr(multi_gpu)
                {
                    min_core_number = host_scalar_allreduce(handle.get_comms(),
                                                            min_core_number,
                                                            raft::comms::op_t::MIN,
                                                            handle.get_stream());
                }
