...
namespace rocgraph
{
    namespace detail
    {
                           ...
                           weight_t              epsilon)
        {
            if(num_rows == num_cols)
            {
                ...
                raft::solver::LinearAssignmentProblem<index_t, weight_t> lpx(
                    handle, num_rows, 1, epsilon);
            ...
            else
            {
                ...
                // Solve LAP(s) for given cost matrix
                lpx.solve(
                    tmp_cost_v.begin(), tmp_row_assignment_v.begin(), tmp_col_assignment_v.begin());

                weight_t tmp_objective_value = lpx.getPrimalObjectiveValue(0);

                raft::copy(
                    d_assignment, tmp_row_assignment_v.begin(), num_rows, handle.get_stream());
        ...
        weight_t hungarian_sparse(raft::handle_t const&                                   handle,
                                  ...
                                  weight_t        epsilon)
        {
            ROCGRAPH_EXPECTS(assignment != nullptr,
                             "Invalid input argument: assignment pointer is NULL");
            ROCGRAPH_EXPECTS(graph.edge_data != nullptr,
            ...
            rmm::device_uvector<vertex_t> temp_tasks_v(graph.number_of_vertices,
                                                       handle.get_stream());
            rmm::device_uvector<vertex_t> temp_workers_v(graph.number_of_vertices,
                                                         handle.get_stream());
            ...
            thrust::fill(handle.get_thrust_policy(),
                         ...
                         vertex_t{-1});
            thrust::fill(
                handle.get_thrust_policy(), temp_tasks_v.begin(), temp_tasks_v.end(), vertex_t{-1});
    ...
    template float
        hungarian<int32_t, int32_t, float>(raft::handle_t const&,
                                           ...
                                           float);
    template double
        hungarian<int32_t, int32_t, double>(raft::handle_t const&,
