    ...
    namespace detail
    {
        ...
        struct rrandom_gen_t
        {
                ...
                , d_ptr_random_(original::raw_ptr(d_random))
            {
                ...
                ROCGRAPH_EXPECTS(rnd_sz >= static_cast<decltype(rnd_sz)>(num_paths),
                                      ...
                                      original::device_vec_t<vertex_t>&     d_col_indx) const
            {
                ...
                thrust::transform_if(
                    ...
                    [] __device__(real_t rnd_val, edge_t crt_out_deg) {
                        vertex_t v_indx = static_cast<vertex_t>(
                            rnd_val >= 1.0 ? crt_out_deg - 1 : rnd_val * crt_out_deg);
            ...
            //
            static void generate_random(raft::handle_t const& handle,
                                        real_t*               p_d_rnd,
                                        size_t                sz,
                                        seed_t                seed)
            {
                raft::random::RngState rng_state{seed};
                rocgraph::detail::uniform_random_fill(
        ...
        struct col_indx_extract_t
        {
            ...
            void operator()(
                original::device_vec_t<vertex_t> const&
                    d_coalesced_src_v, // in: coalesced vector of vertices
                original::device_vec_t<vertex_t> const&
                ...
                const
            {
                thrust::transform_if(
                    ...
                    thrust::make_zip_iterator(thrust::make_tuple(
                        d_v_next_vertices.begin(), d_v_next_weights.begin())), // output
                    [max_depth         = max_depth_,
                     ...
                     values            = values_ ? thrust::optional<weight_t const*>{*values_}
                        ...
                        auto weight_value
                            = (values ? (*values)[start_row + col_indx]
 ...
 */
        ...
        struct random_walker_t
        {
            ...
            //
            void start(original::device_const_vector_view<vertex_t, index_t>&
                                                         d_src_init_v, // in: start set
                       ...
                       original::device_vec_t<index_t>& d_sizes) const // out: init sizes to {1,...}
            {
                ...
                //
                auto dlambda = [stride = max_depth_] __device__(auto indx) -> index_t {
                    return indx * stride;
                };
                // use the transform iterator as map:
                //
                auto map_it_begin = thrust::make_transform_iterator(
                    thrust::make_counting_iterator<index_t>(0), dlambda);
            ...
            void step(graph_view_t<vertex_t, edge_t, false, false> const&          graph_view,
                      std::optional<edge_property_view_t<edge_t, weight_t const*>> edge_weight_view,
                      selector_t const&                                            selector,
                      seed_t                                                       seed,
            ...
            {
                auto how_many_stopped = thrust::count_if(
                    handle_.get_thrust_policy(),
                      ...
                      original::device_vec_t<index_t> const& d_sizes) const // paths sizes
            {
                ...
                d_coalesced_v.resize(thrust::distance(d_coalesced_v.begin(), new_end_v),
                                     handle_.get_stream());
                d_coalesced_w.resize(thrust::distance(d_coalesced_w.begin(), new_end_w),
                                     handle_.get_stream());
            }
                ...
                const // nelems = number of elements to gather (typically num_paths_)
            {
                ...
                //
                auto dlambda
                    = [stride, ptr_d_sizes, ptr_d_coalesced] __device__(auto indx) -> vertex_t {
                    auto delta = ptr_d_sizes[indx] - 1;
                                  ...
                                  original::device_vec_t<index_t> const&  d_sizes) const
            {
                scatter_to_coalesced(
                    d_src, d_coalesced, d_crt_out_degs, d_sizes, max_depth_, num_paths_);
            }
            //
            ...
            original::device_vec_t<edge_t>
                get_out_degs(graph_view_t<vertex_t, edge_t, false, false> const& graph_view) const
            {
                return graph_view.compute_out_degrees(handle_);
            }

            vertex_t get_vertex_padding_value(void) const
            {
                return vertex_padding_value_;
            }

            weight_t get_weight_padding_value(void) const
            {
                return weight_padding_value_;
            }
            ...
            decltype(auto) get_handle(void) const
            {
                return handle_;
            }
 ...
 */
        template <typename vertex_t,
                  ...
                  typename random_engine_t = rrandom_gen_t<vertex_t, edge_t>,
                  typename seeding_policy_t
                  = original::clock_seeding_t<typename random_engine_t::seed_type>,
                  typename index_t = edge_t>
                ...
                = original::clock_seeding_t<typename random_engine_t::seed_type>{})
        {
            ...
            ROCGRAPH_EXPECTS(static_cast<index_t>(how_many_valid) == d_v_start.size(),
            ...
            original::device_vec_t<edge_t>   d_crt_out_degs(tmp_buff_sz,
                                                          stream); // crt vertex set out-degs
            original::device_vec_t<vertex_t> d_col_indx(tmp_buff_sz,
                                                        stream); // \in {0,..,out-deg(v)}
            ...
            //
            if(use_padding)
                rand_walker.init_padding(d_coalesced_v, d_coalesced_w);
            ...
            else
            {
                return std::make_tuple(std::move(d_coalesced_v),
                                       std::move(d_coalesced_w),
                                       original::device_vec_t<index_t>(
                                           0, stream), // purposely empty size array for the padded
        ...
        struct coo_convertor_t
        {
            ...
            {
                ROCGRAPH_EXPECTS(static_cast<index_t>(d_sizes.size()) == num_paths_,
                                 "Invalid size vector.");
                ...
                ROCGRAPH_EXPECTS(static_cast<index_t>(d_coalesced_v.size()) == total_sz_v,
                ...
                // resize to new_end:
                //
                d_sz_w_scan.resize(thrust::distance(d_sz_w_scan.begin(), new_end_it),
                                   handle_.get_stream());
            ...
            std::tuple<original::device_vec_t<int>, index_t, original::device_vec_t<index_t>>
                fill_stencil(original::device_const_vector_view<index_t>& d_sizes) const
            {
            ...
            std::tuple<original::device_vec_t<vertex_t>, original::device_vec_t<vertex_t>>
                gather_pairs(original::device_const_vector_view<vertex_t>& d_coalesced_v,
                             ...
                             index_t                                       total_sz_v) const
            {
                ...
                thrust::transform(
                    ...
                    thrust::make_zip_iterator(
                        thrust::make_tuple(d_src_v.begin(), d_dst_v.begin())), // start_zip
 ...
 */
                     ...
                     std::unique_ptr<sampling_params_t>                           sampling_strategy)
    {
        ...
        //
        detail::original::device_const_vector_view<vertex_t, index_t> d_v_start{ptr_d_start,
                                                                                num_paths};
        ...
        size_t req_mem_common    = sizeof(vertex_t) * coalesced_v_count
                                + sizeof(weight_t) * coalesced_e_count
                                + // coalesced_v + coalesced_w
                                (sizeof(vertex_t) + sizeof(index_t)) * num_paths; // start_v + sizes

        size_t req_mem_horizontal
            = req_mem_common + sizeof(real_t) * coalesced_e_count; // + rnd_buff
        size_t req_mem_vertical
            = req_mem_common
              + (sizeof(edge_t) + 2 * sizeof(vertex_t) + sizeof(weight_t) + sizeof(real_t))
        ...
        if(req_mem_horizontal > req_mem_vertical && req_mem_horizontal > free_mem_sp_bytes)
        {
            ...
            std::cerr << "WARNING: Due to GPU memory availability, slower vertical traversal will "
                         "be used.\n";
        }

        int selector_type{0};
        if(sampling_strategy)
            selector_type = static_cast<int>(sampling_strategy->sampling_type_);

        // node2vec is only possible for weight_t being a floating-point type:
        //
        if constexpr(!std::is_floating_point_v<weight_t>)
        {
            ROCGRAPH_EXPECTS(selector_type != static_cast<int>(sampling_strategy_t::NODE2VEC),
        ...
        if(use_vertical_strategy)
        {
            ...
            {
                ROCGRAPH_EXPECTS(edge_weight_view.has_value(),
                                 "biased selector requires edge weights.");
                auto out_weight_sums
            ...
            else if(selector_type == static_cast<int>(sampling_strategy_t::NODE2VEC))
            {
                ...
                ROCGRAPH_EXPECTS(p > roundoff, "node2vec p parameter is too small.");

                ROCGRAPH_EXPECTS(q > roundoff, "node2vec q parameter is too small.");
        ...
        { // horizontal traversal strategy
            if(selector_type == static_cast<int>(sampling_strategy_t::BIASED))
            {
                ...
                auto quad_tuple = detail::random_walks_impl(handle,
                                                            graph_view,
                                                            edge_weight_view,
                                                            d_v_start,
                                                            max_depth,
 ...
 */
    ...
    std::tuple<rmm::device_uvector<index_t>,
               rmm::device_uvector<index_t>,
               rmm::device_uvector<index_t>>
