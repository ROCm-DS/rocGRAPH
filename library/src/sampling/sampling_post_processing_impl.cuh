...
namespace rocgraph
{
    ...
    namespace
    {
        ...
        struct edge_order_t
        {
            thrust::optional<raft::device_span<size_t const>> edgelist_label_offsets{
                thrust::nullopt};
        ...
        struct is_first_triplet_in_run_t
        {
            ...
            __device__ bool operator()(size_t i) const
            {
                ...
                if(edgelist_label_offsets)
                {
                    auto prev_label = thrust::distance(
                        (*edgelist_label_offsets).begin() + 1,
                        thrust::upper_bound(thrust::seq,
                                            (*edgelist_label_offsets).begin() + 1,
                                            (*edgelist_label_offsets).end(),
        ...
        struct compute_label_index_t
        {
            ...
            {
                return static_cast<label_index_t>(
        ...
        struct optionally_compute_label_index_t
        {
            ...
            __device__ label_index_t operator()(size_t i) const
            {
                return edgelist_label_offsets
                           ? static_cast<label_index_t>(thrust::distance(
                                 (*edgelist_label_offsets).begin() + 1,
                                 thrust::upper_bound(thrust::seq,
                                                     (*edgelist_label_offsets).begin() + 1,
                                                     (*edgelist_label_offsets).end(),
        ...
        {
            ROCGRAPH_EXPECTS(edgelist_majors.size() == edgelist_minors.size(),
                             "Invalid input arguments: edgelist_srcs.size() and "
                             "edgelist_dsts.size() should coincide.");
            ROCGRAPH_EXPECTS(!edgelist_weights.has_value()
                                 || (edgelist_majors.size() == (*edgelist_weights).size()),
                             "Invalid input arguments: if edgelist_weights is valid, "
                             "(*edgelist_weights).size() and "
                             "edgelist_(srcs|dsts).size() should coincide.");
            ROCGRAPH_EXPECTS(!edgelist_edge_ids.has_value()
                                 || (edgelist_majors.size() == (*edgelist_edge_ids).size()),
                             "Invalid input arguments: if edgelist_edge_ids is valid, "
                             "(*edgelist_edge_ids).size() and "
                             "edgelist_(srcs|dsts).size() should coincide.");
            ROCGRAPH_EXPECTS(!edgelist_edge_types.has_value()
                                 || (edgelist_majors.size() == (*edgelist_edge_types).size()),
                             "Invalid input arguments: if edgelist_edge_types is valid, "
                             "(*edgelist_edge_types).size() and "
                             "edgelist_(srcs|dsts).size() should coincide.");
            ROCGRAPH_EXPECTS(
                !edgelist_hops.has_value() || (edgelist_majors.size() == (*edgelist_hops).size()),
                "Invalid input arguments: if edgelist_hops is valid, (*edgelist_hops).size() and "
                "edgelist_(srcs|dsts).size() should coincide.");

            ROCGRAPH_EXPECTS(!edgelist_label_offsets.has_value()
                                 || ((*edgelist_label_offsets).size() == num_labels + 1),
                             "Invalid input arguments: if edgelist_label_offsets is valid, "
                             "(*edgelist_label_offsets).size() "
                             "(size of the offset array) should be num_labels + 1.");

            if(edgelist_majors.size() > 0)
            {
                ROCGRAPH_EXPECTS(
                    (num_labels >= 1) && (num_labels <= std::numeric_limits<label_index_t>::max()),
                ...
                ROCGRAPH_EXPECTS((num_hops >= 1)
                                     && (num_hops <= std::numeric_limits<int32_t>::max()),
                                 "Invalid input arguments: num_hops should be a positive integer "
                                 "and the current "
                                 "implementation "
                                 "assumes that the number of hops is no larger than "
                                 "std::numeric_limits<int32_t>::max().");
                ROCGRAPH_EXPECTS((num_hops == 1) || edgelist_hops.has_value(),
                                 "Invalid input arguments: edgelist_hops.has_value() should be "
                                 "true if num_hops >= 2.");
            }
            else
            {
                ROCGRAPH_EXPECTS("num_labels == 0",
                                 "Invalid input arguments: num_labels should be 0 if the input "
                                 "edge list is empty.");
                ROCGRAPH_EXPECTS("num_hops == 0",
                                 "Invalid input arguments: num_hops should be 0 if the input edge "
                                 "list is empty.");
            }

            ROCGRAPH_EXPECTS(
                (!seed_vertices.has_value() && !seed_vertex_label_offsets.has_value())
            ...
            ROCGRAPH_EXPECTS(!seed_vertex_label_offsets.has_value()
                                 || ((*seed_vertex_label_offsets).size() == num_labels + 1),
                             "Invalid input arguments: if seed_vertex_label_offsets is valid, "
                             "(*seed_vertex_label_offsets).size() (size of the offset array) "
                             "should be num_labels + 1.");

            if(do_expensive_check)
            {
                if(edgelist_label_offsets)
                {
                    ROCGRAPH_EXPECTS(thrust::is_sorted(handle.get_thrust_policy(),
                                                       (*edgelist_label_offsets).begin(),
                                                       (*edgelist_label_offsets).end()),
                                     "Invalid input arguments: if edgelist_label_offsets is valid, "
                                     "*edgelist_label_offsets should be sorted.");
                    size_t front_element{};
                    raft::update_host(&front_element,
                                      (*edgelist_label_offsets).data(),
                                      size_t{1},
                                      handle.get_stream());
                    size_t back_element{};
                    raft::update_host(&back_element,
                                      ...
                                      handle.get_stream());
                    handle.sync_stream();
                    ROCGRAPH_EXPECTS(front_element == size_t{0},
                                     "Invalid input arguments: if edgelist_label_offsets is valid, "
                                     "the first element of "
                                     "*edgelist_label_offsets should be 0.");
                    ROCGRAPH_EXPECTS(
                        back_element == edgelist_majors.size(),
                        "Invalid input arguments: if edgelist_label_offsets is valid, the last "
                        "element of "
                ...
                if(seed_vertices)
                {
                    ...
                    {
                        rmm::device_uvector<vertex_t> this_label_seed_vertices(0,
                                                                               handle.get_stream());
                        {
                            ...
                            {
                                raft::update_host(&start_offset,
                                                  (*seed_vertex_label_offsets).data() + i,
                                                  1,
                                                  handle.get_stream());
                                raft::update_host(&end_offset,
                                                  (*seed_vertex_label_offsets).data() + (i + 1),
                                                  1,
                                                  handle.get_stream());
                                handle.sync_stream();
                            }
                            this_label_seed_vertices.resize(end_offset - start_offset,
                                                            handle.get_stream());
                        ...
                        rmm::device_uvector<vertex_t> this_label_zero_hop_majors(
                            0, handle.get_stream());
                        {
                            size_t start_offset{0};
                            ...
                            {
                                raft::update_host(&start_offset,
                                                  (*edgelist_label_offsets).data() + i,
                                                  1,
                                                  handle.get_stream());
                                raft::update_host(&end_offset,
                                                  (*edgelist_label_offsets).data() + (i + 1),
                                                  1,
                                                  handle.get_stream());
                                handle.sync_stream();
                            }
                            this_label_zero_hop_majors.resize(end_offset - start_offset,
                                                              handle.get_stream());
                            if(edgelist_hops)
                            {
                ...
                std::optional<raft::device_span<size_t const>>   edgelist_label_offsets)
        {
            ...
            if(edgelist_label_offsets)
            {
                ...
                if(edgelist_hops)
                {
                    rmm::device_uvector<size_t> tmp_indices(edgelist_vertices.size(),
                                                            handle.get_stream());
                    ...
                    for(size_t i = 0; i < num_chunks; ++i)
                    {
                        thrust::sort(
                            ...
                            [edgelist_label_offsets = raft::device_span<size_t const>(
                             ...
                             edgelist_vertices,
                             edgelist_hops = *edgelist_hops] __device__(size_t l_idx,
                                                                        size_t r_idx) {
                    ...
                    tmp_hops
                        = rmm::device_uvector<int32_t>(tmp_indices.size(), handle.get_stream());
                    ...
                    auto triplet_first = thrust::make_transform_iterator(
                        ...
                        [edgelist_label_offsets = *edgelist_label_offsets,
                         ...
                         edgelist_hops = *edgelist_hops] __device__(size_t i)
                            -> thrust::tuple<label_index_t, vertex_t, int32_t> {
                            ...
                            return thrust::make_tuple(
                                label_idx, edgelist_vertices[i], edgelist_hops[i]);
                        });

                    thrust::copy(handle.get_thrust_policy(),
                                 triplet_first,
                                 triplet_first + tmp_indices.size(),
                                 thrust::make_zip_iterator(tmp_label_indices.begin(),
                                                           tmp_vertices.begin(),
                                                           (*tmp_hops).begin()));
                }
                else
                {
                    ...
                    auto input_pair_first = thrust::make_transform_iterator(
                        ...
                        [edgelist_label_offsets = *edgelist_label_offsets,
                         edgelist_vertices      = raft::device_span<vertex_t const>(
                             segment_sorted_vertices.data(),
                             segment_sorted_vertices.size())] __device__(size_t i)
                    ...
                    auto output_pair_first = thrust::make_zip_iterator(tmp_label_indices.begin(),
                                                                       tmp_vertices.begin());
                ...
                if(seed_vertices)
                {
                    ...
                    rmm::device_uvector<label_index_t> unique_seed_vertex_label_indices(
                    ...
                    unique_seed_vertex_label_indices.resize(
                        thrust::distance(
                            ...
                            thrust::unique_copy(handle.get_thrust_policy(),
                                                ...
                                                output_pair_first)),
                        handle.get_stream());
                    unique_seed_vertices.resize(unique_seed_vertex_label_indices.size(),
                                                handle.get_stream());
                    ...
                    if(edgelist_hops)
                    {
                        ...
                        thrust::for_each(handle.get_thrust_policy(),
                                         ...
                                         key_pair_from_seed_vertex_first
                                             + unique_seed_vertex_label_indices.size(),
                                         [triplet_from_edgelist_first,
                                          triplet_from_edgelist_last
                                          = triplet_from_edgelist_first
                                            + tmp_label_indices.size()] __device__(auto pair) {
                                             auto it = thrust::lower_bound(
                                                 thrust::seq,
                                                 triplet_from_edgelist_first,
                                                 triplet_from_edgelist_last,
                                                 thrust::make_tuple(thrust::get<0>(pair),
                                                                    thrust::get<1>(pair),
                                                                    int32_t{0}));
                                             ...
                                             {
                                                 // update min. hop to 0
                                                 if(thrust::get<2>(*it) != int32_t{0})
                                                 {
                                                     thrust::get<2>(*it) = int32_t{0};
                                                 }
                                             }
                                         });
                        ...
                        if(unique_seed_vertex_label_indices.size() > 0)
                        {
                            ...
                            rmm::device_uvector<vertex_t> merged_vertices(
                                merged_label_indices.size(), handle.get_stream());
                            rmm::device_uvector<int32_t> merged_hops(merged_label_indices.size(),
                                                                     handle.get_stream());
                            auto triplet_from_seed_vertex_first = thrust::make_zip_iterator(
                                unique_seed_vertex_label_indices.begin(),
                    ...
                    {
                        auto pair_from_edgelist_first = thrust::make_zip_iterator(
                            tmp_label_indices.begin(), tmp_vertices.begin());
            ...
            {
                rmm::device_uvector<vertex_t> tmp_vertices(edgelist_vertices.size(),
                                                           handle.get_stream());
                thrust::copy(handle.get_thrust_policy(),
                ...
                if(edgelist_hops)
                {
                    tmp_hops = rmm::device_uvector<int32_t>((*edgelist_hops).size(),
                                                            handle.get_stream());
                    thrust::copy(handle.get_thrust_policy(),
                                 (*edgelist_hops).begin(),
                                 (*edgelist_hops).end(),
                    ...
                    tmp_vertices.resize(thrust::distance(tmp_vertices.begin(),
                ...
                {
                    thrust::sort(
                        handle.get_thrust_policy(), tmp_vertices.begin(), tmp_vertices.end());
                    tmp_vertices.resize(thrust::distance(tmp_vertices.begin(),
                                                         thrust::unique(handle.get_thrust_policy(),
                                                                        tmp_vertices.begin(),
                                                                        tmp_vertices.end())),
                ...
                if(seed_vertices)
                {
                    ...
                    if(edgelist_hops)
                    {
                        ...
                        {
                            rmm::device_uvector<vertex_t> merged_vertices(
                                tmp_vertices.size() + unique_seed_vertices.size(),
                                handle.get_stream());
                            rmm::device_uvector<int32_t> merged_hops(merged_vertices.size(),
                                                                     handle.get_stream());
                            auto pair_from_seed_vertex_first = thrust::make_zip_iterator(
                                unique_seed_vertices.begin(),
                                thrust::make_constant_iterator(int32_t{0}));
                            thrust::merge(handle.get_thrust_policy(),
                                          ...
                                          thrust::make_zip_iterator(merged_vertices.begin(),
                                                                    merged_hops.begin()));
                            tmp_vertices = std::move(merged_vertices);
                            tmp_hops     = std::move(merged_hops);
                        }
                ...
                std::optional<raft::device_span<size_t const>>   edgelist_label_offsets)
        {
            ...
            if(edgelist_label_offsets)
            {
                ...
                rmm::device_uvector<label_index_t> renumber_map_label_indices(0,
                                                                              handle.get_stream());
                ...
                auto num_chunks
                    = (edgelist_majors.size() + (approx_edges_to_sort_per_iteration - 1))
                      / approx_edges_to_sort_per_iteration;
                auto chunk_size
                    = (num_chunks > 0) ? ((num_labels + (num_chunks - 1)) / num_chunks) : 0;

                size_t copy_offset{0};
                for(size_t i = 0; i < num_chunks; ++i)
                {
                    auto major_start_offset = (*unique_label_major_pair_label_offsets)
                                                  .element(chunk_size * i, handle.get_stream());
                    auto major_end_offset = (*unique_label_major_pair_label_offsets)
                                                .element(std::min(chunk_size * (i + 1), num_labels),
                                                         handle.get_stream());
                    auto minor_start_offset = (*unique_label_minor_pair_label_offsets)
                                                  .element(chunk_size * i, handle.get_stream());
                    auto minor_end_offset = (*unique_label_minor_pair_label_offsets)
                                                .element(std::min(chunk_size * (i + 1), num_labels),
                                                         handle.get_stream());

                    rmm::device_uvector<label_index_t> merged_label_indices(
                        (major_end_offset - major_start_offset)
                            + (minor_end_offset - minor_start_offset),
                        handle.get_stream());
                    rmm::device_uvector<vertex_t> merged_vertices(merged_label_indices.size(),
                                                                  handle.get_stream());
                    rmm::device_uvector<int8_t>   merged_flags(merged_label_indices.size(),
                                                             handle.get_stream());
                    ...
                    if(edgelist_hops)
                    {
                        rmm::device_uvector<int32_t> merged_hops(merged_label_indices.size(),
                                                                 handle.get_stream());
                        auto                         major_quad_first = thrust::make_zip_iterator(
                            (*unique_label_major_pair_label_indices).begin(),
                            unique_label_major_pair_vertices.begin(),
                            (*unique_label_major_pair_hops).begin(),
                            thrust::make_constant_iterator(int8_t{0}));
                        auto minor_quad_first = thrust::make_zip_iterator(
                            (*unique_label_minor_pair_label_indices).begin(),
                        ...
                        auto unique_key_first = thrust::make_zip_iterator(
                            merged_label_indices.begin(), merged_vertices.begin());
                        merged_label_indices.resize(
                            thrust::distance(unique_key_first,
                                             thrust::get<0>(thrust::unique_by_key(
                                                 handle.get_thrust_policy(),
                                                 unique_key_first,
                                                 unique_key_first + merged_label_indices.size(),
                                                 thrust::make_zip_iterator(merged_hops.begin(),
                                                                           merged_flags.begin())))),
                            handle.get_stream());
                    ...
                    {
                        auto major_triplet_first = thrust::make_zip_iterator(
                            (*unique_label_major_pair_label_indices).begin(),
                            unique_label_major_pair_vertices.begin(),
                            thrust::make_constant_iterator(int8_t{0}));
                        auto minor_triplet_first = thrust::make_zip_iterator(
                            (*unique_label_minor_pair_label_indices).begin(),
                            unique_label_minor_pair_vertices.begin(),
                            thrust::make_constant_iterator(int8_t{1}));
                        thrust::merge(handle.get_thrust_policy(),
                        ...
                        auto sort_key_first = thrust::make_zip_iterator(
                            merged_label_indices.begin(), merged_flags.begin());
                        thrust::sort_by_key(handle.get_thrust_policy(),
                                            sort_key_first,
                                            sort_key_first + merged_label_indices.size(),
                                            merged_vertices.begin());
                ...
                return std::make_tuple(std::move(renumber_map),
                                       std::move(renumber_map_label_indices));
            }
            else
            {
                if(edgelist_hops)
                {
                    rmm::device_uvector<vertex_t> merged_vertices(
                        unique_label_major_pair_vertices.size()
                            + unique_label_minor_pair_vertices.size(),
                        handle.get_stream());
                    rmm::device_uvector<int32_t> merged_hops(merged_vertices.size(),
                                                             handle.get_stream());
                    rmm::device_uvector<int8_t>  merged_flags(merged_vertices.size(),
                                                             handle.get_stream());
                ...
                {
                    rmm::device_uvector<vertex_t> output_vertices(
                        unique_label_minor_pair_vertices.size(), handle.get_stream());
                    auto output_last
                        = thrust::set_difference(handle.get_thrust_policy(),
                                                 unique_label_minor_pair_vertices.begin(),
                    ...
                    renumber_map.resize(
                        renumber_map.size()
                            + thrust::distance(output_vertices.begin(), output_last),
                        handle.get_stream());
                    thrust::copy(handle.get_thrust_policy(),
                                 output_vertices.begin(),
                                 output_last,
                ...
                bool                                            do_expensive_check)
        {
            ...
            auto [renumber_map, renumber_map_label_indices]
                = compute_renumber_map<vertex_t, label_index_t>(
                    handle,
                    raft::device_span<vertex_t const>(edgelist_majors.data(),
                                                      edgelist_majors.size()),
                    raft::device_span<vertex_t const>(edgelist_minors.data(),
            ...
            if(edgelist_label_offsets)
            {
                auto num_unique_labels = thrust::count_if(
                    ...
                    detail::is_first_in_run_t<label_index_t const*>{
                        (*renumber_map_label_indices).data()});
                rmm::device_uvector<label_index_t> unique_label_indices(num_unique_labels,
                                                                        handle.get_stream());
                ...
                renumber_map_label_offsets
                    = rmm::device_uvector<size_t>(num_labels + 1, handle.get_stream());
            ...
            if(edgelist_label_offsets)
            {
                rmm::device_uvector<vertex_t> new_vertices(renumber_map.size(),
                                                           handle.get_stream());
                thrust::tabulate(handle.get_thrust_policy(),
                                 ...
                                 [label_indices = raft::device_span<label_index_t const>(
                                      (*renumber_map_label_indices).data(),
                                      (*renumber_map_label_indices).size()),
                                  renumber_map_label_offsets = raft::device_span<size_t const>(
                                     ...
                                     auto label_start_offset
                                         = renumber_map_label_offsets[label_index];
                ...
                for(size_t i = 0; i < num_chunks; ++i)
                {
                    ...
                    auto offset_first = thrust::make_transform_iterator(
                        (*renumber_map_label_offsets).data() + h_label_offsets[i],
                        detail::shift_left_t<size_t>{h_edge_offsets[i]});
                ...
                auto pair_first = thrust::make_zip_iterator(
                    edgelist_majors.begin(), thrust::make_counting_iterator(size_t{0}));
                thrust::transform(
                    handle.get_thrust_policy(),
                    pair_first,
                    ...
                    [edgelist_label_offsets = *edgelist_label_offsets,
                     ...
                     old_vertices = raft::device_span<vertex_t const>(
                         segment_sorted_renumber_map.data(), segment_sorted_renumber_map.size()),
                ...
                if(seed_vertices)
                {
                    pair_first = thrust::make_zip_iterator(
                        (*seed_vertices).begin(), thrust::make_counting_iterator(size_t{0}));
                    thrust::transform(
                        handle.get_thrust_policy(),
                        pair_first,
                        ...
                        [seed_vertex_label_offsets = *seed_vertex_label_offsets,
                         ...
                         = raft::device_span<size_t const>((*renumber_map_label_offsets).data(),
                                                           (*renumber_map_label_offsets).size()),
                         old_vertices
                         = raft::device_span<vertex_t const>(segment_sorted_renumber_map.data(),
            ...
            {
                kv_store_t<vertex_t, vertex_t, false> kv_store(
                    renumber_map.begin(),
                    renumber_map.end(),
                           ...
                           ValueIterator         value_first /* [INOUT] */)
        {
            ...
            auto tmp_buffer = allocate_dataframe_buffer<value_t>(
                thrust::distance(index_first, index_last), handle.get_stream());
            ...
            for(size_t i = 0; i < num_chunks; ++i)
            {
                ...
                if(edgelist_weights)
                {
                    permute_array(handle,
                                  indices.begin(),
                                  indices.end(),
                                  (*edgelist_weights).begin() + h_edge_offsets[i]);
                }
                ...
                if(edgelist_edge_ids)
                {
                    permute_array(handle,
                                  indices.begin(),
                                  indices.end(),
                                  (*edgelist_edge_ids).begin() + h_edge_offsets[i]);
                }
                ...
                if(edgelist_hops)
                {
                    permute_array(handle,
                                  indices.begin(),
                                  indices.end(),
                                  (*edgelist_hops).begin() + h_edge_offsets[i]);
                }
            }
            ...
            bool                                              do_expensive_check)
    {
        ...
        ROCGRAPH_EXPECTS(!doubly_compress || !compress_per_hop,
                         "Invalid input arguments: compress_per_hop should be false if "
                         "doubly_compress is true.");
        ROCGRAPH_EXPECTS(!compress_per_hop || edgelist_hops,
        ...
        if(renumbered_seed_vertices)
        {
            if(seed_vertex_label_offsets)
            {
                ...
                auto pair_first = thrust::make_zip_iterator(label_indices.begin(),
                                                            (*renumbered_seed_vertices).begin());
                thrust::sort(
        ...
        if(do_expensive_check)
        {
            ...
            {
                rmm::device_uvector<vertex_t> min_vertices(num_labels * num_hops,
                                                           handle.get_stream());
                rmm::device_uvector<vertex_t> max_vertices(min_vertices.size(),
                                                           handle.get_stream());
                ...
                auto input_key_first
                    = thrust::make_zip_iterator(label_index_first, (*edgelist_hops).begin());
                rmm::device_uvector<label_index_t> unique_key_label_indices(min_vertices.size(),
                                                                            handle.get_stream());
                rmm::device_uvector<int32_t>       unique_key_hops(min_vertices.size(),
                                                             handle.get_stream());
                auto output_key_first = thrust::make_zip_iterator(unique_key_label_indices.begin(),
                                                                  unique_key_hops.begin());

                auto output_it = thrust::reduce_by_key(
                    handle.get_thrust_policy(),
                    input_key_first,
                ...
                auto num_unique_keys = static_cast<size_t>(
                    thrust::distance(output_key_first, thrust::get<0>(output_it)));
                thrust::reduce_by_key(handle.get_thrust_policy(),
                                      input_key_first,
                                      input_key_first + edgelist_majors.size(),
                                      edgelist_majors.begin(),
                ...
                if(renumbered_seed_vertices)
                {
                    thrust::for_each(
                        ...
                        [seed_vertices
                         = raft::device_span<vertex_t const>((*renumbered_seed_vertices).data(),
                                                             (*renumbered_seed_vertices).size()),
                         seed_vertex_label_offsets
                         = detail::to_thrust_optional(seed_vertex_label_offsets),
                         num_hops,
                         min_vertices
                         = raft::device_span<vertex_t>(min_vertices.data(), min_vertices.size()),
                         max_vertices = raft::device_span<vertex_t>(
                            ...
                            if(label_start_offset < label_end_offset)
                            {
                                min_vertices[l_idx * num_hops]
                                    = hip::std::min(min_vertices[l_idx * num_hops],
                                                    seed_vertices[label_start_offset]);
                                max_vertices[l_idx * num_hops]
                                    = hip::std::max(max_vertices[l_idx * num_hops],
                                                    seed_vertices[label_end_offset - 1]);
                            }
                        });
                }
                ...
                if(num_unique_keys > 1)
                {
                    auto num_invalids = thrust::count_if(
                        ...
                        [output_key_first,
                         min_vertices
                         = raft::device_span<vertex_t const>(min_vertices.data(), num_unique_keys),
                         max_vertices = raft::device_span<vertex_t const>(
                             max_vertices.data(), num_unique_keys)] __device__(size_t i) {
                    ...
                    ROCGRAPH_EXPECTS(
                        ...
                        "edgelist_hops.has_value() is true, majors should be non-decreasing within "
                        "each label "
                        "after renumbering and sorting by (hop, major, minor). Also, majors in hop "
                        "N should not "
                        "appear in any of the previous hops. This condition is satisfied if majors "
                        "in hop N + 1 "
                        "does not have any vertices from the previous hops excluding the minors "
                        "from hop N.");
                }
            }
        }
        ...
        auto compressed_label_indices
            = edgelist_label_offsets ? std::make_optional<rmm::device_uvector<label_index_t>>(
        ...
        if(edgelist_label_offsets)
        {
            ...
            if(edgelist_hops)
            {
                ...
                auto output_key_first
                    = thrust::make_zip_iterator((*compressed_label_indices).begin(),
            ...
            {
                auto input_key_first
                    = thrust::make_zip_iterator(label_index_first, edgelist_majors.begin());
                auto output_key_first = thrust::make_zip_iterator(
                    (*compressed_label_indices).begin(), compressed_nzd_vertices.begin());
                thrust::reduce_by_key(handle.get_thrust_policy(),
                                      input_key_first,
                                      input_key_first + edgelist_majors.size(),
                                      thrust::make_constant_iterator(size_t{1}),
        ...
        else
        {
            if(edgelist_hops)
            {
                ...
                auto output_key_first = thrust::make_zip_iterator((*compressed_hops).begin(),
                                                                  compressed_nzd_vertices.begin());
                thrust::reduce_by_key(handle.get_thrust_policy(),
                                      input_key_first,
                                      input_key_first + edgelist_majors.size(),
        ...
        std::optional<rmm::device_uvector<size_t>> compressed_offset_label_hop_offsets{
            std::nullopt};
        if(doubly_compress)
        {
            if(edgelist_label_offsets || edgelist_hops)
            {
                ...
                if(edgelist_label_offsets)
                {
                    ...
                    {
                        auto pair_first = thrust::make_zip_iterator(
                            (*compressed_label_indices).begin(), (*compressed_hops).begin());
                        auto value_pair_first = thrust::make_transform_iterator(
                            thrust::make_counting_iterator(size_t{0}),
                            [num_hops] __device__(
                                size_t i) -> thrust::tuple<label_index_t, int32_t> {
        ...
        { // !doubly_compress
            rmm::device_uvector<vertex_t> major_vertex_counts(num_labels * num_hops,
                                                              handle.get_stream());
            thrust::tabulate(
                handle.get_thrust_policy(),
                ...
                [edgelist_label_offsets = detail::to_thrust_optional(edgelist_label_offsets),
                 edgelist_hops          = edgelist_hops
                                           ...
                                           (*edgelist_hops).data(), (*edgelist_hops).size())
                                              : thrust::nullopt,
                 edgelist_majors        = raft::device_span<vertex_t const>(edgelist_majors.data(),
                                                                     edgelist_majors.size()),
                 seed_vertices
                 = renumbered_seed_vertices
                 ...
                 compress_per_hop] __device__(size_t i) {
                        ...
                        if(num_hops > 1)
                        {
                            ...
                            auto upper_it
                                = thrust::upper_bound(thrust::seq,
                                                      ...
                                                      (*edgelist_hops).begin() + end_offset,
                                                      h);
                            start_offset = static_cast<size_t>(
                                thrust::distance((*edgelist_hops).begin(), lower_it));
                            end_offset = static_cast<size_t>(
                                thrust::distance((*edgelist_hops).begin(), upper_it));
                        }
                        if(compress_per_hop)
                        {
                    ...
                    if(seed_vertices && (!compress_per_hop || (i % num_hops == 0)))
                    {
                        ...
                        if(label_start_offset < label_end_offset)
                        {
                            num_vertices_from_seed_vertices
                                = (*seed_vertices)[label_end_offset - 1] + 1;
                        }
                    }

                    return hip::std::max(num_vertices_from_edgelist,
                                         num_vertices_from_seed_vertices);
                });
            ...
            if(compress_per_hop)
            {
                minor_vertex_counts = rmm::device_uvector<vertex_t>(major_vertex_counts.size(),
                                                                    handle.get_stream());
                thrust::fill(handle.get_thrust_policy(),
                ...
                {
                    auto pair_first = thrust::make_zip_iterator(
                        (*compressed_hops).begin(), thrust::make_counting_iterator(size_t{0}));
                    thrust::for_each(
                        handle.get_thrust_policy(),
                        pair_first,
            ...
            rmm::device_uvector<size_t> offset_array_offsets(num_labels * num_hops + 1,
                                                             handle.get_stream());
            offset_array_offsets.set_element_to_zero_async(num_labels * num_hops,
                                                           handle.get_stream());
            thrust::tabulate(
                handle.get_thrust_policy(),
                offset_array_offsets.begin(),
                offset_array_offsets.begin() + (num_labels * num_hops),
                [major_vertex_counts = raft::device_span<vertex_t const>(
                     major_vertex_counts.data(), major_vertex_counts.size()),
                 minor_vertex_counts
                 = minor_vertex_counts
                       ? thrust::make_optional<raft::device_span<vertex_t const>>(
                             (*minor_vertex_counts).data(), (*minor_vertex_counts).size())
                    ...
                    if(num_hops > 1)
                    {
                        if(compress_per_hop)
                        {
                            for(size_t j = (i - (i % num_hops)); j < i; ++j)
                            {
                                ...
                                vertex_count
                                    = hip::std::max(vertex_count, (*minor_vertex_counts)[j]);
                            }
                        }
                        else
                        {
                            if(i % num_hops != 0)
                            {
                                vertex_count -= major_vertex_counts[i - 1];
                            }
                        }
                    }
                    return vertex_count;
                ...
                if(edgelist_hops)
                {
                    ...
                    thrust::for_each(
                        ...
                        [compressed_nzd_vertices = raft::device_span<vertex_t const>(
                         ...
                         offset_array_offsets = raft::device_span<size_t const>(
                             offset_array_offsets.data(), offset_array_offsets.size()),
                         compressed_offsets = raft::device_span<size_t>(compressed_offsets.data(),
                                                                        compressed_offsets.size()),
                         tmp_compressed_offsets = raft::device_span<size_t>(
                             tmp_compressed_offsets.data(), tmp_compressed_offsets.size()),
                         compress_per_hop,
                         num_hops] __device__(auto triplet) {
                            auto   nzd_v_idx    = thrust::get<2>(triplet);
                            ...
                            tmp_compressed_offsets
                                [offset_array_offsets[l_idx * num_hops
                                                      + (compress_per_hop ? h : int32_t{0})]
                                 + compressed_nzd_vertices[nzd_v_idx]]
                    ...
                    thrust::for_each(
                        ...
                        [compressed_nzd_vertices = raft::device_span<vertex_t const>(
                         ...
                         compressed_offsets = raft::device_span<size_t>(compressed_offsets.data(),
                                                                        compressed_offsets.size()),
                         tmp_compressed_offsets = raft::device_span<size_t>(
                             tmp_compressed_offsets.data(),
                             tmp_compressed_offsets.size())] __device__(auto pair) {
                            auto   nzd_v_idx    = thrust::get<1>(pair);
                            size_t start_offset = compressed_offsets[nzd_v_idx];
                            size_t end_offset   = compressed_offsets[nzd_v_idx + 1];
                    ...
                    thrust::for_each(
                        ...
                        [compressed_nzd_vertices = raft::device_span<vertex_t const>(
                         ...
                         compress_per_hop] __device__(auto pair) {
                            ...
                            auto   h            = thrust::get<0>(pair);
                            tmp_compressed_offsets
                                [offset_array_offsets[compress_per_hop ? h : int32_t{0}]
                                 + compressed_nzd_vertices[nzd_v_idx]]
                                = end_offset - start_offset;
                        });
                }
                    ...
                    thrust::for_each(
                        ...
                        [compressed_nzd_vertices = raft::device_span<vertex_t const>(
                         ...
                         tmp_compressed_offsets = raft::device_span<size_t>(
                            ...
                            size_t start_offset = compressed_offsets[nzd_v_idx];
                            size_t end_offset   = compressed_offsets[nzd_v_idx + 1];
                            tmp_compressed_offsets[compressed_nzd_vertices[nzd_v_idx]]
                                = end_offset - start_offset;
                        });
                }
            }
