...
namespace rocgraph
{
    ...
    namespace
    {
                ...
                std::optional<raft::device_span<size_t const>>        label_offsets)
        {
            ...
            if(label_indices)
            {
                ...
                if(hops)
                {
                    ...
                    thrust::sort(handle.get_thrust_policy(),
                                 triplet_first,
                                 triplet_first + tmp_label_indices.size());
                    auto key_first   = thrust::make_zip_iterator(tmp_label_indices.begin(),
                                                               tmp_vertices.begin());
                ...
                {
                    rmm::device_uvector<vertex_t> segment_sorted_vertices(vertices.size(),
                                                                          handle.get_stream());
                    ...
                    for(size_t i = 0; i < num_chunks; ++i)
                    {
                        ...
                        auto offset_first = thrust::make_transform_iterator(
                            (*label_offsets).data() + h_label_offsets[i],
                    ...
                    auto pair_first  = thrust::make_zip_iterator(tmp_label_indices.begin(),
                                                                segment_sorted_vertices.begin());
                    auto num_uniques = static_cast<size_t>(
                        thrust::distance(pair_first,
                                         thrust::unique(handle.get_thrust_policy(),
                                                        pair_first,
                                                        pair_first + tmp_label_indices.size())));
                    tmp_label_indices.resize(num_uniques, handle.get_stream());
            ...
            else
            {
                ...
                else
                {
                    ...
                    return std::make_tuple(
                        std::nullopt, std::move(tmp_vertices), std::nullopt, std::nullopt);
                }
            }
        }
                                 ...
                                 std::optional<raft::device_span<size_t const>>  label_offsets)
        {
            ...
            if(label_offsets)
            {
                edgelist_label_indices = detail::expand_sparse_offsets(
            ...
            auto [unique_label_src_pair_label_indices,
                ...
                = compute_min_hop_for_unique_label_vertex_pairs(
                    ...
                    edgelist_label_indices
                        ? std::make_optional<raft::device_span<label_index_t const>>(
            ...
            if(label_offsets)
            {
                ...
                renumber_map.reserve((*unique_label_src_pair_label_indices).size()
                                         + (*unique_label_dst_pair_label_indices).size(),
                                     handle.get_stream());
                ...
                for(size_t i = 0; i < num_chunks; ++i)
                {
                    auto src_start_offset = (*unique_label_src_pair_label_offsets)
                                                .element(chunk_size * i, handle.get_stream());
                    auto src_end_offset = (*unique_label_src_pair_label_offsets)
                                              .element(std::min(chunk_size * (i + 1), num_labels),
                                                       handle.get_stream());
                    auto dst_start_offset = (*unique_label_dst_pair_label_offsets)
                                                .element(chunk_size * i, handle.get_stream());
                    auto dst_end_offset = (*unique_label_dst_pair_label_offsets)
                                              .element(std::min(chunk_size * (i + 1), num_labels),
                                                       handle.get_stream());
                    ...
                    if(edgelist_hops)
                    {
                        rmm::device_uvector<int32_t> merged_hops(merged_label_indices.size(),
                                                                 handle.get_stream());
                        auto                         src_quad_first = thrust::make_zip_iterator(
                        ...
                        auto dst_quad_first = thrust::make_zip_iterator(
                            (*unique_label_dst_pair_label_indices).begin(),
                    ...
                    {
                        auto src_triplet_first = thrust::make_zip_iterator(
                            (*unique_label_src_pair_label_indices).begin(),
                            unique_label_src_pair_vertices.begin(),
                            thrust::make_constant_iterator(int8_t{0}));
                        auto dst_triplet_first = thrust::make_zip_iterator(
                            (*unique_label_dst_pair_label_indices).begin(),
                            unique_label_dst_pair_vertices.begin(),
                            thrust::make_constant_iterator(int8_t{1}));
                        thrust::merge(handle.get_thrust_policy(),
            ...
            else
            {
                if(edgelist_hops)
                {
                    rmm::device_uvector<vertex_t> merged_vertices(
                        unique_label_src_pair_vertices.size()
                            + unique_label_dst_pair_vertices.size(),
                        handle.get_stream());
                    ...
                    auto                         src_triplet_first
                        = thrust::make_zip_iterator(unique_label_src_pair_vertices.begin(),
                                                    (*unique_label_src_pair_hops).begin(),
                                                    thrust::make_constant_iterator(int8_t{0}));
                    auto dst_triplet_first
                        = thrust::make_zip_iterator(unique_label_dst_pair_vertices.begin(),
                                                    (*unique_label_dst_pair_hops).begin(),
                                                    thrust::make_constant_iterator(int8_t{1}));
                    thrust::merge(handle.get_thrust_policy(),
                                  src_triplet_first,
                ...
                else
                {
                    rmm::device_uvector<vertex_t> output_vertices(
                        unique_label_dst_pair_vertices.size(), handle.get_stream());
                    auto output_last
            ...
            bool                                                       do_expensive_check)
    {
        ...
        ROCGRAPH_EXPECTS(edgelist_srcs.size() == edgelist_dsts.size(),
                         "Invalid input arguments: edgelist_srcs.size() and edgelist_dsts.size() "
                         "should coincide.");
        ROCGRAPH_EXPECTS(
            !edgelist_hops.has_value() || (edgelist_srcs.size() == (*edgelist_hops).size()),
            "Invalid input arguments: if edgelist_hops is valid, (*edgelist_hops).size() and "
            "edgelist_srcs.size() should coincide.");
        ROCGRAPH_EXPECTS(
            !label_offsets.has_value()
        ...
        if(do_expensive_check)
        {
            if(label_offsets)
            {
                ...
                raft::update_host(&back_element,
                                  std::get<1>(*label_offsets).data()
                                      + (std::get<1>(*label_offsets).size() - 1),
                                  size_t{1},
                                  handle.get_stream());
                handle.get_stream();
                ROCGRAPH_EXPECTS(
        ...
        auto [renumber_map, renumber_map_label_indices]
            = compute_renumber_map<vertex_t, label_index_t>(
                ...
                edgelist_hops,
                label_offsets ? std::make_optional<raft::device_span<size_t const>>(
                                    std::get<1>(*label_offsets))
                              : std::nullopt);
        ...
        if(label_offsets)
        {
            ...
            renumber_map_label_offsets = rmm::device_uvector<size_t>(
                std::get<0>(*label_offsets).size() + 1, handle.get_stream());
        ...
        if(label_offsets)
        {
            ...
            for(size_t i = 0; i < num_chunks; ++i)
            {
                ...
                {
                    d_tmp_storage
                        = rmm::device_uvector<std::byte>(tmp_storage_bytes, handle.get_stream());
                }
        ...
        else
        {
            ...
            kv_store_view.find(edgelist_srcs.begin(),
                               edgelist_srcs.end(),
                               edgelist_srcs.begin(),
                               handle.get_stream());
            kv_store_view.find(edgelist_dsts.begin(),
                               edgelist_dsts.end(),
                               edgelist_dsts.begin(),
                               handle.get_stream());
        }
