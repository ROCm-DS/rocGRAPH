...
namespace rocgraph
{
    namespace detail
    {
                ...
                bool                           do_expensive_check)
        {
            ...
            ROCGRAPH_FAIL("uniform_neighbor_sample_impl needs unimplemented ROCGRAPH_OPS.");
#else
            ROCGRAPH_EXPECTS(fan_out.size() > 0,
                             "Invalid input argument: number of levels must be non-zero.");
            ROCGRAPH_EXPECTS(fan_out.size()
                                 <= static_cast<size_t>(std::numeric_limits<int32_t>::max()),
                             "Invalid input argument: number of levels should not overflow "
                             "int32_t"); // as we use int32_t
            // to store hops

            if constexpr(!multi_gpu)
            {
                ROCGRAPH_EXPECTS(!label_to_output_comm_rank,
                                 "cannot specify output GPU mapping in SG implementation");
            }

            ROCGRAPH_EXPECTS(!label_to_output_comm_rank || this_frontier_vertex_labels,
                             "cannot specify output GPU mapping without also specifying "
            ...
            if(do_expensive_check)
            {
                ...
                {
                    ROCGRAPH_EXPECTS(rocgraph::detail::is_sorted(
                                         handle, std::get<0>(*label_to_output_comm_rank)),
            ...
            auto level_result_edge_id_vectors
                = edge_id_view ? std::make_optional(std::vector<rmm::device_uvector<edge_t>>{})
                               : std::nullopt;
            ...
            if(level_result_weight_vectors)
            {
                (*level_result_weight_vectors).reserve(fan_out.size());
            }
            if(level_result_edge_id_vectors)
            {
                (*level_result_edge_id_vectors).reserve(fan_out.size());
            }
            if(level_result_edge_type_vectors)
            {
                (*level_result_edge_type_vectors).reserve(fan_out.size());
            }
            if(level_result_label_vectors)
            {
                (*level_result_label_vectors).reserve(fan_out.size());
            }
            ...
            for(auto&& k_level : fan_out)
            {
                ...
                if(weights)
                {
                    (*level_result_weight_vectors).push_back(std::move(*weights));
                }
                if(edge_ids)
                {
                    (*level_result_edge_id_vectors).push_back(std::move(*edge_ids));
                }
                if(edge_types)
                {
                    (*level_result_edge_type_vectors).push_back(std::move(*edge_types));
                }
                if(labels)
                {
                    (*level_result_label_vectors).push_back(std::move(*labels));
                }
                ...
                if(hop < fan_out.size())
                {
                    ...
                    this_frontier_vertices = raft::device_span<vertex_t const>(
                        frontier_vertices.data(), frontier_vertices.size());
            ...
            auto result_weights = level_result_weight_vectors
                                      ? std::make_optional(rmm::device_uvector<weight_t>(
                                            result_size, handle.get_stream()))
                                      : std::nullopt;
            if(result_weights)
            {
                output_offset = 0;
            ...
            auto result_edge_ids = level_result_edge_id_vectors
            ...
            auto result_edge_types = level_result_edge_type_vectors
            ...
            auto result_labels = level_result_label_vectors
