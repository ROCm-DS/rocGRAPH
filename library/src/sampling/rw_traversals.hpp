...
namespace rocgraph
{
    namespace detail
    {
        namespace original
        {
            ...
            {
                device_const_vector_view(value_t const* d_buffer, index_t size)
                    : d_buffer_(d_buffer)
                    , size_(size)
                {
                }
                ...
                value_t const* begin(void) const
                {
                    return d_buffer_;
                }

                value_t const* end() const
                {
                    return d_buffer_ + size_;
                }
            ...
            struct clock_seeding_t
            {
                ...
                seed_t operator()(void)
                {
                    return static_cast<seed_t>(std::time(nullptr));
                }
            };
            ...
            struct uniform_selector_t
            {
                struct sampler_t
                {
                    ...
                    __device__ thrust::optional<thrust::tuple<vertex_t, weight_t>>
                               operator()(vertex_t src_v,
                                   real_t   rnd_val,
                                   vertex_t = 0 /* not used*/,
                                   edge_t   = 0 /* not used*/,
                                   bool     = false /* not used*/) const
                    {
                        ...
                        if(crt_out_deg == 0)
                            return thrust::nullopt; // src_v is a sink
                        ...
                        auto weight_value
                            = (values_ == nullptr
                                   ? weight_t{1}
                                   : values_[start_row
                                             + col_indx]); // account for un-weighted graphs
                ...
                uniform_selector_t(
                    ...
                    , sampler_{graph_view.local_edge_partition_view().offsets().data(),
                               ...
                               edge_weight_view ? (*edge_weight_view).value_firsts()[0]
                                                : static_cast<weight_t*>(nullptr),
                               d_cache_out_degs_.data()}
                {
                }

                device_vec_t<edge_t> const& get_cached_out_degs(void) const
                {
                    return d_cache_out_degs_;
                }

                sampler_t const& get_strategy(void) const
                {
                    return sampler_;
                }
            ...
            private:
                device_vec_t<edge_t>
                    d_cache_out_degs_; // selector-specific: selector must own this resource
                sampler_t
                    sampler_; // which is why the sampling must be separated into a separate object:
            ...
            struct biased_selector_t
            {
                struct sampler_t
                {
                                   ...
                                   bool     = false /* not used*/) const
                    {
                        ...
                        if(col_indx_begin == col_indx_end)
                            return thrust::nullopt; // src_v is a sink
                        ...
                        auto p = thrust::make_tuple(col_indices_[prev_col_indx],
                                                    values_[prev_col_indx]);
                        return thrust::optional<decltype(p)>{p};
            ...
            struct node2vec_selector_t
            {
                struct sampler_t
                {
                    sampler_t(edge_t const*   ro,
                        ...
                        , coalesced_alpha_{
                              (max_degree > 0) && (num_paths > 0) && (ptr_alpha != nullptr)
                                  ? thrust::optional<
                                        thrust::
                                            tuple<vertex_t, edge_t, weight_t*>>{thrust::make_tuple(
                    ...
                    //
                    __device__ weight_t get_alpha(vertex_t prev_v,
                                                  vertex_t src_v,
                                                  vertex_t next_v) const
                    {
                        if(next_v == prev_v)
                        {
                            return 1.0 / p_;
                        ...
                        else
                        {
                            ...
                            auto found_next_from_prev
                                = thrust::binary_search(thrust::seq,
                                                        col_indices_ + prev_indx_begin,
                                                        col_indices_ + prev_indx_end,
                                                        next_v);
                                   ...
                                   bool     start_path) const
                    {
                        ...
                        if(offset_indx_begin == offset_indx_end)
                            return thrust::nullopt; // src_v is a sink

                        // for 1st vertex in path just use biased random selection:
                        //
                        if(start_path)
                        { // `src_v` is starting vertex in path
                            for(; offset_indx < offset_indx_end; ++offset_indx)
                            {
                                weight_t crt_weight
                                    = (values_ == nullptr ? weight_t{1} : values_[offset_indx]);
                            ...
                            auto p = thrust::make_tuple(
                                col_indices_[prev_offset_indx],
                        ...
                        if(coalesced_alpha_.has_value())
                        {
                            ...
                            for(vertex_t nghbr_indx = 0; offset_indx < offset_indx_end;
                                ++offset_indx, ++nghbr_indx)
                            {
                                auto crt_alpha
                                    = get_alpha(prev_v, src_v, col_indices_[offset_indx]);
                                weight_t crt_weight
                                    = (values_ == nullptr ? weight_t{1} : values_[offset_indx]);
                                ...
                                //
                                ptr_d_scaled_weights[start_alpha_offset + nghbr_indx]
                                    = scaled_weight;
                        ...
                        { // uncached solution, with much lower memory footprint but not as efficient
                    ...
                    decltype(auto) get_alpha_buffer(void) const
                    {
                        return coalesced_alpha_;
                    }
                ...
                device_vec_t<weight_t> const& get_alpha_cache(void) const
                {
                    return d_coalesced_alpha_;
                }
            ...
            struct vertical_traversal_t
            {
                ...
                template <typename vertex_t,
                          ...
                          typename seed_t>
                void operator()(
                    graph_view_t<vertex_t, edge_t, false, false> const&
                        graph_view, // graph being traversed
                    std::optional<edge_property_view_t<edge_t, weight_t const*>> edge_weight_view,
                    random_walker_t const&
                        rand_walker, // random walker object for which traversal is driven
                    ...
                    const
                {
                    ...
                    for(decltype(max_depth_) step_indx = 1; step_indx < max_depth_; ++step_indx)
                    {
                        ...
                        //
                        if(rand_walker.all_paths_stopped(d_crt_out_degs))
                            break;
                    }
                }

                size_t get_random_buff_sz(void) const
                {
                    return num_paths_;
                }
            ...
            struct horizontal_traversal_t
            {
                ...
                void operator()(
                    ...
                    device_vec_t<vertex_t>&
                        d_col_indx) // ignored: crt col indices to be used for retrieving next
                        ...
                        [max_depth       = max_depth_,
                         ...
                         sampler] __device__(auto path_index) {
                            ...
                            for(index_t step_indx = 1; step_indx < max_depth; ++step_indx)
                            {
                                ...
                                auto opt_tpl_vn_wn = sampler(
                                    src_vertex, real_rnd_indx, prev_v, path_index, start_path);
                ...
                size_t get_random_buff_sz(void) const
                {
                    return num_paths_ * (max_depth_ - 1);
                }
                size_t get_tmp_buff_sz(void) const
                {
                    return 0;
