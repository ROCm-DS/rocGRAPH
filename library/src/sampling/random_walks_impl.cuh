...
namespace rocgraph
{
    namespace detail
    {
        ...
        inline uint64_t get_current_time_nanoseconds()
        {
            ...
            return static_cast<uint64_t>(
                std::chrono::duration_cast<std::chrono::nanoseconds>(cur.time_since_epoch())
                    .count());
        }
        ...
        struct sample_edges_op_t
        {
            ...
            __device__ std::enable_if_t<std::is_same_v<W, void>, vertex_t>
                       operator()(vertex_t,
                           vertex_t dst,
                           thrust::nullopt_t,
                           thrust::nullopt_t,
            ...
            template <typename W = weight_t>
            __device__ std::enable_if_t<!std::is_same_v<W, void>, thrust::tuple<vertex_t, W>>
        ...
        struct uniform_selector
        {
            ...
            uniform_selector(uint64_t seed)
                : rng_state_(seed)
            {
            }
                        ...
                        current_vertices)
            {
                ...
                rocgraph::vertex_frontier_t<vertex_t, tag_t, GraphViewType::is_multi_gpu, false>
                    vertex_frontier(handle, 1);
                ...
                if(edge_weight_view)
                {
                    auto [sample_offsets, sample_e_op_results]
                        = rocgraph::per_v_random_select_transform_outgoing_e(
                            handle,
                            graph_view,
                            ...
                            *edge_weight_view,
                            ...
                            std::make_optional(thrust::make_tuple(
                                rocgraph::invalid_vertex_id<vertex_t>::value, weight_t{0.0})));
                ...
                else
                {
                    auto [sample_offsets, sample_e_op_results]
                        = rocgraph::per_v_random_select_transform_outgoing_e(
                            ...
                            true,
                            std::make_optional(
                                vertex_t{rocgraph::invalid_vertex_id<vertex_t>::value}));
        ...
        struct biased_selector
        {
                        ...
                        current_vertices)
            {
                ...
                ROCGRAPH_FAIL("biased sampling not implemented");
            }
        };
        ...
        struct node2vec_selector
        {
                        ...
                        current_vertices)
            {
                ...
                ROCGRAPH_FAIL("node2vec not implemented");
            }
        };
                ...
                random_selector_t                                            random_selector)
        {
            ...
            if(result_weights)
                detail::scalar_fill(
                    handle, result_weights->data(), result_weights->size(), weight_t{0});

            rmm::device_uvector<vertex_t> current_vertices(start_vertices.size(),
                                                           handle.get_stream());
            rmm::device_uvector<size_t>   current_position(start_vertices.size(),
                                                         handle.get_stream());
            ...
            raft::copy(current_vertices.data(),
                       start_vertices.data(),
                       start_vertices.size(),
                       handle.get_stream());
            ...
            for(size_t level = 0; level < max_length; ++level)
            {
                if constexpr(multi_gpu)
                {
                    ...
                    std::forward_as_tuple(std::tie(current_vertices, current_gpu, current_position),
                                          std::ignore)
                        = rocgraph::groupby_gpu_id_and_shuffle_values(
                            handle.get_comms(),
                            ...
                            [key_func = rocgraph::detail::compute_gpu_id_from_int_vertex_t<
                                 vertex_t>{{vertex_partition_range_lasts.begin(),
                ...
                std::tie(current_vertices, new_weights) = random_selector.follow_random_edge(
                    handle, graph_view, edge_weight_view, current_vertices);
                ...
                ROCGRAPH_EXPECTS(current_vertices.size()
                ...
                if constexpr(multi_gpu)
                {
                    if(result_weights)
                    {
                        ...
                        auto compacted_length = thrust::distance(
                            ...
                            thrust::remove_if(
                                ...
                                [] __device__(auto dst) {
                                    return (dst == rocgraph::invalid_vertex_id<vertex_t>::value);
                                }));
                        ...
                        std::forward_as_tuple(
                            std::tie(current_vertices, *new_weights, current_gpu, current_position),
                            std::ignore)
                            = rocgraph::groupby_gpu_id_and_shuffle_values(
                                handle.get_comms(),
                                current_iter,
                    ...
                    else
                    {
                        ...
                        std::forward_as_tuple(
                            std::tie(current_vertices, current_gpu, current_position), std::ignore)
                            = rocgraph::groupby_gpu_id_and_shuffle_values(
                                handle.get_comms(),
                                current_iter,
                                current_iter + current_vertices.size(),
                            ...
                            uint64_t                                      seed)
    {
        ...
        return detail::random_walk_impl(
            ...
            start_vertices,
            max_length,
            detail::biased_selector<weight_t>{
                (seed == 0 ? detail::get_current_time_nanoseconds() : seed)});
    }
