...
namespace rocgraph
{
    namespace detail
    {
        ...
        struct shuffle_to_output_comm_rank_t
        {
            ...
            __device__ int32_t operator()(key_t key) const
            {
                auto pos = thrust::lower_bound(
                    thrust::seq, output_label_.begin(), output_label_.end(), key);
                    ...
                    label_to_output_comm_rank)
        {
            ...
            {
                sort_sampled_tuples(
                    handle, majors, minors, weights, edge_ids, edge_types, hops, labels);

                if(label_to_output_comm_rank)
                {
                    ROCGRAPH_EXPECTS(
                        labels, "labels must be specified in order to shuffle sampling results");
                    ...
                    auto element_size     = sizeof(vertex_t) * 2
                                        + (weights ? sizeof(weight_t) : size_t{0})
                    ...
                    if(weights)
                    {
                        if(edge_ids)
                        {
                            if(edge_types)
                            {
                                if(hops)
                                {
                                    auto d_tx_value_counts = rocgraph::groupby_and_count(
                                        labels->begin(),
                                        labels->end(),
                                        thrust::make_zip_iterator(majors.begin(),
                                                                  minors.begin(),
                                                                  ...
                                                                  edge_types->begin(),
                                                                  hops->begin()),
                                        shuffle_to_output_comm_rank_t<label_t>{
                                            std::get<0>(*label_to_output_comm_rank),
                                    ...
                                    std::forward_as_tuple(std::tie(majors,
                                                                   minors,
                                                                   weights,
                                                                   edge_ids,
                                                                   edge_types,
                                ...
                                else
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(
                                            majors, minors, weights, edge_ids, edge_types, labels),
                                        std::ignore)
                                        = shuffle_values(
                                            comm,
                                            thrust::make_zip_iterator(majors.begin(),
                            ...
                            else
                            {
                                if(hops)
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(majors, minors, weights, edge_ids, hops, labels),
                                        std::ignore)
                                        = shuffle_values(
                                            comm,
                                            thrust::make_zip_iterator(majors.begin(),
                        ...
                        else
                        {
                            if(edge_types)
                            {
                                if(hops)
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(majors, minors, weights, edge_types, hops, labels),
                                        std::ignore)
                                        = shuffle_values(
                                            comm,
                                            thrust::make_zip_iterator(majors.begin(),
                            ...
                            else
                            {
                                if(hops)
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(majors, minors, weights, hops, labels),
                                        std::ignore)
                                        = shuffle_values(comm,
                                                         thrust::make_zip_iterator(majors.begin(),
                                                                                   minors.begin(),
                                ...
                                else
                                {
                                    ...
                                    std::forward_as_tuple(std::tie(majors, minors, weights, labels),
                                                          std::ignore)
                                        = shuffle_values(comm,
                                                         thrust::make_zip_iterator(majors.begin(),
                                                                                   minors.begin(),
                        ...
                        if(edge_ids)
                        {
                            if(edge_types)
                            {
                                if(hops)
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(
                                            majors, minors, edge_ids, edge_types, hops, labels),
                                        std::ignore)
                                        = shuffle_values(
                                            comm,
                                            thrust::make_zip_iterator(majors.begin(),
                            ...
                            else
                            {
                                if(hops)
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(majors, minors, edge_ids, hops, labels),
                                        std::ignore)
                                        = shuffle_values(
                                            comm,
                                            thrust::make_zip_iterator(majors.begin(),
                                ...
                                else
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(majors, minors, edge_ids, labels), std::ignore)
                                        = shuffle_values(
                                            comm,
                                            thrust::make_zip_iterator(majors.begin(),
                                                                      minors.begin(),
                        ...
                        else
                        {
                            if(edge_types)
                            {
                                if(hops)
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(majors, minors, edge_types, hops, labels),
                                        std::ignore)
                                        = shuffle_values(
                                            comm,
                                            thrust::make_zip_iterator(majors.begin(),
                                ...
                                else
                                {
                                    ...
                                    std::forward_as_tuple(
                                        std::tie(majors, minors, edge_types, labels), std::ignore)
                                        = shuffle_values(
                                            comm,
                                            thrust::make_zip_iterator(majors.begin(),
                                                                      minors.begin(),
                            ...
                            else
                            {
                                if(hops)
                                {
                                    auto d_tx_value_counts = rocgraph::groupby_and_count(
                                        ...
                                        labels->end(),
                                        thrust::make_zip_iterator(
                                            majors.begin(), minors.begin(), hops->begin()),
                                        shuffle_to_output_comm_rank_t<label_t>{
                                            std::get<0>(*label_to_output_comm_rank),
                                            std::get<1>(*label_to_output_comm_rank)},
                                        comm_size,
                                    ...
                                    std::forward_as_tuple(std::tie(majors, minors, hops, labels),
                                                          std::ignore)
                                        = shuffle_values(comm,
                                                         thrust::make_zip_iterator(majors.begin(),
                                                                                   minors.begin(),
                                ...
                                else
                                {
                                    ...
                                    std::forward_as_tuple(std::tie(majors, minors, labels),
                                                          std::ignore)
                                        = shuffle_values(comm,
                                                         thrust::make_zip_iterator(majors.begin(),
                                                                                   minors.begin(),
