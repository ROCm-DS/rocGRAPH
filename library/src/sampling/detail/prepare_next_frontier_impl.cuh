...
namespace rocgraph
{
    namespace detail
    {
        ...
        {
            vertex_partition_device_view_t<vertex_t, multi_gpu> d_vertex_partition(
                vertex_partition);
            ...
            auto                          frontier_vertex_labels = sampled_dst_vertex_labels
                                                                       ? std::make_optional<rmm::device_uvector<label_t>>(
                                                    frontier_size, handle.get_stream())
                                                                       : std::nullopt;
            ...
            if(vertex_used_as_source)
            {
                ...
                if(sampled_src_vertex_labels)
                {
                    ...
                    thrust::sort(
                        handle.get_thrust_policy(), begin_iter, begin_iter + new_verts_size);
                ...
                else
                {
                    ...
                    auto end_iter
                        = thrust::unique(handle.get_thrust_policy(), verts.begin(), verts.end());
                ...
                std::tie(frontier_vertices, frontier_vertex_labels)
                    = remove_visited_vertices_from_frontier(
                        ...
                        labels ? std::make_optional(raft::device_span<label_t const>{
