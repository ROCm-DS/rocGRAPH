...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        {
            rmm::device_uvector<edge_t> offsets((major_range_last - major_range_first) + 1,
                                                stream_view);
            ...
            {
                thrust::fill(
                    rmm::exec_policy(stream_view), offsets.begin(), offsets.end(), edge_t{0});
        ...
        std::tuple<rmm::device_uvector<edge_t>, rmm::device_uvector<vertex_t>>
            compress_hypersparse_offsets(rmm::device_uvector<edge_t>&& offsets,
                                         ...
                                         rmm::cuda_stream_view         stream_view)
        {
            ...
            rmm::device_uvector<vertex_t> dcs_nzd_vertices = rmm::device_uvector<vertex_t>(
            ...
            ROCGRAPH_EXPECTS(dcs_nzd_vertices.size()
                                 < static_cast<size_t>(std::numeric_limits<int32_t>::max()),
                             "remove_if will fail (https://github.com/NVIDIA/thrust/issues/1302), "
               ...
               < major_range_last - major_hypersparse_first)
            {
                ...
                offsets.resize((major_hypersparse_first - major_range_first)
                                   + dcs_nzd_vertices.size() + 1,
                               stream_view);
        ...
        std::tuple<rmm::device_uvector<edge_t>,
                   ...
                   decltype(allocate_dataframe_buffer<edge_value_t>(size_t{0},
                                                                    rmm::cuda_stream_view{})),
                   std::optional<rmm::device_uvector<vertex_t>>>
            sort_and_compress_edgelist(rmm::device_uvector<vertex_t>&&    edgelist_srcs,
                                       rmm::device_uvector<vertex_t>&&    edgelist_dsts,
                                       decltype(allocate_dataframe_buffer<edge_value_t>(
                                           0, rmm::cuda_stream_view{}))&& edgelist_values,
                                       vertex_t                           major_range_first,
                                       ...
                                       rmm::cuda_stream_view stream_view)
        {
            ...
            if(edgelist_minors.size() > mem_frugal_threshold)
            {
                ...
                auto pivot = major_range_first
                             + static_cast<vertex_t>(
                                 thrust::distance(offsets.begin(),
            ...
            {
                std::tie(offsets, dcs_nzd_vertices)
                    = compress_hypersparse_offsets(std::move(offsets),
                                                   major_range_first,
                                       ...
                                       rmm::cuda_stream_view stream_view)
        {
            ...
            {
                thrust::sort(
                    rmm::exec_policy(stream_view), edge_first, edge_first + edgelist_minors.size());
                offsets = compute_sparse_offsets<edge_t>(edgelist_majors.begin(),
                                                         edgelist_majors.end(),
                                                         major_range_first,
                                                         major_range_last,
            ...
            return std::make_tuple(
                std::move(offsets), std::move(indices), std::move(dcs_nzd_vertices));
        }
                                 ...
                                 EdgeValueIterator               edge_value_first /* [INOUT] */)
        {
            ...
            {
                max_chunk_size = std::max(
                    max_chunk_size, static_cast<size_t>(h_edge_offsets[i + 1] - h_edge_offsets[i]));
            }
            rmm::device_uvector<vertex_t>  segment_sorted_indices(max_chunk_size,
                                                                 handle.get_stream());
            ...
            if constexpr(std::is_arithmetic_v<edge_value_t>)
            {
                for(size_t i = 0; i < num_chunks; ++i)
                {
                    ...
                    auto   offset_first
                        = thrust::make_transform_iterator(offsets.data() + h_vertex_offsets[i],
            ...
            { // cub's segmented sort does not support thrust iterators (so we can't directly sort
                // edge values with thrust::zip_iterator)
                rmm::device_uvector<edge_t> input_edge_value_offsets(max_chunk_size,
                                                                     handle.get_stream());
        ...
        std::tuple<size_t, rmm::device_uvector<packed_bool_container_t>>
            mark_entries(raft::handle_t const& handle, size_t num_entries, comparison_t comparison)
        {
            rmm::device_uvector<packed_bool_container_t> marked_entries(
                rocgraph::packed_bool_size(num_entries), handle.get_stream());

            thrust::tabulate(handle.get_thrust_policy(),
                             marked_entries.begin(),
                             marked_entries.end(),
                             [comparison, num_entries] __device__(size_t idx) {
                                 auto   word        = rocgraph::packed_bool_empty_mask();
                                 size_t start_index = idx * rocgraph::packed_bools_per_word();
                                 size_t bits_in_this_word
                                     = (start_index + rocgraph::packed_bools_per_word()
                                        < num_entries)
                                 ...
                                 {
                                     if(comparison(start_index + bit))
                                         word |= rocgraph::packed_bool_mask(bit);
                                 }
        ...
        rmm::device_uvector<T>
            keep_flagged_elements(raft::handle_t const&                            handle,
                                  rmm::device_uvector<T>&&                         vector,
                                  raft::device_span<packed_bool_container_t const> keep_flags,
                                  size_t                                           keep_count)
        {
