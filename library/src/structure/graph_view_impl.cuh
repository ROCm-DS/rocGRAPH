...
namespace rocgraph
{
    ...
    namespace
    {
        ...
        rmm::device_uvector<edge_t> compute_major_degrees(
            ...
            std::optional<std::vector<raft::device_span<packed_bool_container_t const>>> const&
            ...
            std::vector<vertex_t> const& edge_partition_segment_offsets)
        {
            ...
            for(int i = 0; i < minor_comm_size; ++i)
            {
                ...
                max_num_local_degrees
                    = std::max(max_num_local_degrees, major_range_vertex_partition_size);
            ...
            for(int i = 0; i < minor_comm_size; ++i)
            {
                ...
                auto masks   = edge_partition_masks
                                   ? thrust::make_optional((*edge_partition_masks)[i])
                                   : thrust::nullopt;
                ...
                auto major_hypersparse_first
                    = use_dcs ? major_range_first
                                    + edge_partition_segment_offsets
                                        [segment_offset_size_per_partition * i
                ...
                if(use_dcs)
                {
                    ...
                    thrust::fill(execution_policy,
                                 local_degrees.begin()
                                     + (major_hypersparse_first - major_range_first),
                    ...
                    thrust::for_each(
                        ...
                        thrust::make_counting_iterator(
                            static_cast<vertex_t>(dcs_nzd_vertices.size())),
                        [offsets,
                         dcs_nzd_vertices,
                         masks,
                         major_range_first,
                         ...
                         local_degrees = local_degrees.data()] __device__(auto i) {
                            ...
                            if(masks)
                            {
                                local_degree = static_cast<edge_t>(detail::count_set_bits(
                                    (*masks).begin(), offsets[major_idx], local_degree));
                            }
                ...
                minor_comm.reduce(local_degrees.data(),
                                  i == minor_comm_rank ? degrees.data()
        ...
        rmm::device_uvector<edge_t> compute_major_degrees(
            ...
            std::optional<raft::device_span<packed_bool_container_t const>> masks,
            vertex_t                                                        number_of_vertices)
        {
            ...
            thrust::tabulate(handle.get_thrust_policy(),
                             ...
                             degrees.end(),
                             [offsets,
                              masks = masks ? thrust::make_optional(*masks)
                                            : thrust::nullopt] __device__(auto i) {
                                 auto local_degree = offsets[i + 1] - offsets[i];
                                 if(masks)
                                 {
                                     local_degree = static_cast<edge_t>(detail::count_set_bits(
                                         (*masks).begin(), offsets[i], local_degree));
                                 }
                                 return local_degree;
                             });
            return degrees;
        ...
        {
            rmm::device_uvector<edge_t> minor_degrees(
            ...
            edge_t*                                                   count)
        {
            ...
            using BlockReduce
                = hipcub::BlockReduce<edge_t, count_edge_partition_multi_edges_block_size>;
            ...
            if(segment_offsets)
            {
                ...
                if((*segment_offsets)[1] > 0)
                {
                    ...
                    THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                        (rocgraph::for_all_major_for_all_nbr_high_degree),
                ...
                if((*segment_offsets)[2] - (*segment_offsets)[1] > 0)
                {
                    ...
                    THROW_IF_HIPLAUNCHKERNELGGL_ERROR(
                        (rocgraph::for_all_major_for_all_nbr_mid_degree),
                        dim3(update_grid.num_blocks),
                        dim3(update_grid.block_size),
                        0,
                        handle.get_stream(),
                   ...
                   && (*(edge_partition.dcs_nzd_vertex_count()) > 0))
                {
                    ret += thrust::transform_reduce(
                        ...
                        [edge_partition,
                         major_start_offset = (*segment_offsets)[3]] __device__(auto idx) {
                            auto major_idx
                                = major_start_offset
                ...
                raft::device_span<vertex_t const>                                  edge_minors)
        {
            ...
            rmm::device_uvector<size_t> d_major_range_lasts(h_major_range_lasts.size(),
                                                            handle.get_stream());
            raft::update_device(d_major_range_lasts.data(),
                                h_major_range_lasts.data(),
                                h_major_range_lasts.size(),
                                handle.get_stream());
            rmm::device_uvector<size_t> d_lower_bounds(d_major_range_lasts.size(),
                                                       handle.get_stream());
            auto                        major_first        = edge_majors.begin();
            auto                        sorted_major_first = thrust::make_transform_iterator(
                edge_indices.begin(),
                rocgraph::detail::indirection_t<size_t, decltype(major_first)>{major_first});
              ...
              meta.local_sorted_unique_edge_dst_vertex_partition_offsets)
    {
        ...
        ROCGRAPH_EXPECTS(
            !use_dcs
                || ((*edge_partition_dcs_nzd_vertices).size() == edge_partition_offsets.size()),
        ...
        , segment_offsets_(meta.segment_offsets)
    {
        ...
        ROCGRAPH_EXPECTS(offsets.size() == static_cast<size_t>(meta.number_of_vertices + 1),
                         "Internal Error: offsets.size() returns an invalid value.");
        ROCGRAPH_EXPECTS(indices.size() == static_cast<size_t>(meta.number_of_edges),
        ...
        if(this->has_edge_mask())
        {
            ...
            for(size_t i = 0; i < value_firsts.size(); ++i)
            {
                ret += static_cast<edge_t>(
                    detail::count_set_bits(handle, value_firsts[i], edge_counts[i]));
            }
            ret = host_scalar_allreduce(
                handle.get_comms(), ret, raft::comms::op_t::SUM, handle.get_stream());
            return ret;
        }
        else
        {
            ...
            compute_number_of_edges(raft::handle_t const& handle) const
    {
        if(this->has_edge_mask())
        {
            ...
            return static_cast<edge_t>(
                detail::count_set_bits(handle, value_firsts[0], edge_counts[0]));
        }
        else
        {
            return this->number_of_edges_;
            ...
            compute_in_degrees(raft::handle_t const& handle) const
    {
        if(store_transposed)
        {
            std::optional<std::vector<raft::device_span<packed_bool_container_t const>>>
                edge_partition_masks{std::nullopt};
            if(this->has_edge_mask())
            {
                edge_partition_masks
                    = std::vector<raft::device_span<packed_bool_container_t const>>(
                        this->edge_partition_offsets_.size());
                auto value_firsts = (*(this->edge_mask_view())).value_firsts();
                auto edge_counts  = (*(this->edge_mask_view())).edge_counts();
                for(size_t i = 0; i < (*edge_partition_masks).size(); ++i)
                {
                    (*edge_partition_masks)[i] = raft::device_span<packed_bool_container_t const>(
                        value_firsts[i], edge_counts[i]);
                }
            }
            ...
            compute_in_degrees(raft::handle_t const& handle) const
    {
        if(store_transposed)
        {
            return compute_major_degrees(
                ...
                this->offsets_,
                this->has_edge_mask()
                    ? std::make_optional(raft::device_span<packed_bool_container_t const>(
            ...
            compute_max_in_degree(raft::handle_t const& handle) const
    {
        ...
        auto it
            = thrust::max_element(handle.get_thrust_policy(), in_degrees.begin(), in_degrees.end());
            ...
            compute_max_out_degree(raft::handle_t const& handle) const
    {
        ...
        auto it          = thrust::max_element(
            handle.get_thrust_policy(), out_degrees.begin(), out_degrees.end());
        rmm::device_scalar<edge_t> ret(edge_t{0}, handle.get_stream());
        device_allreduce(handle.get_comms(),
    ...
    rmm::device_uvector<bool>
        graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu, std::enable_if_t<multi_gpu>>::
            has_edge(raft::handle_t const&             handle,
                     ...
                     bool                              do_expensive_check)
    {
        ...
        if(do_expensive_check)
        {
            auto num_invalids = detail::count_invalid_vertex_pairs(
                handle, *this, edge_first, edge_first + edge_srcs.size());
        ...
        auto sorted_edge_first = thrust::make_transform_iterator(
            edge_indices.begin(),
            rocgraph::detail::indirection_t<size_t, decltype(edge_first)>{edge_first});
    ...
    rmm::device_uvector<edge_t>
        graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu, std::enable_if_t<multi_gpu>>::
            compute_multiplicity(raft::handle_t const&             handle,
                                 ...
                                 bool                              do_expensive_check)
    {
        ROCGRAPH_EXPECTS(this->is_multigraph(), "Use has_edge() instead for non-multigraphs.");
        ROCGRAPH_EXPECTS(
            edge_srcs.size() == edge_dsts.size(),
            "Invalid input arguments: edge_srcs.size() does not coincide with edge_dsts.size().");
                ...
                [edge_partition, edge_partition_e_mask] __device__(auto e) {
                    ...
                    if(major_idx)
                    {
                        ...
                        auto upper_it = thrust::upper_bound(
                            thrust::seq, indices, indices + local_degree, minor);
                        auto multiplicity
                            = static_cast<edge_t>(thrust::distance(lower_it, upper_it));
                        if(edge_partition_e_mask && (multiplicity > 0))
                        {
                            multiplicity = static_cast<edge_t>(detail::count_set_bits(
                                (*edge_partition_e_mask).value_first(),
                                static_cast<size_t>(local_edge_offset
                                                    + thrust::distance(indices, lower_it)),
                                 ...
                                 bool                              do_expensive_check)
    {
        ...
        thrust::transform(
            ...
            [edge_partition, edge_partition_e_mask] __device__(auto e) {
                ...
                auto upper_it
                    = thrust::upper_bound(thrust::seq, indices, indices + local_degree, minor);
