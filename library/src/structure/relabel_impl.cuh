...
namespace rocgraph
{
                 ...
                 bool                                         do_expensive_check)
    {
        if(multi_gpu)
        {
            ...
            thrust::copy(
                handle.get_thrust_policy(), labels, labels + num_labels, unique_old_labels.data());
            thrust::sort(
            ...
            rmm::device_uvector<vertex_t> new_labels_for_unique_old_labels(0, handle.get_stream());
            {
                ...
                rmm::device_uvector<vertex_t> rx_label_pair_new_labels(0, handle.get_stream());
                {
                    ...
                    auto pair_first = thrust::make_zip_iterator(thrust::make_tuple(
                        label_pair_old_labels.begin(), label_pair_new_labels.begin()));
...

                {
                    ...
                    {
                        auto device_view
                            = detail::kv_hipco_store_find_device_view_t(relabel_map_view);
                        thrust::transform(
                            ...
                            [device_view,
                             invalid_value
                             = invalid_vertex_id<vertex_t>::value] __device__(auto old_label) {
        ...
        {
            kv_store_t<vertex_t, vertex_t, false> relabel_map(std::get<0>(old_new_label_pairs),
                                                              std::get<0>(old_new_label_pairs)
                                                                  + num_label_pairs,
                                                              std::get<1>(old_new_label_pairs),
                                                              invalid_vertex_id<vertex_t>::value,
            ...
            if(skip_missing_labels)
            {
                ...
                thrust::transform(
                    ...
                    labels + num_labels,
                    labels,
                    [device_view, invalid_value = invalid_vertex_id<vertex_t>::value] __device__(
                        auto old_label) {
                        auto val = device_view.find(old_label);
                        return val != invalid_value ? val : old_label;
                    });
        ...
        {
            ROCGRAPH_EXPECTS(thrust::count(handle.get_thrust_policy(),
                                           labels,
                                           labels + num_labels,
                                           invalid_vertex_id<vertex_t>::value)
                                 == 0,
                             "Invalid input argument: labels include old label values missing in "
                             "old_new_label_pairs.");
        }
