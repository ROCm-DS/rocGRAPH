...
namespace rocgraph
{
    namespace detail
    {
        ...
        struct find_unused_id_t
        {
            ...
            {
                for(size_t i = worker_id; i < sorted_local_vertices.size() + size_t{1};
                    i += num_workers)
                {
                    auto start = (i == size_t{0}) ? std::numeric_limits<vertex_t>::lowest()
                                                  : sorted_local_vertices[i - size_t{1}];
                    if(start != std::numeric_limits<vertex_t>::max())
                    {
                        ++start;
                    }; // now inclusive
                    auto end = (i == sorted_local_vertices.size())
                                   ? std::numeric_limits<vertex_t>::max()
                                   : sorted_local_vertices[i]; // exclusive
                    for(vertex_t v = start; v < end; ++v)
                    {
                        if(gpu_id_op(v) == comm_rank)
                        {
                            return v;
                        }
                    }
        ...
        struct search_and_increment_degree_t
        {
            ...
            __device__ void operator()(thrust::tuple<vertex_t, edge_t> vertex_degree_pair) const
            {
                ...
                *(degrees + thrust::distance(sorted_vertices, it))
                    += thrust::get<1>(vertex_degree_pair);
            }
        };
            ...
            bool                              multi_gpu)
        {
            ...
            if(multi_gpu)
            {
                ...
                if((major_comm_size < comm_size) && (minor_comm_size < comm_size))
                { // if neither of the edge source/destination range covers
                    ...
                    assert(std::find(locally_used.begin(), locally_used.end(), false)
                           != locally_used.end());
                    ...
                    while(true)
                    { // the acutal loop count should be smaller than or comparable to comm_size
                        if(!locally_used[vertex_partition_id_op(v)])
                        {
                            ret = v;
                            break;
                        }
                        if(v == std::numeric_limits<vertex_t>::max())
                        {
                            break;
                        }
                        ++v;
                    ...
                    auto found
                        = static_cast<bool>(host_scalar_allreduce(comm,
                                                                  static_cast<int>(ret.has_value()),
                                                                  raft::comms::op_t::MIN,
                                                                  handle.get_stream()));
                    if(found)
                    {
            ...
            {
                raft::update_host(
                    &min, sorted_local_vertices.data(), size_t{1}, handle.get_stream());
            ...
            {
                min = host_scalar_allreduce(
                    handle.get_comms(), min, raft::comms::op_t::MIN, handle.get_stream());
                max = host_scalar_allreduce(
                    handle.get_comms(), max, raft::comms::op_t::MAX, handle.get_stream());
            }
            if(min > std::numeric_limits<vertex_t>::lowest())
            {
                return std::numeric_limits<vertex_t>::lowest();
            }
            if(max < std::numeric_limits<vertex_t>::max())
            {
            ...
            auto num_workers
                = std::min(static_cast<size_t>(handle.get_device_properties().multiProcessorCount)
                               * size_t{1024},
                           sorted_local_vertices.size() + size_t{1});
            ...
            if(multi_gpu && (handle.get_comms().get_size() > int{1}))
            {
                ...
                auto&      minor_comm
                    = handle.get_subcomm(rocgraph::partition_manager::minor_comm_name());
                auto const minor_comm_size = minor_comm.get_size();
                gpu_id_op                  = compute_gpu_id_from_ext_vertex_t<vertex_t>{
            ...
            auto unused_id = thrust::transform_reduce(
                ...
                find_unused_id_t<vertex_t>{sorted_local_vertices,
                                           ...
                                           std::numeric_limits<vertex_t>::max()},
                std::numeric_limits<
                    vertex_t>::max(), // already taken in the step 1.2, so this can't be a
                // valid answer
        ...
        std::tuple<rmm::device_uvector<vertex_t>, std::vector<vertex_t>, vertex_t>
                                 ...
                                 std::vector<edge_t> const& edgelist_edge_counts)
        {
            ...
            if(!local_vertices)
            {
                ...
                for(size_t i = 0; i < edgelist_majors.size(); ++i)
                {
                    ...
                    major_offset += static_cast<size_t>(
                        thrust::distance(sorted_unique_majors.begin() + major_offset,
            ...
            if(!local_vertices)
            {
                ...
                for(size_t i = 0; i < edgelist_minors.size(); ++i)
                {
                    ...
                    thrust::sort(handle.get_thrust_policy(),
                                 ...
                                 sorted_unique_minors.begin() + minor_offset
                                     + edgelist_edge_counts[i]);
                    minor_offset += static_cast<size_t>(
                        thrust::distance(sorted_unique_minors.begin() + minor_offset,
                                         thrust::unique(handle.get_thrust_policy(),
                                                        sorted_unique_minors.begin() + minor_offset,
                                                        sorted_unique_minors.begin() + minor_offset
                                                            + edgelist_edge_counts[i])));
                ...
                if(edgelist_minors.size() > 1)
                {
                    ...
                    sorted_unique_minors.resize(
                        thrust::distance(sorted_unique_minors.begin(),
                                         thrust::unique(handle.get_thrust_policy(),
                                                        sorted_unique_minors.begin(),
                                                        sorted_unique_minors.end())),
                        handle.get_stream());
            ...
            {
                sorted_local_vertices.resize(
                    sorted_unique_majors.size() + sorted_unique_minors.size(), handle.get_stream());
                ...
                {
                    sorted_local_vertices = rocgraph::detail::
            ...
            auto locally_unused_vertex_id = find_locally_unused_ext_vertex_id(
                ...
                multi_gpu);
            ROCGRAPH_EXPECTS(
                locally_unused_vertex_id.has_value(),
            ...
            if constexpr(multi_gpu)
            {
                ...
                auto edge_partition_major_range_sizes = host_scalar_allgather(
                    minor_comm, sorted_local_vertices.size(), handle.get_stream());

                if((minor_comm_size >= 2) && (handle.get_stream_pool_size() >= 2))
                {
                    auto vertex_edge_counts = host_scalar_allreduce(
                        comm,
                        thrust::make_tuple(static_cast<vertex_t>(sorted_local_vertices.size()),
                                           num_local_edges),
                        raft::comms::op_t::SUM,
                        handle.get_stream());
                    ...
                    auto avg_vertex_degree
                        = thrust::get<0>(vertex_edge_counts) > 0
                    ...
                    auto num_streams = static_cast<size_t>(
                        ...
                        / (static_cast<double>(sizeof(vertex_t) + sizeof(edge_t))
                           + (((avg_vertex_degree / minor_comm_size) / num_chunks)
                              * sizeof(vertex_t) * 2)
                    ...
                    if(num_streams >= 2)
                    {
                        ...
                        std::iota((*stream_pool_indices).begin(),
                                  (*stream_pool_indices).end(),
                                  size_t{0});
                        handle.sync_stream();
                    }
                }
                ...
                for(int i = 0; i < minor_comm_size; ++i)
                {
                    auto loop_stream = stream_pool_indices ? handle.get_stream_from_stream_pool(
                                                                 i % (*stream_pool_indices).size())
                                                           : handle.get_stream();
                    ...
                    rmm::device_uvector<edge_t> sorted_major_degrees(sorted_majors.size(),
                                                                     loop_stream);
                    ...
                    tmp_majors.reserve(
                        (static_cast<size_t>(edgelist_edge_counts[i]) + (num_chunks - 1))
                            / num_chunks,
                        loop_stream);
                    size_t offset{0};
                    for(size_t j = 0; j < num_chunks; ++j)
                    {
                        size_t this_chunk_size
                            = std::min(tmp_majors.capacity(),
                                       static_cast<size_t>(edgelist_edge_counts[i]) - offset);
                        ...
                        thrust::copy(rmm::exec_policy(loop_stream),
                                     ...
                                     tmp_majors.begin());
                        thrust::sort(
                            rmm::exec_policy(loop_stream), tmp_majors.begin(), tmp_majors.end());
                        auto num_unique_majors = thrust::count_if(
                        ...
                        auto kv_pair_first = thrust::make_zip_iterator(
                            thrust::make_tuple(tmp_keys.begin(), tmp_values.begin()));
                    ...
                    {
                        sorted_local_vertex_degrees = std::move(sorted_major_degrees);
                    }
                }
            ...
            else
            {
                ...
                sorted_local_vertex_degrees.resize(sorted_local_vertices.size(),
                                                   handle.get_stream());
                ...
                tmp_majors.reserve(static_cast<size_t>(edgelist_edge_counts[0] + (num_chunks - 1))
                                       / num_chunks,
                                   handle.get_stream());
                size_t offset{0};
                for(size_t i = 0; i < num_chunks; ++i)
                {
                    size_t this_chunk_size
                        = std::min(tmp_majors.capacity(),
                                   static_cast<size_t>(edgelist_edge_counts[0]) - offset);
...
#ifndef NDEBUG
#warning The following static asserts won't work since low_degree_threshold is set to the warp size and that's not known until run time.
#endif
            ...
            size_t low_degree_threshold = raft::host_warp_size(handle.get_device());
            ;
            ...
            auto num_segments_per_vertex_partition
                = detail::num_sparse_segments_per_vertex_partition
                  + (hypersparse_degree_threshold > 0 ? size_t{2}
                                                      : size_t{1}); // last is 0-degree segment
                ...
                edgelist_intra_partition_segment_offsets)
        {
            ...
            if constexpr(multi_gpu)
            {
                ...
                for(size_t i = 0; i < edgelist_majors.size(); ++i)
                {
                    auto edge_first = thrust::make_zip_iterator(
                        thrust::make_tuple(edgelist_majors[i], edgelist_minors[i]));
                    ROCGRAPH_EXPECTS(
                        thrust::count_if(
                            handle.get_thrust_policy(),
                            edge_first,
                            ...
                            [comm_size,
                             ...
                             minor_comm_rank,
                             i,
                             gpu_id_key_func = detail::compute_gpu_id_from_ext_edge_endpoints_t<
                                 vertex_t>{comm_size, major_comm_size, minor_comm_size},
                             local_edge_partition_id_key_func
                             = detail::compute_local_edge_partition_id_from_ext_edge_endpoints_t<
                                 vertex_t>{comm_size,
                                           major_comm_size,
                                           minor_comm_size}] __device__(auto edge) {
                                return (gpu_id_key_func(thrust::get<0>(edge), thrust::get<1>(edge))
                                        != comm_rank)
                            ...
                            ROCGRAPH_EXPECTS(
                                thrust::count_if(
                                    ...
                                    [major_comm_size,
                                     ...
                                     = detail::compute_vertex_partition_id_from_ext_vertex_t<
                                         vertex_t>{comm_size},
                                     minor_range_vertex_partition_id = detail::
                                         compute_local_edge_partition_minor_range_vertex_partition_id_t{major_comm_size,
                                                                                                        minor_comm_size,
                                                                                                        major_comm_rank,
                                                                                                        minor_comm_rank}(
                                             j)] __device__(auto minor) {
                                        return vertex_partition_id_key_func(minor)
                                               != minor_range_vertex_partition_id;
                                    })
                                    == 0,
                                "Invalid input argument: if "
                                "edgelist_intra_partition_segment_offsets.has_value() is "
                                "true, edgelist_majors & edgelist_minors should be properly "
                                "grouped "
                ...
                if(sorted_local_vertices)
                {
                    ROCGRAPH_EXPECTS(
                        thrust::count_if(handle.get_thrust_policy(),
                                         ...
                                         (*sorted_local_vertices).end(),
                                         [comm_rank,
                                          key_func = detail::compute_gpu_id_from_ext_vertex_t<
                                              vertex_t>{comm_size,
                                                        major_comm_size,
                        ...
                        ROCGRAPH_EXPECTS(
                            thrust::count_if(handle.get_thrust_policy(),
                                             ...
                                             check_edge_src_and_dst_t<vertex_t>{
                                                 ...
                                                 raft::device_span<vertex_t const>(
                                                     sorted_minors.data(),
                                                     static_cast<vertex_t>(sorted_minors.size()))})
                                == 0,
            ...
            else
            {
                ...
                {
                    auto edge_first = thrust::make_zip_iterator(
                        thrust::make_tuple(edgelist_majors[0], edgelist_minors[0]));
                    ROCGRAPH_EXPECTS(
                        thrust::count_if(
                            handle.get_thrust_policy(),
                            edge_first,
                ...
                ROCGRAPH_EXPECTS(edgelist_intra_partition_segment_offsets.has_value() == false,
                                 "Invalid input argument: "
                                 "edgelist_intra_partition_segment_offsets.has_value() should "
        ...
        template <typename vertex_t>
        std::vector<vertex_t>
            aggregate_segment_offsets(raft::handle_t const&        handle,
                                      std::vector<vertex_t> const& segment_offsets)
        {
            auto& minor_comm = handle.get_subcomm(rocgraph::partition_manager::minor_comm_name());
            auto const minor_comm_size = minor_comm.get_size();

            rmm::device_uvector<vertex_t> d_segment_offsets(segment_offsets.size(),
                                                            handle.get_stream());
            ...
            handle
                .sync_stream(); // this is necessary as h_aggregate_offsets can be used right after return.
                          ...
                          bool do_expensive_check)
    {
        ...
        if(edgelist_intra_partition_segment_offsets)
        {
            ...
            for(size_t i = 0; i < edgelist_majors.size(); ++i)
            {
                ...
                ROCGRAPH_EXPECTS(
                    ...
                    "Invalid input arguments: (*edgelist_intra_partition_segment_offsets)[] is not "
                    "sorted.");
                ROCGRAPH_EXPECTS(
        ...
        auto vertex_partition_ids = host_scalar_allgather(
            comm,
...

        {
            ...
            for(size_t i = 0; i < edgelist_majors.size(); ++i)
            {
                max_edge_partition_major_range_size
                    = std::max(max_edge_partition_major_range_size,
            ...
            for(size_t i = 0; i < edgelist_majors.size(); ++i)
            {
                ...
                kv_store_t<vertex_t, vertex_t, false> renumber_map(
                    ...
                    thrust::make_counting_iterator(
                        partition.local_edge_partition_major_range_first(i)),
        ...
        { // memory footprint dominated by the O(V/sqrt(P))
            ...
            for(int i = 0; i < major_comm_size; ++i)
            {
                ...
                max_segment_size = std::max(
                    max_segment_size,
                    partition.vertex_partition_range_size(minor_range_vertex_partition_id));
            }
                          ...
                          bool      do_expensive_check)
    {
        ...
        kv_store_t<vertex_t, vertex_t, false> renumber_map(
            ...
            handle.get_stream());
        auto renumber_map_view = renumber_map.view();
        renumber_map_view.find(edgelist_majors,
                               edgelist_majors + num_edgelist_edges,
                               edgelist_majors,
                               handle.get_stream());
        renumber_map_view.find(edgelist_minors,
