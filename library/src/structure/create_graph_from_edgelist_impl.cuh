...
namespace rocgraph
{
    ...
    namespace
    {
                                      ...
                                      bool                                 renumber)
        {
            if(vertices)
            {
                rmm::device_uvector<vertex_t> sorted_vertices((*vertices).size(),
                                                              handle.get_stream());
                ...
                ROCGRAPH_EXPECTS(
                    static_cast<size_t>(thrust::distance(sorted_vertices.begin(),
                ...
                if(!renumber)
                {
                    ROCGRAPH_EXPECTS(static_cast<size_t>(thrust::count_if(
                                         handle.get_thrust_policy(),
                                         sorted_vertices.begin(),
                                         sorted_vertices.end(),
                                         detail::check_out_of_range_t<vertex_t>{
                                             vertex_t{0}, std::numeric_limits<vertex_t>::max()}))
                                         == size_t{0},
                    ...
                    rmm::device_uvector<vertex_t> sequences(sorted_vertices.size(),
                                                            handle.get_stream());
                    thrust::sequence(handle.get_thrust_policy(),
                                     sequences.begin(),
                                     sequences.end(),
                                     vertex_t{0});
                    ROCGRAPH_EXPECTS(thrust::equal(handle.get_thrust_policy(),
                                                   sorted_vertices.begin(),
                                                   sorted_vertices.end(),
                                                   sequences.begin()),
                                     "Invalid input argument: vertex IDs should be consecutive "
                                     "integers starting "
            ...
            if constexpr(multi_gpu)
            {
                ...
                if(vertices)
                {
                    ...
                    ROCGRAPH_EXPECTS(
                        num_unique_vertices
                            < static_cast<size_t>(std::numeric_limits<vertex_t>::max()),
                    ...
                    thrust::count_if(
                        ...
                        [comm_rank,
                         gpu_id_key_func = detail::compute_gpu_id_from_ext_edge_endpoints_t<
                             vertex_t>{comm_size,
                                       major_comm_size,
                                       minor_comm_size}] __device__(auto e) {
                    ...
                    ROCGRAPH_EXPECTS(
                        thrust::count_if(
                            ...
                            check_edge_t<vertex_t>{sorted_majors.data(),
                                                   ...
                                                   sorted_minors.data(),
                                                   sorted_minors.data() + sorted_minors.size()})
                            == 0,
                             ...
                             raft::device_span<vertex_t const> edgelist_dsts)
        {
            ...
            if(org_srcs.size() != symmetrized_srcs.size())
            {
                return false;
            }
                ...
                bool                                              do_expensive_check)
        {
            ...
            if(do_expensive_check)
            {
                ...
                {
                    ROCGRAPH_EXPECTS((check_symmetric<vertex_t, store_transposed, multi_gpu>(
                                         handle,
                                         raft::device_span<vertex_t const>(edgelist_srcs.data(),
                                                                           edgelist_srcs.size()),
                                         raft::device_span<vertex_t const>(edgelist_dsts.data(),
                                                                           edgelist_dsts.size()))),
                                     "Invalid input arguments: graph_properties.is_symmetric is "
                                     "true but the input edge list is "
                                     "not symmetric.");
                }

                if(!graph_properties.is_multigraph)
                {
                    ROCGRAPH_EXPECTS(check_no_parallel_edge(
                                         handle,
                                         raft::device_span<vertex_t const>(edgelist_srcs.data(),
                                                                           edgelist_srcs.size()),
                                         raft::device_span<vertex_t const>(edgelist_dsts.data(),
                                                                           edgelist_dsts.size())),
                                     "Invalid input arguments: graph_properties.is_multigraph is "
                                     "false but the input edge list "
                                     "has parallel edges.");
                }
            }
            ...
            auto d_edge_counts = rocgraph::detail::groupby_and_count_edgelist_by_local_partition_id(
                handle,
                store_transposed ? edgelist_dsts : edgelist_srcs,
                store_transposed ? edgelist_srcs : edgelist_dsts,
                edgelist_weights,
            ...
            for(int i = 0; i < minor_comm_size; ++i)
            {
                edgelist_edge_counts[i]
                    = std::accumulate(h_edge_counts.begin() + major_comm_size * i,
            ...
            for(int i = 0; i < minor_comm_size; ++i)
            {
                rmm::device_uvector<vertex_t> tmp_srcs(edgelist_edge_counts[i],
                                                       handle.get_stream());
                thrust::copy(handle.get_thrust_policy(),
            ...
            for(int i = 0; i < minor_comm_size; ++i)
            {
                rmm::device_uvector<vertex_t> tmp_dsts(edgelist_edge_counts[i],
                                                       handle.get_stream());
                thrust::copy(handle.get_thrust_policy(),
            ...
            if(edgelist_weights)
            {
                ...
                for(int i = 0; i < minor_comm_size; ++i)
                {
                    rmm::device_uvector<weight_t> tmp_weights(edgelist_edge_counts[i],
                                                              handle.get_stream());
                    thrust::copy(handle.get_thrust_policy(),
            ...
            std::optional<std::vector<rmm::device_uvector<edge_id_t>>>
            ...
            if(edgelist_edge_ids)
            {
                ...
                for(int i = 0; i < minor_comm_size; ++i)
                {
                    rmm::device_uvector<edge_id_t> tmp_edge_ids(edgelist_edge_counts[i],
                                                                handle.get_stream());
                    thrust::copy(handle.get_thrust_policy(),
            ...
            if(edgelist_edge_types)
            {
                ...
                for(int i = 0; i < minor_comm_size; ++i)
                {
                    rmm::device_uvector<edge_type_t> tmp_edge_types(edgelist_edge_counts[i],
                                                                    handle.get_stream());
                    thrust::copy(handle.get_thrust_policy(),
            ...
            auto [renumber_map_labels, meta]
                = rocgraph::renumber_edgelist<vertex_t, edge_t, multi_gpu>(
                    handle,
            ...
            auto use_dcs = num_segments_per_vertex_partition
            ...
            std::optional<std::vector<rmm::device_uvector<weight_t>>> edge_partition_weights{
                std::nullopt};
            std::optional<std::vector<rmm::device_uvector<edge_id_t>>> edge_partition_edge_ids{
                std::nullopt};
            ...
            std::optional<std::vector<rmm::device_uvector<vertex_t>>>
                edge_partition_dcs_nzd_vertices{std::nullopt};
            ...
            for(size_t i = 0; i < edge_partition_edgelist_srcs.size(); ++i)
            {
                ...
                auto                                            major_hypersparse_first
                    = use_dcs ? std::make_optional<vertex_t>(
                                    ...
                                    + meta.edge_partition_segment_offsets
                                          [num_segments_per_vertex_partition * i
                ...
                if(edgelist_weights)
                {
                    if(edgelist_edge_ids)
                    {
                        if(edgelist_edge_types)
                        {
                            std::forward_as_tuple(offsets,
                                                  indices,
                                                  std::tie(weights, edge_ids, edge_types),
                                                  dcs_nzd_vertices)
                                = detail::sort_and_compress_edgelist<
                                    vertex_t,
                                    edge_t,
                        ...
                        else
                        {
                            std::forward_as_tuple(
                                offsets, indices, std::tie(weights, edge_ids), dcs_nzd_vertices)
                                = detail::sort_and_compress_edgelist<
                                    vertex_t,
                                    edge_t,
                    ...
                    else
                    {
                        ...
                        {
                            std::forward_as_tuple(
                                offsets, indices, std::tie(weights, edge_types), dcs_nzd_vertices)
                                = detail::sort_and_compress_edgelist<
                                    vertex_t,
                                    edge_t,
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            std::forward_as_tuple(offsets, indices, dcs_nzd_vertices) = detail::
                                sort_and_compress_edgelist<vertex_t, edge_t, store_transposed>(
                                    std::move(edge_partition_edgelist_srcs[i]),
                                    std::move(edge_partition_edgelist_dsts[i]),
                                    major_range_first,
                                    major_hypersparse_first,
                ...
                {
                    (*edge_partition_weights).push_back(std::move(*weights));
                }
                if(edge_partition_edge_ids)
                {
                    (*edge_partition_edge_ids).push_back(std::move(*edge_ids));
                }
            ...
            if(edge_partition_weights)
            {
                edge_weights
                    = edge_property_t<graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu>,
                                      weight_t>(std::move(*edge_partition_weights));
            }
            ...
            if(edge_partition_edge_ids)
            {
                edge_ids
                    = edge_property_t<graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu>,
                                      edge_id_t>(std::move(*edge_partition_edge_ids));
            }
            ...
            if(edge_partition_edge_types)
            {
                edge_types
                    = edge_property_t<graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu>,
                                      edge_type_t>(std::move(*edge_partition_edge_types));
            }
            ...
            return std::make_tuple(
                rocgraph::graph_t<vertex_t, edge_t, store_transposed, multi_gpu>(
                    ...
                    rocgraph::graph_meta_t<vertex_t, edge_t, multi_gpu>{
                ...
                bool                                              do_expensive_check)
        {
            ...
            else
            {
                ...
                {
                    num_vertices = 1
                                   + rocgraph::detail::compute_maximum_vertex_id(
            ...
            if(edgelist_weights)
            {
                if(edgelist_edge_ids)
                {
                    ...
                    {
                        std::forward_as_tuple(
                            offsets, indices, std::tie(weights, ids, types), std::ignore)
                            = detail::sort_and_compress_edgelist<
                                vertex_t,
                                edge_t,
                                thrust::tuple<weight_t, edge_id_t, edge_type_t>,
                                store_transposed>(std::move(edgelist_srcs),
                ...
                else
                {
                    ...
                    {
                        std::forward_as_tuple(
                            offsets, indices, std::tie(weights, types), std::ignore)
                            = detail::sort_and_compress_edgelist<
                                vertex_t,
                                edge_t,
                                thrust::tuple<weight_t, edge_type_t>,
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        std::forward_as_tuple(offsets, indices, std::ignore) = detail::
                            sort_and_compress_edgelist<vertex_t, edge_t, store_transposed>(
                                std::move(edgelist_srcs),
                                std::move(edgelist_dsts),
                                vertex_t{0},
            ...
            return std::make_tuple(
                rocgraph::graph_t<vertex_t, edge_t, store_transposed, multi_gpu>(
                    ...
                    rocgraph::graph_meta_t<vertex_t, edge_t, multi_gpu>{
                        num_vertices,
                        graph_properties,
                        renumber ? std::optional<std::vector<vertex_t>>{meta.segment_offsets}
                                 : std::nullopt}),
                std::move(edge_weights),
                std::move(edge_ids),
                std::move(edge_types),
