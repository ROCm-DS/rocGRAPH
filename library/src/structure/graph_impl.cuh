...
namespace rocgraph
{
    ...
    namespace
    {
        ...
        {
            packed_bool_container_t* bitmaps{nullptr};
            vertex_t                 minor_range_first{};

            __device__ void operator()(vertex_t minor) const
            {
                auto minor_offset = minor - minor_range_first;
                auto mask         = packed_bool_container_t{1}
                            << (minor_offset % (sizeof(packed_bool_container_t) * 8));
                atomicOr(bitmaps + (minor_offset / (sizeof(packed_bool_container_t) * 8)), mask);
            }
        };
        ...
        struct popc_t
        {
            __device__ vertex_t operator()(packed_bool_container_t bitmap) const
            {
                    ...
                    edge_partition_dcs_nzd_vertices)
        {
            ...
            std::optional<std::vector<rmm::device_uvector<vertex_t>>>
                local_sorted_unique_edge_majors{std::nullopt};
            std::optional<std::vector<rmm::device_uvector<vertex_t>>>
            ...
            std::optional<rmm::device_uvector<vertex_t>> local_sorted_unique_edge_minors{
                std::nullopt};
            std::optional<rmm::device_uvector<vertex_t>>
                local_sorted_unique_edge_minor_chunk_start_offsets{std::nullopt};
            std::optional<vertex_t> local_sorted_unique_edge_minor_chunk_size{std::nullopt};
            std::optional<std::vector<vertex_t>>
                local_sorted_unique_edge_minor_vertex_partition_offsets{std::nullopt};
...

            {
                ...
                rmm::device_uvector<packed_bool_container_t> minor_bitmaps(
                    (minor_range_size + (sizeof(packed_bool_container_t) * 8 - 1))
                        / (sizeof(packed_bool_container_t) * 8),
                    handle.get_stream());
                thrust::fill(handle.get_thrust_policy(),
                             minor_bitmaps.begin(),
                             minor_bitmaps.end(),
                             packed_bool_container_t{0});
                ...
                auto count_first
                    = thrust::make_transform_iterator(minor_bitmaps.begin(), popc_t<vertex_t>{});
                ...
                auto max_minor_properties_fill_ratio
                    = host_scalar_allreduce(comm,
                                            static_cast<double>(num_local_unique_edge_minors)
                                                / static_cast<double>(minor_range_size),
                                            raft::comms::op_t::MAX,
                                            handle.get_stream());
                ...
                {
                    auto const chunk_size = static_cast<size_t>(
                        std::min(1.0 / max_minor_properties_fill_ratio, 1024.0));
    ...
    // thrust::remove_if, and thrust::copy_if (https://github.com/NVIDIA/thrust/issues/1302)
                    ...
                    while(num_scanned < static_cast<size_t>(minor_range_size))
                    {
                        size_t this_scan_size
                            = std::min(size_t{1} << 30,
                                       static_cast<size_t>(minor_range_last
                                                           - (minor_range_first + num_scanned)));
                        num_copied += static_cast<size_t>(thrust::distance(
                            ...
                            thrust::copy_if(
                                ...
                                thrust::make_counting_iterator(minor_range_first + num_scanned
                                                               + this_scan_size),
                                unique_edge_minors.begin() + num_copied,
                                rocgraph::detail::check_bit_set_t<packed_bool_container_t const*,
                                                                  vertex_t>{minor_bitmaps.data(),
                                                                            minor_range_first})));
                    ...
                    thrust::copy_if(
                        ...
                        rocgraph::detail::check_bit_set_t<packed_bool_container_t const*, vertex_t>{
                            minor_bitmaps.data(), minor_range_first});
#endif

                    auto num_chunks = static_cast<size_t>(
                        (minor_range_size + (chunk_size - size_t{1})) / chunk_size);
                    rmm::device_uvector<vertex_t> unique_edge_minor_chunk_start_offsets(
                        num_chunks + size_t{1}, handle.get_stream());

                    auto chunk_start_vertex_first = thrust::make_transform_iterator(
                        thrust::make_counting_iterator(vertex_t{0}),
                        detail::multiply_and_add_t<vertex_t>{static_cast<vertex_t>(chunk_size),
                                                             minor_range_first});
                    ...
                    unique_edge_minor_chunk_start_offsets.set_element(
                        num_chunks,
                        static_cast<vertex_t>(unique_edge_minors.size()),
                        handle.get_stream());

                    std::vector<vertex_t> h_minor_range_vertex_partition_firsts(major_comm_size
                                                                                - 1);
                    for(int i = 1; i < major_comm_size; ++i)
                    {
                        auto minor_range_vertex_partition_id = detail::
                        ...
                        h_minor_range_vertex_partition_firsts[i - 1]
                            = meta.partition.vertex_partition_range_first(
                    ...
                    raft::update_device(d_minor_range_vertex_partition_firsts.data(),
                                        ...
                                        handle.get_stream());
                    rmm::device_uvector<vertex_t> d_key_offsets(
                        d_minor_range_vertex_partition_firsts.size(), handle.get_stream());
                    ...
                    raft::update_host(h_key_offsets.data() + 1,
                                      d_key_offsets.data(),
                                      d_key_offsets.size(),
                                      handle.get_stream());
                    ...
                    local_sorted_unique_edge_minor_vertex_partition_offsets
                        = std::move(h_key_offsets);
                }
            }
            ...
            for(size_t i = 0; i < edge_partition_offsets.size(); ++i)
            {
                num_local_unique_edge_major_counts[i] += thrust::count_if(
                    ...
                    thrust::make_counting_iterator(
                        static_cast<vertex_t>(edge_partition_offsets[i].size() - 1)),
                    has_nzd_t<vertex_t, edge_t>{edge_partition_offsets[i].data(), vertex_t{0}});
            }
            auto num_local_unique_edge_majors
                = std::reduce(num_local_unique_edge_major_counts.begin(),
            ...
            for(size_t i = 0; i < meta.partition.number_of_local_edge_partitions(); ++i)
            {
                aggregate_major_range_size
                    += meta.partition.local_edge_partition_major_range_size(i);
            }
               ...
               < detail::edge_partition_src_dst_property_values_kv_pair_fill_ratio_threshold)
            {
                ...
                (*local_sorted_unique_edge_major_chunk_start_offsets)
                    .reserve(edge_partition_offsets.size());
                for(size_t i = 0; i < edge_partition_offsets.size(); ++i)
                {
                    ...
                    rmm::device_uvector<vertex_t> unique_edge_majors(
                        num_local_unique_edge_major_counts[i], handle.get_stream());
                    ROCGRAPH_EXPECTS(
                    ...
                    auto cur_size = thrust::distance(
                        ...
                        thrust::copy_if(handle.get_thrust_policy(),
                                        ...
                                        unique_edge_majors.begin(),
                                        has_nzd_t<vertex_t, edge_t>{
                                            edge_partition_offsets[i].data(), major_range_first}));
                    if(use_dcs)
                    {
                    ...
                    auto num_chunks = static_cast<size_t>(
                        ((major_range_last - major_range_first) + (chunk_size - size_t{1}))
                        / chunk_size);
                    rmm::device_uvector<vertex_t> unique_edge_major_chunk_start_offsets(
                        num_chunks + size_t{1}, handle.get_stream());
                    ...
                    unique_edge_major_chunk_start_offsets.set_element(
                        num_chunks,
                        static_cast<vertex_t>(unique_edge_majors.size()),
                        handle.get_stream());
            ...
            return std::make_tuple(
                std::move(local_sorted_unique_edge_majors),
    ...
    {
        ROCGRAPH_EXPECTS(edge_partition_offsets.size() == edge_partition_indices.size(),
                         "Invalid input argument: edge_partition_offsets.size() != "
                         "edge_partition_indices.size().");
        ROCGRAPH_EXPECTS(
        ...
        if constexpr(store_transposed)
        {
            std::tie(local_sorted_unique_edge_dsts_,
                ...
                = update_local_sorted_unique_edge_majors_minors<vertex_t,
                                                                edge_t,
                                                                store_transposed,
                                                                multi_gpu>(
                    handle,
