...
namespace rocgraph
{
    ...
    namespace
    {
                ...
                bool do_expensive_check)
        {
            ...
            rmm::device_uvector<vertex_t> edgelist_majors(number_of_local_edges,
                                                          handle.get_stream());
            ...
            auto edgelist_ids     = edge_id_view ? std::make_optional<rmm::device_uvector<edge_t>>(
                                                 ...
                                                 : std::nullopt;
            auto edgelist_weights = edge_weight_view
                                        ? std::make_optional<rmm::device_uvector<weight_t>>(
                                              edgelist_majors.size(), handle.get_stream())
                                        : std::nullopt;
            auto edgelist_types   = edge_type_view
                ...
                detail::decompress_edge_partition_to_edgelist<vertex_t,
                    ...
                    edge_type_view
                        ? std::make_optional<detail::edge_partition_edge_property_device_view_t<
                              edge_t,
                              edge_type_t const*>>((*edge_type_view), i)
                        : std::nullopt,
                    graph_view.has_edge_mask()
                        ? std::make_optional<detail::edge_partition_edge_property_device_view_t<
                              edge_t,
                              packed_bool_container_t const*,
                              bool>>(*(graph_view.edge_mask_view()), i)
                        : std::nullopt,
                    raft::device_span<vertex_t>(edgelist_majors.data() + cur_size,
                                                edgelist_edge_counts[i]),
                    raft::device_span<vertex_t>(edgelist_minors.data() + cur_size,
                                                edgelist_edge_counts[i]),
                    edgelist_weights
                ...
                for(size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i)
                {
                        ...
                        if(edgelist_ids)
                        {
                            if(edgelist_types)
                            {
                                auto zip_itr = thrust::make_zip_iterator(
                                    thrust::make_tuple(major_ptrs[i],
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                auto zip_itr = thrust::make_zip_iterator(thrust::make_tuple(
                                    major_ptrs[i], (*edgelist_types).data() + cur_size));
                    ...
                    rmm::device_uvector<size_t> d_segment_offsets(d_thresholds.size(),
                                                                  handle.get_stream());
                ...
                bool do_expensive_check)
        {
            ...
            if(graph_view.has_edge_mask())
            {
                num_edges = detail::count_set_bits(
                    handle, (*(graph_view.edge_mask_view())).value_firsts()[0], num_edges);
            }
            ...
            detail::decompress_edge_partition_to_edgelist<vertex_t,
                ...
                graph_view.has_edge_mask()
                    ? std::make_optional<detail::edge_partition_edge_property_device_view_t<
                          edge_t,
                          packed_bool_container_t const*,
                          bool>>(*(graph_view.edge_mask_view()), 0)
                    : std::nullopt,
                ...
                edgelist_types ? std::make_optional<raft::device_span<edge_type_t>>(
                                     (*edgelist_types).data(), (*edgelist_types).size())
                               : std::nullopt,
