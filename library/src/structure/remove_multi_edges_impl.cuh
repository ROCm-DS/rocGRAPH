...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        struct hash_src_dst_pair
        {
            ...
            int32_t __device__ operator()(thrust::tuple<vertex_t, vertex_t> t) const
            {
                ...
                return hash_func.compute_hash(reinterpret_cast<std::byte*>(pair),
                                              2 * sizeof(vertex_t))
                       % num_groups;
            }
        };
                              ...
                              size_t                          mem_frugal_threshold)
        {
            ...
            if(edgelist_srcs.size() > mem_frugal_threshold)
            {
                ...
                raft::update_host(h_group_counts.data(),
                                  group_counts.data(),
                                  group_counts.size(),
                                  handle.get_stream());

                thrust::sort(
                    handle.get_thrust_policy(), pair_first, pair_first + h_group_counts[0]);
                              ...
                              bool                               keep_min_value_edge)
        {
            ...
            if(edgelist_srcs.size() > mem_frugal_threshold)
            {
                ...
                {
                    thrust::sort(
                        handle.get_thrust_policy(), edge_first, edge_first + h_group_counts[0]);
                    thrust::sort(handle.get_thrust_policy(),
    ...
    void remove_multi_edges_helper(
                           ...
                           bool                                              keep_min_value_edge)
    {
            ...
            if(edgelist_edge_ids)
            {
                if(edgelist_edge_types)
                {
                    std::forward_as_tuple(
                        ...
                        = detail::group_multi_edges<vertex_t,
            ...
            else
            {
                if(edgelist_edge_types)
                {
                    std::forward_as_tuple(edgelist_srcs,
                                          edgelist_dsts,
                                          std::tie(edgelist_weights, edgelist_edge_types))
        ...
        else
        {
            if(edgelist_edge_ids)
            {
                if(edgelist_edge_types)
                {
                    std::forward_as_tuple(edgelist_srcs,
                                          edgelist_dsts,
                                          std::tie(edgelist_edge_ids, edgelist_edge_types))
            ...
            else
            {
                ...
                {
                    std::forward_as_tuple(
                        edgelist_srcs, edgelist_dsts, std::tie(edgelist_edge_types))
                ...
                else
                {
                    std::tie(edgelist_srcs, edgelist_dsts)
                        = detail::group_multi_edges(handle,
                                                    std::move(edgelist_srcs),
                                                    std::move(edgelist_dsts),
                                                    mem_frugal_threshold);
                }
        ...
        remove_multi_edges_helper(handle,
                                  edgelist_srcs,
                                  edgelist_dsts,
                                  edgelist_weights,
                                  edgelist_edge_ids,
