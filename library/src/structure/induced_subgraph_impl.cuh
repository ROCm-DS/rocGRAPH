...
namespace rocgraph
{
    ...
    namespace detail
    {
        ...
        {
            using return_type
                = thrust::optional<thrust::tuple<vertex_t, vertex_t, weight_t, size_t>>;
            ...
            bool                                                               do_expensive_check)
    {
        ...
        if(do_expensive_check)
        {
            ...
            raft::update_host(&num_aggregate_subgraph_vertices,
                              ...
                              handle.get_stream());
            handle.sync_stream();
            ROCGRAPH_EXPECTS(should_be_zero == 0,
        ...
        if constexpr(multi_gpu)
        {
            ...
            dst_subgraph_vertices_v
                = rocgraph::device_allgatherv(handle, major_comm, subgraph_vertices);

            graph_ids_v = rocgraph::device_allgatherv(
                handle,
                major_comm,
                raft::device_span<size_t const>(graph_ids_v.data(), graph_ids_v.size()));
            ...
            dst_subgraph_offsets = raft::device_span<size_t const>(dst_subgraph_offsets_v.data(),
                                                                   dst_subgraph_offsets_v.size());
        }
        else
        {
        ...
        if(edge_weight_view)
        {
            ...
            std::tie(edge_majors, edge_minors, *edge_weights, subgraph_edge_graph_ids)
                = extract_transform_v_frontier_outgoing_e(
                    ...
                    *edge_weight_view,
                    detail::
                        induced_subgraph_weighted_edge_op<vertex_t, weight_t, thrust::nullopt_t>{
        ...
        else
        {
            ...
            thrust::sort(handle.get_thrust_policy(),
                         thrust::make_zip_iterator(subgraph_edge_graph_ids.begin(),
                                                   edge_majors.begin(),
                                                   edge_minors.begin()),
                         thrust::make_zip_iterator(
                             subgraph_edge_graph_ids.end(), edge_majors.end(), edge_minors.end()));
