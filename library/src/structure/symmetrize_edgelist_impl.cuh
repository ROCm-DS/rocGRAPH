...
namespace rocgraph
{
    ...
    namespace
    {
        ...
        struct compare_upper_triangular_edges_as_lower_triangular_t
        {
                                       ...
                                       thrust::tuple<vertex_t, vertex_t, weight_t> const& rhs) const
            {
                return thrust::make_tuple(
                       ...
                       < thrust::make_tuple(
                           thrust::get<1>(rhs), thrust::get<0>(rhs), thrust::get<2>(rhs));
            }
        };
        ...
        struct symmetrize_op_t
        {
            ...
            {
                using weight_t = typename thrust::tuple_element<
                    2,
                    typename thrust::iterator_traits<EdgeIterator>::value_type>::type;

                auto min_run_length
                    = lower_run_length < upper_run_length ? lower_run_length : upper_run_length;
                auto max_run_length
                    = lower_run_length < upper_run_length ? upper_run_length : lower_run_length;
                for(size_t i = 0; i < max_run_length; ++i)
                {
                    if(i < min_run_length)
                    {
                        thrust::get<2>(*(edge_first + i))
                            = (thrust::get<2>(*(edge_first + i))
        ...
        {
            __device__ thrust::tuple<vertex_t, vertex_t>
                       operator()(thrust::tuple<vertex_t, vertex_t> e) const
            {
    ...
    namespace detail
    {
                                ...
                                bool                                           reciprocal)
        {
            ...
            rmm::device_uvector<vertex_t> upper_triangular_majors(
                edgelist_majors.size() - num_lower_triangular_edges - num_diagonal_edges,
                handle.get_stream());
            ...
            auto diagonal_weights = edgelist_weights
                                        ? std::make_optional<rmm::device_uvector<weight_t>>(
                                              diagonal_majors.size(), handle.get_stream())
                                        : std::nullopt;
            auto upper_triangular_weights
                = edgelist_weights ? std::make_optional<rmm::device_uvector<weight_t>>(
            ...
            if(edgelist_weights)
            {
                auto lower_triangular_edge_first = thrust::make_zip_iterator(
                    thrust::make_tuple(lower_triangular_majors.begin(),
                ...
                merged_lower_triangular_majors.resize(lower_triangular_majors.size()
                                                          + upper_triangular_majors.size(),
                                                      handle.get_stream());
                ...
                auto merged_first = thrust::make_zip_iterator(
                    thrust::make_tuple(merged_lower_triangular_majors.begin(),
                ...
                thrust::for_each(
                    ...
                    update_edge_weights_and_flags_t<decltype(merged_first)>{
                        merged_first,
                        includes.data(),
                        merged_lower_triangular_majors.size(),
                        symm_op});
                ...
                auto merged_major_minor_range_first = thrust::make_zip_iterator(
                    thrust::make_tuple(merged_lower_triangular_majors.begin(),
                                       merged_lower_triangular_minors.begin()));
            ...
            else
            {
                auto lower_triangular_edge_first = thrust::make_zip_iterator(thrust::make_tuple(
                    lower_triangular_majors.begin(), lower_triangular_minors.begin()));
                thrust::sort(handle.get_thrust_policy(),
                             lower_triangular_edge_first,
                             lower_triangular_edge_first + lower_triangular_majors.size());
                auto upper_triangular_edge_first = thrust::make_zip_iterator(thrust::make_tuple(
                ...
                auto merged_first = thrust::make_zip_iterator(
                    thrust::make_tuple(merged_lower_triangular_majors.begin(),
                                       merged_lower_triangular_minors.begin()));
                auto merged_last
                    = reciprocal ? thrust::set_intersection(
                                       handle.get_thrust_policy(),
                                       lower_triangular_edge_first,
                                       lower_triangular_edge_first + lower_triangular_majors.size(),
                                       upper_triangular_edge_first,
            ...
            upper_triangular_majors.resize(merged_lower_triangular_majors.size(),
                                           handle.get_stream());
            thrust::copy(handle.get_thrust_policy(),
                         merged_lower_triangular_minors.begin(),
            ...
            edgelist_majors.resize(edgelist_majors.size() + diagonal_majors.size()
                                       + upper_triangular_majors.size(),
                                   handle.get_stream());
            thrust::copy(handle.get_thrust_policy(),
                         diagonal_majors.begin(),
                         diagonal_majors.end(),
                         edgelist_majors.end() - diagonal_majors.size()
                             - upper_triangular_majors.size());
            thrust::copy(handle.get_thrust_policy(),
                         upper_triangular_majors.begin(),
            ...
            if(edgelist_weights)
            {
                ...
                thrust::copy(handle.get_thrust_policy(),
                             ...
                             (*edgelist_weights).end() - (*diagonal_weights).size()
                            ...
                            bool                                           reciprocal)
    {
        ...
        return std::make_tuple(
            store_transposed ? std::move(edgelist_minors) : std::move(edgelist_majors),
