    ...
    namespace detail
    {
                                                    ...
                                                    bool                    do_expensive_check)
        {
            if(do_expensive_check && multi_gpu)
            {
                ...
                auto global_start = rocgraph::host_scalar_bcast(
                    handle.get_comms(), local_range_start, 0, handle.get_stream());
                auto sub_range_sizes = rocgraph::host_scalar_allgather(
                    handle.get_comms(), local_range_size, handle.get_stream());
                std::exclusive_scan(sub_range_sizes.begin(),
                                    sub_range_sizes.end(),
                                    sub_range_sizes.begin(),
                                    global_start);
                ROCGRAPH_EXPECTS(sub_range_sizes[comm_rank] == local_range_start,
                                 "Invalid input arguments: a rage must have contiguous and "
            ...
            if(multi_gpu)
            {
                ...
                {
                    rmm::device_uvector<vertex_t> d_target_ranks(permuted_integers.size(),
                                                                 handle.get_stream());

                    rocgraph::detail::uniform_random_fill(handle.get_stream(),
                                                          d_target_ranks.data(),
                                                          d_target_ranks.size(),
                                                          vertex_t{0},
                                                          vertex_t{comm_size},
                    ...
                    auto nr_output_pairs = static_cast<vertex_t>(
                        thrust::distance(d_reduced_ranks.begin(), output_end.first));

                    std::vector<vertex_t> h_reduced_ranks(comm_size);
                    std::vector<vertex_t> h_reduced_counts(comm_size);

                    raft::update_host(h_reduced_ranks.data(),
                                      d_reduced_ranks.data(),
                                      nr_output_pairs,
                                      handle.get_stream());

                    raft::update_host(h_reduced_counts.data(),
                                      d_reduced_counts.data(),
                                      nr_output_pairs,
                                      handle.get_stream());

                    for(int i = 0; i < static_cast<int>(nr_output_pairs); i++)
                    {
                        tx_value_counts[h_reduced_ranks[i]]
                            = static_cast<size_t>(h_reduced_counts[i]);
                    }
                }

                std::tie(permuted_integers, std::ignore)
                    = rocgraph::shuffle_values(handle.get_comms(),
                                               permuted_integers.begin(),
                                               tx_value_counts,
                                               handle.get_stream());
            }
            ...
            if(multi_gpu)
            {
                ...
                auto extra_cluster_ids = rocgraph::detail::device_allgatherv(
                    handle,
                    comm,
                ...
                auto deficits = rocgraph::host_scalar_allgather(
                    handle.get_comms(), nr_deficits, handle.get_stream());

                std::exclusive_scan(
                    deficits.begin(), deficits.end(), deficits.begin(), vertex_t{0});
