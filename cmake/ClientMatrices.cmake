...
if(BUILD_ADDRESS_SANITIZER)
  execute_process(
    COMMAND
      ${CMAKE_CXX_COMPILER} ${CONVERT_SOURCE} -O3 -fsanitize=address
      -shared-libasan -L${ROCM_PATH}/${CMAKE_INSTALL_LIBDIR}
      -Wl,--enable-new-dtags,--build-id=sha1,--rpath,$ENV{ROCM_ASAN_EXE_RPATH}
      -o ${PROJECT_BINARY_DIR}/mtx2csr.exe
    RESULT_VARIABLE STATUS)
else()
  execute_process(
    COMMAND
      ${CMAKE_CXX_COMPILER} ${CONVERT_SOURCE} -O3
      -L${ROCM_PATH}/${CMAKE_INSTALL_LIBDIR}
      -Wl,--enable-new-dtags,--build-id=sha1,--rpath,$ENV{ROCM_EXE_RPATH} -o
      ${PROJECT_BINARY_DIR}/mtx2csr.exe
    RESULT_VARIABLE STATUS)
endif()
  ...
  if(NOT EXISTS "${CMAKE_MATRICES_DIR}/${mat}.csr")
    if(NOT ROCGRAPH_MTX_DIR)
      ...
      if(DEFINED ENV{ROCGRAPH_TEST_MIRROR} AND NOT $ENV{ROCGRAPH_TEST_MIRROR}
        ...
        file(
          DOWNLOAD https://sparse.tamu.edu/MM/${m}.tar.gz
          ${CMAKE_MATRICES_DIR}/${mat}.tar.gz
          INACTIVITY_TIMEOUT 10
          STATUS DL)
    ...
    # mtx2csr.exe is having relative RUNPATH with respect to package install
    # directory Set LD_LIBRARY_PATH for running the executable from build
    # directory
    set(ENV{LD_LIBRARY_PATH}
        "$ENV{LD_LIBRARY_PATH}:${ROCM_PATH}/${CMAKE_INSTALL_LIBDIR}")
    execute_process(
      COMMAND ${PROJECT_BINARY_DIR}/mtx2csr.exe ${mat}.mtx ${mat}.csr
      RESULT_VARIABLE STATUS
      WORKING_DIRECTORY ${CMAKE_MATRICES_DIR})
    if(STATUS AND NOT STATUS EQUAL 0)
